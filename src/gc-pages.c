...
#ifdef _OS_WINDOWS_
    char *mem =
        (char *)VirtualAlloc(NULL, pages_sz + GC_PAGE_SZ, MEM_RESERVE, PAGE_READWRITE);
    if (mem == NULL)
        return NULL;
#else
...
NOINLINE jl_gc_pagemeta_t *jl_gc_alloc_page(void) JL_NOTSAFEPOINT
{
    ...
    for (info.pagetable_i32 = memory_map.lb;
         info.pagetable_i32 < (REGION2_PG_COUNT + 31) / 32; info.pagetable_i32++) {
        ...
        for (info.pagetable_i = 0; freemap1; info.pagetable_i++, freemap1 >>= 1) {
            ...
            for (info.pagetable1_i32 = info.pagetable1->lb;
                 info.pagetable1_i32 < REGION1_PG_COUNT / 32; info.pagetable1_i32++) {
                ...
                for (info.pagetable1_i = 0; freemap0; info.pagetable1_i++, freemap0 >>= 1) {
                    ...
                    info.pagetable0 =
                        info.pagetable1
                            ->meta0[info.pagetable1_i + info.pagetable1_i32 * 32];
                    ...
                    for (info.pagetable0_i32 = info.pagetable0->lb;
                        ...
                        if (freemap) {
                            ...
                            info.meta =
                                info.pagetable0
                                    ->meta[info.pagetable0_i + info.pagetable0_i32 * 32];
                    ...
                    info.pagetable1->freemap0[info.pagetable1_i32] &=
                        ~(uint32_t)(1 << info.pagetable1_i); // record that this was full
                }
            }
            memory_map.freemap1[info.pagetable_i32] &=
                ~(uint32_t)(1 << info.pagetable_i); // record that this was full
        }
    }
...
void jl_gc_free_page(void *p) JL_NOTSAFEPOINT
{
    ...
    if (GC_PAGE_SZ < jl_page_size) {
        ...
        void *otherp =
            (void *)((uintptr_t)p &
                     ~(jl_page_size - 1)); // round down to the nearest physical page
        p = otherp;
        while (n_pages--) {
