...
{
    return jl_options.outputo || jl_options.outputbc || jl_options.outputunoptbc ||
           jl_options.outputji || jl_options.outputasm;
}
...
void jl_write_compiler_output(void)
{
    ...
    if (jl_options.incremental) {
        ...
        if (jl_options.outputbc || jl_options.outputunoptbc)
            jl_printf(JL_STDERR,
                      "WARNING: incremental output to a .bc file is not implemented\n");
        if (jl_options.outputo)
            jl_printf(JL_STDERR,
                      "WARNING: incremental output to a .o file is not implemented\n");
        if (jl_options.outputasm)
            jl_printf(JL_STDERR,
                      "WARNING: incremental output to a .s file is not implemented\n");
    }
    else {
        ios_t *s = NULL;
        ...
        if (jl_options.outputo || jl_options.outputbc || jl_options.outputunoptbc ||
            ...
            jl_dump_native(native_code, jl_options.outputbc, jl_options.outputunoptbc,
                           jl_options.outputo, jl_options.outputasm, (const char *)s->buf,
                           (size_t)s->size);
        }
    }
...
static void _compile_all_tvar_union(jl_value_t *methsig)
{
    ...
    for (i = 0; i < tvarslen; /* incremented by inner loop */) {
        ...
        JL_TRY {
            ...
            // currently doesn't matter much, since jl_compile_hint doesn't work on abstract
            // types
...
static void _compile_all_deq(jl_array_t *found)
{
    ...
    for (found_i = 0; found_i < found_l; found_i++) {
        ...
        assert(mi == m->unspecialized); // make sure we didn't get tricked by a generated
                                        // function, since we can't handle those
        jl_code_instance_t *ucache =
            jl_get_method_inferred(mi, (jl_value_t *)jl_any_type, 1, ~(size_t)0);
        if (ucache->invoke != NULL)
            continue;
        src = m->source;
        assert(src);
        // TODO: we could now enable storing inferred function pointers in the
        // `unspecialized` cache
        // src = jl_type_infer(mi, jl_world_counter, 1);
        // if (ucache->invoke != NULL)
        //    continue;

        // first try to create leaf signatures from the signature declaration and compile
        // those
    ...
    for (size_t i = 0; i < jl_array_len(m); i++) {
        ...
        if (jl_is_method_instance(item)) {
            ...
            if (!jl_isa_compileable_sig((jl_tupletype_t *)mi->specTypes, mi->def.method))
                mi = jl_get_specialization1((jl_tupletype_t *)mi->specTypes,
                                            jl_world_counter, &min_world, &max_world, 0);
            if (mi)
