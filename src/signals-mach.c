...
// Otherwise return `0`
static int jl_mach_gc_wait(jl_ptls_t ptls2, mach_port_t thread, int16_t tid)
{
...
static void allocate_segv_handler()
{
    ...
    for (int16_t tid = 0; tid < jl_n_threads; tid++) {
        attach_exception_port(
            pthread_mach_thread_np((pthread_t)jl_all_tls_states[tid]->system_id), 0);
    }
}
...
enum x86_trap_flags { USER_MODE = 0x4, WRITE_FAULT = 0x2, PAGE_PRESENT = 0x1 };
...
JL_DLLEXPORT
kern_return_t catch_exception_raise(mach_port_t exception_port, mach_port_t thread,
                                    mach_port_t task, exception_type_t exception,
                                    exception_data_t code,
                                    mach_msg_type_number_t code_count)
{
    ...
    kern_return_t ret = thread_get_state(thread, x86_EXCEPTION_STATE64,
    ...
    if (msync((void *)(fault_addr & ~(jl_page_size - 1)), 1, MS_ASYNC) ==
        ...
        if (is_addr_on_stack(ptls2, (void *)fault_addr)) {
            ...
            if (!(exc_state.__err & WRITE_FAULT))
                return KERN_INVALID_ARGUMENT; // rethrow the SEGV since it wasn't an error
                                              // with writing to read-only memory
        ...
        kern_return_t ret =
            thread_get_state(thread, x86_THREAD_STATE64, (thread_state_t)&state, &count);
        HANDLE_MACH_ERROR("thread_get_state", ret);
        jl_critical_error(SIGSEGV, (unw_context_t *)&state, ptls2->bt_data,
                          &ptls2->bt_size);
        return KERN_INVALID_ARGUMENT;
    }
}
...
static void attach_exception_port(thread_port_t thread, int segv_only)
{
    ...
    ret = thread_set_exception_ports(thread, mask, segv_port, EXCEPTION_DEFAULT,
                                           ...
                                           mach_msg_type_number_t code_count)
{
    ...
    kern_return_t ret =
        thread_set_state(thread, x86_THREAD_STATE64, (thread_state_t)&state, count);
    HANDLE_MACH_ERROR("thread_set_state", ret);
    ...
    while (1) {
        ...
        for (i = jl_n_threads; i-- > 0;) {
            ...
            if (running) {
                ...
                if (forceDwarf == 0) {
                    ...
                    bt_size_cur +=
                        rec_backtrace_ctx((jl_bt_element_t *)bt_data_prof + bt_size_cur,
                                          bt_size_max - bt_size_cur - 1, uc, NULL, 1);
                }
                ...
                // Reset the alarm
                kern_return_t ret =
                    clock_alarm(clk, TIME_RELATIVE, timerprof, profile_port);
