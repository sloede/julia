...
// This is a manually constructed dual of the gvars array, which would be produced by
// codegen for Julia code, for C.
static void *const _tags[] = {
    // builtin types
    &jl_any_type, &jl_symbol_type, &jl_ssavalue_type, &jl_datatype_type,
    &jl_slotnumber_type, &jl_simplevector_type, &jl_array_type, &jl_typedslot_type,
    &jl_expr_type, &jl_globalref_type, &jl_string_type, &jl_module_type, &jl_tvar_type,
    &jl_method_instance_type, &jl_method_type, &jl_code_instance_type,
    &jl_linenumbernode_type, &jl_lineinfonode_type, &jl_gotonode_type, &jl_quotenode_type,
    ...
    &jl_addrspace_pointer_type, &jl_vararg_type, &jl_abstractarray_type,
    ...
    &jl_array_any_type, &jl_intrinsic_type, &jl_abstractslot_type, &jl_methtable_type,
    &jl_typemap_level_type, &jl_typemap_entry_type, &jl_voidpointer_type,
    &jl_newvarnode_type, &jl_anytuple_type_type, &jl_anytuple_type, &jl_namedtuple_type,
    &jl_emptytuple_type, &jl_array_symbol_type, &jl_array_uint8_type, &jl_array_int32_type,
    &jl_int32_type, &jl_int64_type, &jl_bool_type, &jl_uint8_type, &jl_uint32_type,
    &jl_uint64_type, &jl_char_type, &jl_weakref_type, &jl_int8_type, &jl_int16_type,
    &jl_uint16_type, &jl_float16_type, &jl_float32_type, &jl_float64_type,
    &jl_floatingpoint_type, &jl_number_type, &jl_signed_type,
    // special typenames
    &jl_tuple_typename, &jl_pointer_typename, &jl_addrspace_pointer_typename,
    &jl_array_typename, &jl_type_typename, &jl_vararg_typename, &jl_namedtuple_typename,
    &jl_vecelement_typename,
    // special exceptions
    &jl_errorexception_type, &jl_argumenterror_type, &jl_typeerror_type,
    &jl_methoderror_type, &jl_loaderror_type, &jl_initerror_type, &jl_undefvarerror_type,
    &jl_stackovf_exception, &jl_diverror_exception, &jl_interrupt_exception,
    &jl_boundserror_type, &jl_memory_exception, &jl_undefref_exception,
    ...
    &jl_module_init_order, &jl_core_module, &jl_base_module, &jl_main_module,
    &jl_top_module, &jl_typeinf_func, &jl_type_type_mt, &jl_nonfunction_mt,
    ...
    &jl_builtin__apply_iterate, &jl_builtin_isdefined, &jl_builtin_nfields,
    &jl_builtin_tuple, &jl_builtin_svec, &jl_builtin_getfield, &jl_builtin_setfield,
    &jl_builtin_fieldtype, &jl_builtin_arrayref, &jl_builtin_const_arrayref,
    &jl_builtin_arrayset, &jl_builtin_arraysize, &jl_builtin_apply_type,
    &jl_builtin_applicable, &jl_builtin_invoke, &jl_builtin__expr, &jl_builtin_ifelse,
    &jl_builtin__typebody, NULL};
...
// This is a manually constructed dual of the fvars array, which would be produced by
// codegen for Julia code, for C.
static const jl_fptr_args_t id_to_fptrs[] = {&jl_f_throw,
                                             &jl_f_is,
                                             &jl_f_typeof,
                                             &jl_f_issubtype,
                                             &jl_f_isa,
...
// if a larger size is required, will need to add support for writing larger relocations in
// many cases below
...
static void jl_load_sysimg_so(void)
{
    ...
    if (!imaging_mode &&
        ...
        jl_dlsym(jl_sysimg_handle, "jl_sysimg_gvars_offsets",
                 (void **)&sysimg_gvars_offsets, 1);
...
{
    assert((ios_pos(s) & (sizeof(void *) - 1)) == 0 &&
           "stream misaligned for writing a word-sized value");
...
static uintptr_t _backref_id(jl_serializer_state *s, jl_value_t *v) JL_NOTSAFEPOINT
{
    ...
    if (jl_is_symbol(v)) {
        ...
        if (idx == HT_NOTFOUND) {
            ...
            idx = (void *)((char *)HT_NOTFOUND +
                           ((uintptr_t)SymbolRef << RELOC_TAG_OFFSET) + offset);
            *pidx = idx;
        }
    }
...
static void jl_write_module(jl_serializer_state *s, uintptr_t item, jl_module_t *m)
{
    ...
    if (m->usings.items == &m->usings._space[0]) {
        ...
        arraylist_push(&s->relocs_list,
                       (void *)(reloc_offset + offsetof(jl_module_t, usings.items)));
        arraylist_push(&s->relocs_list,
                       (void *)(((uintptr_t)DataRef << RELOC_TAG_OFFSET) + item));
        size_t i;
        for (i = 0; i < m->usings.len; i++) {
            arraylist_push(
                &s->relocs_list,
                (void *)(reloc_offset + offsetof(jl_module_t, usings._space[i])));
...
static void jl_write_values(jl_serializer_state *s)
{
    ...
    for (i = 0, len = backref_table_numel * 2; i < len; i += 2) {
        ...
        jl_datatype_t *t = (jl_datatype_t *)jl_typeof(v);
        assert((t->instance == NULL || t->instance == v) &&
               "detected singleton construction corruption");
        ...
        record_gvar(s, jl_get_llvm_gv(native_functions, v),
                    ((uintptr_t)DataRef << RELOC_TAG_OFFSET) + reloc_offset);
...
#define JL_ARRAY_ALIGN(jl_value, nbytes) LLT_ALIGN(jl_value, nbytes)
            ...
            size_t tsz = JL_ARRAY_ALIGN(sizeof(jl_array_t) + ndimwords * sizeof(size_t),
            ...
            if (!ar->flags.ptrarray && !ar->flags.hasptr) {
                ...
                data /= sizeof(void *);
                assert(data < ((uintptr_t)1 << RELOC_TAG_OFFSET) &&
                       "offset to constant data too large");
                arraylist_push(&s->relocs_list,
                               (void *)(reloc_offset +
                                        offsetof(jl_array_t, data))); // relocation location
                arraylist_push(&s->relocs_list,
                               (void *)(((uintptr_t)ConstDataRef << RELOC_TAG_OFFSET) +
                ...
                arraylist_push(&s->relocs_list,
                               (void *)(reloc_offset +
                                        offsetof(jl_array_t, data))); // relocation location
                arraylist_push(&s->relocs_list,
                               (void *)(((uintptr_t)DataRef << RELOC_TAG_OFFSET) +
                                        item)); // relocation target
                if (ar->flags.hasptr) {
                    ...
                    for (i = 0; i < alen; i++) {
                        for (j = 0; j < np; j++) {
                            size_t offset =
                                i * elsz + jl_ptr_offset(((jl_datatype_t *)et), j) *
                                               sizeof(jl_value_t *);
                            jl_value_t *fld = *(jl_value_t **)&data[offset];
                            if (fld != NULL) {
                                arraylist_push(
                                    &s->relocs_list,
                                    (void *)(uintptr_t)(reloc_offset + tsz +
                                                        offset)); // relocation location
                                arraylist_push(
                                    &s->relocs_list,
                                    (void *)backref_id(s, fld)); // relocation target
                                memset(&s->s->buf[reloc_offset + tsz + offset], 0,
                                       sizeof(fld)); // relocation offset (none)
                            }
                            else {
                                assert(*(jl_value_t **)&s->s
                                            ->buf[reloc_offset + tsz + offset] == NULL);
                            }
                        }
                    }
                }
            ...
            assert(data < ((uintptr_t)1 << RELOC_TAG_OFFSET) &&
                   "offset to constant data too large");
            arraylist_push(&s->relocs_list,
                           (void *)(reloc_offset + 8)); // relocation location
            arraylist_push(&s->relocs_list,
                           (void *)(((uintptr_t)ConstDataRef << RELOC_TAG_OFFSET) +
                                    data)); // relocation target
            ...
            for (i = 0; i < np; i++) {
                ...
                if (fld != NULL) {
                    arraylist_push(
                        &s->relocs_list,
                        (void *)(uintptr_t)(offset + reloc_offset)); // relocation location
                    arraylist_push(&s->relocs_list,
                                   (void *)backref_id(s, fld)); // relocation target
                    memset(&s->s->buf[offset + reloc_offset], 0,
                           sizeof(fld)); // relocation offset (none)
                }
            }
            ...
            if (jl_is_method(v)) {
                ...
                if (m->invoke == jl_fptr_const_return) {
                    ...
                    if (jl_is_method(m->def->def.method)) {
                        ...
                        if (builtin_id) { // found in the table of builtins
                            ...
                            jl_get_function_id(
                                native_functions, m, &invokeptr_id,
                                &specfptr_id); // see if we generated code for it
                            if (invokeptr_id) {
                                if (invokeptr_id == -1) {
                                    ...
                                    ios_ensureroom(s->fptr_record,
                                                   invokeptr_id * sizeof(void *));
                                    ios_seek(s->fptr_record,
                                             (invokeptr_id - 1) * sizeof(void *));
                                ...
                                if (specfptr_id) {
                                    ...
                                    ios_ensureroom(s->fptr_record,
                                                   specfptr_id * sizeof(void *));
                                    ios_seek(s->fptr_record,
                                             (specfptr_id - 1) * sizeof(void *));
                                    write_uint32(s->fptr_record, reloc_offset);
#ifdef _P64
                                    write_padding(s->fptr_record, 4);
#endif
                ...
                if (fptr_id != JL_API_NULL) {
                    arraylist_push(
                        &s->relocs_list,
                        (void *)(reloc_offset + offsetof(jl_code_instance_t,
                                                         invoke))); // relocation location
                    arraylist_push(&s->relocs_list,
                                   (void *)(((uintptr_t)FunctionRef << RELOC_TAG_OFFSET) +
                                            fptr_id)); // relocation target
                }
                if (builtin_id >= 2) {
                    arraylist_push(&s->relocs_list,
                                   (void *)(reloc_offset +
                                            offsetof(jl_code_instance_t,
                                                     specptr.fptr))); // relocation location
                    arraylist_push(
                        &s->relocs_list,
                        (void *)(((uintptr_t)BuiltinFunctionRef << RELOC_TAG_OFFSET) +
                                 builtin_id - 2)); // relocation target
                }
            }
                ...
                if (dt->layout != NULL) {
                    ...
                    write_padding(s->const_data,
                                  layout - ios_pos(s->const_data)); // realign stream
                    newdt->layout = NULL; // relocation offset
                    layout /= sizeof(void *);
                    arraylist_push(
                        &s->relocs_list,
                        (void *)(reloc_offset +
                                 offsetof(jl_datatype_t, layout))); // relocation location
                    arraylist_push(&s->relocs_list,
                                   (void *)(((uintptr_t)ConstDataRef << RELOC_TAG_OFFSET) +
                                            layout)); // relocation target
...
static uintptr_t get_reloc_for_item(uintptr_t reloc_item, size_t reloc_offset)
{
    ...
    if (tag == DataRef) {
        ...
        assert(
            reloc_offset == 0 &&
            "offsets for relocations to builtin objects should be precomposed in the reloc_item");
        size_t offset = (reloc_item & (((uintptr_t)1 << RELOC_TAG_OFFSET) - 1));
        switch (tag) {
        case ConstDataRef: break;
        case SymbolRef: assert(offset < nsym_tag && "corrupt relocation item id"); break;
        case TagRef:
            assert(offset < 2 * NBOX_C + 257 && "corrupt relocation item id");
            break;
        case BindingRef: assert(offset == 0 && "corrupt relocation offset"); break;
        case BuiltinFunctionRef:
            assert(offset < sizeof(id_to_fptrs) / sizeof(*id_to_fptrs) &&
        ...
        case FunctionRef:
            ...
            break;
        case DataRef:
        default: assert(0 && "corrupt relocation item id"); abort();
        }
#endif
...
static inline uintptr_t get_item_for_reloc(jl_serializer_state *s, uintptr_t base,
                                           size_t size, uint32_t reloc_id)
{
    enum RefTags tag = (enum RefTags)(reloc_id >> RELOC_TAG_OFFSET);
    size_t offset = (reloc_id & (((uintptr_t)1 << RELOC_TAG_OFFSET) - 1));
    switch (tag) {
    case DataRef: assert(offset <= size); return base + offset;
    case ConstDataRef: return (uintptr_t)s->const_data->buf + (offset * sizeof(void *));
    case SymbolRef:
        assert(offset < deser_sym.len && deser_sym.items[offset] &&
               "corrupt relocation item id");
        return (uintptr_t)deser_sym.items[offset];
    case BindingRef: return jl_buff_tag | GC_OLD_MARKED;
    case TagRef:
        if (offset == 0)
    ...
    case FunctionRef:
        ...
        case JL_API_CONST: return (uintptr_t)jl_fptr_const_return;
        case JL_API_INTERPRETED: return (uintptr_t)jl_fptr_interpret_call;
        case JL_API_BUILTIN: return (uintptr_t)jl_fptr_args;
        case JL_API_NULL:
        case JL_API_MAX:
            // default:
...
static void jl_update_all_fptrs(jl_serializer_state *s)
{
    ...
    for (i = 0; i < sysimg_fvars_max; i++) {
        ...
        if (offset != 0) {
            ...
            assert(jl_is_method(codeinst->def->def.method) &&
...
static void jl_reinit_item(jl_value_t *v, int how)
{
    ...
    case 3: { // install ccallable entry point in JIT
        ...
        jl_compile_extern_c(NULL, NULL, jl_sysimg_handle, jl_svecref(sv, 0),
                            jl_svecref(sv, 1));
        break;
    }
...
static void jl_prune_type_cache(jl_svec_t *cache)
{
    ...
    for (i = 0; i < l; i++) {
        ...
        if (ptrhash_get(&backref_table, ti) != HT_NOTFOUND ||
            jl_get_llvm_gv(native_functions, ti) != 0)
...
// Takes in a path of the form "usr/lib/julia/sys.so" (jl_restore_system_image should be
// passed the same string)
...
JL_DLLEXPORT void jl_set_sysimg_so(void *handle)
{
    ...
    int symbol_found = jl_dlsym(handle, "jl_RTLD_DEFAULT_handle_pointer",
                                (void **)&jl_RTLD_DEFAULT_handle_pointer, 0);
    if (!symbol_found || (void *)&jl_RTLD_DEFAULT_handle != *jl_RTLD_DEFAULT_handle_pointer)
        jl_error(
            "System image file failed consistency check: maybe opened the wrong version?");
...
static void jl_restore_system_image_from_stream(ios_t *f)
{
    ...
    if (0) {
        printf("sysimg size breakdown:\n"
               ...
               "    gvar list: %8u\n"
               "    fptr list: %8u\n",
               (unsigned)sizeof_sysimg, (unsigned)sizeof_constdata,
               (unsigned)sizeof_symbols, (unsigned)sizeof_tags,
               (unsigned)(sizeof_relocations - sizeof_tags), (unsigned)sizeof_gvar_record,
