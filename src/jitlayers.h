...
void addOptimizationPasses(legacy::PassManagerBase *PM, int opt_level,
                           bool lower_intrinsics = true, bool dump_native = false);
...
struct jl_returninfo_t {
    llvm::Function *decl;
    enum CallingConv { Boxed = 0, Register, SRet, Union, Ghosts } cc;
    size_t union_bytes;
    size_t union_align;
...
typedef std::vector<std::tuple<jl_code_instance_t *, jl_returninfo_t::CallingConv, unsigned,
                               llvm::Function *, bool>>
    jl_codegen_call_targets_t;
...
jl_compile_result_t jl_emit_code(jl_method_instance_t *mi, jl_code_info_t *src,
...
enum CompilationPolicy { Default = 0, Extern = 1 };
...
static inline GlobalVariable *global_proto(GlobalVariable *G, Module *M = NULL)
{
    ...
    GlobalVariable *proto =
        new GlobalVariable(G->getType()->getElementType(), G->isConstant(),
                           GlobalVariable::ExternalLinkage, NULL, G->getName(),
...
static inline Constant *literal_static_pointer_val(const void *p, Type *T)
{
    ...
    return ConstantExpr::getIntToPtr(
        ConstantInt::get(Type::getInt64Ty(T->getContext()), (uint64_t)p), T);
#else
    return ConstantExpr::getIntToPtr(
        ConstantInt::get(Type::getInt32Ty(T->getContext()), (uint32_t)p), T);
#endif
}

static const inline char *name_from_method_instance(jl_method_instance_t *li)
{
    return jl_is_method(li->def.method) ? jl_symbol_name(li->def.method->name) :
                                          "top-level scope";
}
...
class JuliaOJIT {
    ...
    struct CompilerT {
        CompilerT(JuliaOJIT *pjit) : jit(*pjit) {}
...
public:
    ...
    void NotifyFinalizer(RTDyldObjHandleT Key, const object::ObjectFile &Obj,
