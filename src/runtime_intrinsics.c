...
#define SELECTOR_FUNC(intrinsic)                                        \
    ...
    static inline intrinsic##_t select_##intrinsic(                     \
        unsigned sz, const select_##intrinsic##_t list) JL_NOTSAFEPOINT \
    {                                                                   \
...
#define fp_select(a, func) sizeof(a) == sizeof(float) ? func##f((float)a) : func(a)
#define fp_select2(a, b, func) sizeof(a) == sizeof(float) ? func##f(a, b) : func(a, b)
...
#define ter_intrinsic_ctype(OP, name, nbits, c_type)                                   \
    static void jl_##name##nbits(unsigned runtime_nbits, void *pa, void *pb, void *pc, \
                                 void *pr) JL_NOTSAFEPOINT                             \
    {                                                                                  \
        c_type a = *(c_type *)pa;                                                      \
        c_type b = *(c_type *)pb;                                                      \
        c_type c = *(c_type *)pc;                                                      \
...
#define un_iintrinsic(name, u)                                         \
    ...
    {                                                                  \
        return jl_iintrinsic_1(jl_typeof(a), a, #name, u##signbitbyte, \
                               jl_intrinsiclambda_ty1, name##_list);   \
    }
#define un_iintrinsic_fast(LLVMOP, OP, name, u)                            \
    ...
    static const select_intrinsic_1_t name##_list = {                      \
        LLVMOP, jl_##name##8, jl_##name##16, jl_##name##32, jl_##name##64, \
    };                                                                     \
    un_iintrinsic(name, u)
#define un_iintrinsic_slow(LLVMOP, name, u)                   \
    static const select_intrinsic_1_t name##_list = {LLVMOP}; \
    un_iintrinsic(name, u)
...
#define uu_iintrinsic_slow(LLVMOP, name, u)                    \
    static const select_intrinsic_u1_t name##_list = {LLVMOP}; \
    uu_iintrinsic(name, u)

static inline jl_value_t *jl_iintrinsic_1(
    jl_value_t *ty, jl_value_t *a, const char *name, char (*getsign)(void *, unsigned),
    jl_value_t *(*lambda1)(jl_value_t *, void *, unsigned, unsigned, const void *),
    const void *list)
{
...
static inline jl_value_t *jl_intrinsiclambda_ty1(jl_value_t *ty, void *pa, unsigned osize,
                                                 unsigned osize2, const void *voidlist)
{
...
static inline jl_value_t *jl_intrinsiclambda_u1(jl_value_t *ty, void *pa, unsigned osize,
                                                unsigned osize2, const void *voidlist)
{
...
static inline jl_value_t *jl_intrinsic_cvt(jl_value_t *ty, jl_value_t *a, const char *name,
                                           intrinsic_cvt_t op)
{
...
static inline jl_value_t *jl_fintrinsic_1(jl_value_t *ty, jl_value_t *a, const char *name,
                                          fintrinsic_op1 *floatop, fintrinsic_op1 *doubleop)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    if (!jl_is_primitivetype(jl_typeof(a)))
        jl_errorf("%s: value is not a primitive type", name);
    ...
    case 4: floatop(sz2 * host_char_bit, pa, pr); break;
    case 8: doubleop(sz2 * host_char_bit, pa, pr); break;
    default:
        jl_errorf(
            "%s: runtime floating point intrinsics are not implemented for bit sizes other than 32 and 64",
            name);
    }
    return newv;
}
...
#define bi_iintrinsic(name, u, cvtb)                                              \
    ...
    {                                                                             \
        return jl_iintrinsic_2(a, b, #name, u##signbitbyte, jl_intrinsiclambda_2, \
                               name##_list, cvtb);                                \
    }
...
#define cmp_iintrinsic(name, u)                                                     \
    JL_DLLEXPORT jl_value_t *jl_##name(jl_value_t *a, jl_value_t *b)                \
    {                                                                               \
        return jl_iintrinsic_2(a, b, #name, u##signbitbyte, jl_intrinsiclambda_cmp, \
                               name##_list, 0);                                     \
    }
...
#define checked_iintrinsic(name, u, lambda_checked)                                      \
    JL_DLLEXPORT jl_value_t *jl_##name(jl_value_t *a, jl_value_t *b)                     \
    {                                                                                    \
        return jl_iintrinsic_2(a, b, #name, u##signbitbyte, lambda_checked, name##_list, \
                               0);                                                       \
    }
...
#define checked_iintrinsic_slow(LLVMOP, name, u)                    \
    static const select_intrinsic_checked_t name##_list = {LLVMOP}; \
    checked_iintrinsic(name, u, jl_intrinsiclambda_checked)
#define checked_iintrinsic_div(LLVMOP, name, u)                     \
    static const select_intrinsic_checked_t name##_list = {LLVMOP}; \
    checked_iintrinsic(name, u, jl_intrinsiclambda_checkeddiv)

static inline jl_value_t *jl_iintrinsic_2(
    jl_value_t *a, jl_value_t *b, const char *name, char (*getsign)(void *, unsigned),
    ...
    const void *list, int cvtb)
{
    ...
    if (sz2 > szb) {
         ...
         * (zero-extend if cvtb is set, since in that case b is unsigned while the sign of a
         * comes from the op)
         */
        void *pb2 = alloca(sz2);
        memcpy(pb2, pb, szb);
...
static inline jl_value_t *jl_intrinsiclambda_2(jl_value_t *ty, void *pa, void *pb,
                                               unsigned sz, unsigned sz2,
                                               const void *voidlist)
{
    void *pr = alloca(sz2);
...
static inline jl_value_t *jl_intrinsiclambda_cmp(jl_value_t *ty, void *pa, void *pb,
                                                 unsigned sz, unsigned sz2,
                                                 const void *voidlist)
{
...
static inline jl_value_t *jl_intrinsiclambda_checked(jl_value_t *ty, void *pa, void *pb,
                                                     unsigned sz, unsigned sz2,
                                                     const void *voidlist)
{
    jl_value_t *params[2];
    ...
    intrinsic_checked_t op =
        select_intrinsic_checked(sz2, (const intrinsic_checked_t *)voidlist);
...
static inline jl_value_t *jl_intrinsiclambda_checkeddiv(jl_value_t *ty, void *pa, void *pb,
                                                        unsigned sz, unsigned sz2,
                                                        const void *voidlist)
{
    void *pr = alloca(sz2);
...
#define bi_fintrinsic(OP, name)                                                                                \
    ...
    {                                                                                                          \
        ...
        case 4: jl_##name##32(32, pa, pb, pr); break;                                                          \
        case 8: jl_##name##64(64, pa, pb, pr); break;                                                          \
        default:                                                                                               \
            jl_error(                                                                                          \
                #name                                                                                          \
                ": runtime floating point intrinsics are not implemented for bit sizes other than 32 and 64"); \
        }                                                                                                      \
        return newv;                                                                                           \
    }
...
#define bool_fintrinsic(OP, name)                                                                              \
    ...
    {                                                                                                          \
        ...
        switch (sz) {                                                                                          \
        /* choose the right size c-type operation */                                                           \
        case 4: cmp = jl_##name##32(32, pa, pb); break;                                                        \
        case 8: cmp = jl_##name##64(64, pa, pb); break;                                                        \
        default:                                                                                               \
            jl_error(                                                                                          \
                #name                                                                                          \
                ": runtime floating point intrinsics are not implemented for bit sizes other than 32 and 64"); \
...
#define ter_fintrinsic(OP, name)                                                                               \
    ...
    {                                                                                                          \
        ...
        int sz = jl_datatype_size(ty);                                                                         \
        jl_value_t *newv = jl_gc_alloc(ptls, sz, ty);                                                          \
        void *pa = jl_data_ptr(a), *pb = jl_data_ptr(b), *pc = jl_data_ptr(c),                                 \
             *pr = jl_data_ptr(newv);                                                                          \
        switch (sz) {                                                                                          \
        /* choose the right size c-type operation */                                                           \
        case 4: jl_##name##32(32, pa, pb, pc, pr); break;                                                      \
        case 8: jl_##name##64(64, pa, pb, pc, pr); break;                                                      \
        default:                                                                                               \
            jl_error(                                                                                          \
                #name                                                                                          \
                ": runtime floating point intrinsics are not implemented for bit sizes other than 32 and 64"); \
...
#define frem(a, b) fp_select2(a, b, fmod)
...
// ternary operators //
#define fma(a, b, c) sizeof(a) == sizeof(float) ? fmaf(a, b, c) : fma(a, b, c)
...
fpiseq_n(float, 32)
fpiseq_n(double, 64)
#define fpiseq(a, b) sizeof(a) == sizeof(float) ? fpiseq32(a, b) : fpiseq64(a, b)
...
fpislt_n(float, 32)
fpislt_n(double, 64)
#define fpislt(a, b) sizeof(a) == sizeof(float) ? fpislt32(a, b) : fpislt64(a, b)
...
#define fptrunc(pr, a)                                               \
    ...
    if (osize == 32)                                                 \
        ...
        *(double *)pr = a;                                           \
        ...
        jl_error(                                                    \
            "fptrunc: runtime floating point intrinsics are not implemented for bit sizes other than 32 and 64");
#define fpext(pr, a)                                               \
    if (!(osize > 8 * sizeof(a)))                                  \
    ...
    if (osize == 32)                                               \
        ...
        *(double *)pr = a;                                         \
        ...
        jl_error(                                                  \
            "fpext: runtime floating point intrinsics are not implemented for bit sizes other than 32 and 64");
...
#define uTYPEMIN(a) (0)
#define uTYPEMAX(a) \
    (8 * sizeof(~a) == runtime_nbits ? (~(a - a)) : (~((~(a - a)) << runtime_nbits)))
...
// functions
#define flipsign(a, b) (b >= 0) ? a : -a
                                        bi_iintrinsic_fast(jl_LLVMFlipSign, flipsign,
                                                           flipsign_int, )
