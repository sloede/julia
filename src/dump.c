...
static void jl_serialize_value_(jl_serializer_state *s, jl_value_t *v,
                                int as_literal) JL_GC_DISABLED;
static jl_value_t *jl_deserialize_value(jl_serializer_state *s,
                                        jl_value_t **loc) JL_GC_DISABLED;
...
static void jl_serialize_datatype(jl_serializer_state *s, jl_datatype_t *dt) JL_GC_DISABLED
{
    ...
    if (!internal && jl_unwrap_unionall(dt->name->wrapper) == (jl_value_t *)dt) {
        ...
        if (jl_unwrap_unionall(dt->name->wrapper) ==
            (jl_value_t *)dt) // comes up often since functions create types
        ...
        tag = 11; // external, but definitely new (still needs caching, but not full
                  // unique-ing)
    }
    else {
    ...
    if (dtnl > 4 && strcmp(&dtname[dtnl - 4], "##kw") == 0 && !internal && tag != 0) {
        /* XXX: yuck, this is horrible, but the auto-generated kw types from the serializer
         * isn't a real type, so we *must* be very careful */
        assert(tag == 6); // other struct types should never exist
        tag = 9;
        if (jl_type_type_mt->kwsorter != NULL &&
            dt == (jl_datatype_t *)jl_typeof(jl_type_type_mt->kwsorter)) {
            dt = jl_datatype_type; // any representative member with this MethodTable
        }
        else if (jl_nonfunction_mt->kwsorter != NULL &&
                 dt == (jl_datatype_t *)jl_typeof(jl_nonfunction_mt->kwsorter)) {
            ...
            assert(primarydt == (jl_value_t *)jl_any_type ||
                   jl_typeof(((jl_datatype_t *)primarydt)->name->mt->kwsorter) ==
                       (jl_value_t *)dt);
    ...
    write_uint8(s->s,
                dt->abstract | (dt->mutabl << 1) | (has_layout << 2) | (has_instance << 3));
    write_uint8(s->s, dt->hasfreetypevars | (dt->isconcretetype << 1) |
                          (dt->isdispatchtuple << 2) | (dt->isbitstype << 3) |
                          (dt->zeroinit << 4) | (dt->isinlinealloc << 5) |
    ...
    if (has_layout) {
        uint8_t layout = 0;
        if (dt->layout ==
            ((jl_datatype_t *)jl_unwrap_unionall((jl_value_t *)jl_array_type))->layout) {
            layout = 1;
        }
            ...
            layout = 2;
                 ...
                 ((jl_datatype_t *)jl_unwrap_unionall((jl_value_t *)jl_pointer_type))
                     ->layout) {
            layout = 3;
        }
...
static void jl_serialize_module(jl_serializer_state *s, jl_module_t *m)
{
    ...
    if (!module_in_worklist(m)) {
        if (m == m->parent) {
            ...
            for (i = 0; i < jl_array_len(s->loaded_modules_array); i++) {
                jl_module_t *mi =
                    (jl_module_t *)jl_array_ptr_ref(s->loaded_modules_array, i);
    ...
    for (i = 1; i < m->bindings.size; i += 2) {
        if (table[i] != HT_NOTFOUND) {
            ...
            if (!b->constp && e && jl_is_cpointer(e) &&
            ...
            write_int8(s->s, (b->deprecated << 3) | (b->constp << 2) | (b->exportp << 1) |
                                 (b->imported));
        }
    }
...
static int is_ir_node(jl_value_t *v)
{
    ...
    return jl_is_slot(v) || jl_is_ssavalue(v) || jl_is_uniontype(v) || jl_is_expr(v) ||
           jl_is_newvarnode(v) || jl_is_svec(v) || jl_is_tuple(v) ||
           ((jl_datatype_t *)jl_typeof(v))->instance || jl_is_int32(v) || jl_is_int64(v) ||
           jl_is_bool(v) || jl_is_uint8(v) || jl_is_quotenode(v) || jl_is_gotonode(v) ||
           jl_is_linenode(v) || jl_is_globalref(v) || jl_is_phinode(v) ||
           jl_is_phicnode(v) || jl_is_upsilonnode(v) || jl_is_pinode(v) ||
    ...
    if (s->mode == MODE_IR) {
        ...
        if (jl_is_module(v)) {
            ...
            if (module_in_worklist(m) && !module_in_worklist(m->parent)) {
                // will need to reinsert this into parent bindings, later (in case of any
                // errors during reinsert)
                arraylist_push(&reinit_list, (void *)pos);
    ...
    if (jl_is_svec(v)) {
        ...
        if (s->mode == MODE_IR && jl_globalref_mod(v) == s->method->module) {
            ...
            jl_serialize_value(s, jl_globalref_name(v));
             ...
             ((jl_ssavalue_t *)v)->id >= 0) {
        write_uint8(s->s, TAG_SSAVALUE);
        write_uint8(s->s, ((jl_ssavalue_t *)v)->id);
    }
    else if (jl_is_ssavalue(v) && ((jl_ssavalue_t *)v)->id <= UINT16_MAX &&
             ((jl_ssavalue_t *)v)->id >= 0) {
        write_uint8(s->s, TAG_LONG_SSAVALUE);
        write_uint16(s->s, ((jl_ssavalue_t *)v)->id);
    }
    else if (jl_typeis(v, jl_slotnumber_type) && jl_slot_number(v) <= UINT16_MAX &&
             jl_slot_number(v) >= 0) {
        ...
        if (ar->flags.ndims == 1 && ar->elsize <= 0x1f) {
            ...
            write_uint8(s->s, (ar->flags.ptrarray << 7) | (ar->flags.hasptr << 6) |
                                  (isunion << 5) | (ar->elsize & 0x1f));
        }
        else {
            write_uint8(s->s, TAG_ARRAY);
            write_uint16(s->s, ar->flags.ndims);
            write_uint16(s->s, (ar->flags.ptrarray << 15) | (ar->flags.hasptr << 14) |
                                   (isunion << 13) | (ar->elsize & 0x1fff));
        }
        ...
        if (ar->flags.ptrarray) {
            for (i = 0; i < l; i++) {
                ...
                if (e && jl_is_cpointer(e) && jl_unbox_voidpointer(e) != (void *)-1 &&
                    jl_unbox_voidpointer(e) != NULL)
        ...
        for (i = 0, j = 0; i < nf + 1; i++) {
            ...
            if (j < np) {
                ...
                while (ptr > prevptr) {
                    // previous field contained pointers; write them and their interleaved
                    // data
                    if (prevptr > last)
...
static int jl_collect_methcache_from_mod(jl_typemap_entry_t *ml,
                                         void *closure) JL_GC_DISABLED
{
...
static void jl_collect_backedges_to(jl_method_instance_t *caller,
                                    htable_t *all_callees) JL_GC_DISABLED
{
            ...
            for (j = 0; valid && j < all_callees.size; j += 2) {
                if (pc[j + 1] != HT_NOTFOUND) {
                    ...
                    if (target == HT_NOTFOUND) {
                        ...
                        jl_value_t *matches =
                            jl_matching_methods((jl_tupletype_t *)sig, -1, 0,
                                                jl_world_counter, &min_valid, &max_valid);
                        if (matches == jl_false) {
                            valid = 0;
                            break;
                        }
                        ...
                        for (k = 0; k < jl_array_len(matches); k++) {
                            jl_array_ptr_set(matches, k,
                                             jl_svecref(jl_array_ptr_ref(matches, k), 2));
                        }
                    ...
                    ((int32_t *)jl_array_data(callees))[jl_array_len(callees) - 1] =
                        (char *)target - (char *)HT_NOTFOUND - 1;
                }
            }
...
static int64_t write_dependency_list(ios_t *s, jl_array_t **udepsp, jl_array_t *mod_array)
{
    ...
    if (!deps)
        deps =
            (jl_array_t *)jl_get_global(jl_base_module, jl_symbol("_require_dependencies"));
    ...
    jl_array_t *udeps =
        (*udepsp = deps && unique_func ? (jl_array_t *)jl_apply(uniqargs, 2) : NULL);
    ...
    if (udeps) {
        ...
        for (i = 0; i < l; i++) {
            ...
            jl_module_t *depmod =
                (jl_module_t *)jl_fieldref(deptuple, 0); // evaluating module
            ...
            for (j = 0; j < lj; j++) {
                jl_module_t *workmod =
                    (jl_module_t *)jl_array_ptr_ref(serializer_worklist, j);
                if (workmod->parent == jl_main_module || workmod->parent == workmod) {
                    ++provides;
...
static jl_value_t *jl_deserialize_datatype(jl_serializer_state *s, int pos,
                                           jl_value_t **loc) JL_GC_DISABLED
{
    ...
    assert(s->method == NULL && s->mode != MODE_IR &&
           "no new data-types expected during MODE_IR");
    assert(pos == backref_list.len - 1 &&
           "nothing should have been deserialized since assigning pos");
    ...
    if (has_layout) {
        ...
        if (layout == 1) {
            ...
            jl_datatype_layout_t *layout = (jl_datatype_layout_t *)jl_gc_perm_alloc(
                sizeof(jl_datatype_layout_t) + fldsize, 0, 4, 0);
            *layout = buffer;
...
static jl_value_t *jl_deserialize_value_svec(jl_serializer_state *s,
                                             uint8_t tag) JL_GC_DISABLED
{
...
static jl_value_t *jl_deserialize_value_symbol(jl_serializer_state *s,
                                               uint8_t tag) JL_GC_DISABLED
{
    int usetable = (s->mode != MODE_IR);
    size_t len;
...
static jl_value_t *jl_deserialize_value_array(jl_serializer_state *s,
                                              uint8_t tag) JL_GC_DISABLED
{
    int usetable = (s->mode != MODE_IR);
    int16_t i, ndims;
    ...
    jl_array_t *a = jl_new_array_for_deserialization((jl_value_t *)NULL, ndims, dims,
                                                     !isptr, hasptr, isunion, elsize);
    if (usetable)
...
static jl_value_t *jl_deserialize_value_expr(jl_serializer_state *s,
                                             uint8_t tag) JL_GC_DISABLED
{
    int usetable = (s->mode != MODE_IR);
    size_t i, len;
...
static jl_value_t *jl_deserialize_value_phi(jl_serializer_state *s,
                                            uint8_t tag) JL_GC_DISABLED
{
    int usetable = (s->mode != MODE_IR);
...
static jl_value_t *jl_deserialize_value_phic(jl_serializer_state *s,
                                             uint8_t tag) JL_GC_DISABLED
{
    int usetable = (s->mode != MODE_IR);
    size_t i, len;
                                               ...
                                               jl_value_t **loc) JL_GC_DISABLED
{
    int usetable = (s->mode != MODE_IR);
    jl_method_t *m =
        (jl_method_t *)jl_gc_alloc(s->ptls, sizeof(jl_method_t), jl_method_type);
    ...
    m->specializations =
        (jl_svec_t *)jl_deserialize_value(s, (jl_value_t **)&m->specializations);
    ...
    if (m->source)
        jl_gc_wb(m, m->source);
    m->unspecialized =
        (jl_method_instance_t *)jl_deserialize_value(s, (jl_value_t **)&m->unspecialized);
    if (m->unspecialized)
...
static jl_value_t *jl_deserialize_value_method_instance(jl_serializer_state *s,
                                                        jl_value_t **loc) JL_GC_DISABLED
{
    int usetable = (s->mode != MODE_IR);
    jl_method_instance_t *mi = (jl_method_instance_t *)jl_gc_alloc(
        s->ptls, sizeof(jl_method_instance_t), jl_method_instance_type);
...
static jl_value_t *jl_deserialize_value_code_instance(jl_serializer_state *s,
                                                      jl_value_t **loc) JL_GC_DISABLED
{
    int usetable = (s->mode != MODE_IR);
    jl_code_instance_t *codeinst = (jl_code_instance_t *)jl_gc_alloc(
        s->ptls, sizeof(jl_code_instance_t), jl_code_instance_type);
    ...
    codeinst->def =
        (jl_method_instance_t *)jl_deserialize_value(s, (jl_value_t **)&codeinst->def);
    ...
    codeinst->next =
        (jl_code_instance_t *)jl_deserialize_value(s, (jl_value_t **)&codeinst->next);
...
static jl_value_t *jl_deserialize_value_singleton(jl_serializer_state *s,
                                                  jl_value_t **loc) JL_GC_DISABLED
{
    if (s->mode == MODE_IR) {
    ...
    if (s->mode == MODE_MODULE) {
        ...
        // if loc == NULL, then the caller can't provide the address where the instance will
        // be stored. this happens if a field might store a 0-size value, but the field
        // itself is not 0 size, e.g. `::Union{Int,Nothing}`
        if (loc != NULL) {
            arraylist_push(&flagref_list, loc);
            arraylist_push(&flagref_list, (void *)pos);
        }
    }
    jl_datatype_t *dt = (jl_datatype_t *)jl_deserialize_value(
        s, (jl_value_t **)HT_NOTFOUND); // no loc, since if dt is replaced, then
                                        // dt->instance would be also
    jl_set_typeof(v, dt);
    if (dt->instance == NULL)
        return v;
    return dt->instance;
...
static jl_value_t *jl_deserialize_value_any(jl_serializer_state *s, uint8_t tag,
                                            jl_value_t **loc) JL_GC_DISABLED
{
    int usetable = (s->mode != MODE_IR);
    ...
    if (usetable)
        arraylist_push(&backref_list, v);
    jl_datatype_t *dt =
        (jl_datatype_t *)jl_deserialize_value(s, &jl_astaggedvalue(v)->type);
                                        ...
                                        jl_value_t **loc) JL_GC_DISABLED
{
    ...
    case 0: tag = read_uint8(s->s); return deser_tag[tag];
    case TAG_BACKREF: JL_FALLTHROUGH;
    case TAG_SHORT_BACKREF:
        assert(s->method == NULL && s->mode != MODE_IR);
    ...
    case TAG_METHODROOT: return jl_array_ptr_ref(s->method->roots, read_uint8(s->s));
    case TAG_LONG_METHODROOT: return jl_array_ptr_ref(s->method->roots, read_uint16(s->s));
    case TAG_SVEC: JL_FALLTHROUGH;
    case TAG_LONG_SVEC: return jl_deserialize_value_svec(s, tag);
    case TAG_COMMONSYM: return deser_symbols[read_uint8(s->s)];
    case TAG_SYMBOL: JL_FALLTHROUGH;
    case TAG_LONG_SYMBOL: return jl_deserialize_value_symbol(s, tag);
    case TAG_SSAVALUE:
    ...
    case TAG_SLOTNUMBER:
        ...
        if (usetable)
            arraylist_push(&backref_list, v);
        return v;
    case TAG_ARRAY: JL_FALLTHROUGH;
    case TAG_ARRAY1D: return jl_deserialize_value_array(s, tag);
    case TAG_EXPR: JL_FALLTHROUGH;
    case TAG_LONG_EXPR: JL_FALLTHROUGH;
    case TAG_CALL1: JL_FALLTHROUGH;
    case TAG_CALL2: return jl_deserialize_value_expr(s, tag);
    case TAG_PHINODE: JL_FALLTHROUGH;
    case TAG_LONG_PHINODE: return jl_deserialize_value_phi(s, tag);
    case TAG_PHICNODE: JL_FALLTHROUGH;
    case TAG_LONG_PHICNODE: return jl_deserialize_value_phic(s, tag);
    case TAG_GOTONODE: JL_FALLTHROUGH;
    case TAG_QUOTENODE:
        v = jl_new_struct_uninit(tag == TAG_GOTONODE ? jl_gotonode_type :
                                                       jl_quotenode_type);
        if (usetable)
            arraylist_push(&backref_list, v);
        set_nth_field(tag == TAG_GOTONODE ? jl_gotonode_type : jl_quotenode_type, (void *)v,
                      0, jl_deserialize_value(s, NULL));
        return v;
    case TAG_UNIONALL:
        ...
        if (usetable)
            backref_list.items[pos] = v;
        ((jl_unionall_t *)v)->var =
            (jl_tvar_t *)jl_deserialize_value(s, (jl_value_t **)&((jl_unionall_t *)v)->var);
    ...
    case TAG_METHOD: return jl_deserialize_value_method(s, loc);
    case TAG_METHOD_INSTANCE: return jl_deserialize_value_method_instance(s, loc);
    case TAG_CODE_INSTANCE: return jl_deserialize_value_code_instance(s, loc);
    case TAG_MODULE: return jl_deserialize_value_module(s);
    ...
    case TAG_INT32:
        ...
        if (usetable)
            arraylist_push(&backref_list, v);
        return v;
    case TAG_UINT8: return jl_box_uint8(read_uint8(s->s));
    ...
    case TAG_GLOBALREF: return jl_deserialize_value_globalref(s);
    case TAG_SINGLETON: return jl_deserialize_value_singleton(s, loc);
    case TAG_CORE: return (jl_value_t *)jl_core_module;
    case TAG_BASE: return (jl_value_t *)jl_base_module;
    case TAG_VECTORTY:
        v = jl_deserialize_value(s, NULL);
    ...
    case TAG_LINEINFO:
        ...
        for (i = 0; i < jl_datatype_nfields(jl_lineinfonode_type); i++) {
            ...
            set_nth_field(jl_lineinfonode_type, (void *)v, i,
                          jl_deserialize_value(s, (jl_value_t **)((char *)v + offs)));
        }
        return v;
    case TAG_DATATYPE:
        pos = backref_list.len;
...
static void jl_verify_edges(jl_array_t *targets, jl_array_t **pvalids)
{
    ...
    *pvalids = valids;
    for (i = 0; i < l; i++) {
        ...
        size_t min_valid = 0;
        size_t max_valid = ~(size_t)0;
        jl_value_t *matches = jl_matching_methods((jl_tupletype_t *)sig, -1, 0,
                                                  jl_world_counter, &min_valid, &max_valid);
...
{
    // map(enable, ((list[i] => targets[list[i + 1] .* 2]) for i in 1:2:length(list) if
    // all(valids[list[i + 1]])))
    size_t i, l = jl_array_len(list);
    ...
    for (i = 0; i < l; i += 2) {
        ...
        assert(caller->def.method->primary_world ==
               jl_world_counter); // caller should be new
    ...
    for (i = 0;; i++) {
        ...
        if (len == 0 || i == l)
            return jl_get_exceptionf(jl_errorexception_type,
                                     "Wrong number of entries in module list.");
        ...
        if (!m || !jl_is_module(m) || m->uuid.hi != uuid.hi || m->uuid.lo != uuid.lo ||
            m->name != sym || m->build_id != build_id) {
            return jl_get_exceptionf(jl_errorexception_type,
...
{
    uint16_t bom;
    return (readstr_verify(s, JI_MAGIC) && read_uint16(s) == JI_FORMAT_VERSION &&
            ios_read(s, (char *)&bom, 2) == 2 && bom == BOM &&
            read_uint8(s) == sizeof(void *) && readstr_verify(s, JL_BUILD_UNAME) &&
            !read_uint8(s) && readstr_verify(s, JL_BUILD_ARCH) && !read_uint8(s) &&
...
static void jl_finalize_serializer(jl_serializer_state *s)
{
    ...
    if (jl_module_init_order != NULL) {
        ...
        for (i = 0; i < l; i++) {
            ...
            assert(ptrhash_get(&backref_table, jl_array_ptr_ref(jl_module_init_order, i)) !=
                   HT_NOTFOUND);
        }
    }
...
static void jl_reinit_item(jl_value_t *v, int how, arraylist_t *tracee_list)
{
    JL_TRY {
        ...
        case 2: { // reinsert module v into parent (const)
            ...
            if (b->value != NULL) {
                ...
                if (jl_generating_output() && jl_options.incremental) {
                    jl_errorf("Cannot replace module %s during incremental precompile.",
        ...
        default: assert(0 && "corrupt deserialization state"); abort();
        }
    }
    JL_CATCH {
...
JL_DLLEXPORT jl_array_t *jl_compress_ir(jl_method_t *m, jl_code_info_t *code)
{
    ...
    jl_serializer_state s = {&dest, MODE_IR, m, jl_get_ptls_states(), NULL};

    uint8_t flags = (code->inferred << 3) | (code->inlineable << 2) |
                    (code->propagate_inbounds << 1) | (code->pure << 0);
    write_uint8(s.s, flags);
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (i == 1) { // skip codelocs
            assert(jl_field_offset(jl_code_info_type, i) ==
                   offsetof(jl_code_info_t, codelocs));
            continue;
        }
        if (i == 4) { // don't copy contents of method_for_inference_limit_heuristics field
            assert(jl_field_offset(jl_code_info_type, i) ==
                   offsetof(jl_code_info_t, method_for_inference_limit_heuristics));
            copy = 0;
        }
...
JL_DLLEXPORT jl_code_info_t *jl_uncompress_ir(jl_method_t *m, jl_code_instance_t *metadata,
                                              jl_array_t *data)
{
    ...
    int en = jl_gc_enable(0); // Might GC
    jl_serializer_state s = {&src, MODE_IR, m, jl_get_ptls_states(), NULL};
    ...
    for (i = 0; i < 6; i++) {
        ...
        jl_value_t **fld = (jl_value_t **)((char *)jl_data_ptr(code) +
                                           jl_field_offset(jl_code_info_type, i));
        *fld = jl_deserialize_value(&s, fld);
    }
...
JL_DLLEXPORT int jl_save_incremental(const char *fname, jl_array_t *worklist)
{
    ...
    jl_idtable_type =
        jl_base_module ? jl_get_global(jl_base_module, jl_symbol("IdDict")) : NULL;
    jl_idtable_typename =
        jl_base_module ?
            ((jl_datatype_t *)jl_unwrap_unionall((jl_value_t *)jl_idtable_type))->name :
            NULL;
    jl_bigint_type =
        jl_base_module ? jl_get_global(jl_base_module, jl_symbol("BigInt")) : NULL;
    if (jl_bigint_type) {
        gmp_limb_size = jl_unbox_long(jl_get_global(
                            (jl_module_t *)jl_get_global(jl_base_module, jl_symbol("GMP")),
                            jl_symbol("BITS_PER_LIMB"))) /
                        8;
    }
    ...
    jl_serializer_state s = {&f, MODE_MODULE, NULL, jl_get_ptls_states(), mod_array};
    ...
    if (udeps) {
        ...
        for (i = 0; i < len; i++) {
            ...
            if (depmod != (jl_value_t *)jl_main_module) {
                ...
                if (!srctp) {
                    jl_printf(JL_STDERR,
                              "WARNING: could not cache source text for \"%s\".\n",
                              jl_string_data(dep));
                    continue;
                }
                size_t slen = jl_string_len(dep);
...
{
    return jl_subtype((jl_value_t *)a, (jl_value_t *)b) &&
           jl_subtype((jl_value_t *)b, (jl_value_t *)a);
}
...
static jl_method_t *jl_recache_method(jl_method_t *m)
{
    ...
    assert((jl_value_t *)mt != jl_nothing);
    jl_set_typeof(m,
                  (void *)(intptr_t)0x30); // invalidate the old value to help catch errors
...
static jl_method_instance_t *jl_recache_method_instance(jl_method_instance_t *mi)
{
    ...
    jl_set_typeof(mi,
                  (void *)(intptr_t)0x40); // invalidate the old value to help catch errors
    jl_svec_t *env = jl_emptysvec;
    jl_value_t *ti =
        jl_type_intersection_env((jl_value_t *)argtypes, (jl_value_t *)m->sig, &env);
    // assert(ti != jl_bottom_type); (void)ti;
    if (ti == jl_bottom_type)
        env = jl_emptysvec; // the intersection may fail now if the type system had made an
                            // incorrect subtype env in the past
    jl_method_instance_t *_new =
        jl_specializations_get_linfo(m, (jl_value_t *)argtypes, env);
    return _new;
}
...
static jl_value_t *_jl_restore_incremental(ios_t *f, jl_array_t *mod_array)
{
    ...
    jl_serializer_state s = {f, MODE_MODULE, NULL, ptls, mod_array};
    ...
    jl_insert_methods(
        (jl_array_t *)external_methods); // hook up methods of external generic functions
                                         // (needs to be after recache types)
    jl_recache_other(); // make all of the other objects identities correct (needs to be
                        // after insert methods)
    htable_free(&uniquing_table);
    jl_array_t *init_order = jl_finalize_deserializer(
        &s, tracee_list); // done with f and s (needs to be after recache)
    ...
    jl_insert_backedges(
        (jl_array_t *)external_backedges,
        (jl_array_t *)external_edges); // restore external backedges (needs to be last)
    ...
    jl_gc_enable_finalizers(
        ptls, 1); // make sure we don't run any Julia code concurrently before this point
    if (tracee_list) {
        jl_methtable_t *mt;
...
JL_DLLEXPORT jl_value_t *jl_restore_incremental(const char *fname, jl_array_t *mod_array)
{
    ...
    if (ios_file(&f, fname, 1, 0, 0, 0) == NULL) {
        return jl_get_exceptionf(jl_errorexception_type, "Cache file \"%s\" not found.\n",
                                 fname);
    }
    return _jl_restore_incremental(&f, mod_array);
}
...
#include "common_symbols1.inc"
                    ...
                    jl_anytuple_type,
                    jl_tparam0(jl_anytuple_type),
                    jl_emptytuple_type,
                    jl_array_uint8_type,
                    jl_code_info_type,
                    jl_typeofbottom_type,
                    jl_typeofbottom_type->super,
                    jl_namedtuple_type,
                    jl_array_int32_type,
                    jl_typedslot_type,
                    jl_uint32_type,
