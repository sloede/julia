...
// The compilation signature is not used to cache the method if the number of overlapping
// methods is greater than this
...
static int speccache_eq(size_t idx, const void *ty, jl_svec_t *data, uint_t hv)
{
    ...
    if (ty == sig)
        return 1;
    uint_t h2 =
        ((jl_datatype_t *)(jl_is_unionall(sig) ? jl_unwrap_unionall(sig) : sig))->hash;
    if (h2 != hv)
        return 0;
...
JL_DLLEXPORT jl_method_instance_t *
jl_specializations_get_linfo(jl_method_t *m JL_PROPAGATES_ROOT, jl_value_t *type,
                             jl_svec_t *sparams)
{
    uint_t hv =
        ((jl_datatype_t *)(jl_is_unionall(type) ? jl_unwrap_unionall(type) : type))->hash;
    for (int locked = 0;; locked++) {
        ...
        if (hv) {
            ssize_t idx =
                jl_smallintset_lookup(speckeyset, speccache_eq, type, specializations, hv);
            if (idx != -1) {
                jl_method_instance_t *mi =
                    (jl_method_instance_t *)jl_svecref(specializations, idx);
                if (locked)
            ...
            jl_method_instance_t **data =
                (jl_method_instance_t **)jl_svec_data(specializations);
            JL_GC_PUSH1(&specializations); // clang-sa doesn't realize this loop uses
                                           // specializations
            for (i = cl; i > 0; i--) {
        ...
        if (!locked) {
            ...
            JL_GC_PUSH1(&mi);
            if (hv ? (i + 1 >= cl || jl_svecref(specializations, i + 1) != NULL) :
                     (i <= 1 || jl_svecref(specializations, i - 2) != NULL)) {
                size_t ncl = cl < 8 ? 8 : (cl * 3) >> 1;
                jl_svec_t *nc = jl_alloc_svec_uninit(ncl);
                if (i > 0)
                    memcpy((char *)jl_svec_data(nc), jl_svec_data(specializations),
                           sizeof(void *) * i);
                memset((char *)jl_svec_data(nc) + sizeof(void *) * i, 0,
                       sizeof(void *) * (ncl - cl));
                if (i < cl)
            ...
            if (hv) {
                ...
                jl_smallintset_insert(&m->speckeyset, (jl_value_t *)m, speccache_hash, i,
                                      specializations);
            }
            JL_UNLOCK(&m->writelock);
            JL_GC_POP();
...
JL_DLLEXPORT jl_value_t *jl_methtable_lookup(jl_methtable_t *mt, jl_value_t *type,
                                             size_t world)
{
    struct jl_typemap_assoc search = {type, world, NULL, 0, ~(size_t)0};
    jl_typemap_entry_t *sf =
        jl_typemap_assoc_by_type(mt->defs, &search, /*offs*/ 0, /*subtype*/ 0);
    if (!sf)
        return jl_nothing;
...
JL_DLLEXPORT jl_code_instance_t *
jl_set_method_inferred(jl_method_instance_t *mi, jl_value_t *rettype,
                       jl_value_t *inferred_const, jl_value_t *inferred,
                       int32_t const_flags, size_t min_world, size_t max_world);

jl_datatype_t *jl_mk_builtin_func(jl_datatype_t *dt, const char *name,
                                  jl_fptr_args_t fptr) JL_GC_DISABLED
{
    jl_sym_t *sname = jl_symbol(name);
    if (dt == NULL) {
        jl_value_t *f =
            jl_new_generic_function_with_supertype(sname, jl_core_module, jl_builtin_type);
    ...
    JL_GC_PUSH1(&m);
    jl_method_instance_t *mi =
        jl_get_specialized(m, (jl_value_t *)jl_anytuple_type, jl_emptysvec);
    m->unspecialized = mi;
    jl_gc_wb(m, mi);

    jl_code_instance_t *codeinst = jl_set_method_inferred(
        mi, (jl_value_t *)jl_any_type, jl_nothing, jl_nothing, 0, 1, ~(size_t)0);
    codeinst->specptr.fptr1 = fptr;
    codeinst->invoke = jl_fptr_args;

    jl_methtable_t *mt = dt->name->mt;
    jl_typemap_insert(&mt->cache, (jl_value_t *)mt, jl_anytuple_type, NULL, jl_emptysvec,
                      (jl_value_t *)mi, 0, &lambda_cache, 1, ~(size_t)0);
    mt->frozen = 1;
    JL_GC_POP();
    return dt;
}
...
jl_code_info_t *jl_type_infer(jl_method_instance_t *mi, size_t world, int force)
{
    ...
    if (mi->inInference && !force)
        return NULL;
    if (jl_is_method(mi->def.method) && mi->def.method->unspecialized == mi)
        return NULL; // be careful never to infer the unspecialized method, this would not
                     // be valid
...
JL_DLLEXPORT jl_value_t *jl_rettype_inferred(jl_method_instance_t *mi, size_t min_world,
                                             size_t max_world) JL_NOTSAFEPOINT
{
...
JL_DLLEXPORT jl_code_instance_t *jl_get_method_inferred(jl_method_instance_t *mi,
                                                        jl_value_t *rettype,
                                                        size_t min_world, size_t max_world)
{
    jl_code_instance_t *codeinst = mi->cache;
    while (codeinst) {
        if (codeinst->min_world == min_world && codeinst->max_world == max_world &&
            jl_egal(codeinst->rettype, rettype)) {
            return codeinst;
        }
        codeinst = codeinst->next;
    }
    return jl_set_method_inferred(mi, rettype, NULL, NULL, 0, min_world, max_world);
}

JL_DLLEXPORT jl_code_instance_t *
jl_set_method_inferred(jl_method_instance_t *mi JL_PROPAGATES_ROOT, jl_value_t *rettype,
                       jl_value_t *inferred_const, jl_value_t *inferred,
                       int32_t const_flags, size_t min_world, size_t max_world
                       /*, jl_array_t *edges, int absolute_max*/)
{
    ...
    jl_code_instance_t *codeinst = (jl_code_instance_t *)jl_gc_alloc(
        ptls, sizeof(jl_code_instance_t), jl_code_instance_type);
...
static void foreach_mtable_in_module(jl_module_t *m,
                                     void (*visit)(jl_methtable_t *mt, void *env),
                                     void *env, jl_array_t *visited)
{
    size_t i;
...
{
    return (t == (jl_value_t *)jl_any_type ||
            jl_types_equal(t, (jl_value_t *)jl_type_type));
}
...
// if concrete_match returns false, the sig may specify `Type{T::DataType}`, while the `tt`
// contained DataType in this case, subtyping is wrong, and this may not actually match at
// runtime since it may instead match any kind of `Type{T::Type}`
...
static jl_value_t *ml_matches(jl_typemap_t *ml, int offs, jl_tupletype_t *type, int lim,
                              int include_ambiguous, size_t world, size_t *min_valid,
                              size_t *max_valid);

// get the compilation signature specialization for this method
static void jl_compilation_sig(
    jl_tupletype_t *const tt, // the original tupletype of the call : this is expected to be
                              // a relative simple type (no Varags, Union, UnionAll, etc.)
    jl_svec_t *sparams, jl_method_t *definition, intptr_t nspec,
    // output:
    ...
    for (i = 0; i < np; i++) {
        ...
        if (jl_is_kind(decl_i)) {
            ...
            if (jl_subtype(kind, decl_i) &&
                !jl_subtype((jl_value_t *)jl_type_type, decl_i)) {
                // if we can prove the match was against the kind (not a Type)
            ...
            // if we get a kind, where we don't expect to accept one, widen it to something
            // more expected (Type{T})
            if (!(jl_subtype(elt, decl_i) &&
                  !jl_subtype((jl_value_t *)jl_type_type, decl_i))) {
                if (!*newparams)
                    *newparams = jl_svec_copy(tt->parameters);
        ...
        else if (!jl_is_datatype(elt) &&
                 jl_subtype(elt, (jl_value_t *)jl_type_type)) { // elt <: Type{T}
            // not triggered for isdispatchtuple(tt), this attempts to handle
            // some cases of adapting a random signature into a compilation signature
            if (!*newparams)
                *newparams = jl_svec_copy(tt->parameters);
            ...
            if (very_general_type(decl_i)) {
                ...
                */
                ...
                jl_svecset(*newparams, i, jl_typetype_type);
                     ...
                     // try to give up on specializing type parameters for
                     // Type{Type{Type{...}}}
                     (jl_is_type_type(jl_tparam0(jl_tparam0(elt))) ||
                      !jl_has_free_typevars(decl_i))) {
                // TODO: this is probably solidly unsound and would corrupt the cache in
                // many cases
                /*
                ...
                */
                ...
                if (i < nargs || !definition->isva) {
                    jl_value_t *di =
                        jl_type_intersection(decl_i, (jl_value_t *)jl_typetype_type);
                    assert(di != (jl_value_t *)jl_bottom_type);
                    // issue #11355: DataType has a UID and so would take precedence in the
                    // cache
                    if (jl_is_kind(di))
    ...
    if (jl_nparams(tt) >= nspec &&
        ...
        if (all_are_subtypes) {
            ...
            if (nsp > 0 && jl_has_free_typevars(lastdeclt)) {
                ...
                lastdeclt = jl_instantiate_type_in_env(lastdeclt, (jl_unionall_t *)decl,
    ...
    for (i = 0; i < np; i++) {
        ...
        if (jl_is_vararg_type(elt)) {
            ...
            if (jl_has_free_typevars(decl_i)) {
                // TODO: in this case, answer semi-conservatively that these varargs are
                // always compilable we don't have the ability to get sparams, so deciding
                // if elt is a potential result of jl_instantiate_type_in_env for decl_i for
                // any sparams that is consistent with the rest of the arguments seems like
                // it would be extremely difficult and hopefully the upstream code probably
...
cache_method(jl_methtable_t *mt, jl_typemap_t **cache,
             jl_value_t *parent JL_PROPAGATES_ROOT,
             jl_tupletype_t *tt, // the original tupletype of the signature
             jl_method_t *definition, size_t world, jl_svec_t *sparams)
{
    ...
    { // scope block
        ...
        jl_typemap_entry_t *entry =
            jl_typemap_assoc_by_type(*cache, &search, offs, /*subtype*/ 1);
    ...
    intptr_t nspec = (mt == NULL || mt == jl_type_type_mt || mt == jl_nonfunction_mt ?
                          definition->nargs + 1 :
                          mt->max_args + 2);
    ...
    newmeth =
        jl_specializations_get_linfo(definition, (jl_value_t *)compilationsig, sparams);
    ...
    if (!cache_with_orig && mt) {
        ...
        temp = ml_matches(mt->defs, 0, compilationsig, MAX_UNSPECIALIZED_CONFLICTS, 1,
        ...
        if (!cache_with_orig && guards > 0) {
            ...
            for (i = 0, l = jl_array_len(temp); i < l; i++) {
                ...
                if (other != definition) {
                    ...
                    //        NULL, jl_emptysvec, /*guard*/NULL, jl_cachearg_offset(mt),
                    //        &lambda_cache, other->min_world, other->max_world);
                }
            }
        }
        if (cache_with_orig) {
            ...
            // determined above that there's no ambiguity in also using compilationsig as
            // the cacheablesig
    ...
    // now scan `cachett` and ensure that `Type{T}` in the cache will be matched exactly by
    // `typeof(T)` and also reduce the complexity of rejecting this entry in the cache by
    // replacing non-simple types with jl_any_type to build a new `type` (for example, if
    // the signature contains jl_function_type)
    ...
    for (i = 0; i < np; i++) {
        ...
        if (jl_is_vararg_type(elt)) {
            ...
            jl_value_t *kind = jl_typeof(jl_tparam0(elt));
            if (!newparams)
                newparams = jl_svec_copy(cachett->parameters);
            jl_svecset(newparams, i, kind);
        }
        else if (!jl_is_concrete_type(
                     elt)) { // for example, jl_function_type or jl_tuple_type
            if (!newparams)
                newparams = jl_svec_copy(cachett->parameters);
    ...
    if (cachett != tt && simplett == NULL) {
        struct jl_typemap_assoc search = {(jl_value_t *)cachett, min_valid, NULL, 0,
                                          ~(size_t)0};
        jl_typemap_entry_t *entry =
            jl_typemap_assoc_by_type(*cache, &search, offs, /*subtype*/ 1);
    ...
    jl_typemap_insert(cache, parent, cachett, simplett, guardsigs, (jl_value_t *)newmeth,
                      offs, &lambda_cache, min_valid, max_valid);

    JL_GC_POP();
    return newmeth;
}

// this is the general entry point for taking a match (returned by jl_typemap_assoc_by_type
// with subtype=1) and getting the most-specific one (or NULL, if there isn't one that is
// most specific)
static jl_typemap_entry_t *
jl_typemap_morespecific_by_type(jl_typemap_entry_t *first JL_PROPAGATES_ROOT,
                                jl_value_t *types, jl_svec_t **penv, size_t world)
{
    jl_typemap_entry_t *candidate = first;
    jl_value_t *resorted = first->func.method->resorted;
    // pick a method out of the resorted list that is more specific than any other
    // applicable method
    if ((jl_value_t *)resorted != jl_nothing) {
        ...
        for (i = 0; i < l; i++) {
            ...
            if (prior->min_world <= world && world <= prior->max_world &&
                jl_subtype(types, (jl_value_t *)prior->sig)) {
                if (candidate == first ||
                    jl_type_morespecific((jl_value_t *)prior->sig,
                                         (jl_value_t *)candidate->sig)) {
                    candidate = prior;
                }
                // else if (!jl_type_morespecific((jl_value_t*)candidate->sig,
                // (jl_value_t*)prior->sig)) {
                //    isambig = 1;
                //}
            }
        }
        // and then make sure it is more specific than all other applicable (unsorted)
        // methods
        // if (isambig)
        //    return NULL;
        if (candidate != first) {
            ...
            if ((jl_value_t *)ambigs != jl_nothing) {
                ...
                for (i = 0; i < l; i++) {
                    jl_typemap_entry_t *ambig =
                        (jl_typemap_entry_t *)jl_array_ptr_ref(ambigs, i);
                    if (ambig->min_world <= world && world <= ambig->max_world &&
                        jl_subtype(types, (jl_value_t *)ambig->sig)) {
                        if (!jl_type_morespecific((jl_value_t *)candidate->sig,
                                                  (jl_value_t *)ambig->sig))
                            return NULL;
                    }
                }
            }
            // for (i = 0; i < l; i++) {
            //    jl_typemap_entry_t *prior =
            //    (jl_typemap_entry_t*)jl_array_ptr_ref(resorted, i); if (prior ==
            //    candidate)
...
static jl_method_instance_t *jl_mt_assoc_by_type(jl_methtable_t *mt, jl_datatype_t *tt,
                                                 int mt_cache, size_t world)
{
    // caller must hold the mt->writelock
    struct jl_typemap_assoc search = {(jl_value_t *)tt, world, NULL, 0, ~(size_t)0};
    jl_typemap_entry_t *entry =
        jl_typemap_assoc_by_type(mt->cache, &search, jl_cachearg_offset(mt), /*subtype*/ 1);
    if (entry && entry->func.value)
        return entry->func.linfo;
    ...
    if (entry != NULL) {
        entry =
            jl_typemap_morespecific_by_type(entry, (jl_value_t *)tt, &search.env, world);
        if (entry != NULL) {
...
/*
  ...
  ambiguous. however, (AbstractArray, AbstractMatrix, Foo) and (AbstractMatrix,
  AbstractArray, Bar) are fine since Foo and Bar are disjoint, so there would be no
  confusion over which one to call.
*/
                                   ...
                                   struct typemap_intersection_env *closure0)
{
    ...
    if (reorder) {
        ...
        // TODO: we might like to use `subtype = exact1 && exact2` here, but
        // check_disabled_ambiguous_visitor won't be able to handle that, so we might end up
        // making some unnecessary mambig entries here but I don't have any examples of such
        struct jl_typemap_assoc search = {(jl_value_t *)isect, world, NULL, 0, ~(size_t)0};
        jl_typemap_entry_t *l =
            jl_typemap_assoc_by_type(closure->defs, &search, /*offs*/ 0, /*subtype*/ 0);
        // assert((!subtype || l != after) && "bad typemap lookup result"); // should find
        // `before` first
        if (l != NULL && l != before && l != after) {
            if ((exact1 ||
                 jl_type_morespecific_no_subtype(
                     (jl_value_t *)l->sig,
                     (jl_value_t *)sig)) && // no subtype since `l->sig >: isect >: sig`
                (exact2 ||
                 jl_type_morespecific_no_subtype(
                     (jl_value_t *)l->sig,
                     (jl_value_t *)type))) { // no subtype since `l->sig >: isect >: type`
                // ok, intersection is already covered by a more specific method
                reorder = 0; // this lack of ordering doesn't matter (unless we delete this
                             // method--then it will)
...
static jl_value_t *check_ambiguous_matches(jl_typemap_t *defs, jl_typemap_entry_t *newentry,
...
static int check_disabled_ambiguous_visitor(jl_typemap_entry_t *oldentry,
                                            struct typemap_intersection_env *closure0)
{
    struct ambiguous_matches_env *closure =
        container_of(closure0, struct ambiguous_matches_env, match);
    ...
    for (i = 0; i < l; i++) {
        jl_typemap_entry_t *before =
            (jl_typemap_entry_t *)jl_array_ptr_ref(closure->shadowed, i);
        ...
        if (jl_types_equal(type, isect2)) {
            ...
            if (msp) {
                ...
                jl_array_ptr_1d_push((jl_array_t *)beforem->resorted,
                                     (jl_value_t *)oldentry);
            }
                                       ...
                                       int depth)
{
    ...
    while (codeinst) {
        if (codeinst->max_world == ~(size_t)0) {
            assert(
                codeinst->min_world - 1 <= max_world &&
...
static int invalidate_backedges(jl_method_instance_t *replaced_linfo, size_t max_world)
{
    ...
    if (backedges) {
        ...
        size_t i, l = jl_array_len(backedges);
        jl_method_instance_t **replaced =
            (jl_method_instance_t **)jl_array_ptr_data(backedges);
        for (i = 0; i < l; i++) {
...
JL_DLLEXPORT void jl_method_instance_add_backedge(jl_method_instance_t *callee,
...
JL_DLLEXPORT void jl_method_table_add_backedge(jl_methtable_t *mt, jl_value_t *typ,
                                               jl_value_t *caller)
{
...
static int invalidate_mt_cache(jl_typemap_entry_t *oldentry, void *closure0)
{
    ...
    if (oldentry->max_world == ~(size_t)0) {
        ...
        if (jl_is_method(env->shadowed)) {
            ...
            jl_typemap_entry_t **d =
                (jl_typemap_entry_t **)jl_array_ptr_data(env->shadowed);
...
static jl_typemap_entry_t *do_typemap_search(jl_methtable_t *mt JL_PROPAGATES_ROOT,
...
JL_DLLEXPORT void jl_method_table_disable(jl_methtable_t *mt, jl_method_t *method)
{
    if (jl_options.incremental && jl_generating_output())
        jl_printf(
            JL_STDERR,
            "WARNING: method deletion during Module precompile may lead to undefined behavior"
            "\n  ** incremental compilation may be fatally broken for this module **\n\n");
    ...
    // Recompute ambiguities (deleting a more specific method might reveal ambiguities that
...
JL_DLLEXPORT void jl_method_table_insert(jl_methtable_t *mt, jl_method_t *method,
                                         jl_tupletype_t *simpletype)
{
    ...
    struct jl_typemap_assoc search = {(jl_value_t *)type, method->primary_world, NULL, 0,
                                      ~(size_t)0};
    ...
    jl_typemap_entry_t *newentry =
        jl_typemap_insert(&mt->defs, (jl_value_t *)mt, (jl_tupletype_t *)type, simpletype,
                          jl_emptysvec, (jl_value_t *)method, 0, &method_defs,
    ...
    if (oldvalue) {
        ...
        for (j = 0; j < n; j++) {
            ...
            for (i = 0; i < l; i++) {
                jl_method_instance_t *mi =
                    (jl_method_instance_t *)jl_svecref(specializations, i);
                if (mi != NULL &&
                    !jl_has_empty_intersection(type, (jl_value_t *)mi->specTypes))
...
static void JL_NORETURN jl_method_error_bare(jl_function_t *f, jl_value_t *args,
                                             size_t world)
{
...
jl_method_instance_t *jl_method_lookup(jl_value_t **args, size_t nargs, int cache,
                                       size_t world)
{
    assert(nargs > 0 && "expected caller to handle this case");
    jl_methtable_t *mt = jl_gf_mtable(args[0]);
    jl_typemap_entry_t *entry = jl_typemap_assoc_exact(mt->cache, args[0], &args[1], nargs,
...
JL_DLLEXPORT jl_value_t *jl_matching_methods(jl_tupletype_t *types, int lim,
                                             int include_ambiguous, size_t world,
                                             size_t *min_valid, size_t *max_valid)
{
    ...
    return ml_matches(mt->defs, 0, types, lim, include_ambiguous, world, min_valid,
                      max_valid);
}
...
jl_code_instance_t *jl_compile_method_internal(jl_method_instance_t *mi, size_t world)
{
    ...
    if (jl_options.compile_enabled == JL_OPTIONS_COMPILE_OFF ||
        ...
        if (jl_is_method(def) && def->unspecialized) {
            ...
            if (unspec && unspec->invoke != NULL) {
                jl_code_instance_t *codeinst = jl_set_method_inferred(
                    mi, (jl_value_t *)jl_any_type, NULL, NULL, 0, 1, ~(size_t)0);
                codeinst->isspecsig = 0;
    ...
    if (!codeinst) {
        ...
        jl_code_instance_t *ucache =
            jl_get_method_inferred(unspec, (jl_value_t *)jl_any_type, 1, ~(size_t)0);
        // ask codegen to make the fptr for unspec
        if (ucache->invoke == NULL)
            jl_generate_fptr_for_unspecialized(ucache);
        if (ucache->invoke != jl_fptr_sparam && ucache->invoke != jl_fptr_interpret_call) {
            return ucache;
        }
        codeinst = jl_set_method_inferred(mi, (jl_value_t *)jl_any_type, NULL, NULL, 0, 1,
                                          ~(size_t)0);
        codeinst->isspecsig = 0;
...
JL_DLLEXPORT jl_value_t *jl_fptr_const_return(jl_value_t *f, jl_value_t **args,
                                              uint32_t nargs, jl_code_instance_t *m)
{
    return m->rettype_const;
}

JL_DLLEXPORT jl_value_t *jl_fptr_args(jl_value_t *f, jl_value_t **args, uint32_t nargs,
                                      jl_code_instance_t *m)
{
    return m->specptr.fptr1(f, args, nargs);
}

JL_DLLEXPORT jl_value_t *jl_fptr_sparam(jl_value_t *f, jl_value_t **args, uint32_t nargs,
                                        jl_code_instance_t *m)
{
...
jl_method_instance_t *jl_get_specialization1(jl_tupletype_t *types JL_PROPAGATES_ROOT,
                                             size_t world, size_t *min_valid,
                                             size_t *max_valid, int mt_cache)
{
    ...
    if (jl_is_datatype(ti)) {
        ...
        if ((jl_value_t *)mt != jl_nothing) {
            ...
            if (mt_cache && ((jl_datatype_t *)ti)->isdispatchtuple) {
                ...
                // used via dispatch later (e.g. because it was hinted via a call to
                // `precompile`)
                JL_LOCK(&mt->writelock);
                ...
                intptr_t nspec =
                    (mt == jl_type_type_mt || mt == jl_nonfunction_mt ? m->nargs + 1 :
                                                                        mt->max_args + 2);
                jl_compilation_sig(ti, env, m, nspec, &newparams);
                tt = (newparams ? jl_apply_tuple_type(newparams) : ti);
                int is_compileable =
                    ((jl_datatype_t *)ti)->isdispatchtuple || jl_isa_compileable_sig(tt, m);
                if (is_compileable) {
                    nf = jl_specializations_get_linfo(m, (jl_value_t *)tt, env);
                }
            }
...
{
    int generating_llvm = jl_options.outputo || jl_options.outputbc ||
                          jl_options.outputunoptbc || jl_options.outputasm;
    // If we are saving ji files (e.g. package pre-compilation or intermediate sysimg build
    // steps), don't bother generating anything since it won't be saved.
    if (jl_rettype_inferred(mi, world, world) == jl_nothing)
        (void)jl_type_infer(mi, world, 1);
    // If we are saving ji files (e.g. package pre-compilation or intermediate sysimg build
    // steps), don't bother generating output in the current environment
    if (generating_llvm) {
...
JL_DLLEXPORT int jl_compile_hint(jl_tupletype_t *types)
{
    ...
    size_t max_valid = ~(size_t)0;
    jl_method_instance_t *mi =
        jl_get_specialization1(types, world, &min_valid, &max_valid, 1);
    if (mi == NULL)
        return 0;
    JL_GC_PROMISE_ROOTED(mi);
    if (jl_generating_output()) {
        jl_compile_now(mi);
        // In addition to full compilation of the compilation-signature, if `types` is more
        // specific (e.g. due to nospecialize), also run inference now on the original
        // `types`, since that may help us guide inference to find additional useful methods
        // that should be compiled
        // ALT: if (jl_is_datatype(types) && ((jl_datatype_t*)types)->isdispatchtuple &&
        // !jl_egal(mi->specTypes, types)) ALT: if (jl_subtype(types, mi->specTypes))
        if (!jl_subtype(mi->specTypes, (jl_value_t *)types)) {
            jl_svec_t *tpenv2 = jl_emptysvec;
            jl_value_t *types2 = NULL;
            JL_GC_PUSH2(&tpenv2, &types2);
            types2 = jl_type_intersection_env((jl_value_t *)types,
                                              (jl_value_t *)mi->def.method->sig, &tpenv2);
            jl_method_instance_t *li2 =
                jl_specializations_get_linfo(mi->def.method, (jl_value_t *)types2, tpenv2);
            JL_GC_POP();
...
{
    jl_method_instance_t *mi =
        jl_get_specialization1(types, world, min_valid, max_valid, 0);
    if (!mi)
        return jl_nothing;
...
STATIC_INLINE jl_value_t *_jl_invoke(jl_value_t *F, jl_value_t **args, uint32_t nargs,
                                     jl_method_instance_t *mfunc, size_t world)
{
...
JL_DLLEXPORT jl_value_t *jl_invoke(jl_value_t *F, jl_value_t **args, uint32_t nargs,
...
STATIC_INLINE int sig_match_fast(jl_value_t *arg1t, jl_value_t **args, jl_value_t **sig,
                                 size_t n)
{
...
STATIC_INLINE jl_method_instance_t *jl_lookup_generic_(jl_value_t *F, jl_value_t **args,
                                                       uint32_t nargs, uint32_t callsite,
                                                       size_t world)
{
#ifdef JL_GF_PROFILE
    ncalls++;
...
#undef LOOP_BODY
    ...
    if (i == 4) {
        ...
        mt = jl_gf_mtable(F);
        entry = jl_typemap_assoc_exact(mt->cache, F, args, nargs, jl_cachearg_offset(mt),
                                       world);
...
jl_method_instance_t *jl_lookup_generic(jl_value_t **args, uint32_t nargs,
...
JL_DLLEXPORT jl_value_t *jl_apply_generic(jl_value_t *F, jl_value_t **args, uint32_t nargs)
{
    size_t world = jl_get_ptls_states()->world_age;
    jl_method_instance_t *mfunc =
        jl_lookup_generic_(F, args, nargs, jl_int32hash_fast(jl_return_address()), world);
    JL_GC_PROMISE_ROOTED(mfunc);
    return _jl_invoke(F, args, nargs, mfunc, world);
}

JL_DLLEXPORT jl_value_t *jl_gf_invoke_lookup(jl_value_t *types JL_PROPAGATES_ROOT,
                                             size_t world)
{
    ...
    jl_typemap_entry_t *entry =
        jl_typemap_assoc_by_type(mt->defs, &search, /*offs*/ 0, /*subtype*/ 1);
    if (entry == NULL)
        return jl_nothing;
    jl_typemap_entry_t *m =
        jl_typemap_morespecific_by_type(entry, (jl_value_t *)types, NULL, world);
    if (m == NULL)
        return jl_nothing;
    return (jl_value_t *)m;
}

static jl_value_t *jl_gf_invoke_by_method(jl_method_t *method, jl_value_t *gf,
...
jl_value_t *jl_gf_invoke(jl_value_t *types0, jl_value_t *gf, jl_value_t **args,
                         size_t nargs)
{
    size_t world = jl_get_ptls_states()->world_age;
                                          ...
                                          jl_value_t **args, size_t nargs)
{
    ...
    if (tm) {
        ...
        if (jl_is_unionall(method->sig)) {
            int sub =
                jl_subtype_matching((jl_value_t *)tt, (jl_value_t *)method->sig, &tpenv);
            assert(sub);
            (void)sub;
        }
        ...
        mfunc = cache_method(NULL, &method->invokes, (jl_value_t *)method, tt, method, 1,
                             tpenv);
...
JL_DLLEXPORT jl_value_t *jl_get_invoke_lambda(jl_typemap_entry_t *entry, jl_value_t *tt)
{
    ...
    JL_GC_PUSH1(&tpenv);
    if (jl_is_unionall(method->sig)) {
        jl_value_t *ti = jl_type_intersection_env(tt, (jl_value_t *)method->sig, &tpenv);
...
jl_function_t *jl_new_generic_function_with_supertype(jl_sym_t *name, jl_module_t *module,
                                                      jl_datatype_t *st)
{
...
JL_DLLEXPORT jl_function_t *jl_get_kwsorter(jl_value_t *ty)
{
    ...
    if (!mt->kwsorter) {
        ...
        if (!mt->kwsorter) {
            ...
            mt->kwsorter =
                jl_new_generic_function_with_supertype(fname, mt->module, jl_function_type);
                              ...
                              struct typemap_intersection_env *closure0)
{
    ...
    */
    ...
    if ((jl_value_t *)meth->resorted != jl_nothing) {
        // first consider adding any more specific matching methods that got put in the
        // table later than us
        ...
        for (j = 0; j < l; j++) {
            jl_typemap_entry_t *prior =
                (jl_typemap_entry_t *)jl_array_ptr_ref(meth->resorted, j);
            ...
            closure->match.ti =
                jl_type_intersection_env_s((jl_value_t *)closure->match.type,
            ...
            if (closure->match.ti != (jl_value_t *)jl_bottom_type) {
                ...
                if (!closure->include_ambiguous &&
                    (jl_value_t *)meth->ambig != jl_nothing) {
                    size_t j, l = jl_array_len(meth->ambig);
                    for (j = 0; j < l; j++) {
                        jl_typemap_entry_t *mambig =
                            (jl_typemap_entry_t *)jl_array_ptr_ref(meth->ambig, j);
                        jl_value_t *sig2 = (jl_value_t *)mambig->sig;
                        // TODO: involve and update world
                        if (jl_subtype(closure->match.ti, sig2)) {
                            if (!jl_type_morespecific((jl_value_t *)prior->sig,
                                                      (jl_value_t *)sig2))
                                break;
                        }
                    }
                ...
                ptrhash_put(&closure->visited, (void *)prior,
                            (void *)prior); // won't need to consider it again
            }
        }
        JL_GC_POP();
        closure->match.ti = ti;
    ...
    if (return_this_match && !closure->include_ambiguous &&
        (jl_value_t *)meth->ambig != jl_nothing) {
    ...
    if (return_this_match && closure->lim >= 0) {
        ...
        int i;
        for (i = 0; i < len; i++) {
            jl_method_t *prior_match =
                (jl_method_t *)jl_svecref(jl_array_ptr_ref(closure->t, i), 2);
            jl_value_t *sig2 = (jl_value_t *)prior_match->sig;
            if (closure->include_ambiguous && (jl_value_t *)meth->ambig != jl_nothing) {
                // check that prior sig is not actually just ambiguous with this--may need
                // to include both
                size_t j, l = jl_array_len(meth->ambig);
                for (j = 0; j < l; j++) {
                    jl_typemap_entry_t *mambig =
    ...
    if (closure->include_ambiguous && (jl_value_t *)meth->ambig != jl_nothing) {
        ...
        for (j = 0; j < l; j++) {
            ...
            closure->match.env = jl_emptysvec;
            closure->match.ti =
                jl_type_intersection_env((jl_value_t *)closure->match.type,
                                         (jl_value_t *)prior->sig, &closure->match.env);
            if (closure->match.ti != (jl_value_t *)jl_bottom_type) {
                if (!ml_matches_visitor(prior, closure0))
                    return 0; // enough matches already--fast terminate
...
static jl_value_t *ml_matches(jl_typemap_t *defs, int offs, jl_tupletype_t *type, int lim,
                              int include_ambiguous, size_t world, size_t *min_valid,
                              size_t *max_valid)
{
    ...
    if (((jl_datatype_t *)unw)->isdispatchtuple) {
        jl_typemap_entry_t *ml =
            jl_typemap_assoc_by_type(defs, &search, offs, /*subtype*/ 1);
