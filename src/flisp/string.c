...
value_t fl_string(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    FOR_ARGS(i, 0, arg, args) { fl_print(fl_ctx, s, args[i]); }
...
static value_t mem_find_byte(fl_context_t *fl_ctx, char *s, char c, size_t start,
                             size_t len)
{
...
value_t fl_numbertostring(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    if (isfixnum(n))
        ...
        type_error(fl_ctx, "number->string", "integer", n);
    else
        num = conv_to_uint64(cp_data((cprim_t *)ptr(n)), cp_numtype((cprim_t *)ptr(n)));
...
static const builtinspec_t stringfunc_info[] = {{"string", fl_string},
                                                {"string?", fl_stringp},
