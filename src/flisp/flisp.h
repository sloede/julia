...
#ifdef MEMDEBUG2
#define ismanaged(ctx, v) \
    (!issymbol(v) && !isfixnum(v) && ((v) > (N_OPCODES << 3)) && !iscbuiltin(ctx, v))
#else
#define ismanaged(ctx, v)                             \
    ((((unsigned char *)ptr(v)) >= ctx->fromspace) && \
...
#define iscbuiltin(ctx, x) \
    (iscvalue(x) && (cv_class((cvalue_t *)ptr(x)) == ctx->builtintype))
...
#define FL_TRY_EXTERN(fl_ctx)    \
    ...
    int l__tr, l__ca;            \
    fl_savestate(fl_ctx, &_ctx); \
    fl_ctx->exc_ctx = &_ctx;     \
    if (!fl_setjmp(_ctx.buf))    \
        for (l__tr = 1; l__tr; l__tr = 0, (void)(fl_ctx->exc_ctx = fl_ctx->exc_ctx->prev))

#define FL_CATCH_EXTERN(fl_ctx) \
    else for (l__ca = 1; l__ca; l__ca = 0, fl_restorestate(fl_ctx, &_ctx))

#if defined(_OS_WINDOWS_)
__declspec(noreturn) void lerrorf(fl_context_t *fl_ctx, value_t e, const char *format, ...);
__declspec(noreturn) void lerror(fl_context_t *fl_ctx, value_t e,
                                 const char *msg) JL_NOTSAFEPOINT;
__declspec(noreturn) void fl_raise(fl_context_t *fl_ctx, value_t e);
__declspec(noreturn) void type_error(fl_context_t *fl_ctx, const char *fname,
                                     const char *expected, value_t got);
__declspec(noreturn) void bounds_error(fl_context_t *fl_ctx, const char *fname, value_t arr,
                                       value_t ind);
#else
...
void lerror(fl_context_t *fl_ctx, value_t e, const char *msg)
    __attribute__((__noreturn__)) JL_NOTSAFEPOINT;
...
#define cptr(v) (iscprim(v) ? cp_data((cprim_t *)ptr(v)) : cv_data((cvalue_t *)ptr(v)))
...
static inline void argcount(fl_context_t *fl_ctx, const char *fname, uint32_t nargs,
                            uint32_t c)
{
    if (__unlikely(nargs != c))
        lerrorf(fl_ctx, fl_ctx->ArgError, "%s: too %s arguments", fname,
                nargs < c ? "few" : "many");
}
