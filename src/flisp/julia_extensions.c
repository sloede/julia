...
static int is_wc_cat_id_start(uint32_t wc, utf8proc_category_t cat)
{
    return (
        ...
        (wc >= 0x1d6c1 && // variants of \nabla and \partial
         (wc == 0x1d6c1 || wc == 0x1d6db || wc == 0x1d6fb || wc == 0x1d715 ||
          wc == 0x1d735 || wc == 0x1d74f || wc == 0x1d76f || wc == 0x1d789 ||
          wc == 0x1d7a9 || wc == 0x1d7c3)) ||

        // super- and subscript +-=()
        (wc >= 0x207a && wc <= 0x207e) || (wc >= 0x208a && wc <= 0x208e) ||
        ...
        (wc >= 0x1D7CE &&
         wc <= 0x1D7E1)); // 𝟎 through 𝟗 (inclusive), 𝟘 through 𝟡 (inclusive)
}
...
static int never_id_char(uint32_t wc)
{
    ...
    return (
        ...
        (wc < 0xff && cat >= UTF8PROC_CATEGORY_PD && cat <= UTF8PROC_CATEGORY_PO) ||
...
value_t fl_julia_strip_op_suffix(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    if (!op[i])
        return args[0]; // no suffix to strip
    if (!i)
        return args[0]; // only suffix chars --- might still be a valid identifier
...
value_t fl_julia_underscore_symbolp(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    char *op = symbol_name(fl_ctx, args[0]);
    if (*op == '\0')
        return fl_ctx->F; // return false for empty symbol
    while (*op == '_')
        ++op;
...
static char *normalize(fl_context_t *fl_ctx, char *s)
{
    ...
    if (newlen > fl_ctx->jlbuflen) {
        ...
        if (!fl_ctx->jlbuf)
            lerror(fl_ctx, fl_ctx->OutOfMemoryError, "error allocating UTF8 buffer");
    }
    result = utf8proc_decompose_custom((uint8_t *)s, 0, (int32_t *)fl_ctx->jlbuf, result,
                                       (utf8proc_option_t)options, jl_charmap_map, NULL);
    if (result < 0)
        goto error;
    result =
        utf8proc_reencode((int32_t *)fl_ctx->jlbuf, result, (utf8proc_option_t)options);
    if (result < 0)
        goto error;
