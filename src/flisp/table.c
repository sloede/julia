...
value_t fl_table_foldl(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    for (i = 0; i < n; i += 2) {
        if (table[i + 1] != HT_NOTFOUND) {
            zero = fl_applyn(fl_ctx, 3, f, (value_t)table[i], (value_t)table[i + 1], zero);
            // reload pointer
...
static const builtinspec_t tablefunc_info[] = {{"table", fl_table},
                                               {"table?", fl_tablep},
                                               {"put!", fl_table_put},
                                               {"get", fl_table_get},
                                               {"has?", fl_table_has},
...
void table_init(fl_context_t *fl_ctx)
{
    ...
    fl_ctx->tabletype =
        define_opaque_type(fl_ctx->tablesym, sizeof(htable_t), &fl_ctx->table_vtable, NULL);
