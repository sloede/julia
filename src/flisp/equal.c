...
static void eq_union(fl_context_t *fl_ctx, htable_t *table, value_t a, value_t b, value_t c,
                     value_t cb)
{
...
static value_t bounded_compare(fl_context_t *fl_ctx, value_t a, value_t b, int bound,
                               int eq);
static value_t cyc_compare(fl_context_t *fl_ctx, value_t a, value_t b, htable_t *table,
                           int eq);

static value_t bounded_vector_compare(fl_context_t *fl_ctx, value_t a, value_t b, int bound,
                                      int eq)
{
    ...
    for (i = 0; i < m; i++) {
        value_t d =
            bounded_compare(fl_ctx, vector_elt(a, i), vector_elt(b, i), bound - 1, eq);
        if (d == fl_ctx->NIL || numval(d) != 0)
            return d;
    }
    if (la < lb)
        return fixnum(-1);
...
static value_t cyc_vector_compare(fl_context_t *fl_ctx, value_t a, value_t b,
                                  htable_t *table, int eq)
{
    size_t la = vector_size(a);
    size_t lb = vector_size(b);
    size_t m, i;
...
static uintptr_t bounded_hash(fl_context_t *fl_ctx, value_t a, int bound, int *oob)
{
    *oob = 0;
    ...
    switch (tg) {
    case TAG_NUM:
    case TAG_NUM1: u.d = (double)numval(a); return doublehash(u.i64);
    case TAG_FUNCTION:
        if (uintval(a) > N_BUILTINS)
            return bounded_hash(fl_ctx, ((function_t *)ptr(a))->bcode, bound, oob);
        return inthash(a);
    case TAG_SYM: return ((symbol_t *)ptr(a))->hash;
    case TAG_CPRIM:
        cp = (cprim_t *)ptr(a);
        data = cp_data(cp);
