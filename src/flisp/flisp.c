...
static const char *const builtin_names[] = {
    ...
    // predicates
    "eq?", "eqv?", "equal?", "atom?", "not", "null?", "boolean?", "symbol?", "number?",
    "bound?", "pair?", "builtin?", "vector?", "fixnum?", "function?",
...
#define FL_CATCH(fl_ctx)                                                          \
    else for (l__ca = 1; l__ca; l__ca = 0, fl_ctx->lasterror = fl_ctx->NIL,       \
                                fl_ctx->throwing_frame = 0, fl_ctx->SP = _ctx.sp, \
...
{
    fl_raise(fl_ctx, fl_listn(fl_ctx, 4, fl_ctx->TypeError, symbol(fl_ctx, fname),
                              symbol(fl_ctx, expected), got));
}

void bounds_error(fl_context_t *fl_ctx, const char *fname, value_t arr, value_t ind)
{
    fl_raise(fl_ctx,
             fl_listn(fl_ctx, 4, fl_ctx->BoundsError, symbol(fl_ctx, fname), arr, ind));
}
...
#define CHECK_ALIGN8(p)                     \
    assert((((uintptr_t)(p)) & 0x7) == 0 && \
           "flisp requires malloc to return 8-aligned pointers")
...
value_t fl_gensym(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    char *n = uint2str(fl_ctx->gsname[fl_ctx->gsnameno] + 1, sizeof(fl_ctx->gsname[0]) - 1,
                       fl_ctx->gensym_ctr++, 10);
    *(--n) = 'g';
...
char *symbol_name(fl_context_t *fl_ctx, value_t v)
{
    ...
    if (ismanaged(fl_ctx, v)) {
        ...
        fl_ctx->gsnameno = 1 - fl_ctx->gsnameno;
        char *n = uint2str(fl_ctx->gsname[fl_ctx->gsnameno] + 1,
                           sizeof(fl_ctx->gsname[0]) - 1, gs->id, 10);
        *(--n) = 'g';
        return n;
    }
#else
...
{
    if (n == 0)
        return fl_ctx->the_empty_vector;
...
void gc(fl_context_t *fl_ctx, int mustgrow)
{
    ...
    while (rs) {
        for (i = 0; i < rs->backrefs.size; i++)
            rs->backrefs.table[i] =
                (void *)relocate(fl_ctx, (value_t)rs->backrefs.table[i]);
    ...
    printf("GC: found %d/%d live conses\n",
           (fl_ctx->curheap - fl_ctx->tospace) / sizeof(cons_t),
           fl_ctx->heapsize / sizeof(cons_t));
#endif
    ...
    if (fl_ctx->gc_grew ||
        ...
        if (fl_ctx->gc_grew) {
            fl_ctx->heapsize *= 2;
            temp = bitvector_resize(fl_ctx->consflags, 0, fl_ctx->heapsize / sizeof(cons_t),
                                    1);
            if (temp == NULL)
                fl_raise(fl_ctx, fl_ctx->memory_exception_value);
...
static value_t do_trycatch(fl_context_t *fl_ctx)
{
    ...
    FL_TRY(fl_ctx) { v = apply_cl(fl_ctx, 0); }
    FL_CATCH(fl_ctx)
    {
...
*/
static uint32_t process_keys(fl_context_t *fl_ctx, value_t kwtable, uint32_t nreq,
                             uint32_t nkw, uint32_t nopt, uint32_t bp, uint32_t nargs,
                             int va)
{
    uintptr_t n;
    ...
    do {
        ...
        if (vector_elt(kwtable, x) == v) {
            ...
            lerrorf(fl_ctx, fl_ctx->ArgError, "unsupported keyword %s",
                    symbol_name(fl_ctx, v));
        }
        i++;
        if (i >= nargs)
...
no_kw:
    ...
    if (nrestargs)
        memmove(&fl_ctx->Stack[bp + ntot], &fl_ctx->Stack[bp + i],
...
#define GET_INT32(a)                                                                       \
    ((int32_t)((((int32_t)a[0]) << 0) | (((int32_t)a[1]) << 8) | (((int32_t)a[2]) << 16) | \
               (((int32_t)a[3]) << 24)))
#define GET_INT16(a) ((int16_t)((((int16_t)a[0]) << 0) | (((int16_t)a[1]) << 8)))
...
dispatch:
    ...
    do_vargc:
                ...
                if (fl_apply_v != UNBOUND)
                    PUSH(fl_ctx, fl_ctx->T);
                else
                    PUSH(fl_ctx, fl_ctx->F);
                NEXT_OP;
                OP(OP_DUP) fl_ctx->SP++;
                fl_ctx->Stack[fl_ctx->SP - 1] = fl_ctx->Stack[fl_ctx->SP - 2];
                NEXT_OP;
                OP(OP_POP) POPN(fl_ctx, 1);
                NEXT_OP;
                OP(OP_TCALL)
                n = *ip++; // nargs
    do_tcall:
                ...
                if (tag(fl_apply_func) == TAG_FUNCTION) {
                    if (fl_apply_func > (N_BUILTINS << 3)) {
                        ...
                        default: op = (uint8_t)i; goto dispatch;
                        }
#endif
                        }
                    }
                    ...
                    s = fl_ctx->SP;
                    fl_apply_v = ((builtin_t)(uintptr_t)(((void **)ptr(fl_apply_func))[3]))(
                        fl_ctx, &fl_ctx->Stack[fl_ctx->SP - n], n);
                    fl_ctx->SP = s - n;
    ...
    do_call:
                ...
                if (fl_ctx->curr_frame == top_frame)
                    return fl_apply_v;
                fl_ctx->SP -= (4 + nargs);
                ...
                else if (!leafp(fl_ctx->Stack[fl_ctx->SP - 2]) ||
                         !leafp(fl_ctx->Stack[fl_ctx->SP - 1])) {
                    fl_apply_v = fl_ctx->F;
                }
                else {
                    fl_apply_v = (compare_(fl_ctx, fl_ctx->Stack[fl_ctx->SP - 2],
                                           fl_ctx->Stack[fl_ctx->SP - 1], 1) == 0 ?
                                      fl_ctx->T :
                                      fl_ctx->F);
                }
                fl_ctx->Stack[fl_ctx->SP - 2] = fl_apply_v;
                POPN(fl_ctx, 1);
                NEXT_OP;
                OP(OP_EQUAL)
                if (fl_ctx->Stack[fl_ctx->SP - 2] == fl_ctx->Stack[fl_ctx->SP - 1]) {
                ...
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    (iscons(fl_ctx->Stack[fl_ctx->SP - 1]) ? fl_ctx->T : fl_ctx->F);
                NEXT_OP;
                OP(OP_ATOMP)
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    (iscons(fl_ctx->Stack[fl_ctx->SP - 1]) ? fl_ctx->F : fl_ctx->T);
                NEXT_OP;
                OP(OP_NOT)
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    ((fl_ctx->Stack[fl_ctx->SP - 1] == fl_ctx->F) ? fl_ctx->T : fl_ctx->F);
                NEXT_OP;
                OP(OP_NULLP)
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    ((fl_ctx->Stack[fl_ctx->SP - 1] == fl_ctx->NIL) ? fl_ctx->T :
                                                                      fl_ctx->F);
                NEXT_OP;
                OP(OP_BOOLEANP)
                fl_apply_v = fl_ctx->Stack[fl_ctx->SP - 1];
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    ((fl_apply_v == fl_ctx->T || fl_apply_v == fl_ctx->F) ? fl_ctx->T :
                                                                            fl_ctx->F);
                NEXT_OP;
                OP(OP_SYMBOLP)
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    (issymbol(fl_ctx->Stack[fl_ctx->SP - 1]) ? fl_ctx->T : fl_ctx->F);
                NEXT_OP;
                OP(OP_NUMBERP)
                fl_apply_v = fl_ctx->Stack[fl_ctx->SP - 1];
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    (fl_isnumber(fl_ctx, fl_apply_v) ? fl_ctx->T : fl_ctx->F);
                NEXT_OP;
                OP(OP_FIXNUMP)
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    (isfixnum(fl_ctx->Stack[fl_ctx->SP - 1]) ? fl_ctx->T : fl_ctx->F);
                NEXT_OP;
                OP(OP_BOUNDP)
                sym = tosymbol(fl_ctx, fl_ctx->Stack[fl_ctx->SP - 1], "bound?");
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    ((sym->binding == UNBOUND) ? fl_ctx->F : fl_ctx->T);
                NEXT_OP;
                OP(OP_BUILTINP)
                fl_apply_v = fl_ctx->Stack[fl_ctx->SP - 1];
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    (isbuiltin(fl_apply_v) || iscbuiltin(fl_ctx, fl_apply_v)) ? fl_ctx->T :
                                                                                fl_ctx->F;
                NEXT_OP;
                OP(OP_FUNCTIONP)
                fl_apply_v = fl_ctx->Stack[fl_ctx->SP - 1];
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    ((tag(fl_apply_v) == TAG_FUNCTION &&
                      (uintval(fl_apply_v) <= OP_ASET || fl_apply_v > (N_BUILTINS << 3))) ||
                     iscbuiltin(fl_ctx, fl_apply_v)) ?
                        fl_ctx->T :
                        fl_ctx->F;
                NEXT_OP;
                OP(OP_VECTORP)
                fl_ctx->Stack[fl_ctx->SP - 1] =
                    (isvector(fl_ctx->Stack[fl_ctx->SP - 1]) ? fl_ctx->T : fl_ctx->F);
                NEXT_OP;
            ...
            fl_ctx->curheap += sizeof(cons_t);
                ...
                OP(OP_CAR)
                fl_apply_v = fl_ctx->Stack[fl_ctx->SP - 1];
                if (!iscons(fl_apply_v))
                    type_error(fl_ctx, "car", "cons", fl_apply_v);
                fl_ctx->Stack[fl_ctx->SP - 1] = car_(fl_apply_v);
                NEXT_OP;
                OP(OP_CDR)
                fl_apply_v = fl_ctx->Stack[fl_ctx->SP - 1];
                if (!iscons(fl_apply_v))
                    type_error(fl_ctx, "cdr", "cons", fl_apply_v);
    ...
    add_ovf:
                        fl_apply_v =
                            fl_add_any(fl_ctx, &fl_ctx->Stack[i], fl_ctx->SP - i, s);
                        break;
                    }
                }
                if (i == fl_ctx->SP)
                ...
                NEXT_OP;
                OP(OP_ADD2)
                if (bothfixnums(fl_ctx->Stack[fl_ctx->SP - 1],
                                fl_ctx->Stack[fl_ctx->SP - 2])) {
                    s = numval(fl_ctx->Stack[fl_ctx->SP - 1]) +
                        numval(fl_ctx->Stack[fl_ctx->SP - 2]);
                    if (fits_fixnum(s))
                        fl_apply_v = fixnum(s);
                    else
    ...
    apply_sub:
                ...
                fl_ctx->Stack[i + 1] =
                    fl_neg(fl_ctx, fl_add_any(fl_ctx, &fl_ctx->Stack[i], n, 0));
    ...
    do_neg:
                if (isfixnum(fl_ctx->Stack[fl_ctx->SP - 1]))
                    fl_ctx->Stack[fl_ctx->SP - 1] =
                        fixnum(-numval(fl_ctx->Stack[fl_ctx->SP - 1]));
                else
                    fl_ctx->Stack[fl_ctx->SP - 1] =
                        fl_neg(fl_ctx, fl_ctx->Stack[fl_ctx->SP - 1]);
                NEXT_OP;
                OP(OP_SUB2)
    do_sub2:
                if (bothfixnums(fl_ctx->Stack[fl_ctx->SP - 2],
                                fl_ctx->Stack[fl_ctx->SP - 1])) {
                    s = numval(fl_ctx->Stack[fl_ctx->SP - 2]) -
                        numval(fl_ctx->Stack[fl_ctx->SP - 1]);
                    if (fits_fixnum(s))
                        fl_apply_v = fixnum(s);
                    else
                        fl_apply_v = mk_ptrdiff(fl_ctx, s);
    ...
    apply_mul:
                ...
                for (; i < fl_ctx->SP; i++) {
                    if (isfixnum(fl_ctx->Stack[i])) {
                        ...
                        fl_apply_v = fl_mul_any(fl_ctx, &fl_ctx->Stack[i], fl_ctx->SP - i,
                                                fl_apply_accum);
                        break;
                    }
                }
    ...
    apply_div:
                ...
                if (n == 1) {
                    fl_ctx->Stack[fl_ctx->SP - 1] =
                        fl_div2(fl_ctx, fixnum(1), fl_ctx->Stack[i]);
                }
                else {
                    if (n > 2) {
                        PUSH(fl_ctx, fl_ctx->Stack[i]);
                ...
                if (bothfixnums(fl_apply_v, fl_apply_e))
                    ...
                    fl_apply_v =
                        (!numeric_compare(fl_ctx, fl_apply_v, fl_apply_e, 1, 0, "=")) ?
                            fl_ctx->T :
                            fl_ctx->F;
                POPN(fl_ctx, 1);
                fl_ctx->Stack[fl_ctx->SP - 1] = fl_apply_v;
                ...
                if (bothfixnums(fl_ctx->Stack[fl_ctx->SP - 2],
                                fl_ctx->Stack[fl_ctx->SP - 1])) {
                    fl_apply_v = (numval(fl_ctx->Stack[fl_ctx->SP - 2]) <
                                  numval(fl_ctx->Stack[fl_ctx->SP - 1])) ?
                                     fl_ctx->T :
                                     fl_ctx->F;
                }
                else {
                    fl_apply_v = (numval(fl_compare(fl_ctx, fl_ctx->Stack[fl_ctx->SP - 2],
                                                    fl_ctx->Stack[fl_ctx->SP - 1])) < 0) ?
                                     fl_ctx->T :
                                     fl_ctx->F;
                }
                POPN(fl_ctx, 1);
                ...
                OP(OP_COMPARE)
                fl_ctx->Stack[fl_ctx->SP - 2] =
                    compare_(fl_ctx, fl_ctx->Stack[fl_ctx->SP - 2],
                             fl_ctx->Stack[fl_ctx->SP - 1], 0);
                POPN(fl_ctx, 1);
                NEXT_OP;
    ...
    apply_vector:
                ...
                if (n) {
                    memcpy(&vector_elt(fl_apply_v, 0), &fl_ctx->Stack[fl_ctx->SP - n],
                           n * sizeof(value_t));
                    POPN(fl_ctx, n);
                }
                PUSH(fl_ctx, fl_apply_v);
                ...
                if (isvector(fl_apply_e)) {
                    ...
                    if ((unsigned)i >= vector_size(fl_apply_e))
                        bounds_error(fl_ctx, "aset!", fl_apply_v,
                                     fl_ctx->Stack[fl_ctx->SP - 1]);
                    vector_elt(fl_apply_e, i) =
                        (fl_apply_v = fl_ctx->Stack[fl_ctx->SP - 1]);
                }
                ...
                OP(OP_LOADT) PUSH(fl_ctx, fl_ctx->T);
                NEXT_OP;
                OP(OP_LOADF) PUSH(fl_ctx, fl_ctx->F);
                NEXT_OP;
                OP(OP_LOADNIL) PUSH(fl_ctx, fl_ctx->NIL);
                NEXT_OP;
                OP(OP_LOAD0) PUSH(fl_ctx, fixnum(0));
                NEXT_OP;
                OP(OP_LOAD1) PUSH(fl_ctx, fixnum(1));
                NEXT_OP;
                OP(OP_LOADI8) s = (int8_t)*ip++;
                PUSH(fl_ctx, fixnum(s));
                NEXT_OP;
                OP(OP_LOADV)
                fl_apply_v = fn_vals(fl_ctx->Stack[bp - 1]);
                assert(*ip < vector_size(fl_apply_v));
                fl_apply_v = vector_elt(fl_apply_v, *ip);
                ip++;
                PUSH(fl_ctx, fl_apply_v);
                NEXT_OP;
                OP(OP_LOADVL)
                fl_apply_v = fn_vals(fl_ctx->Stack[bp - 1]);
                fl_apply_v = vector_elt(fl_apply_v, GET_INT32(ip));
                ip += 4;
                PUSH(fl_ctx, fl_apply_v);
                NEXT_OP;
                OP(OP_LOADGL)
    ...
    do_setg:
            ...
            fl_ctx->curheap += (4 * sizeof(value_t));
                ...
                s = GET_INT32(ip);
                ip += 4;
                nargs = process_keys(fl_ctx, fl_apply_v, i, n, llabs(s) - (i + n), bp,
                                     nargs, s < 0);
                NEXT_OP;
    ...
    static value_t fl_function_code(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
    {
        ...
        value_t v = args[0];
        if (!isclosure(v))
            type_error(fl_ctx, "function:code", "function", v);
        return fn_bcode(v);
    }
    static value_t fl_function_vals(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
    {
        ...
        value_t v = args[0];
        if (!isclosure(v))
            type_error(fl_ctx, "function:vals", "function", v);
        return fn_vals(v);
    }
    static value_t fl_function_env(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
    {
        ...
        value_t v = args[0];
        if (!isclosure(v))
            type_error(fl_ctx, "function:env", "function", v);
        return fn_env(v);
    }
    static value_t fl_function_name(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
    {
        ...
        value_t v = args[0];
        if (!isclosure(v))
            type_error(fl_ctx, "function:name", "function", v);
        return fn_name(v);
    }
    ...
    value_t fl_stacktrace(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
    {
        ...
        return _stacktrace(fl_ctx, fl_ctx->throwing_frame ? fl_ctx->throwing_frame :
                                                            fl_ctx->curr_frame);
    }
    ...
    value_t fl_map1(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
    {
        ...
        if (nargs == 2) {
            ...
            size_t i;
            while (fl_ctx->SP + nargs + 1 > fl_ctx->N_STACK)
                grow_stack(fl_ctx);
            PUSH(fl_ctx, fl_ctx->Stack[argSP]);
            for (i = 1; i < nargs; i++) {
    ...
    static const builtinspec_t core_builtin_info[] = {{"function", fl_function},
    ...
    fl_ctx->tospace = (unsigned char *)LLT_ALLOC(fl_ctx->heapsize);
        ...
        fl_ctx->LAMBDA = symbol(fl_ctx, "lambda");
        fl_ctx->FUNCTION = symbol(fl_ctx, "function");
        fl_ctx->QUOTE = symbol(fl_ctx, "quote");
        fl_ctx->TRYCATCH = symbol(fl_ctx, "trycatch");
        fl_ctx->BACKQUOTE = symbol(fl_ctx, "quasiquote");
        fl_ctx->COMMA = symbol(fl_ctx, "unquote");
        fl_ctx->COMMAAT = symbol(fl_ctx, "unquote-splicing");
        fl_ctx->COMMADOT = symbol(fl_ctx, "unquote-nsplicing");
        fl_ctx->IOError = symbol(fl_ctx, "io-error");
        fl_ctx->ParseError = symbol(fl_ctx, "parse-error");
        fl_ctx->TypeError = symbol(fl_ctx, "type-error");
        fl_ctx->ArgError = symbol(fl_ctx, "arg-error");
        fl_ctx->UnboundError = symbol(fl_ctx, "unbound-error");
        fl_ctx->KeyError = symbol(fl_ctx, "key-error");
        fl_ctx->OutOfMemoryError = symbol(fl_ctx, "memory-error");
        ...
        fl_ctx->symbolsym = symbol(fl_ctx, "symbol");
        fl_ctx->fixnumsym = symbol(fl_ctx, "fixnum");
        fl_ctx->vectorsym = symbol(fl_ctx, "vector");
        fl_ctx->builtinsym = symbol(fl_ctx, "builtin");
        fl_ctx->booleansym = symbol(fl_ctx, "boolean");
        fl_ctx->nullsym = symbol(fl_ctx, "null");
        fl_ctx->definesym = symbol(fl_ctx, "define");
        fl_ctx->defmacrosym = symbol(fl_ctx, "define-macro");
        fl_ctx->forsym = symbol(fl_ctx, "for");
        fl_ctx->setqsym = symbol(fl_ctx, "set!");
        fl_ctx->evalsym = symbol(fl_ctx, "eval");
        fl_ctx->vu8sym = symbol(fl_ctx, "vu8");
        fl_ctx->fnsym = symbol(fl_ctx, "fn");
        fl_ctx->nulsym = symbol(fl_ctx, "nul");
        fl_ctx->alarmsym = symbol(fl_ctx, "alarm");
        fl_ctx->backspacesym = symbol(fl_ctx, "backspace");
        fl_ctx->tabsym = symbol(fl_ctx, "tab");
        fl_ctx->linefeedsym = symbol(fl_ctx, "linefeed");
        fl_ctx->vtabsym = symbol(fl_ctx, "vtab");
        fl_ctx->pagesym = symbol(fl_ctx, "page");
        fl_ctx->returnsym = symbol(fl_ctx, "return");
        fl_ctx->escsym = symbol(fl_ctx, "esc");
        fl_ctx->spacesym = symbol(fl_ctx, "space");
        fl_ctx->deletesym = symbol(fl_ctx, "delete");
        fl_ctx->newlinesym = symbol(fl_ctx, "newline");
        fl_ctx->tsym = symbol(fl_ctx, "t");
        fl_ctx->Tsym = symbol(fl_ctx, "T");
        fl_ctx->fsym = symbol(fl_ctx, "f");
        fl_ctx->Fsym = symbol(fl_ctx, "F");
        ...
        set(fl_ctx->printwidthsym = symbol(fl_ctx, "*print-width*"),
            fixnum(fl_ctx->SCR_WIDTH));
    ...
    set(symbol(fl_ctx, "*os-name*"), symbol(fl_ctx, "unknown"));
        ...
        if (uv_exepath(exename, &exe_size) == 0) {
            setc(symbol(fl_ctx, "*install-dir*"),
                 cvalue_static_cstring(fl_ctx, strdup(dirname(exename))));
        }

        fl_ctx->memory_exception_value =
            fl_list2(fl_ctx, fl_ctx->OutOfMemoryError,
    ...
    int fl_load_system_image(fl_context_t *fl_ctx, value_t sys_image_iostream)
    {
        ...
        FL_TRY(fl_ctx)
        {
            while (1) {
                ...
                if (ios_eof(value2c(ios_t *, fl_ctx->Stack[fl_ctx->SP - 1])))
                    break;
                if (isfunction(e)) {
