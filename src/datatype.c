...
JL_DLLEXPORT jl_methtable_t *jl_new_method_table(jl_sym_t *name, jl_module_t *module)
{
    ...
    jl_methtable_t *mt =
        (jl_methtable_t *)jl_gc_alloc(ptls, sizeof(jl_methtable_t), jl_methtable_type);
...
JL_DLLEXPORT jl_typename_t *jl_new_typename_in(jl_sym_t *name, jl_module_t *module)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_typename_t *tn =
        (jl_typename_t *)jl_gc_alloc(ptls, sizeof(jl_typename_t), jl_typename_type);
    tn->name = name;
    tn->module = module;
    tn->wrapper = NULL;
...
{
    return jl_new_datatype((jl_sym_t *)name, module, super, parameters, jl_emptysvec,
                           jl_emptysvec, 1, 0, 0);
}
...
jl_datatype_t *jl_new_uninitialized_datatype(void)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_datatype_t *t =
        (jl_datatype_t *)jl_gc_alloc(ptls, sizeof(jl_datatype_t), jl_datatype_type);
    t->hash = 0;
    t->hasfreetypevars = 0;
    t->isdispatchtuple = 0;
    t->isbitstype = 0;
...
static jl_datatype_layout_t *jl_get_layout(uint32_t nfields, uint32_t npointers,
                                           uint32_t alignment, int haspadding,
                                           ...
                                           uint32_t pointers[]) JL_NOTSAFEPOINT
{
    ...
    // TODO: lots of these are the same--take advantage of the fact these are immutable to
    // combine them
...
{
    return (!d->abstract && jl_datatype_size(d) == 0 && d != jl_symbol_type &&
            d->name != jl_array_typename && d->isconcretetype && !d->mutabl);
}
...
static unsigned union_isinlinable(jl_value_t *ty, int pointerfree, size_t *nbytes,
                                  size_t *align) JL_NOTSAFEPOINT
{
    ...
    if (jl_is_datatype(ty) && jl_datatype_isinlinealloc(ty) &&
        (!pointerfree || ((jl_datatype_t *)ty)->layout->npointers == 0)) {
...
int jl_pointer_egal(jl_value_t *t)
{
    if (t == (jl_value_t *)jl_any_type)
        return 0; // when setting up the initial types, jl_is_type_type gets confused about
                  // this
    if (t == (jl_value_t *)jl_symbol_type)
        return 1;
    if (jl_is_mutable_datatype(t) && // excludes abstract types
        t !=
            (jl_value_t *)jl_string_type && // technically mutable, but compared by contents
...
static int references_name(jl_value_t *p, jl_typename_t *name,
                           int affects_layout) JL_NOTSAFEPOINT
{
...
static void throw_ovf(int should_malloc, void *desc, jl_datatype_t *st, int offset)
{
    if (should_malloc)
        free(desc);
    jl_errorf("type %s has field offset %d that exceeds the page size",
              jl_symbol_name(st->name->name), offset);
}
...
void jl_compute_field_offsets(jl_datatype_t *st)
{
    ...
    if (st->types == NULL || st->name->wrapper == NULL)
        return;
    if ((jl_is_tuple_type(st) || jl_is_namedtuple_type(st)) &&
        !jl_is_concrete_type((jl_value_t *)st))
        return;
    ...
    if (st == w && st->layout) {
        // this check allows us to force re-computation of the layout for some types during
        // init
        st->layout = NULL;
        st->size = 0;
        st->zeroinit = 0;
    ...
    if (w->layout) {
        ...
        if (st == jl_symbol_type || st == jl_string_type) {
            ...
            static const jl_datatype_layout_t opaque_ptr_layout = {0, 1, -1, sizeof(void *),
                                                                   0, 0};
        ...
        // compute a conservative estimate of whether there could exist an instance of a
        // subtype of this
        for (i = 0; st->has_concrete_subtype && i < st->ninitialized; i++) {
            ...
            if (fld == jl_bottom_type)
                ...
                st->has_concrete_subtype =
                    !jl_is_datatype(fld) || ((jl_datatype_t *)fld)->has_concrete_subtype;
        }
        ...
        for (i = 0; i < nfields; i++) {
            ...
            if (jl_islayout_inline(fld, &fsz, &al)) { // aka jl_datatype_isinlinealloc
                ...
                if (!jl_pointer_egal(fld)) {
                    // this somewhat poorly named flag says whether some of the bits can be
                    // non-unique
                    haspadding = 1;
                }
            }
                                            ...
                                            int abstract, int mutabl, int ninitialized)
{
    ...
    if (jl_is_typename(name)) {
        ...
        if (super == jl_function_type || super == jl_builtin_type ||
            is_anonfn_typename(jl_symbol_name(name))) {
            // Callable objects (including compiler-generated closures) get independent
            // method tables as an optimization
    ...
    if (t->name->wrapper == NULL) {
        ...
        for (i = np - 1; i >= 0; i--) {
            t->name->wrapper = jl_new_struct(jl_unionall_type, jl_svecref(parameters, i),
                                             t->name->wrapper);
...
JL_DLLEXPORT jl_datatype_t *jl_new_foreign_type(jl_sym_t *name, jl_module_t *module,
                                                jl_datatype_t *super,
                                                jl_markfunc_t markfunc,
                                                jl_sweepfunc_t sweepfunc, int haspointers,
                                                int large)
{
    jl_datatype_t *bt = jl_new_datatype(name, module, super, jl_emptysvec, jl_emptysvec,
                                        jl_emptysvec, 0, 1, 0);
    bt->size = large ? GC_MAX_SZCLASS + 1 : 0;
    jl_datatype_layout_t *layout = (jl_datatype_layout_t *)jl_gc_perm_alloc(
        sizeof(jl_datatype_layout_t) + sizeof(jl_fielddescdyn_t), 0, 4, 0);
    layout->nfields = 0;
    layout->alignment = sizeof(void *);
    layout->haspadding = 1;
    layout->npointers = haspointers;
    layout->fielddesc_type = 3;
    jl_fielddescdyn_t *desc = (jl_fielddescdyn_t *)((char *)layout + sizeof(*layout));
...
JL_DLLEXPORT jl_value_t *jl_new_bits(jl_value_t *dt, void *data)
{
    ...
    if (bt == jl_bool_type)
        return (1 & *(int8_t *)data) ? jl_true : jl_false;
    if (bt == jl_uint8_type)
    ...
    if (bt == jl_int8_type)
        return jl_box_int8(*(int8_t *)data);
    if (bt == jl_int16_type)
        return jl_box_int16(*(int16_t *)data);
    if (bt == jl_uint64_type)
        return jl_box_uint64(*(uint64_t *)data);
    if (bt == jl_uint32_type)
        return jl_box_uint32(*(uint32_t *)data);
    if (bt == jl_uint16_type)
        return jl_box_uint16(*(uint16_t *)data);
    if (bt == jl_char_type)
        return jl_box_char(*(uint32_t *)data);
    ...
    case 16: memcpy(jl_assume_aligned(v, 16), data, 16); break;
...
void jl_assign_bits(void *dest, jl_value_t *bits)
{
    ...
    case 16: memcpy(dest, jl_assume_aligned(bits, 16), 16); break;
...
#define BOX_FUNC(typ, c_type, pfx, nw)                                           \
    ...
    {                                                                            \
        jl_ptls_t ptls = jl_get_ptls_states();                                   \
        jl_value_t *v = jl_gc_alloc(ptls, nw * sizeof(void *), jl_##typ##_type); \
