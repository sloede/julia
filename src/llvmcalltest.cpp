...
JL_DLLEXPORT llvm::Function *MakeIdentityFunction(llvm::PointerType *AnyTy)
{
    ...
    Function *F = Function::Create(FunctionType::get(TrackedTy, {TrackedTy}, false),
                                   llvm::GlobalValue::ExternalLinkage, "identity", M);
...
JL_DLLEXPORT llvm::Function *MakeLoadGlobalFunction(llvm::PointerType *AnyTy)
{
    ...
    auto G = new GlobalVariable(*M, intType, true, GlobalValue::InternalLinkage,
                                Constant::getNullValue(intType), "test_global_var");

    auto resultType = Type::getInt64Ty(AnyTy->getContext());
    auto F = Function::Create(FunctionType::get(resultType, {}, false),
                              GlobalValue::ExternalLinkage, "load_global_var", M);
