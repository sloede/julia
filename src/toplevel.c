...
// current file name
JL_DLLEXPORT const char *jl_filename =
    "none"; // need to update jl_critical_error if this is TLS
...
void jl_init_main_module(void)
{
    ...
    jl_set_const(jl_main_module, jl_symbol("Core"), (jl_value_t *)jl_core_module);
    jl_set_global(jl_core_module, jl_symbol("Main"), (jl_value_t *)jl_main_module);
}
...
void jl_module_run_initializer(jl_module_t *m)
{
    ...
    JL_CATCH {
        if (jl_initerror_type == NULL) {
            ...
            jl_rethrow_other(
                jl_new_struct(jl_initerror_type, m->name, jl_current_exception()));
        }
    }
}
...
void jl_register_root_module(jl_module_t *m)
{
    ...
    if (register_module_func == NULL)
        register_module_func =
            jl_get_global(jl_base_module, jl_symbol("register_root_module"));
...
jl_array_t *jl_get_loaded_modules(void)
{
    ...
    if (loaded_modules_array == NULL && jl_base_module != NULL)
        loaded_modules_array =
            jl_get_global(jl_base_module, jl_symbol("loaded_modules_array"));
...
jl_value_t *jl_eval_module_expr(jl_module_t *parent_module, jl_expr_t *ex)
{
    ...
    if (jl_is__toplevel__mod(parent_module)) {
        ...
        if (b->value != NULL) {
            ...
            uintptr_t *refcnt =
                (uintptr_t *)ptrhash_bp(&jl_current_modules, (void *)b->value);
            *refcnt += 1;
        }
    ...
    for (int i = 0; i < jl_array_len(exprs); i++) {
        ...
        form = jl_expand_stmt_with_loc(jl_array_ptr_ref(exprs, i), newm, jl_filename,
                                       jl_lineno);
        ptls->world_age = jl_world_counter;
...
static void body_attributes(jl_array_t *body, int *has_intrinsics, int *has_defs,
                            int *has_loops)
{
    size_t i;
    *has_loops = 0;
...
static jl_module_t *eval_import_path(jl_module_t *where,
                                     jl_module_t *from JL_PROPAGATES_ROOT, jl_array_t *args,
                                     jl_sym_t **name,
                                     const char *keyword) JL_GLOBALLY_ROOTED
{
    ...
    *name = NULL;
    ...
    while (1) {
        ...
        JL_GC_PROMISE_ROOTED(m);
        if (!jl_is_module(m))
            jl_errorf("invalid %s path: \"%s\" does not name a module", keyword,
                      jl_symbol_name(var));
        i++;
    }
    *name = var;
...
{
    return jl_is_expr(e) &&
           (((jl_expr_t *)e)->head == module_sym || ((jl_expr_t *)e)->head == import_sym ||
            ((jl_expr_t *)e)->head == using_sym || ((jl_expr_t *)e)->head == export_sym ||
            ((jl_expr_t *)e)->head == thunk_sym || ((jl_expr_t *)e)->head == global_sym ||
            ((jl_expr_t *)e)->head == const_sym || ((jl_expr_t *)e)->head == toplevel_sym ||
...
static jl_method_instance_t *method_instance_for_thunk(jl_code_info_t *src,
                                                       jl_module_t *module)
{
...
static void import_module(jl_module_t *JL_NONNULL m, jl_module_t *import)
{
    ...
    if (jl_binding_resolved_p(m, name)) {
        ...
        if ((!b->constp && b->owner != m) ||
            (b->value && b->value != (jl_value_t *)import)) {
...
static jl_module_t *eval_import_from(jl_module_t *m JL_PROPAGATES_ROOT, jl_expr_t *ex,
                                     const char *keyword)
{
        ...
        if (fr->head == colon_sym) {
            if (jl_expr_nargs(fr) > 0 && jl_is_expr(jl_exprarg(fr, 0))) {
                ...
                if (((jl_expr_t *)path)->head == dot_sym) {
                    jl_sym_t *name = NULL;
                    jl_module_t *from =
                        eval_import_path(m, NULL, path->args, &name, keyword);
                    if (name != NULL) {
                        from = (jl_module_t *)jl_eval_global_var(from, name);
                        if (!jl_is_module(from))
                            jl_errorf("invalid %s path: \"%s\" does not name a module",
                                      keyword, jl_symbol_name(name));
                    }
                    return from;
                }
            }
...
jl_value_t *jl_toplevel_eval_flex(jl_module_t *JL_NONNULL m, jl_value_t *e, int fast,
                                  int expanded)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    if (!jl_is_expr(e)) {
    ...
    if (head == module_sym) {
        ...
        for (; i < jl_expr_nargs(ex); i++) {
            ...
            if (jl_is_expr(a) && ((jl_expr_t *)a)->head == dot_sym) {
                name = NULL;
                jl_module_t *import =
                    eval_import_path(m, from, ((jl_expr_t *)a)->args, &name, "using");
                ...
                if (from) {
                    ...
                    if (m == jl_main_module && name == NULL) {
                        // TODO: for now, `using A` in Main also creates an explicit binding
                        // for `A` This will possibly be extended to all modules.
                        import_module(m, u);
                    }
                }
            }
        ...
        for (; i < jl_expr_nargs(ex); i++) {
            ...
            if (jl_is_expr(a) && ((jl_expr_t *)a)->head == dot_sym) {
                name = NULL;
                jl_module_t *import =
                    eval_import_path(m, from, ((jl_expr_t *)a)->args, &name, "import");
                if (name == NULL) {
    ...
    if (jl_options.incremental && jl_generating_output()) {
        if (!ptrhash_has(&jl_current_modules, (void *)m) && !jl_is__toplevel__mod(m)) {
            if (m != jl_main_module) { // TODO: this was grand-fathered in
                ...
                jl_errorf(
                    "Evaluation into the closed module `%s` breaks incremental compilation "
...
JL_DLLEXPORT jl_value_t *jl_load_rewrite(jl_module_t *module, const char *fname,
                                         jl_value_t *mapexpr)
{
    uv_stat_t stbuf;
