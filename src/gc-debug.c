...
JL_DLLEXPORT jl_taggedvalue_t *jl_gc_find_taggedvalue_pool(char *p, size_t *osize_p)
{
    ...
    if (!(info.pagetable0->allocmap[info.pagetable0_i32] &
...
/*
 ...
 is rare enough this may not be straightforward. If the backtracking goes well you should
 know which object and which of its slots was written to without being caught by the write
 ...
 - Once you have a completely deterministic program which crashes on gc_verify, the
 addresses should stay constant between different runs (with same binary, same environment
...
static void gc_verify_track(jl_ptls_t ptls)
{
    ...
    do {
        ...
        if (lostval_parents.len == 0) {
            jl_safe_printf(
                "Could not find the missing link. We missed a toplevel root. This is odd.\n");
            break;
        }
        ...
        for (int i = 0; i < lostval_parents.len; i++) {
            ...
            for (int j = 0; j < clean_len + bits_save[GC_OLD].len; j++) {
                void *p = bits_save[j >= clean_len ? GC_OLD : GC_CLEAN]
                              .items[j >= clean_len ? j - clean_len : j];
            ...
            if (lostval_parent != NULL)
                break;
        }
        if (lostval_parent ==
            NULL) { // all parents of lostval were also scheduled for deletion
...
void gc_verify(jl_ptls_t ptls)
{
    ...
    for (int i = 0; i < clean_len + bits_save[GC_OLD].len; i++) {
        jl_taggedvalue_t *v =
            (jl_taggedvalue_t *)bits_save[i >= clean_len ? GC_OLD : GC_CLEAN]
                .items[i >= clean_len ? i - clean_len : i];
    ...
    while ((char *)v <= lim) {
        ...
        if (!in_freelist) {
            ...
            if (dt != (jl_value_t *)jl_buff_tag &&
                ...
                v->header != 0x10 && v->header != 0x20 && v->header != 0x30 &&
                v->header != 0x40 && v->header != 0x50 && v->header != 0x60) {
            ...
            while (next) {
                ...
                if (gc_page_data(next) != gc_page_data(last)) {
                    ...
                    if (gc_page_data(next) != allocating) {
                        // when not currently allocating on this page, fl_begin_offset
                        // should be correct
...
#ifdef GC_DEBUG_ENV
JL_DLLEXPORT jl_gc_debug_env_t jl_gc_debug_env = {0,
                                                  0,
...
static void gc_debug_alloc_init(jl_alloc_num_t *num, const char *name)
{
    ...
    num->interv = 1;
    num->max = UINT64_MAX;
    sscanf(env, "%" SCNd64 ":%" SCNd64 ":%" SCNd64, (int64_t *)&num->min,
           (int64_t *)&num->interv, (int64_t *)&num->max);
    if (num->interv == 0)
        num->interv = 1;
    num->next = num->min;
}
...
static void objprofile_print(htable_t nums, htable_t sizes)
{
    for (int i = 0; i < nums.size; i += 2) {
        if (nums.table[i + 1] != HT_NOTFOUND) {
            ...
            if (sz > 2e9) {
                jl_safe_printf(" %6d : %*.1f GB of (%*p) ", num, 6,
                               ((double)sz) / 1024 / 1024 / 1024, ptr_hex_width, ty);
            }
            else if (sz > 2e6) {
                jl_safe_printf(" %6d : %*.1f MB of (%*p) ", num, 6,
                               ((double)sz) / 1024 / 1024, ptr_hex_width, ty);
            }
            else if (sz > 2e3) {
                jl_safe_printf(" %6d : %*.1f kB of (%*p) ", num, 6, ((double)sz) / 1024,
                               ptr_hex_width, ty);
            }
            else {
                jl_safe_printf(" %6d : %*d  B of (%*p) ", num, 6, (int)sz, ptr_hex_width,
                               ty);
            }
...
void jl_print_gc_stats(JL_STREAM *s)
{
    ...
    if (gc_num.pause > 0) {
        jl_safe_printf("gc time  \t%.5f sec (%2.1f%%) in %d (%d full) collections\n",
                       jl_ns2s(gc_num.total_time), jl_ns2s(gc_num.total_time) / ptime * 100,
                       gc_num.pause, gc_num.full_sweep);
        jl_safe_printf("gc pause \t%.2f ms avg\n\t\t%2.0f ms max\n",
                       jl_ns2ms(gc_num.total_time) / gc_num.pause, jl_ns2ms(max_pause));
    ...
    jl_safe_printf("page table max utilization : %u (%.1f%%) - %u (%.1f%%) - %u (%.1f%%)\n",
                   p2, p2 * 100.0 / REGION2_PG_COUNT, p1,
                   p1 * 100.0 / REGION1_PG_COUNT / p2, p0,
                   p0 * 100.0 / REGION0_PG_COUNT / p1);
#ifdef _OS_LINUX_
...
void gc_time_pool_end(int sweep_full)
{
    ...
    jl_safe_printf("GC sweep pools end %.2f ms at %.1f GB/s "
                   ...
                   total_pages, total_pages - skipped_pages, freed_pages, lazy_freed_pages,
...
void gc_time_mark_pause(int64_t t0, int64_t scanned_bytes, int64_t perm_scanned_bytes)
{
    ...
    jl_safe_printf("GC mark pause %.2f ms | "
                   ...
                   (scanned_bytes + perm_scanned_bytes) / 1024, scanned_bytes / 1024,
                   perm_scanned_bytes / 1024, last_remset_len, remset_nptr);
}
                         ...
                         int64_t estimate_freed, int sweep_full)
{
    ...
    jl_safe_printf("GC sweep pause %.2f ms live %" PRId64 " kB "
                   ...
                   jl_ns2ms(sweep_pause), live_bytes / 1024, gc_num.freed / 1024,
                   estimate_freed / 1024, gc_num.freed - estimate_freed, pct,
                   gc_num.since_sweep / 1024, jl_ns2ms(gc_postmark_end - gc_premark_end),
    ...
    *pnold = nold;
    if (npgs != 0) {
        jl_safe_printf(
            ...
            p->osize, nused, nused + nfree, nused ? (nold * 100) / nused : 0, npgs,
            (nused * p->osize) / 1024, *pwaste / 1024);
    }
...
void gc_stats_big_obj(void)
{
    ...
    jl_safe_printf("%d kB (%d%% old) in %d large objects (%d%% old)\n",
                   ...
                   nused + nused_old,
                   nused + nused_old ? (nused_old * 100) / (nused + nused_old) : 0);
}
#endif // MEMPROFILE
    ...
    while (sp.pc < top) {
        ...
        if (pc == gc_mark_label_addrs[GC_MARK_L_marked_obj]) {
            ...
            jl_safe_printf("%p: Root object: %p :: %p (bits: %d)\n        of type ",
                           (void *)data, (void *)data->obj, (void *)data->tag,
                           (int)data->bits);
            jl_((void *)data->tag);
            isroot = 1;
        }
            ...
            jl_safe_printf("%p:  %s Array in object %p :: %p -- [%p, %p)\n        of type ",
                           ...
                           ((void **)data->parent)[-1], (void *)data->begin,
                           (void *)data->end);
            ...
            jl_safe_printf("%p:  %s Object (8bit) %p :: %p -- [%d, %d)\n        of type ",
                           (void *)data, prefix, (void *)data->parent,
                           ((void **)data->parent)[-1], (int)(data->begin - desc),
                           (int)(data->end - desc));
            jl_(jl_typeof(data->parent));
        }
            ...
            jl_safe_printf("%p:  %s Stack frame %p -- %d of %d (%s)\n", (void *)data,
                           prefix, (void *)data->s, (int)data->i, (int)data->nroots >> 1,
