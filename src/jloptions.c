...
static const char opts[] =
    ...
    // TODO: These TOKENS are defined in `runtime_ccall.cpp`. A more verbose `--help` should
...
restart_switch:
        switch (c) {
        case 0: break;
        case 1: jl_errorf("--lisp must be specified as the first argument"); break;
        case '?':
        case ':':
            if (optopt) {
                if (optopt == 'g') {
                ...
                for (const struct option *o = longopts; o->val; o++) {
                    if (optopt == o->val) {
                        if (o->has_arg == optional_argument) {
                            ...
                            jl_errorf("option `-%c/--%s` is missing an argument", o->val,
                                      o->name);
                        }
                        else {
        ...
        case 'h': // help
            ...
            exit(0);
        case opt_help_hidden: jl_printf(JL_STDOUT, "%s%s", usage, opts_hidden); exit(0);
        case 'g': // debug info
            if (optarg != NULL) {
                if (!strcmp(optarg, "0"))
        ...
        case opt_sysimage_native_code:
            if (!strcmp(optarg, "yes"))
                jl_options.use_sysimage_native_code =
                    JL_OPTIONS_USE_SYSIMAGE_NATIVE_CODE_YES;
            else if (!strcmp(optarg, "no"))
                jl_options.use_sysimage_native_code =
                    JL_OPTIONS_USE_SYSIMAGE_NATIVE_CODE_NO;
            else
                jl_errorf("julia: invalid argument to --sysimage-native-code={yes|no} (%s)",
                          optarg);
            break;
        case opt_compiled_modules:
            if (!strcmp(optarg, "yes"))
                ...
                jl_errorf("julia: invalid argument to --compiled-modules={yes|no} (%s)",
                          optarg);
            break;
        case 'C': // cpu-target
        ...
        case 't': // threads
            ...
            if (!strcmp(optarg, "auto")) {
                ...
                if (errno != 0 || optarg == endptr || *endptr != 0 || nthreads < 1 ||
                    nthreads >= INT_MAX)
        ...
        case 'p': // procs
            ...
            if (!strcmp(optarg, "auto")) {
                ...
                if (errno != 0 || optarg == endptr || *endptr != 0 || nprocs < 1 ||
                    nprocs >= INT_MAX)
        ...
        case opt_machine_file:
            ...
            if (!jl_options.machine_file)
                jl_error("julia: failed to allocate memory");
            break;
        case opt_project: jl_options.project = optarg ? strdup(optarg) : "@."; break;
        case opt_color:
            if (!strcmp(optarg, "yes"))
        ...
        case opt_warn_overwrite:
            if (!strcmp(optarg, "yes"))
                ...
                jl_errorf("julia: invalid argument to --warn-overwrite={yes|no} (%s)",
                          optarg);
            break;
        case opt_warn_scope:
            if (!strcmp(optarg, "yes"))
