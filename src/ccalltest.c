...
JL_DLLEXPORT complex_t *cptest(complex_t *a)
{
    ...
    if (verbose)
        fprintf(stderr, "%" PRIjint " + %" PRIjint " i\n", a->real, a->imag);
    a->real += 1;
    a->imag -= 2;
    return a;
}
...
JL_DLLEXPORT struct1 test_1long_a(jint x1, jint x2, jint x3, struct1 a, float b)
{
    // Unpack a "small" struct { float, double }
    if (verbose)
        fprintf(stderr, "(%" PRIjint ", %" PRIjint ", %" PRIjint ") & %g + %g i & %g\n", x1,
                x2, x3, a.x, a.y, b);
    a.x += b + x1 + x2 + x3;
    a.y -= b * 2;
    return a;
...
JL_DLLEXPORT struct1 test_1long_b(jint x1, double x2, jint x3, struct1 a, float b)
{
    // Unpack a "small" struct { float, double }
    if (verbose)
        fprintf(stderr, "(%" PRIjint ", %g, %" PRIjint ") & %g + %g i & %g\n", x1, x2, x3,
                a.x, a.y, b);
    a.x += b + x1 + x2 + x3;
    a.y -= b * 2;
    return a;
...
JL_DLLEXPORT struct1 test_1long_c(jint x1, double x2, jint x3, jint x4, struct1 a, float b)
{
    // Unpack a "small" struct { float, double }
    if (verbose)
        fprintf(stderr, "(%" PRIjint ", %g, %" PRIjint ", %" PRIjint ") & %g + %g i & %g\n",
...
JL_DLLEXPORT struct2a test_2a(struct2a a, int32_t b)
{
    ...
    if (verbose)
        fprintf(stderr, "%" PRId32 " + %" PRId32 " i & %" PRId32 "\n", a.x.x, a.y.y, b);
    a.x.x += b * 1;
    a.y.y -= b * 2;
    return a;
}
...
JL_DLLEXPORT struct3a test_3a(struct3a a, int64_t b)
{
    ...
    if (verbose)
        fprintf(stderr, "%" PRId64 " + %" PRId64 " i & %" PRId64 "\n", a.x.x, a.y.y, b);
    a.x.x += b * 1;
    a.y.y -= b * 2;
    return a;
}
...
{
    if (verbose)
        fprintf(stderr, "%" PRId32 ",%" PRId32 ",%" PRId32 " & %" PRId32 "\n", a.x, a.y,
                a.z, b);
    a.x += b * 1;
    a.y -= b * 2;
    a.z += b * 3;
...
{
    if (verbose)
        fprintf(stderr, "%" PRId32 ",%" PRId32 ",%" PRId32 ",%" PRId32 " & %" PRId32 "\n",
                a.x, a.y, a.z, a.a, b);
    a.x += b * 1;
    a.y -= b * 2;
    a.z += b * 3;
...
{
    if (verbose)
        fprintf(stderr, "%" PRId64 ",%" PRId8 " & %" PRId8 "\n", a.x, a.y, b);
    a.x += b * 1;
    a.y -= b * 2;
    return a;
}
...
{
    if (verbose)
        fprintf(stderr, "%" PRId32 ",%" PRId8 " & %" PRId8 "\n", a.x, a.y, b);
    a.x += b * 1;
    a.y -= b * 2;
    return a;
}
...
{
    if (verbose)
        fprintf(stderr, "%" PRId32 ",%" PRId16 " & %" PRId16 "\n", a.x, a.y, b);
    a.x += b * 1;
    a.y -= b * 2;
    return a;
}
...
{
    if (verbose)
        fprintf(stderr, "%" PRId8 ",%" PRId8 ",%" PRId8 ",%" PRId8 " & %" PRId8 "\n", a.x,
                a.y, a.z, a.a, b);
    a.x += b * 1;
    a.y -= b * 2;
    a.z += b * 3;
...
JL_DLLEXPORT struct12 test_12(struct12 a, float b)
{
    ...
    if (verbose)
        fprintf(stderr, "%g + %g i & %g + %g i & %g\n", creal(a.x), cimag(a.x), creal(a.y),
                cimag(a.y), b);
    a.x += b * 1 - (b * 2.0 * I);
    a.y += b * 3 - (b * 4.0 * I);
    return a;
...
JL_DLLEXPORT int128_t test_128(int128_t a, int64_t b)
{
    // Unpack a Int128
    if (verbose)
        fprintf(stderr, "0x%016" PRIx64 "%016" PRIx64 " & %" PRId64 "\n", a.y, a.x, b);
    a.x += b * 1;
    if (a.x == 0)
        a.y += b * 1;
    return a;
...
JL_DLLEXPORT struct_big test_big(struct_big a)
{
    ...
    if (verbose)
        fprintf(stderr, "%" PRIjint " %" PRIjint " %c\n", a.x, a.y, a.z);
    a.x += 1;
    a.y -= 2;
    a.z -= 'A';
    return a;
...
JL_DLLEXPORT struct_big test_big_long(jint x1, jint x2, jint x3, struct_big a)
{
    ...
    if (verbose)
        fprintf(stderr,
                "(%" PRIjint ", %" PRIjint ", %" PRIjint ") %" PRIjint " %" PRIjint " %c\n",
...
#define test_huge(suffix, reg)                                                        \
    JL_DLLEXPORT struct_huge##suffix test_huge##suffix(char a, struct_huge##suffix b, \
                                                       char c)                        \
    {                                                                                 \
        if (verbose)                                                                  \
            fprintf(stderr, "%c-%c\n", a, c);                                         \
...
JL_DLLEXPORT int test_long_args_intp(int *a1, int *a2, int *a3, int *a4, int *a5, int *a6,
...
JL_DLLEXPORT int test_long_args_int(int a1, int a2, int a3, int a4, int a5, int a6, int a7,
...
JL_DLLEXPORT float test_long_args_float(float a1, float a2, float a3, float a4, float a5,
...
JL_DLLEXPORT double test_long_args_double(double a1, double a2, double a3, double a4,
...
JL_DLLEXPORT int test_spill_int4(int *v1, int *v2, int *v3, int *v4, struct_spill_pint s)
{
...
JL_DLLEXPORT int test_spill_int6(int *v1, int *v2, int *v3, int *v4, int *v5, int *v6,
                                 struct_spill_pint s)
{
    return *v1 + *v2 + *v3 + *v4 + *v5 + *v6 + *s.a + *s.b;
}

JL_DLLEXPORT int test_spill_int7(int *v1, int *v2, int *v3, int *v4, int *v5, int *v6,
                                 int *v7, struct_spill_pint s)
{
    return *v1 + *v2 + *v3 + *v4 + *v5 + *v6 + *v7 + *s.a + *s.b;
}

JL_DLLEXPORT int test_spill_int8(int *v1, int *v2, int *v3, int *v4, int *v5, int *v6,
                                 int *v7, int *v8, struct_spill_pint s)
{
    return *v1 + *v2 + *v3 + *v4 + *v5 + *v6 + *v7 + *v8 + *s.a + *s.b;
}

JL_DLLEXPORT int test_spill_int9(int *v1, int *v2, int *v3, int *v4, int *v5, int *v6,
                                 int *v7, int *v8, int *v9, struct_spill_pint s)
{
    return *v1 + *v2 + *v3 + *v4 + *v5 + *v6 + *v7 + *v8 + *v9 + *s.a + *s.b;
}

JL_DLLEXPORT int test_spill_int10(int *v1, int *v2, int *v3, int *v4, int *v5, int *v6,
                                  int *v7, int *v8, int *v9, int *v10, struct_spill_pint s)
{
...
JL_DLLEXPORT float test_spill_float3(float v1, float v2, float v3, struct_spill_float s)
{
    return v1 + v2 + v3 + s.a + s.b;
}
...
JL_DLLEXPORT float test_spill_float5(float v1, float v2, float v3, float v4, float v5,
                                     struct_spill_float s)
{
    return v1 + v2 + v3 + v4 + v5 + s.a + s.b;
}

JL_DLLEXPORT float test_spill_float6(float v1, float v2, float v3, float v4, float v5,
                                     float v6, struct_spill_float s)
{
    return v1 + v2 + v3 + v4 + v5 + v6 + s.a + s.b;
}

JL_DLLEXPORT float test_spill_float7(float v1, float v2, float v3, float v4, float v5,
                                     float v6, float v7, struct_spill_float s)
{
    return v1 + v2 + v3 + v4 + v5 + v6 + v7 + s.a + s.b;
}

JL_DLLEXPORT float test_spill_float8(float v1, float v2, float v3, float v4, float v5,
                                     float v6, float v7, float v8, struct_spill_float s)
{
    return v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + s.a + s.b;
}

JL_DLLEXPORT float test_spill_float9(float v1, float v2, float v3, float v4, float v5,
...
JL_DLLEXPORT float test_spill_float10(float v1, float v2, float v3, float v4, float v5,
...
JL_DLLEXPORT __m128i test_m128i(__m128i a, __m128i b, __m128i c, __m128i d)
{
    ...
    return _mm_add_epi32(a, _mm_cvtps_epi32(_mm_mul_ps(
                                _mm_cvtepi32_ps(b), _mm_cvtepi32_ps(_mm_sub_epi32(c, d)))));
}
...
JL_DLLEXPORT struct_aa64_1 test_aa64_i128_2(int64_t v1, __int128 v2, struct_aa64_1 v3)
{
...
JL_DLLEXPORT struct_aa64_2 test_aa64_fp16_2(int v1, float v2, double v3, __fp16 v4)
{
...
JL_DLLEXPORT int64_t test_ppc64_vec1long(int64_t d1, int64_t d2, int64_t d3, int64_t d4,
                                         int64_t d5, int64_t d6, int64_t d7, int64_t d8,
                                         int64_t d9, struct_huge1_ppc64 vs)
{
    return d1 + d2 + d3 + d4 + d5 + d6 + d7 + d8 + d9 + vs.m + vs.v[0] + vs.v[1] + vs.v[2] +
           vs.v[3];
}

JL_DLLEXPORT int64_t test_ppc64_vec1long_vec(int64_t d1, int64_t d2, int64_t d3, int64_t d4,
                                             int64_t d5, int64_t d6, int64_t d7, int64_t d8,
                                             int64_t d9, float32x4_t vs)
{
    return d1 + d2 + d3 + d4 + d5 + d6 + d7 + d8 + d9 + vs[0] + vs[1] + vs[2] + vs[3];
}

JL_DLLEXPORT float32x4_t test_ppc64_vec2(int64_t d1, float32x4_t a, float32x4_t b,
                                         float32x4_t c, float32x4_t d, float32x4_t e,
                                         float32x4_t f, float32x4_t g, float32x4_t h,
                                         float32x4_t i, float32x4_t j, float32x4_t k,
                                         float32x4_t l, float32x4_t m, float32x4_t n)
{
    float32x4_t r;
    r[0] = d1 + a[0] + b[0] + c[0] + d[0] + e[0] + f[0] + g[0] + h[0] + i[0] + j[0] + k[0] +
           l[0] + m[0] + n[0];
    r[1] = d1 + a[1] + b[1] + c[1] + d[1] + e[1] + f[1] + g[1] + h[1] + i[1] + j[1] + k[1] +
           l[1] + m[1] + n[1];
    r[2] = d1 + a[2] + b[2] + c[2] + d[2] + e[2] + f[2] + g[2] + h[2] + i[2] + j[2] + k[2] +
           l[2] + m[2] + n[2];
    r[3] = d1 + a[3] + b[3] + c[3] + d[3] + e[3] + f[3] + g[3] + h[3] + i[3] + j[3] + k[3] +
           l[3] + m[3] + n[3];
    return r;
}
