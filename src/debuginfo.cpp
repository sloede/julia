                                     ...
                                     uint8_t *UnwindData)
{
    ...
    if (0) {
        ...
        jl_in_stackwalk = 1;
        if (mod_size &&
            !SymLoadModuleEx(GetCurrentProcess(), NULL, NULL, NULL, (DWORD64)Section,
                             mod_size, NULL, SLMFLAG_VIRTUAL)) {
            static int warned = 0;
            if (!warned) {
                jl_printf(JL_STDERR,
                          "WARNING: failed to insert module info for backtrace: %lu\n",
                          GetLastError());
                warned = 1;
            }
        }
            ...
            name[len - 1] = 0;
            if (!SymAddSymbol(GetCurrentProcess(), (ULONG64)Section, name, (DWORD64)Code,
                              (DWORD)Size, 0)) {
                jl_printf(
                    JL_STDERR,
                    "WARNING: failed to insert function name %s into debug info: %lu\n",
                    name, GetLastError());
            }
        }
        jl_in_stackwalk = 0;
    ...
    if (!RtlAddFunctionTable(tbl, 1, (DWORD64)Section)) {
        ...
        if (!warned) {
            jl_printf(JL_STDERR,
                      "WARNING: failed to insert function stack unwind info: %lu\n",
                      GetLastError());
            warned = 1;
        }
    }
...
public:
                                      ...
                                      RTDyldMemoryManager *memmgr)
    {
        ...
        if (!SavedObject.first) {
            ...
            auto NewObj =
                object::ObjectFile::createObjectFile(NewBuffer->getMemBufferRef());
            assert(NewObj);
...
#if defined(_OS_WINDOWS_)
        uint64_t SectionAddrCheck =
            0; // assert that all of the Sections are at the same location
        ...
        for (const object::SymbolRef &sym_iter : debugObj.symbols()) {
            ...
            if (pAddr) {
                ...
                *pAddr = (uint8_t *)Addr;
                ...
                if (memmgr)
                    SectionAddr =
                        (uintptr_t)lookupWriteAddressFor(memmgr, (void *)SectionLoadAddr);
        ...
        *(uint64_t *)(&catchjmp[SectionLoadOffset + 2]) = (uint64_t)&__julia_personality;
        ...
        *(DWORD *)&UnwindData[SectionLoadOffset + 8] = (DWORD)(
            catchjmp - (uint8_t *)SectionAddrCheck); // relative location of catchjmp
        ...
        for (const auto &sym_size : symbols) {
            ...
            if (SectionAddrCheck)
                ...
                SectionAddrCheck = SectionLoadAddr;
            create_PRUNTIME_FUNCTION((uint8_t *)(uintptr_t)Addr, (size_t)Size, sName,
                                     (uint8_t *)(uintptr_t)SectionLoadAddr,
                                     (size_t)SectionSize, UnwindData);
#endif
            StringMap<jl_code_instance_t *>::iterator codeinst_it =
                codeinst_in_flight.find(sName);
...
static std::pair<char *, bool> jl_demangle(const char *name) JL_NOTSAFEPOINT
{
    ...
    if (end <= start)
        goto done;
    if (strncmp(name, "japi1_", 6) && strncmp(name, "japi3_", 6) &&
        strncmp(name, "julia_", 6) && strncmp(name, "jsys1_", 6) &&
...
static int lookup_pointer(object::SectionRef Section, DIContext *context,
                          jl_frame_t **frames, size_t pointer, int64_t slide, bool demangle,
                          bool noInline) JL_NOTSAFEPOINT
{
    ...
    auto inlineInfo =
        context->getInliningInfoForAddress(makeAddress(Section, pointer + slide), infoSpec);
    ...
    if (n_frames == 0) {
        ...
        return lookup_pointer(object::SectionRef(), NULL, frames, pointer, slide, demangle,
                              noInline);
    }
    if (noInline)
        n_frames = 1;
    ...
    for (int i = 0; i < n_frames; i++) {
        ...
        if (!noInline) {
            ...
            info = context->getLineInfoForAddress(makeAddress(Section, pointer + slide),
                                                  infoSpec);
        }
        ...
        if (inlined_frame) {
            ...
            if (!fromC) {
                ...
                if (semi_pos != std::string::npos) {
                    ...
                    frame->linfo = NULL; // TODO: if (new_frames[n_frames - 1].linfo)
                                         // frame->linfo = lookup(func_name in linfo)?
                }
            }
        }
    ...
    for (const object::SectionRef &Section : Obj.sections()) {
...
#endif
        {
            ...
            if (found) {
                ...
                info.crc32 =
                    *(const uint32_t *)Contents.substr(LLT_ALIGN(length + 1, 4), 4).data();
                break;
            }
        }
    }
...
openDebugInfo(StringRef debuginfopath, const debug_link_info &info)
{
    ...
    uint32_t crc32 = calc_gnu_debuglink_crc32(SplitFile.get()->getBufferStart(),
    ...
    auto error_splitobj = object::ObjectFile::createObjectFile(
        SplitFile.get().get()->getMemBufferRef(), llvm_file_magic::unknown);
    if (!error_splitobj) {
        return error_splitobj.takeError();
    }

    // successfully validated and loaded split debug info file
    return object::OwningBinary<object::ObjectFile>(std::move(error_splitobj.get()),
...
static void get_function_name_and_base(llvm::object::SectionRef Section, size_t pointer,
                                       int64_t slide, bool insysimage, void **saddr,
                                       char **name, bool untrusted_dladdr) JL_NOTSAFEPOINT
{
...
#ifdef _OS_WINDOWS_
    // For ntdll and msvcrt since we are currently only parsing DWARF debug info through
    // LLVM
    if (!insysimage && needs_name) {
        static char frame_info_func[sizeof(SYMBOL_INFO) + MAX_SYM_NAME * sizeof(TCHAR)];
...
static objfileentry_t &find_object_file(uint64_t fbase, StringRef fname) JL_NOTSAFEPOINT
{
    ...
    if (isdarwin) {
        ...
        std::unique_ptr<MemoryBuffer> membuf =
            MemoryBuffer::getMemBuffer(StringRef((const char *)fbase, msize), "", false);
        auto origerrorobj = llvm::object::ObjectFile::createObjectFile(
            membuf->getMemBufferRef(), llvm_file_magic::unknown);
        if (!origerrorobj)
            return entry;

        llvm::object::MachOObjectFile *morigobj =
            (llvm::object::MachOObjectFile *)origerrorobj.get().get();
        ...
        CFBundleRef dsfmwkbundle = CFBundleCreate(kCFAllocatorDefault, dsfmwkurl);
        CFRelease(dsfmwkurl);
        ...
        if (DBGCopyFullDSYMURLForUUID != NULL) {
            ...
            CFUUIDRef objuuid =
                CFUUIDCreateWithBytes(kCFAllocatorDefault, uuid[0], uuid[1], uuid[2],
                                      uuid[3], uuid[4], uuid[5], uuid[6], uuid[7], uuid[8],
            ...
            if (dsympathurl != NULL &&
                CFURLGetFileSystemRepresentation(dsympathurl, true, (UInt8 *)objpathcstr,
        ...
        // On Linux systems we need to mmap another copy because of the permissions on the
        // mmap'ed shared library. On Windows we need to mmap another copy since reading the
        // in-memory copy seems to return object_error:unexpected_eof
        objpath = fname;
    }
    ...
    if (errorobj) {
        ...
        if (islinux) {
            ...
            if (!info.filename.empty()) {
                size_t sep = fname.rfind('/');
                Expected<object::OwningBinary<object::ObjectFile>> DebugInfo(
                    errorCodeToError(
...
static object::SectionRef getModuleSectionForAddress(const object::ObjectFile *obj,
                                                     uint64_t Address) JL_NOTSAFEPOINT
{
...
bool jl_dylib_DI_for_fptr(size_t pointer, object::SectionRef *Section, int64_t *slide,
                          llvm::DIContext **context, bool onlySysImg, bool *isSysImg,
                          void **saddr, char **name, char **filename) JL_NOTSAFEPOINT
{
    *Section = object::SectionRef();
    *context = NULL;
    // On Windows and FreeBSD, `dladdr` (or its equivalent) returns the closest exported
    // symbol without checking the size. This causes the lookup to return incorrect non-NULL
    // result for local functions when better result is available through other methods.
...
// Assigning fname, fbase
    ...
    dladdr_success =
        dladdr1((void *)pointer, &dlinfo, (void **)&extra_info, RTLD_DL_LINKMAP) != 0;
#else
    ...
    *context = entry.ctx;
    ...
    get_function_name_and_base(*Section, pointer, entry.slide, insysimage, saddr, name,
                               untrusted_dladdr);
    return true;
}
                                   ...
                                   int noInline) JL_NOTSAFEPOINT
{
    ...
    if (SymGetLineFromAddr64(GetCurrentProcess(), dwAddress, &dwDisplacement,
                             &frame_info_line)) {
    ...
    bool isSysImg;
    void *saddr;
    if (!jl_dylib_DI_for_fptr(pointer, &Section, &slide, &context, skipC, &isSysImg, &saddr,
                              &frame0->func_name, &frame0->file_name)) {
        frame0->fromC = 1;
        return 1;
    }
    ...
    if (isSysImg && sysimg_fptrs.base && saddr) {
        ...
        for (size_t i = 0; i < sysimg_fptrs.nclones; i++) {
            if (diff == sysimg_fptrs.clone_offsets[i]) {
                ...
                if (idx <
                    sysimg_fvars_n) // items after this were cloned but not referenced
                                    // directly by a method (such as our ccall PLT thunks)
                       ...
                       int noInline) JL_NOTSAFEPOINT
{
    ...
    *frames_out = frames;
    ...
    if (jl_DI_for_fptr(pointer, &symsize, &slide, &Section, &context)) {
        ...
        int nf =
            lookup_pointer(Section, context, frames_out, pointer, slide, true, noInline);
        return nf;
    }
    ...
    processFDEs((char *)Addr, Size, [](const char *Entry) {
        if (!libc_deregister_frame) {
            libc_deregister_frame =
                (void (*)(void *))dlsym(RTLD_NEXT, "__deregister_frame");
        }
...
#elif defined(_OS_LINUX_) && defined(JL_UNW_HAS_FORMAT_IP) && !defined(_CPU_ARM_)
#include <type_traits>
...
template<typename T>
static T parse_leb128(const uint8_t *&Addr, const uint8_t *End)
{
...
static DW_EH_PE parseCIE(const uint8_t *Addr, const uint8_t *End)
{
    ...
    for (const char *augp = augmentation;; augp++) {
        ...
        case 'P': {
            ...
            case DW_EH_PE_sleb128: p = consume_leb128(p, cie_end); break;
            case DW_EH_PE_udata2:
            case DW_EH_PE_sdata2: p += 2; break;
            case DW_EH_PE_udata4:
            case DW_EH_PE_sdata4: p += 4; break;
            case DW_EH_PE_udata8:
            case DW_EH_PE_sdata8: p += 8; break;
            case DW_EH_PE_signed: p += sizeof(void *); break;
            default:
...
void register_eh_frames(uint8_t *Addr, size_t Size)
{
    ...
    processFDEs((char *)Addr, Size, [&](const char *Entry) {
        ...
        if (encoding == DW_EH_PE_absptr || encoding == DW_EH_PE_omit) {
            ...
            assert((encoding & 0xf0) == 0x10 && "Only pcrel mode is supported");
            switch (encoding & 0xf) {
            case DW_EH_PE_uleb128:
            ...
            case DW_EH_PE_sdata8:
                ...
                break;
            default: assert(0 && "Invalid FDE encoding."); break;
            }
        }
        ...
        if (end_ip < (start + size))
            end_ip = start + size;
        table[cur_entry].fde_offset = safe_trunc<int32_t>((intptr_t)Entry - (intptr_t)Addr);
...
extern "C" uint64_t jl_getUnwindInfo(uint64_t dwAddr)
{
...
extern "C" uint64_t jl_trygetUnwindInfo(uint64_t dwAddr)
{
    // Might be called from unmanaged thread
    Optional<std::map<size_t, ObjectInfo, revcomp> *> maybeobjmap =
        jl_jit_events->trygetObjectMap();
    if (maybeobjmap) {
