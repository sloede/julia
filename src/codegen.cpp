...
// use ELF because RuntimeDyld COFF X86_64 doesn't seem to work (fails to generate function
// pointers)?
#define FORCE_ELF
#endif
...
#else
JL_DLLEXPORT uintptr_t __stack_chk_guard =
    (uintptr_t)0xBAD57ACCBAD67ACC; // 0xBADSTACKBADSTACK
JL_DLLEXPORT void __stack_chk_fail()
{
    /* put your panic function or similar in here */
    fprintf(stderr, "fatal error: stack corruption detected\n");
    gc_debug_critical_error();
    abort(); // end with abort, since the compiler destroyed the stack upon entry to this
             // function, there's no going back now
}
#endif
...
jl_cgparams_t jl_default_cgparams = {1,
                                     1,
                                     1,
                                     0,
#ifdef _OS_WINDOWS_
...
struct jl_cgval_t {
    Value *V; // may be of type T* or T, or set to NULL if ghost (or if the value has not
              // been initialized yet, for a variable definition)
    // For unions, we may need to keep a reference to the boxed part individually.
    // If this is non-NULL, then, at runtime, we satisfy the invariant that (for the
    // corresponding runtime values) if `(TIndex | 0x80) != 0`, then `Vboxed == V` (by
    // value). For convenience, we also set this value of isboxed values, in which case it
    // is equal (at compile time) to V.
    Value *Vboxed;
    Value *TIndex; // if `V` is an unboxed (tagged) Union described by `typ`, this gives the
                   // DataType index (1-based, small int) as an i8
    jl_value_t *constant; // constant value (rooted in linfo.def.roots)
    jl_value_t *typ; // the original type of V, never NULL
    bool isboxed; // whether this value is a jl_value_t* allocated on the heap with the
                  // right type tag
    ...
    jl_cgval_t(Value *V, Value *gcroot, bool isboxed, jl_value_t *typ, Value *tindex)
      : // general constructor (with pointer type auto-detect)
    ...
    explicit jl_cgval_t(jl_value_t *typ)
      : // ghost value constructor
        // mark explicit to avoid being used implicitly for conversion from NULL (use
        // jl_cgval_t() instead)
        V(NULL),
        Vboxed(NULL),
        TIndex(NULL),
    ...
    jl_cgval_t(const jl_cgval_t &v, jl_value_t *typ, Value *tindex)
      : // copy constructor with new type
        V(v.V),
        Vboxed(v.Vboxed),
        TIndex(tindex),
        constant(v.constant),
...
struct jl_varinfo_t {
    Instruction *boxroot; // an address, if the var might be in a jl_value_t** stack slot
                          // (marked tbaa_const, if appropriate)
...
public:
    ...
    std::vector<std::tuple<jl_cgval_t, BasicBlock *, AllocaInst *, PHINode *, jl_value_t *>>
        PhiNodes;
    ...
    ~jl_codectx_t() { assert(this->roots == NULL); }
};

static Type *julia_type_to_llvm(jl_codectx_t &ctx, jl_value_t *jt, bool *isboxed = NULL);
static jl_returninfo_t get_specsig_function(jl_codectx_t &ctx, Module *M, StringRef name,
                                            jl_value_t *sig, jl_value_t *jlrettype);
static jl_cgval_t emit_expr(jl_codectx_t &ctx, jl_value_t *expr, ssize_t ssaval = -1);
static Value *global_binding_pointer(jl_codectx_t &ctx, jl_module_t *m, jl_sym_t *s,
                                     jl_binding_t **pbnd, bool assign);
static jl_cgval_t emit_checked_var(jl_codectx_t &ctx, Value *bp, jl_sym_t *name, bool isvol,
                                   MDNode *tbaa);
static jl_cgval_t emit_sparam(jl_codectx_t &ctx, size_t i);
static Value *emit_condition(jl_codectx_t &ctx, const jl_cgval_t &condV,
...
{
    GlobalVariable *gv = new GlobalVariable(M, val->getType(), true,
                                            GlobalVariable::PrivateLinkage, val, name);
...
static void undef_derived_strct(IRBuilder<> &irbuilder, Value *ptr, jl_datatype_t *sty,
                                MDNode *tbaa)
{
    ...
    if (np == 0)
        return;
    ptr = irbuilder.CreateBitCast(
        ptr, T_prjlvalue->getPointerTo(ptr->getType()->getPointerAddressSpace()));
    Value *V_null = ConstantPointerNull::get(cast<PointerType>(T_prjlvalue));
    for (i = 0; i < np; i++) {
        Value *fld =
            irbuilder.CreateConstInBoundsGEP1_32(T_prjlvalue, ptr, jl_ptr_offset(sty, i));
...
static inline jl_cgval_t mark_julia_slot(Value *v, jl_value_t *typ, Value *tindex,
                                         MDNode *tbaa)
{
...
static inline jl_cgval_t value_to_pointer(jl_codectx_t &ctx, Value *v, jl_value_t *typ,
                                          Value *tindex)
{
    Value *loc;
    if (valid_as_globalinit(
            v)) { // llvm can't handle all the things that could be inside a ConstantExpr
...
static inline jl_cgval_t mark_julia_type(jl_codectx_t &ctx, Value *v, bool isboxed,
                                         jl_value_t *typ)
{
    ...
    if (v && !isboxed && v->getType()->isAggregateType() && !jl_is_vecelement_type(typ) &&
        CountTrackedPointers(v->getType()).count == 0) {
...
static inline jl_cgval_t update_julia_type(jl_codectx_t &ctx, const jl_cgval_t &v,
                                           jl_value_t *typ)
{
    if (v.typ == typ || v.typ == jl_bottom_type || v.constant ||
        typ == (jl_value_t *)jl_any_type || jl_egal(v.typ, typ))
        return v; // fast-path
    ...
    if (v.TIndex) {
        ...
        if (jl_is_datatype(utyp)) {
            ...
            if (jl_is_concrete_type(utyp))
                ...
                alwaysboxed =
                    !((jl_datatype_t *)utyp)->abstract && ((jl_datatype_t *)utyp)->mutabl;
            if (alwaysboxed) {
        ...
        if (!jl_is_concrete_type(typ))
            return v; // not generally worth trying to change type info (which would require
                      // recomputing tindex)
    }
    Type *T = julia_type_to_llvm(ctx, typ);
    if (type_is_ghost(T))
...
static jl_cgval_t convert_julia_type(jl_codectx_t &ctx, const jl_cgval_t &v,
                                     jl_value_t *typ);
...
static void CreateTrap(IRBuilder<> &irbuilder)
{
    ...
    Function *trap_func = Intrinsic::getDeclaration(f->getParent(), Intrinsic::trap);
...
static jl_cgval_t convert_julia_type_union(jl_codectx_t &ctx, const jl_cgval_t &v,
                                           jl_value_t *typ)
{
        ...
        for_each_uniontype_small(
            ...
            [&](unsigned idx, jl_datatype_t *jt) {
                ...
                if (new_idx) {
                    ...
                    Value *cmp =
                        ctx.builder.CreateICmpEQ(tindex, ConstantInt::get(T_int8, idx));
                    new_tindex = ctx.builder.CreateSelect(
                        cmp, ConstantInt::get(T_int8, new_idx), new_tindex);
                    t = true;
                }
    ...
    if (!isa<Constant>(new_tindex)) {
            ...
            for_each_uniontype_small(
                ...
                [&](unsigned idx, jl_datatype_t *jt) {
                    ...
                    if (old_idx == 0) {
                        ...
                        Value *cmp = ctx.builder.CreateICmpEQ(
                            maybe_decay_untracked(
                                literal_pointer_val(ctx, (jl_value_t *)jt)),
                            union_box_dt);
                        union_box_tindex = ctx.builder.CreateSelect(
                            cmp, ConstantInt::get(T_int8, 0x80 | idx), union_box_tindex);
                    }
                },
                typ, counter);
            if (union_box_dt) {
                BasicBlock *postBB =
                    BasicBlock::Create(jl_LLVMContext, "post_union_isa", ctx.f);
                ctx.builder.CreateBr(postBB);
                ctx.builder.SetInsertPoint(currBB);
                Value *wasunknown =
                    ctx.builder.CreateICmpEQ(v.TIndex, ConstantInt::get(T_int8, 0x80));
        ...
        if (!skip_box.all()) {
            ...
            if (v.Vboxed) {
                ...
                boxv = ctx.builder.CreateSelect(ctx.builder.CreateAnd(wasboxed, isboxed),
                                                v.Vboxed, boxv);
            }
            if (v.V == NULL) {
                ...
                slotv = ctx.builder.CreateSelect(
                    isboxv, decay_derived(boxv),
...
// given a value marked with type `v.typ`, compute the mapping and/or boxing to return a
// value of type `typ`
    ...
    if (jl_is_concrete_type(typ)) {
        ...
        if (v.TIndex) {
            ...
            if (new_idx) {
                ...
                if (v.V && !v.ispointer()) {
                    // TODO: remove this branch once all consumers of v.TIndex understand
                    // how to handle a non-ispointer value
...
static std::pair<bool, bool> uses_specsig(jl_method_instance_t *lam, jl_value_t *rettype,
                                          bool prefer_specsig)
{
    ...
    if (jl_is_method(lam->def.method)) {
        if ((size_t)jl_subtype_env_size(lam->def.method->sig) !=
            jl_svec_len(lam->sparam_vals))
            needsparams = true;
    ...
    if (jl_is_uniontype(rettype)) {
        ...
        if (nbytes > 0)
            return std::make_pair(true, false); // some elements of the union could be
                                                // returned unboxed avoiding allocation
    }
    bool allSingleton = true;
    for (size_t i = 0; i < jl_nparams(sig); i++) {
        jl_value_t *sigt = jl_tparam(sig, i);
        bool issing =
            jl_is_datatype(sigt) && jl_is_datatype_singleton((jl_datatype_t *)sigt);
        allSingleton &= issing;
...
const int logdata_blocksize = 32; // target getting nearby lines in the same general cache
                                  // area and reducing calls to malloc by chunking
...
static void visitLine(jl_codectx_t &ctx, std::vector<logdata_block *> &vec, int line,
                      Value *addend, const char *name)
{
    uint64_t *ptr = allocLine(vec, line);
    Value *pv =
        ConstantExpr::getIntToPtr(ConstantInt::get(T_size, (uintptr_t)ptr), T_pint64);
    Value *v = ctx.builder.CreateLoad(pv, true, name);
    v = ctx.builder.CreateAdd(v, addend);
    ctx.builder.CreateStore(v, pv, true); // volatile, not atomic, so this might be an
                                          // underestimate, but it's faster this way
}
...
static void mallocVisitLine(jl_codectx_t &ctx, StringRef filename, int line, Value *sync)
{
    ...
    if (filename == "" || filename == "none" || filename == "no file" ||
        ...
        return;
    Value *addend =
        sync ? ctx.builder.CreateCall(prepare_call(sync_gc_total_bytes_func), {sync}) :
...
static void write_lcov_data(logdata_t &logData, const std::string &outfile)
{
    std::ofstream outf(outfile.c_str(),
                       std::ofstream::ate | std::ofstream::out | std::ofstream::binary);
    // std::string base = std::string(jl_options.julia_bindir);
    // base = base + "/../share/julia/base/";
    logdata_t::iterator it = logData.begin();
    for (; it != logData.end(); it++) {
...
static jl_value_t *static_apply_type(jl_codectx_t &ctx, const jl_cgval_t *args,
                                     size_t nargs)
{
...
static jl_value_t *static_eval(jl_codectx_t &ctx, jl_value_t *ex, int sparams = true,
                               int allow_alloc = true)
{
    if (!JL_FEAT_TEST(ctx, static_alloc))
        allow_alloc = 0;
    if (jl_is_symbol(ex)) {
        ...
        if (e->head == call_sym) {
            ...
            if (f) {
                if (jl_array_dim0(e->args) == 3 && f == jl_builtin_getfield) {
                    m = (jl_module_t *)static_eval(ctx, jl_exprarg(e, 1), sparams,
                                                   allow_alloc);
                    ...
                    s = (jl_sym_t *)static_eval(ctx, jl_exprarg(e, 2), sparams,
                                                allow_alloc);
                    if (s && jl_is_symbol(s)) {
                    ...
                    for (i = 0; i < n; i++) {
                        v[i + 1] =
                            static_eval(ctx, jl_exprarg(e, i + 1), sparams, allow_alloc);
                        if (v[i + 1] == NULL) {
                            JL_GC_POP();
                            return NULL;
                        }
                    ...
                    size_t last_age = jl_get_ptls_states()->world_age;
                    // here we know we're calling specific builtin functions that work in
                    // world 1.
                    jl_get_ptls_states()->world_age = 1;
                    jl_value_t *result;
                    JL_TRY {
...
static void jl_add_method_root(jl_codectx_t &ctx, jl_value_t *val)
{
    if (jl_is_concrete_type(val) || jl_is_bool(val) || jl_is_symbol(val) ||
        val == jl_nothing || val == (jl_value_t *)jl_any_type ||
        val == (jl_value_t *)jl_bottom_type || val == (jl_value_t *)jl_core_module)
        return;
    JL_GC_PUSH1(&val);
    if (ctx.roots == NULL) {
...
static jl_cgval_t emit_globalref(jl_codectx_t &ctx, jl_module_t *mod, jl_sym_t *name)
{
    ...
    if (bnd && bnd->value != NULL) {
        ...
        return mark_julia_type(ctx, tbaa_decorate(tbaa_binding, ctx.builder.CreateLoad(bp)),
                               true, (jl_value_t *)jl_any_type);
    }
...
static jl_cgval_t emit_getfield(jl_codectx_t &ctx, const jl_cgval_t &strct, jl_sym_t *name)
{
    ...
    jl_cgval_t myargs_array[2] = {strct, mark_julia_const((jl_value_t *)name)};
    Value *result = emit_jlcall(ctx, jlgetfield_func, maybe_decay_untracked(V_null),
                                myargs_array, 2, JLCALL_F_CC);
    return mark_julia_type(ctx, result, true, jl_any_type);
}

static Value *emit_box_compare(jl_codectx_t &ctx, const jl_cgval_t &arg1,
                               const jl_cgval_t &arg2)
{
    if (jl_pointer_egal(arg1.typ) || jl_pointer_egal(arg2.typ)) {
        ...
        assert(varg1 && varg2 && (arg1.isboxed || arg1.TIndex) &&
               (arg2.isboxed || arg2.TIndex) &&
               "Only boxed types are valid for pointer comparison.");
        varg1 = maybe_decay_tracked(varg1);
        varg2 = maybe_decay_tracked(varg2);
        if (cast<PointerType>(varg1->getType())->getAddressSpace() !=
            cast<PointerType>(varg2->getType())->getAddressSpace()) {
    ...
    return ctx.builder.CreateTrunc(
        ctx.builder.CreateCall(prepare_call(jlegal_func), {varg1, varg2}), T_int1);
}

static Value *emit_bits_compare(jl_codectx_t &ctx, jl_cgval_t arg1, jl_cgval_t arg2);

static Value *emit_bitsunion_compare(jl_codectx_t &ctx, const jl_cgval_t &arg1,
                                     const jl_cgval_t &arg2)
{
    assert(arg1.typ == arg2.typ && arg1.TIndex && arg2.TIndex &&
           jl_is_uniontype(arg1.typ) && "unimplemented");
    ...
    Function *trap_func = Intrinsic::getDeclaration(ctx.f->getParent(), Intrinsic::trap);
...
static Value *emit_bits_compare(jl_codectx_t &ctx, jl_cgval_t arg1, jl_cgval_t arg2)
{
    ...
    if (at->isVectorTy()) {
        ...
        for (size_t i = 0, l = jl_svec_len(types); i < l; i++) {
            ...
            subAns = emit_bits_compare(ctx, mark_julia_type(ctx, fld1, false, fldty),
    ...
    if (at->isAggregateType()) { // Struct or Array
        ...
        Value *varg1 =
            arg1.ispointer() ? maybe_decay_tracked(data_pointer(ctx, arg1)) : arg1.V;
        Value *varg2 =
            arg2.ispointer() ? maybe_decay_tracked(data_pointer(ctx, arg2)) : arg2.V;
        if (sz > 512 && !sty->layout->haspadding) {
            ...
            Value *answer = ctx.builder.CreateCall(
                prepare_call(memcmp_func),
                {ctx.builder.CreateBitCast(varg1, T_pint8),
                 ctx.builder.CreateBitCast(varg2, T_pint8), ConstantInt::get(T_size, sz)},
            ...
            Value *answer = ConstantInt::get(T_int1, 1);
            for (size_t i = 0, l = jl_svec_len(types); i < l; i++) {
                // TODO: should we replace this with `subAns =
                // emit_f_is(emit_getfield_knownidx(ctx, arg1, i, arg1.typ),
                // emit_getfield_knownidx(ctx, arg2, i, arg2.typ))` or is there any value in
                ...
                Value *subAns, *fld1, *fld2;
                unsigned llvm_idx = (i > 0 && isa<StructType>(at)) ?
                                        convert_struct_offset(ctx, at, byte_offset) :
                                        i;
                if (arg1.ispointer())
                ...
                if (jl_field_isptr(sty, i)) {
                    ...
                    subAns = emit_box_compare(ctx, mark_julia_type(ctx, fld1, true, fldty),
                    ...
                    if (arg1.ispointer()) {
                        Value *tindex1 = ctx.builder.CreateNUWAdd(
                            ConstantInt::get(T_int8, 1),
                            ctx.builder.CreateLoad(T_int8, emit_struct_gep(ctx, at, varg1,
                                                                           tindex_offset)));
                    ...
                    if (arg2.ispointer()) {
                        Value *tindex2 = ctx.builder.CreateNUWAdd(
                            ConstantInt::get(T_int8, 1),
                            ctx.builder.CreateLoad(T_int8, emit_struct_gep(ctx, at, varg2,
                                                                           tindex_offset)));
                    ...
                    jl_cgval_t fld1_info =
                        arg1.ispointer() ? mark_julia_slot(fld1, fldty, NULL, arg1.tbaa) :
                                           mark_julia_type(ctx, fld1, false, fldty);
                    jl_cgval_t fld2_info =
                        arg2.ispointer() ? mark_julia_slot(fld2, fldty, NULL, arg2.tbaa) :
                                           mark_julia_type(ctx, fld2, false, fldty);
...
static Value *emit_f_is(jl_codectx_t &ctx, const jl_cgval_t &arg1, const jl_cgval_t &arg2)
{
    ...
    if (jl_is_concrete_type(rt1) && jl_is_concrete_type(rt2) && !jl_is_kind(rt1) &&
        !jl_is_kind(rt2) && rt1 != rt2) {
    ...
    if (arg1.isghost || arg2.isghost) {
        ...
        return ctx.builder.CreateICmpEQ(mark_callee_rooted(boxed(ctx, arg1)),
                                        mark_callee_rooted(boxed(ctx, arg2)));
    }

    if (jl_type_intersection(rt1, rt2) ==
        (jl_value_t *)jl_bottom_type) // types are disjoint (exhaustive test)
        return ConstantInt::get(T_int1, 0);
...
// returns true if the call has been handled
{
    if (f == jl_builtin_is && nargs == 2) {
        ...
        if (v1.constant) {
            if (v2.constant) {
                *ret = mark_julia_type(
                    ctx, ConstantInt::get(T_int8, jl_egal(v1.constant, v2.constant)), false,
                    jl_bool_type);
                return true;
            }
        }
        ...
        *ret =
            mark_julia_type(ctx, ctx.builder.CreateZExt(ans, T_int8), false, jl_bool_type);
        return true;
    }
        ...
        *ret = emit_typeof(ctx, argv[1]);
        ...
        if (jl_is_type_type(ta.typ) && !jl_has_free_typevars(ta.typ) &&
            ...
            *ret =
                mark_julia_type(ctx, ConstantInt::get(T_int8, issub), false, jl_bool_type);
            return true;
        }
    }

    else if (((f == jl_builtin__apply && nargs == 2) ||
              (f == jl_builtin__apply_iterate && nargs == 3)) &&
             ctx.vaSlot > 0) {
        int arg_start = f == jl_builtin__apply ? 2 : 3;
        // turn Core._apply(f, Tuple) ==> f(Tuple...) using the jlcall calling convention if
        // Tuple is the va allocation
...
#endif
                Value *theArgs = ctx.builder.CreateInBoundsGEP(
                    T_prjlvalue, ctx.argArray, ConstantInt::get(T_size, ctx.nReqArgs));
                Value *r = ctx.builder.CreateCall(prepare_call(jlapplygeneric_func),
                                                  {theF, theArgs, nva});
        ...
        if (jl_is_tuple_type(rt) && jl_is_concrete_type(rt) &&
            nargs == jl_datatype_nfields(rt)) {
        ...
        *ret = jl_cgval_t();
                ...
                if (idx.constant) {
                    ...
                    if (idx_const > 0 && idx_const <= ndims) {
                        ...
                        *ret = mark_julia_type(ctx,
                                               emit_arraysize(ctx, ary, ary_ex, idx_const),
                                               false, jl_long_type);
                        return true;
                    }
                    else if (idx_const > ndims) {
                        *ret = mark_julia_type(ctx, ConstantInt::get(T_size, 1), false,
                                               jl_long_type);
                        return true;
                    }
                }
                    ...
                    Value *idx_dyn =
                        emit_unbox(ctx, T_size, idx, (jl_value_t *)jl_long_type);
                    error_unless(ctx,
                                 ctx.builder.CreateICmpSGT(idx_dyn,
                                                           ConstantInt::get(T_size, 0)),
                                 "arraysize: dimension out of range");
                    BasicBlock *outBB =
                        BasicBlock::Create(jl_LLVMContext, "outofrange", ctx.f);
                    BasicBlock *inBB = BasicBlock::Create(jl_LLVMContext, "inrange");
                    BasicBlock *ansBB = BasicBlock::Create(jl_LLVMContext, "arraysize");
                    ctx.builder.CreateCondBr(
                        ctx.builder.CreateICmpSLE(idx_dyn, ConstantInt::get(T_size, ndims)),
                        inBB, outBB);
            ...
            if (!jl_has_free_typevars(ety) && (jl_is_long(ndp) || nargs == 3)) {
                ...
                if (!isboxed && jl_is_datatype(ety) && jl_datatype_size(ety) == 0) {
                    ...
                    *ret = ghostValue(ety);
                    ...
                    Type *AT = ArrayType::get(IntegerType::get(jl_LLVMContext, 8 * al),
                                              (elsz + al - 1) / al);
                    Value *data = emit_bitcast(ctx, emit_arrayptr(ctx, ary, ary_ex),
                                               AT->getPointerTo());
                    // isbits union selector bytes are stored after a->maxsize
                    Value *ndims = (nd == -1 ? emit_arrayndims(ctx, ary) :
                                               ConstantInt::get(T_int16, nd));
                    Value *is_vector =
                        ctx.builder.CreateICmpEQ(ndims, ConstantInt::get(T_int16, 1));
                    Value *offset = emit_arrayoffset(ctx, ary, nd);
                    Value *selidx_v =
                        ctx.builder.CreateSub(emit_vectormaxsize(ctx, ary),
                                              ctx.builder.CreateZExt(offset, T_size));
                    ...
                    Instruction *tindex = tbaa_decorate(
                        tbaa_arrayselbyte, ctx.builder.CreateLoad(T_int8, ptindex));
                    tindex->setMetadata(
                        LLVMContext::MD_range,
                        MDNode::get(jl_LLVMContext,
                                    {ConstantAsMetadata::get(ConstantInt::get(T_int8, 0)),
                                     ConstantAsMetadata::get(
                                         ConstantInt::get(T_int8, union_max))}));
                    AllocaInst *lv = emit_static_alloca(ctx, AT);
                    if (al > 1)
                        lv->setAlignment(Align(al));
                    emit_memcpy(ctx, lv, tbaa_arraybuf,
                                ctx.builder.CreateInBoundsGEP(AT, data, idx), tbaa_arraybuf,
                                elsz, al, false);
                    *ret = mark_julia_slot(
                        lv, ety,
                        ctx.builder.CreateNUWAdd(ConstantInt::get(T_int8, 1), tindex),
                        tbaa_arraybuf);
                }
                else {
                    MDNode *aliasscope =
                        (f == jl_builtin_const_arrayref) ? ctx.aliasscope : nullptr;
                    *ret =
                        typed_load(ctx, emit_arrayptr(ctx, ary, ary_ex), idx, ety,
        ...
        if (jl_is_array_type(aty_dt) && indices_ok) {
            ...
            if (!jl_has_free_typevars(ety) && (jl_is_long(ndp) || nargs == 4)) {
                if (jl_subtype(
                        val.typ,
                        ety)) { // TODO: probably should just convert this to a type-assert
                    size_t elsz = 0, al = 0;
                    int union_max = jl_islayout_inline(ety, &elsz, &al);
                    bool isboxed = (union_max == 0);
                    if (isboxed)
                    ...
                    ssize_t nd = jl_is_long(ndp) ? jl_unbox_long(ndp) : -1;
                    jl_value_t *boundscheck = argv[1].constant;
                    Value *idx = emit_array_nd_index(ctx, ary, ary_ex, nd, &argv[4],
                                                     nargs - 3, boundscheck);
                    if (!isboxed && jl_is_datatype(ety) && jl_datatype_size(ety) == 0) {
                        // no-op
                    }
                        ...
                        PHINode *data_owner =
                            NULL; // owner object against which the write barrier must check
                        if (isboxed || (jl_is_datatype(ety) &&
                                        ((jl_datatype_t *)ety)->layout->npointers >
                                            0)) { // if elements are just bits, don't need a
                                                  // write barrier
                            ...
                            Value *is_owned = ctx.builder.CreateICmpEQ(
                                flags, ConstantInt::get(T_int16, 3));
                            BasicBlock *curBB = ctx.builder.GetInsertBlock();
                            BasicBlock *ownedBB =
                                BasicBlock::Create(jl_LLVMContext, "array_owned", ctx.f);
                            BasicBlock *mergeBB =
                                BasicBlock::Create(jl_LLVMContext, "merge_own", ctx.f);
                            ctx.builder.CreateCondBr(is_owned, ownedBB, mergeBB);
                            ctx.builder.SetInsertPoint(ownedBB);
                            // load owner pointer
                            Instruction *own_ptr;
                            if (jl_is_long(ndp)) {
                                own_ptr = ctx.builder.CreateLoad(
                                    T_prjlvalue, ctx.builder.CreateConstGEP1_32(
                                                     T_prjlvalue,
                                                     emit_bitcast(ctx, decay_derived(aryv),
                                                                  T_pprjlvalue),
                                ...
                                own_ptr = ctx.builder.CreateCall(
                                    prepare_call(jlarray_data_owner_func), {aryv});
                            }
                        ...
                        if (jl_is_uniontype(ety)) {
                            ...
                            tindex = ctx.builder.CreateNUWSub(tindex,
                                                              ConstantInt::get(T_int8, 1));
                            Value *ndims = (nd == -1 ? emit_arrayndims(ctx, ary) :
                                                       ConstantInt::get(T_int16, nd));
                            Value *is_vector = ctx.builder.CreateICmpEQ(
                                ndims, ConstantInt::get(T_int16, 1));
                            Value *offset = emit_arrayoffset(ctx, ary, nd);
                            Value *selidx_v = ctx.builder.CreateSub(
                                emit_vectormaxsize(ctx, ary),
                                ctx.builder.CreateZExt(offset, T_size));
                            Value *selidx_m = emit_arraylen(ctx, ary);
                            Value *selidx =
                                ctx.builder.CreateSelect(is_vector, selidx_v, selidx_m);
                            Value *ptindex =
                                ctx.builder.CreateInBoundsGEP(AT, data, selidx);
                            ptindex = emit_bitcast(ctx, ptindex, T_pint8);
                            ptindex =
                                ctx.builder.CreateInBoundsGEP(T_int8, ptindex, offset);
                            ptindex = ctx.builder.CreateInBoundsGEP(T_int8, ptindex, idx);
                            tbaa_decorate(tbaa_arrayselbyte,
                            ...
                            typed_store(ctx, emit_arrayptr(ctx, ary, ary_ex, isboxed), idx,
                                        val, ety,
        ...
        if (fld.typ == (jl_value_t *)jl_long_type) {
            if (ctx.vaSlot > 0) {
                ...
                if (LoadInst *load = dyn_cast_or_null<LoadInst>(obj.V)) {
                    if (load->getPointerOperand() == ctx.slots[ctx.vaSlot].boxroot &&
                        ...
                        jl_cgval_t va_ary( // fake instantiation of a cgval, in order to
                                           // call emit_bounds_check
                            ctx.builder.CreateInBoundsGEP(T_prjlvalue, ctx.argArray,
                                                          ConstantInt::get(T_size,
                                                                           ctx.nReqArgs)),
                            NULL, false, NULL, NULL);
                        Value *idx =
                            emit_unbox(ctx, T_size, fld, (jl_value_t *)jl_long_type);
                        jl_value_t *boundscheck = (nargs == 3 ? argv[3].constant : jl_true);
                        idx = emit_bounds_check(ctx, va_ary, NULL, idx, valen, boundscheck);
                        idx = ctx.builder.CreateAdd(idx,
                                                    ConstantInt::get(T_size, ctx.nReqArgs));
                        Instruction *v = ctx.builder.CreateLoad(
                            T_prjlvalue, ctx.builder.CreateInBoundsGEP(ctx.argArray, idx));
                        // if we know the result type of this load, we will mark that
                        // information here too
                        tbaa_decorate(tbaa_value,
                                      maybe_mark_load_dereferenceable(v, false, rt));
            ...
            if (jl_is_datatype(utt) && utt->layout) {
                if ((jl_is_structtype(utt) || jl_is_tuple_type(utt)) &&
                    !jl_subtype((jl_value_t *)jl_module_type, obj.typ)) {
                ...
                if (jl_is_tuple_type(utt) && is_tupletype_homogeneous(utt->types, true)) {
                    ...
                    if (obj.ispointer()) {
                        ...
                        if (!bounds_check_enabled(ctx, boundscheck)) {
                            ...
                            vidx = emit_bounds_check(
                                ctx, obj, (jl_value_t *)obj.typ, vidx,
        ...
        if (jl_is_type_type(obj.typ)) {
            ...
            if (jl_is_concrete_type(tp0)) {
                *ret =
                    mark_julia_type(ctx, ConstantInt::get(T_size, jl_datatype_nfields(tp0)),
                                    false, jl_long_type);
                return true;
            }
        }
        ...
        if ((jl_is_type_type(typ.typ) && jl_is_concrete_type(jl_tparam0(typ.typ))) ||
            ...
            if (fld.typ == (jl_value_t *)jl_long_type) {
                ...
                emit_bounds_check(ctx, typ, (jl_value_t *)jl_datatype_type, idx, types_len,
                                  boundscheck);
                Value *fieldtyp_p = ctx.builder.CreateInBoundsGEP(
                    T_prjlvalue, decay_derived(emit_bitcast(ctx, types_svec, T_pprjlvalue)),
                    idx);
                Value *fieldtyp = tbaa_decorate(
                    tbaa_const, ctx.builder.CreateLoad(T_prjlvalue, fieldtyp_p));
        ...
        if (sty == jl_string_type || sty == jl_simplevector_type) {
            ...
            *ret = mark_julia_type(ctx, ctx.builder.CreateMul(len, elsize), false,
                                   jl_long_type);
            return true;
        }
        ...
        if (jl_is_datatype(sty) && sty != jl_symbol_type &&
            sty->name != jl_array_typename && sty != jl_simplevector_type &&
            sty != jl_string_type &&
            ...
            if (jl_is_concrete_type((jl_value_t *)sty) ||
                ...
                *ret = mark_julia_type(ctx, ConstantInt::get(T_size, jl_datatype_size(sty)),
                                       false, jl_long_type);
                return true;
            }
        }
        ...
        if (jl_is_type_type((jl_value_t *)stt)) {
            ...
            // TODO: could use `issingletontype` predicate here, providing better type
            // knowledge than only handling DataType
        ...
        else if (jl_field_isptr(stt, fieldidx) ||
                 jl_type_hasptr(jl_field_type(stt, fieldidx))) {
            Value *fldv;
            size_t offs = jl_field_offset(stt, fieldidx) / sizeof(jl_value_t *);
            if (obj.ispointer()) {
                if (!jl_field_isptr(stt, fieldidx))
                    offs +=
                        ((jl_datatype_t *)jl_field_type(stt, fieldidx))->layout->first_ptr;
                Value *ptr = emit_bitcast(ctx, maybe_decay_tracked(data_pointer(ctx, obj)),
                                          T_pprjlvalue);
                Value *addr =
                    ctx.builder.CreateConstInBoundsGEP1_32(T_prjlvalue, ptr, offs);
                             ...
                             jl_cgval_t *argv, size_t nargs, CallingConv::ID cc)
{
    ...
    CallInst *result = ctx.builder.CreateCall(
        FTy,
...
static jl_cgval_t emit_call_specfun_other(jl_codectx_t &ctx, jl_method_instance_t *mi,
                                          jl_value_t *jlretty, StringRef specFunctionObject,
                                          jl_cgval_t *argv, size_t nargs,
                                          jl_returninfo_t::CallingConv *cc,
                                          unsigned *return_roots,
                                          jl_value_t *inferred_retty)
{
    // emit specialized call site
    jl_returninfo_t returninfo =
        get_specsig_function(ctx, jl_Module, specFunctionObject, mi->specTypes, jlretty);
    ...
    *return_roots = returninfo.return_roots;
    ...
    case jl_returninfo_t::Ghosts: break;
    ...
    for (size_t i = 0; i < nargs; i++) {
        ...
        if (isboxed) {
            ...
            argvals[idx] = decay_derived(maybe_bitcast(ctx, data_pointer(ctx, arg), at));
        }
        else {
            assert(at == et);
    ...
    assert(idx == nfargs);
    CallInst *call =
        ctx.builder.CreateCall(returninfo.decl, ArrayRef<Value *>(&argvals[0], nfargs));
    ...
    case jl_returninfo_t::Union: {
        ...
        Value *derived = ctx.builder.CreateSelect(
            ctx.builder.CreateICmpEQ(ctx.builder.CreateAnd(tindex,
                                                           ConstantInt::get(T_int8, 0x80)),
                                     ConstantInt::get(T_int8, 0)),
            decay_derived(ctx.builder.CreateBitCast(argvals[0], T_pjlvalue)),
            decay_derived(box));
        retval = mark_julia_slot(derived, jlretty, tindex, tbaa_stack);
        retval.Vboxed = box;
        break;
    }
...
static jl_cgval_t emit_call_specfun_boxed(jl_codectx_t &ctx, StringRef specFunctionObject,
                                          ...
                                          jl_value_t *inferred_retty)
{
    auto theFptr = jl_Module
                       ->getOrInsertFunction(specFunctionObject, jl_func_sig)
...
static jl_cgval_t emit_invoke(jl_codectx_t &ctx, jl_expr_t *ex, jl_value_t *rt)
{
    ...
    if (lival.constant) {
        ...
        if (mi == ctx.linfo) {
            ...
            if (ft == jl_func_sig) {
                ...
                result = emit_call_specfun_other(ctx, mi, ctx.rettype, protoname, argv,
                                                 nargs, &cc, &return_roots, rt);
                handled = true;
            }
        }
        else {
            jl_value_t *ci = jl_rettype_inferred(
                mi, ctx.world, ctx.world); // TODO: need to use the right pair world here
            jl_code_instance_t *codeinst = (jl_code_instance_t *)ci;
            if (ci != jl_nothing &&
                codeinst->invoke != jl_fptr_sparam) { // check if we know we definitely
                                                      // can't handle this specptr
                if (codeinst->invoke == jl_fptr_const_return) {
                    ...
                    std::tie(specsig, needsparams) =
                        uses_specsig(mi, codeinst->rettype, ctx.params->prefer_specsig);
                    std::string name;
                    StringRef protoname;
                    ...
                    if (ctx.use_cache) {
                        ...
                        if (codeinst->specptr.fptr) {
                            if (specsig ? codeinst->isspecsig :
                                          codeinst->invoke == jl_fptr_args) {
                                protoname = jl_ExecutionEngine->getFunctionAtAddress(
                                    (uintptr_t)codeinst->specptr.fptr, codeinst);
                                need_to_emit = false;
                            }
                        }
                    }
                    if (need_to_emit) {
                        ...
                        namestream << (specsig ? "j_" : "j1_")
                                   << name_from_method_instance(mi) << "_"
                                   << globalUnique++;
                        name = namestream.str();
                    ...
                    if (specsig)
                        result =
                            emit_call_specfun_other(ctx, mi, codeinst->rettype, protoname,
                                                    argv, nargs, &cc, &return_roots, rt);
                    else
                        result = emit_call_specfun_boxed(ctx, protoname, argv, nargs, rt);
                    handled = true;
                    if (need_to_emit) {
                        Function *trampoline_decl =
                            cast<Function>(jl_Module->getNamedValue(protoname));
                        ctx.call_targets.push_back(std::make_tuple(
                            codeinst, cc, return_roots, trampoline_decl, specsig));
                    }
                }
            }
        }
    ...
    if (!handled) {
        Value *r = emit_jlcall(ctx, prepare_call(jlinvoke_func).getCallee(),
                               boxed(ctx, lival), argv, nargs, JLCALL_F2_CC);
...
static jl_cgval_t emit_call(jl_codectx_t &ctx, jl_expr_t *ex, jl_value_t *rt)
{
    ...
    if (f.constant && jl_isa(f.constant, (jl_value_t *)jl_builtin_type)) {
        ...
        std::map<jl_fptr_args_t, Function *>::iterator it =
            builtin_func_map.find(jl_get_builtin_fptr(f.constant));
        if (it != builtin_func_map.end()) {
            Value *theFptr = it->second;
            Value *ret = emit_jlcall(ctx, theFptr, maybe_decay_untracked(V_null), &argv[1],
                                     nargs - 1, JLCALL_F_CC);
    ...
    Value *callval =
        emit_jlcall(ctx, jlapplygeneric_func, nullptr, argv, nargs, JLCALL_F_CC);
...
static void undef_var_error_ifnot(jl_codectx_t &ctx, Value *ok, jl_sym_t *name)
{
    ...
    ctx.builder.CreateCall(
        prepare_call(jlundefvarerror_func),
    ...
    if (assign) {
        ...
        if (b->owner != m) {
            ...
            (void)asprintf(&msg, "cannot assign a value to variable %s.%s from module %s",
                           jl_symbol_name(b->owner->name), jl_symbol_name(s),
        ...
        if (b == NULL) {
            ...
            GlobalVariable *bindinggv =
                new GlobalVariable(*ctx.f->getParent(), T_pjlvalue, false,
                                   GlobalVariable::InternalLinkage, initnul, name.str());
            ...
            ctx.builder.CreateCondBr(ctx.builder.CreateICmpNE(cachedval, initnul), have_val,
                                     not_found);
            ctx.f->getBasicBlockList().push_back(not_found);
            ctx.builder.SetInsertPoint(not_found);
            Value *bval =
                ctx.builder.CreateCall(prepare_call(jlgetbindingorerror_func),
...
static jl_cgval_t emit_sparam(jl_codectx_t &ctx, size_t i)
{
    ...
    Value *bp = ctx.builder.CreateConstInBoundsGEP1_32(
        T_prjlvalue, ctx.spvals_ptr, i + sizeof(jl_svec_t) / sizeof(jl_value_t *));
    Value *sp = tbaa_decorate(tbaa_const, ctx.builder.CreateLoad(T_prjlvalue, bp));
    Value *isnull = ctx.builder.CreateICmpNE(
        emit_typeof(ctx, sp),
...
static jl_cgval_t emit_global(jl_codectx_t &ctx, jl_sym_t *sym)
{
    ...
    if (jbp && jbp->value != NULL) {
        ...
        return mark_julia_type(
            ctx, tbaa_decorate(tbaa_binding, ctx.builder.CreateLoad(T_prjlvalue, bp)), true,
            jl_any_type);
    }
...
static jl_cgval_t emit_isdefined(jl_codectx_t &ctx, jl_value_t *sym)
{
    ...
    if (jl_is_slot(sym)) {
        ...
        if (vi.boxroot != NULL) {
            ...
            Value *box_isnull =
                ctx.builder.CreateICmpNE(boxed, maybe_decay_untracked(V_null));
            if (vi.pTIndex) {
        ...
        assert(((jl_expr_t *)sym)->head == static_parameter_sym &&
               "malformed isdefined expression");
        ...
        Value *bp = ctx.builder.CreateConstInBoundsGEP1_32(
            T_prjlvalue, ctx.spvals_ptr, i + sizeof(jl_svec_t) / sizeof(jl_value_t *));
        Value *sp = tbaa_decorate(tbaa_const, ctx.builder.CreateLoad(T_prjlvalue, bp));
        isnull = ctx.builder.CreateICmpNE(
            emit_typeof(ctx, sp),
            maybe_decay_untracked(literal_pointer_val(ctx, (jl_value_t *)jl_tvar_type)));
    }
    else {
        ...
        if (bnd) {
            ...
            Value *v =
                ctx.builder.CreateCall(prepare_call(jlboundp_func),
...
static jl_cgval_t emit_varinfo(jl_codectx_t &ctx, jl_varinfo_t &vi, jl_sym_t *varname,
                               jl_value_t *better_typ = NULL)
{
    jl_value_t *typ = better_typ ? better_typ : vi.value.typ;
    jl_cgval_t v;
    Value *isnull = NULL;
    if (vi.boxroot == NULL || vi.pTIndex != NULL) {
        if ((!vi.isVolatile && vi.isSA) || vi.isArgument || vi.value.constant ||
            !vi.value.V) {
            v = vi.value;
            if (vi.pTIndex)
            ...
            if (vi.isVolatile) {
                Value *unbox = ctx.builder.CreateAlignedLoad(
                    ssaslot->getAllocatedType(), varslot, varslot->getAlignment(), true);
        ...
        if (vi.pTIndex) {
            ...
            if (v.V) { // v.V will be null if it is a union of all ghost values
                v.V = ctx.builder.CreateSelect(
                    load_unbox, emit_bitcast(ctx, decay_derived(v.V), boxed->getType()),
                    decay_derived(boxed));
            }
            else
                v.V = boxed;
...
static void emit_vi_assignment_unboxed(jl_codectx_t &ctx, jl_varinfo_t &vi, Value *isboxed,
        ...
        if (vi.value.V == NULL) {
            ...
            if (rval_info.isghost) {
                ...
                if (rval_info.typ != vi.value.typ && !vi.pTIndex && !rval_info.TIndex) {
                    // isbits cast-on-assignment is invalid. this branch should be
                    // dead-code.
                    CreateTrap(ctx.builder);
                }
                else {
                    ...
                    if (vi.pTIndex)
                        ctx.builder.CreateStore(
                            UndefValue::get(
                                cast<AllocaInst>(vi.value.V)->getAllocatedType()),
                            vi.value.V);
                    Type *store_ty =
                        julia_type_to_llvm(ctx, rval_info.constant ?
                                                    jl_typeof(rval_info.constant) :
                                                    rval_info.typ);
                    Type *dest_ty = store_ty->getPointerTo();
                    if (dest_ty != dest->getType())
                        dest = emit_bitcast(ctx, dest, dest_ty);
                    tbaa_decorate(tbaa_stack,
                                  ctx.builder.CreateStore(
            ...
            if (vi.pTIndex == NULL) {
                ...
                if (vi.value.V != rval_info.V) {
                    Value *copy_bytes =
                        ConstantInt::get(T_int32, jl_datatype_size(vi.value.typ));
                    emit_memcpy(ctx, vi.value.V, tbaa_stack, rval_info, copy_bytes,
                                julia_alignment(rval_info.typ), vi.isVolatile);
                }
            }
            else {
                emit_unionmove(ctx, vi.value.V, tbaa_stack, rval_info, isboxed,
                               vi.isVolatile);
            }
        }
    }
...
static void emit_phinode_assign(jl_codectx_t &ctx, ssize_t idx, jl_value_t *r)
{
    ...
    if (jl_is_uniontype(phiType)) {
        ...
        dest = try_emit_union_alloca(ctx, ((jl_uniontype_t *)phiType), allunbox, min_align,
                                     nbytes);
        if (dest) {
            Instruction *phi = dest->clone();
            phi->insertAfter(dest);
            PHINode *Tindex_phi =
                PHINode::Create(T_int8, jl_array_len(edges), "tindex_phi");
            ...
            ctx.builder.CreateMemCpy(phi, MaybeAlign(min_align), dest, MaybeAlign(0),
                                     nbytes, false);
#else
            ctx.builder.CreateMemCpy(phi, min_align, dest, 0, nbytes, false);
#endif
            ctx.builder.CreateLifetimeEnd(dest);
            Value *ptr = ctx.builder.CreateSelect(
                isboxed, maybe_bitcast(ctx, decay_derived(ptr_phi), T_pint8),
                maybe_bitcast(ctx, decay_derived(phi), T_pint8));
            jl_cgval_t val =
                mark_julia_slot(ptr, phiType, Tindex_phi,
                                tbaa_stack); // XXX: this TBAA is wrong for ptr_phi
            val.Vboxed = ptr_phi;
...
#if JL_LLVM_VERSION >= 110000
        ctx.builder.CreateMemCpy(phi, MaybeAlign(julia_alignment(phiType)), dest,
                                 MaybeAlign(0), jl_datatype_size(phiType), false);
#else
        ctx.builder.CreateMemCpy(phi, julia_alignment(phiType), dest, 0,
...
static void emit_ssaval_assign(jl_codectx_t &ctx, ssize_t idx, jl_value_t *r)
{
    ...
    if (jl_is_phicnode(r)) {
        ...
        slot = emit_expr(
            ctx, r, idx); // slot could be a jl_value_t (unboxed) or jl_value_t* (ispointer)
    }
...
static void emit_varinfo_assign(jl_codectx_t &ctx, jl_varinfo_t &vi, jl_cgval_t rval_info,
                                jl_value_t *l = NULL)
{
    if (!vi.used)
        return;
...
static void emit_assignment(jl_codectx_t &ctx, jl_value_t *l, jl_value_t *r, ssize_t ssaval)
{
    ...
    if (jl_is_symbol(l))
        ...
        bp = global_binding_pointer(ctx, jl_globalref_mod(l), jl_globalref_name(l), &bnd,
                                    true); // now bp != NULL
    else
        assert(jl_is_slot(l));
    if (bp == NULL && s != NULL)
    ...
    if (bp != NULL) { // it's a global
        ...
        ctx.builder.CreateCall(prepare_call(jlcheckassign_func),
                               {literal_pointer_val(ctx, bnd), rval});
...
static jl_cgval_t emit_cfunction(jl_codectx_t &ctx, jl_value_t *output_type,
                                 const jl_cgval_t &fexpr, jl_value_t *rt, jl_svec_t *argt);
                             ...
                             const std::string &msg)
{
    ...
    if (isbool) {
        ...
        return ctx.builder.CreateXor(ctx.builder.CreateTrunc(cond, T_int1),
                                     ConstantInt::get(T_int1, 1));
    }
    if (condV.isboxed) {
        return ctx.builder.CreateICmpEQ(
            boxed(ctx, condV), maybe_decay_untracked(literal_pointer_val(ctx, jl_false)));
    }
    // not a boolean
...
static jl_cgval_t emit_expr(jl_codectx_t &ctx, jl_value_t *expr, ssize_t ssaval)
{
    ...
    if (jl_is_ssavalue(expr)) {
        ...
        if (!ctx.ssavalue_assigned.at(idx)) {
            ...
            return ctx.SAvalues.at(
                idx); // at this point, SAvalues[idx] actually contains the SAvalue
        }
    }
    ...
    if (jl_is_pinode(expr)) {
        return convert_julia_type(ctx, emit_expr(ctx, jl_fieldref_noalloc(expr, 0)),
                                  jl_fieldref_noalloc(expr, 1));
    }
    if (!jl_is_expr(expr)) {
        int needroot = true;
        ...
        if (needroot &&
            jl_is_method(ctx.linfo->def.method)) { // toplevel exprs and some integers are
                                                   // already rooted
    ...
    if (head == isdefined_sym) {
        ...
        Value *cond = ctx.builder.CreateTrunc(
            emit_unbox(ctx, T_int8, emit_expr(ctx, args[1]), (jl_value_t *)jl_bool_type),
            T_int1);
        ...
        assert(ssaval >= 0);
        jl_value_t *expr_t = jl_is_long(ctx.source->ssavaluetypes) ?
        ...
        if (ssaval < 0)
            ...
            expr_t = jl_is_long(ctx.source->ssavaluetypes) ?
                         (jl_value_t *)jl_any_type :
                         jl_array_ptr_ref(ctx.source->ssavaluetypes, ssaval);
        ...
        if (jl_is_symbol(sym)) {
            jl_binding_t *bnd = NULL;
            (void)global_binding_pointer(ctx, mod, sym, &bnd, true);
            assert(bnd);
        ...
        return mark_julia_type(
            ctx, ctx.builder.CreateCall(prepare_call(jl_current_exception_func)), true,
            jl_any_type);
    }
        ...
        return mark_julia_type(
            ...
            ctx.builder.CreateCall(prepare_call(jlcopyast_func),
                                   maybe_decay_untracked(boxed(ctx, ast))),
            true, jl_expr_type);
    }
        ...
        if (jl_is_toplevel_only_expr(expr) && !jl_is_method(ctx.linfo->def.method)) {
            // call interpreter to run a toplevel expr from inside a
            // compiled toplevel thunk.
            Value *args[2] = {literal_pointer_val(ctx, (jl_value_t *)ctx.module),
...
static Function *emit_tojlinvoke(jl_code_instance_t *codeinst, Module *M,
                                 jl_codegen_params_t &params)
{
    ...
    Function *f =
        Function::Create(jl_func_sig, GlobalVariable::PrivateLinkage, name.str(), M);
    jl_init_function(f);
    f->addFnAttr(Thunk);
    // f->setAlwaysInline();
    ...
    if (codeinst->invoke != NULL) {
        StringRef theFptrName =
            jl_ExecutionEngine->getFunctionAtAddress((uintptr_t)codeinst->invoke, codeinst);
...
static void emit_cfunc_invalidate(Function *gf_thunk, jl_returninfo_t::CallingConv cc,
                                  unsigned return_roots, jl_value_t *calltype,
                                  jl_value_t *rettype, size_t nargs,
                                  jl_codegen_params_t &params,
                                  Function *target = jlapplygeneric_func)
{
    ...
    switch (cc) {
    case jl_returninfo_t::Boxed: ctx.builder.CreateRet(gf_ret); break;
    case jl_returninfo_t::Register: {
        ...
        if (gfrt->isVoidTy()) {
            ...
            ctx.builder.CreateRet(
                ctx.builder.CreateAlignedLoad(gf_ret, julia_alignment(rettype)));
        }
        break;
    }
    case jl_returninfo_t::SRet: {
        ...
        emit_memcpy(ctx, &*gf_thunk->arg_begin(), nullptr, gf_ret, nullptr,
                    jl_datatype_size(rettype), julia_alignment(rettype));
        ctx.builder.CreateRetVoid();
        break;
    }
    case jl_returninfo_t::Union: {
        ...
        Value *tindex = compute_box_tindex(ctx, emit_typeof_boxed(ctx, gf_retbox),
                                           (jl_value_t *)jl_any_type, rettype);
        tindex = ctx.builder.CreateOr(tindex, ConstantInt::get(T_int8, 0x80));
...
static Function *gen_cfun_wrapper(Module *into, jl_codegen_params_t &params,
                                  const function_sig_t &sig, jl_value_t *ff,
                                  const char *aliasname, jl_value_t *declrt,
                                  jl_method_instance_t *lam, jl_unionall_t *unionall_env,
                                  jl_svec_t *sparam_vals, jl_array_t **closure_types)
{
    ...
    if (lam && params.cache) {
        // TODO: this isn't ideal to be unconditionally calling type inference (and compile)
        // from here
    ...
    if (nest) {
        ...
        functype =
            FunctionType::get(sig.sret ? T_void : sig.prt, fargt_sig, /*isVa*/ false);
    ...
    Function *cw =
        Function::Create(functype, GlobalVariable::ExternalLinkage, funcName.str(), M);
    ...
    ctx.world_age_field =
        ctx.builder.CreateSelect(have_tls, ctx.world_age_field, dummy_world);
    Value *last_age =
        tbaa_decorate(tbaa_gcframe, ctx.builder.CreateLoad(ctx.world_age_field));
    Value *valid_tls = ctx.builder.CreateIsNotNull(last_age);
    have_tls = ctx.builder.CreateAnd(have_tls, valid_tls);
    ctx.world_age_field =
        ctx.builder.CreateSelect(valid_tls, ctx.world_age_field, dummy_world);
    Value *world_v = ctx.builder.CreateLoad(prepare_global(jlgetworld_global));

    Value *age_ok = NULL;
    if (calltype) {
        Value *lam_max = ctx.builder.CreateLoad(
            T_size, ctx.builder.CreateConstInBoundsGEP1_32(
                        T_size,
                        emit_bitcast(ctx, literal_pointer_val(ctx, (jl_value_t *)codeinst),
                                     T_psize),
                        offsetof(jl_code_instance_t, max_world) / sizeof(size_t)));
        // XXX: age is always OK if we don't have a TLS. This is a hack required due to
        // `@threadcall` abuse. and adds quite a bit of complexity here, even though it's
        // still wrong (anything that tries to interact with the runtime will fault)
        age_ok = ctx.builder.CreateICmpUGE(lam_max, world_v);
        world_v = ctx.builder.CreateSelect(ctx.builder.CreateOr(have_tls, age_ok), world_v,
                                           lam_max);
    ...
    for (size_t i = 0; i < nargs; ++i, ++AI) {
        ...
        // so that the julia_to_native type checks are more likely to be doable (e.g.
        // concrete types) at compile-time
        jl_value_t *jargty_proper = jargty;
        bool static_at =
            !(unionall_env && jl_has_typevar_from_unionall(jargty, unionall_env));
        if (!static_at) {
            if (sparam_vals) {
                jargty_proper = rt1 = jl_instantiate_type_in_env(jargty, unionall_env,
                                                                 jl_svec_data(sparam_vals));
                ...
                jargty_proper = rt1 =
                    jl_rewrap_unionall(jargty, (jl_value_t *)unionall_env);
            }
        }
        ...
        if (aref) {
            if (jargty == (jl_value_t *)jl_any_type) {
                inputarg =
                    mark_julia_type(ctx,
                                    ctx.builder.CreateLoad(
                                        T_prjlvalue, emit_bitcast(ctx, val, T_pprjlvalue)),
                                    true, jl_any_type);
            }
            else if (static_at && jl_is_concrete_immutable(
                                      jargty)) { // anything that could be stored unboxed
                bool isboxed;
                ...
                if (type_is_ghost(T)) {
                    ...
                    val = ctx.builder.CreateAlignedLoad(
                        val, 1); // make no alignment assumption about pointer from C
                    inputarg = mark_julia_type(ctx, val, false, jargty);
                }
            }
            else if (static_at ||
                     (!jl_is_typevar(jargty) && !jl_is_immutable_datatype(jargty))) {
                // must be a jl_value_t* (because it's mutable or contains gc roots)
                inputarg = mark_julia_type(
                    ctx, maybe_decay_untracked(emit_bitcast(ctx, val, T_prjlvalue)), true,
                    jargty_proper);
            }
            else {
                ...
                Value *runtime_dt = ctx.builder.CreateLoad(
                    T_prjlvalue, ctx.builder.CreateConstGEP1_32(
                                     T_prjlvalue, nestPtr, jl_array_len(*closure_types)));
                ...
                BasicBlock *unboxedBB =
                    BasicBlock::Create(jl_LLVMContext, "maybe-unboxed", cw);
                ...
                Value *p2 = ctx.builder.CreateLoad(
                    T_prjlvalue, ctx.builder.CreateBitCast(val, T_pprjlvalue));
                ctx.builder.CreateBr(afterBB);
            ...
            if (argboxed) {
                ...
                if (sig.byRefList.at(i)) {
                    assert(cast<PointerType>(val->getType())->getElementType() ==
                           sig.fargt[i]);
                    val = ctx.builder.CreateAlignedLoad(val, 1); // unknown alignment from C
                }
                else {
                    bool issigned = jl_signed_type &&
                                    jl_subtype(jargty_proper, (jl_value_t *)jl_signed_type);
                    val = llvm_type_rewrite(ctx, val, sig.fargt[i], issigned);
                }
                // passed an unboxed T, but may need something boxed (not valid to be
                // unboxed)
                if (static_at) {
                    bool isboxed;
                    ...
                    if (isboxed)
                        inputarg = mark_julia_type(
                            ctx,
                            box_ccall_result(ctx, val, literal_pointer_val(ctx, jargty),
                                             jargty),
                            true, jargty_proper);
                    else
                        inputarg = mark_julia_type(ctx, val, false, jargty);
                    ...
                    Value *runtime_dt = ctx.builder.CreateLoad(
                        T_prjlvalue,
                        ctx.builder.CreateConstGEP1_32(T_prjlvalue, nestPtr,
                                                       jl_array_len(*closure_types)));
    ...
    jl_cgval_t retval;
    if (calltype == 2) {
        nargs = 0; // arguments not needed -- TODO: not really true, should emit an age_ok
                   // test and jlcall
        jlfunc_sret = false;
        ...
        if (calltype == 1) {
            StringRef fname =
                jl_ExecutionEngine->getFunctionAtAddress((uintptr_t)callptr, codeinst);
        ...
        Value *ret = emit_jlcall(ctx, prepare_call(jlapplygeneric_func).getCallee(), NULL,
                                 inputargs, nargs + 1, JLCALL_F_CC);
        if (age_ok) {
            ctx.builder.CreateBr(b_after);
        ...
        StringRef protoname =
            jl_ExecutionEngine->getFunctionAtAddress((uintptr_t)callptr, codeinst);
        jl_returninfo_t returninfo =
            get_specsig_function(ctx, M, protoname, lam->specTypes, astrt);
        FunctionType *cft = returninfo.decl->getFunctionType();
        ...
        if (jlfunc_sret || returninfo.cc == jl_returninfo_t::Union) {
            ...
            if (sig.sret && jlfunc_sret) {
                ...
                result =
                    emit_static_alloca(ctx, cft->getParamType(0)->getPointerElementType());
            }
        ...
        if (age_ok) {
            funcName << "_gfthunk";
            Function *gf_thunk =
                Function::Create(returninfo.decl->getFunctionType(),
            ...
            // this builds a method that calls jl_apply_generic (as a closure over a
            // singleton function pointer), but which has the signature of a specsig
            emit_cfunc_invalidate(gf_thunk, returninfo.cc, returninfo.return_roots,
                                  lam->specTypes, codeinst->rettype, nargs + 1,
                                  ctx.emission_context);
            theFptr = ctx.builder.CreateSelect(age_ok, theFptr, gf_thunk);
        }
        CallInst *call = ctx.builder.CreateCall(
            cast<FunctionType>(theFptr->getType()->getPointerElementType()), theFptr,
            ArrayRef<Value *>(args));
        call->setAttributes(returninfo.decl->getAttributes());
        switch (returninfo.cc) {
        case jl_returninfo_t::Boxed:
        ...
        case jl_returninfo_t::Union: {
            ...
            Value *derived = ctx.builder.CreateSelect(
                ...
                decay_derived(box));
            retval = mark_julia_slot(derived, astrt, tindex, tbaa_stack);
            retval.Vboxed = box;
            break;
        }
        case jl_returninfo_t::Ghosts:
    ...
    if (nest) {
        funcName << "make";
        Function *cw_make =
            Function::Create(FunctionType::get(T_pint8, {T_pint8, T_ppjlvalue}, false),
                             GlobalVariable::ExternalLinkage, funcName.str(), M);
        ...
        Function *init_trampoline =
            Intrinsic::getDeclaration(cw_make->getParent(), Intrinsic::init_trampoline);
        Function *adjust_trampoline =
            Intrinsic::getDeclaration(cw_make->getParent(), Intrinsic::adjust_trampoline);
        cwbuilder.CreateCall(init_trampoline, {Tramp, cwbuilder.CreateBitCast(cw, T_pint8),
    ...
    if (aliasname) {
        GlobalAlias::create(cw->getType()->getElementType(),
                            cw->getType()->getAddressSpace(), GlobalValue::ExternalLinkage,
                            aliasname, cw, M);
    }
    if (!into)
...
static jl_cgval_t emit_cfunction(jl_codectx_t &ctx, jl_value_t *output_type,
                                 const jl_cgval_t &fexpr_rt, jl_value_t *declrt,
                                 jl_svec_t *argt)
{
    jl_unionall_t *unionall_env = (jl_is_method(ctx.linfo->def.method) &&
                                   jl_is_unionall(ctx.linfo->def.method->sig)) ?
    ...
    jl_value_t *sigt = NULL; // dispatch-sig = type signature with Ref{} annotations removed
    ...
    const std::string err = verify_ccall_sig(
        /* inputs:  */
        rt, (jl_value_t *)argt, unionall_env, sparam_vals, &ctx.emission_context,
        /* outputs: */
    ...
    function_sig_t sig("cfunction", lrt, rt, retboxed, argt, unionall_env, false,
                       CallingConv::C, false, &ctx.emission_context);
    ...
    // compute+verify the dispatch signature, and see if it depends on the environment
    // sparams
    bool approx = false;
    sigt = (jl_value_t *)jl_alloc_svec(nargt + 1);
    jl_svecset(sigt, 0, fexpr_rt.typ);
    if (!fexpr_rt.constant &&
        (!jl_is_concrete_type(fexpr_rt.typ) || jl_is_kind(fexpr_rt.typ)))
        approx = true;
    for (size_t i = 0; i < nargt; i++) {
        ...
        if (unionall_env && jl_has_typevar_from_unionall(jargty, unionall_env)) {
            if (sparam_vals)
                jargty = jl_instantiate_type_in_env(jargty, unionall_env,
                                                    jl_svec_data(sparam_vals));
            else
                approx = true;
        }
    ...
    jl_method_instance_t *lam = sigt ? jl_get_specialization1((jl_tupletype_t *)sigt, world,
                                                              &min_valid, &max_valid, 0) :
                                       NULL;
    Value *F =
        gen_cfun_wrapper(jl_Module, ctx.emission_context, sig, fexpr_rt.constant, NULL,
                         declrt, lam, unionall_env, sparam_vals, &closure_types);
    bool outboxed;
    if (nest) {
        ...
        Value *cache =
            new GlobalVariable(*jl_Module, T_htable, false, GlobalVariable::InternalLinkage,
                               ConstantAggregateZero::get(T_htable), cname.str());
        F = ctx.builder.CreateCall(
            prepare_call(jlgetcfunctiontrampoline_func),
            {fobj, literal_pointer_val(ctx, output_type),
             ctx.builder.CreateBitCast(cache, T_pint8),
             literal_pointer_val(ctx, (jl_value_t *)fill), F,
             closure_types ? literal_pointer_val(ctx, (jl_value_t *)unionall_env) : V_null,
             closure_types ? ctx.spvals_ptr :
                             ConstantPointerNull::get(cast<PointerType>(T_pprjlvalue))});
        outboxed = true;
    }
    else {
        ...
        if (outboxed) {
            ...
            Value *strct =
                emit_allocobj(ctx, jl_datatype_size(output_type),
            ...
            tbaa_decorate(tbaa,
                          ctx.builder.CreateStore(
                              ctx.builder.CreatePtrToInt(
                                  literal_pointer_val(ctx, fexpr_rt.constant), T_size),
                              ctx.builder.CreateConstGEP1_32(T_size, derived_strct, 1)));
            Value *zero = ConstantInt::get(T_size, 0);
            tbaa_decorate(tbaa,
                          ctx.builder.CreateStore(zero, ctx.builder.CreateConstGEP1_32(
                                                            T_size, derived_strct, 2)));
            tbaa_decorate(tbaa,
                          ctx.builder.CreateStore(zero, ctx.builder.CreateConstGEP1_32(
                                                            T_size, derived_strct, 3)));
            F = strct;
        }
    }
    JL_GC_POP();
                                ...
                                jl_value_t *sigt, jl_codegen_params_t &params)
{
    ...
    jl_value_t *err;
    { // scope block for sig
        function_sig_t sig("cfunction", lcrt, crt, toboxed, argtypes, NULL, false,
                           CallingConv::C, false, &params);
        if (sig.err_msg.empty()) {
            size_t world = jl_world_counter;
            size_t min_valid = 0;
            size_t max_valid = ~(size_t)0;
            ...
            if (sysimg_handle) {
                ...
                if (found) {
                    ...
                    F = Function::Create(ftype, GlobalVariable::ExternalLinkage, name,
                                         shadow_output);
                    add_named_global(F, addr);
                }
            }
            else {
                jl_method_instance_t *lam = jl_get_specialization1(
                    (jl_tupletype_t *)sigt, world, &min_valid, &max_valid, 0);
...
static Function *gen_invoke_wrapper(jl_method_instance_t *lam, jl_value_t *jlretty,
                                    const jl_returninfo_t &f, int retarg,
                                    StringRef funcName, Module *M,
                                    jl_codegen_params_t &params)
{
    Function *w =
        Function::Create(jl_func_sig, GlobalVariable::ExternalLinkage, funcName, M);
    ...
    for (size_t i = 0; i < jl_nparams(lam->specTypes) && idx < nfargs; ++i) {
        ...
        if (i == 0) {
            ...
            Value *argPtr =
                ctx.builder.CreateConstInBoundsGEP1_32(T_prjlvalue, argArray, i - 1);
            theArg = maybe_mark_load_dereferenceable(
                ctx.builder.CreateLoad(T_prjlvalue, argPtr), false, ty);
        }
        if (!isboxed) {
            theArg = decay_derived(emit_bitcast(ctx, theArg, PointerType::get(lty, 0)));
            if (!lty->isAggregateType()) // keep "aggregate" type values in place as
                                         // pointers
    ...
    if (retarg != -1) {
        ...
        case jl_returninfo_t::Union:
            ...
            retval = mark_julia_slot(result, jlretty,
                                     ctx.builder.CreateExtractValue(call, 1), tbaa_stack);
                                            ...
                                            jl_value_t *sig, jl_value_t *jlrettype)
{
    ...
    Type *rt;
    if (jl_is_structtype(jlrettype) &&
        jl_is_datatype_singleton((jl_datatype_t *)jlrettype)) {
        rt = T_void;
        props.cc = jl_returninfo_t::Register;
    }
    else if (jl_is_uniontype(jlrettype)) {
        bool allunbox;
        union_alloca_type((jl_uniontype_t *)jlrettype, allunbox, props.union_bytes,
                          props.union_align, props.union_minalign);
    ...
    for (size_t i = 0; i < jl_nparams(sig); i++) {
        ...
        if (ty->isAggregateType()) { // aggregate types are passed by pointer
            attributes =
                attributes.addParamAttribute(jl_LLVMContext, argno, Attribute::NoCapture);
            attributes =
                attributes.addParamAttribute(jl_LLVMContext, argno, Attribute::ReadOnly);
...
static void emit_sret_roots(jl_codectx_t &ctx, bool isptr, Value *Src, Type *T,
                            Value *Shadow, unsigned count)
{
    if (isptr)
        Src = maybe_decay_tracked(Src);
    if (isptr && Src->getType()->getPointerElementType() != T)
        Src = ctx.builder.CreateBitCast(
            Src, T->getPointerTo(Src->getType()->getPointerAddressSpace()));
...
static DISubroutineType *get_specsig_di(jl_codectx_t &ctx, jl_value_t *rt, jl_value_t *sig,
                                        DIBuilder &dbuilder)
{
    size_t nargs = jl_nparams(sig); // TODO: if this is a Varargs function, our debug info
                                    // for the `...` var may be misleading
              ...
              jl_codegen_params_t &params)
{
    ...
    int n_ssavalues = jl_is_long(src->ssavaluetypes) ? jl_unbox_long(src->ssavaluetypes) :
    ...
    bool specsig, needsparams;
    std::tie(specsig, needsparams) =
        uses_specsig(lam, jlrettype, params.params->prefer_specsig);
    if (!src->inferred)
        specsig = false;
    ...
    for (i = 0; i < vinfoslen; i++) {
        ...
        varinfo.usedUndef =
            (jl_vinfo_usedundef(flags) != 0) || (!varinfo.isArgument && !src->inferred);
        if (!varinfo.isArgument) {
            varinfo.value =
                mark_julia_type(ctx, (Value *)NULL, false, (jl_value_t *)jl_any_type);
        }
    }
    ...
    if (specsig) { // assumes !va and !needsparams
        returninfo = get_specsig_function(ctx, M, declarations.specFunctionObject,
                                          lam->specTypes, jlrettype);
        f = returninfo.decl;
        has_sret = (returninfo.cc == jl_returninfo_t::SRet ||
                    returninfo.cc == jl_returninfo_t::Union);
        jl_init_function(f);
        ...
        (void)gen_invoke_wrapper(lam, jlrettype, returninfo, retarg,
                                 declarations.functionObject, M, ctx.emission_context);
    }
    else {
        ...
        add_return_attr(f, Attribute::NonNull);
        f->addFnAttr(Thunk);
        // TODO: (if needsparams) add attributes: dereferenceable<sizeof(void*) *
        // length(sp)>, readonly, nocapture
        // TODO: add attributes: dereferenceable<sizeof(ft)>, readonly, nocapture - e.g.
    ...
    if (returninfo.cc == jl_returninfo_t::Union) {
        f->addAttribute(1, Attribute::getWithDereferenceableBytes(jl_LLVMContext,
                                                                  returninfo.union_bytes));
        f->addAttribute(1, Attribute::getWithAlignment(jl_LLVMContext,
                                                       Align(returninfo.union_align)));
    }
    ...
    if (ctx.debug_enabled) {
        DICompileUnit::DebugEmissionKind emissionKind =
            (DICompileUnit::DebugEmissionKind)ctx.params->debug_info_kind;
        ...
        DICompileUnit *CU =
            dbuilder.createCompileUnit(llvm::dwarf::DW_LANG_Julia, topfile // File
                                       ,
                                       "julia" // Producer
                                       ,
                                       true // isOptimized
        ...
        SP = dbuilder.createFunction(CU, dbgFuncName // Name
                                     ,
                                     ...
                                     DISubprogram::SPFlagDefinition |
                                         DISubprogram::SPFlagOptimized // SPFlags
                                     ,
        ...
        if (jl_options.debug_level >= 2) {
            ...
            // Go over all arguments and local variables and initialize their debug
            // information
            for (i = 0; i < nreq; i++) {
                jl_sym_t *argname = (jl_sym_t *)jl_array_ptr_ref(src->slotnames, i);
                if (argname == unused_sym)
            ...
            if (va && ctx.vaSlot != -1) {
                ctx.slots[ctx.vaSlot].dinfo = dbuilder.createParameterVariable(
                    SP, // Scope (current function will be fill in later)
                    std::string(jl_symbol_name(slot_symbol(ctx, ctx.vaSlot))) +
                        "...", // Variable name
                    has_sret + nreq + 1, // Argument number (1-based)
                    topfile, // File
                    toplineno == -1 ?
                        0 :
                        toplineno, // Line (for now, use lineno of the function)
                    julia_type_to_di(ctx, ctx.slots[ctx.vaSlot].value.typ, &dbuilder,
                                     false),
                    AlwaysPreserve, // May be deleted if optimized out
                    DINode::FlagZero); // Flags (TODO: Do we need any)
            }
            for (i = 0; i < vinfoslen; i++) {
                ...
                varinfo.dinfo = dbuilder.createAutoVariable(
                    ...
                    toplineno == -1 ?
                        0 :
                        toplineno, // Line (for now, use lineno of the function)
                    julia_type_to_di(ctx, varinfo.value.typ, &dbuilder,
                                     false), // Variable type
                    AlwaysPreserve, // May be deleted if optimized out
                    DINode::FlagZero // Flags (TODO: Do we need any)
                );
    ...
    if (!specsig) {
        ...
        ctx.builder.CreateStore(argArray, pargArray,
                                true /*volatile store to prevent removal of this alloca*/);
        argCount = &*AI++;
    ...
    */
    ...
    auto allocate_local = [&](jl_varinfo_t &varinfo, jl_sym_t *s) {
        ...
        if (varinfo.value.constant) {
            ...
            size_t align, nbytes;
            Value *lv =
                try_emit_union_alloca(ctx, (jl_uniontype_t *)jt, allunbox, align, nbytes);
            if (lv) {
        ...
        if (!varinfo.isArgument || // always need a slot if the variable is assigned
            specsig || // for arguments, give them stack slots if they aren't in `argArray`
                       // (otherwise, will use that pointer)
            (va && (int)i == ctx.vaSlot) || // or it's the va arg tuple
            i == 0) { // or it is the first argument (which isn't in `argArray`)
            AllocaInst *av = new AllocaInst(T_prjlvalue, 0, jl_symbol_name(s),
                                            /*InsertBefore*/ ctx.ptlsStates);
            StoreInst *SI = new StoreInst(
                ConstantPointerNull::get(cast<PointerType>(T_prjlvalue)), av, false);
            ...
            if (ctx.debug_enabled && varinfo.dinfo) {
                ...
                dbuilder.insertDeclare(av, varinfo.dinfo, expr, topdebugloc,
    ...
    auto get_specsig_arg = [&](jl_value_t *argType, Type *llvmArgType, bool isboxed) {
        ...
        if (type_is_ghost(llvmArgType)) { // this argument is not actually passed
            ...
            ++AI;
            maybe_mark_argument_dereferenceable(Arg, argType);
            theArg = mark_julia_slot(Arg, argType, NULL,
                                     tbaa_const); // this argument is by-pointer
        }
        else {
            Argument *Arg = &*AI;
            ++AI;
        ...
        if (s == unused_sym || vi.value.constant) {
            ...
            if (specsig) {
                ...
                if (i == 0) {
                    ...
                    Value *argPtr = ctx.builder.CreateInBoundsGEP(
                        T_prjlvalue, argArray, ConstantInt::get(T_size, i - 1));
                    auto load = maybe_mark_load_dereferenceable(
                        ctx.builder.CreateLoad(T_prjlvalue, argPtr), false, vi.value.typ);
                    ...
                    if (ctx.debug_enabled && vi.dinfo && !vi.boxroot && !vi.value.V) {
                        ...
                        if ((Metadata *)vi.dinfo->getType() != jl_pvalue_dillvmt)
                            addr.push_back(llvm::dwarf::DW_OP_deref);
                        dbuilder.insertDeclare(pargArray, vi.dinfo,
            ...
            if (vi.boxroot == NULL) {
                assert(vi.value.V == NULL &&
                       "unexpected variable slot created for argument");
                // keep track of original (possibly boxed) value to avoid re-boxing or
                // moving
                vi.value = theArg;
                if (specsig && theArg.V && ctx.debug_enabled && vi.dinfo) {
                    ...
                    if (theArg.ispointer()) {
                        ...
                        parg = ctx.builder.CreateAlloca(theArg.V->getType(), NULL,
                                                        jl_symbol_name(s));
    ...
    if (va && ctx.vaSlot != -1) {
        ...
        if (vi.value.constant || !vi.used) {
            ...
            for (size_t i = nreq; i < jl_nparams(lam->specTypes); ++i) {
                ...
                bool isboxed = deserves_argbox(argType);
                Type *llvmArgType =
                    isboxed ? T_prjlvalue : julia_type_to_llvm(ctx, argType);
            ...
            if (jl_is_concrete_type(vi.value.typ)) {
                ...
                restTuple = emit_jlcall(ctx, prepare_call(jltuple_func).getCallee(),
                                        maybe_decay_untracked(V_null), vargs, ctx.nvargs,
                                        JLCALL_F_CC);
            ...
            restTuple = ctx.builder.CreateCall(
                prepare_call(jltuple_func),
                {maybe_decay_untracked(V_null),
                 ctx.builder.CreateInBoundsGEP(T_prjlvalue, argArray,
                                               ConstantInt::get(T_size, nreq - 1)),
                 ctx.builder.CreateSub(argCount, ConstantInt::get(T_int32, nreq - 1))});
    ...
    std::vector<DebugLineTable> linetable;
    {
        ...
        for (size_t i = 0; i < nlocs; i++) {
            // LineInfoNode(mod::Module, method::Any, file::Symbol, line::Int,
            // inlined_at::Int)
            ...
            if (jl_is_method(method)) {
                ...
                info.is_user_code = (info.inlined_at == 0) ?
                                        mod_is_user_mod :
                                        linetable.at(info.inlined_at).is_user_code;
            }
            ...
            if (ctx.debug_enabled) {
                ...
                if (info.inlined_at == 0 &&
                    info.file ==
                        ctx.file) { // if everything matches, emit a toplevel line number
                    ...
                    DebugLoc inl_loc = (info.inlined_at == 0) ?
                                           DebugLoc::get(0, 0, SP, NULL) :
                                           linetable.at(info.inlined_at).loc;
        ...
        for (i = 0; i < nstmts; i++) {
            ...
            if (expr) {
                if (expr->head == aliasscope_sym) {
                    ...
                    MDNode *scope_list =
                        MDNode::get(jl_LLVMContext, ArrayRef<Metadata *>(scope_stack));
        ...
        for (size_t i = 0; i < stmtslen; ++i) {
            ...
            if (jl_is_expr(stmt)) {
                if (((jl_expr_t *)stmt)->head == goto_ifnot_sym) {
                    int dest =
                        jl_unbox_long(jl_array_ptr_ref(((jl_expr_t *)stmt)->args, 1));
                    ...
                    if (i + 2 <= stmtslen)
                        branch_targets.insert(i + 2);
                    int dest =
                        jl_unbox_long(jl_array_ptr_ref(((jl_expr_t *)stmt)->args, 0));
                    branch_targets.insert(dest);
                }
            }
    ...
    for (int label : branch_targets) {
        BasicBlock *bb = BasicBlock::Create(jl_LLVMContext, "L" + std::to_string(label), f);
        BB[label] = bb;
    }
    ...
    while (cursor != -1) {
        ...
        if (expr && expr->head == return_sym) {
            ...
            case jl_returninfo_t::Register:
                ...
                break;
            case jl_returninfo_t::SRet: retval = NULL; break;
            case jl_returninfo_t::Union: {
                Value *data, *tindex;
                if (retvalinfo.TIndex) {
                    ...
                    if (retvalinfo.V == NULL) {
                        ...
                        if (retvalinfo.Vboxed) {
                            // also need to account for the possibility the return object is
                            // boxed and avoid / skip copying it to the stack
                            isboxed_union = ctx.builder.CreateICmpNE(
                                ctx.builder.CreateAnd(tindex,
                                                      ConstantInt::get(T_int8, 0x80)),
                                ConstantInt::get(T_int8, 0));
                            data = ctx.builder.CreateSelect(isboxed_union,
                                                            retvalinfo.Vboxed, data);
                        }
                    }
                }
                else {
            ...
            if (sret) {
                if (retvalinfo.ispointer()) {
                    if (returninfo.return_roots) {
                        ...
                        emit_sret_roots(ctx, true, data_pointer(ctx, retvalinfo), store_ty,
                                        f->arg_begin() + 1, returninfo.return_roots);
                    }
                    if (returninfo.cc == jl_returninfo_t::SRet) {
                        assert(jl_is_concrete_type(jlrettype));
                        emit_memcpy(ctx, sret, nullptr, retvalinfo,
                                    jl_datatype_size(jlrettype),
                                    julia_alignment(jlrettype));
                    }
                    else { // must be jl_returninfo_t::Union
                        emit_unionmove(ctx, sret, nullptr, retvalinfo,
                                       /*skip*/ isboxed_union);
                    }
                }
                else {
                    ...
                    if (returninfo.return_roots) {
                        ...
                        emit_sret_roots(ctx, false, Val, store_ty, f->arg_begin() + 1,
                                        returninfo.return_roots);
                    }
                    if (dest_ty != sret->getType())
                        sret = emit_bitcast(ctx, sret, dest_ty);
                    ctx.builder.CreateAlignedStore(Val, sret,
                                                   julia_alignment(retvalinfo.typ));
                    assert(retvalinfo.TIndex == NULL &&
                           "unreachable"); // unimplemented representation
                }
            }
        ...
        if (jl_is_upsilonnode(stmt)) {
            ...
            if (val) {
                ...
                // tindex).
                ctx.builder.CreateStore(vi.boxroot ? ConstantInt::get(T_int8, 0x80) :
        ...
        if (expr && expr->head == goto_ifnot_sym) {
            ...
            ctx.SAvalues.at(cursor) =
                jl_cgval_t(excstack_state, NULL, false, (jl_value_t *)jl_ulong_type, NULL);
    ...
    auto undef_value_for_type = [&](Type *T) {
        ...
        Value *undef;
        if (tracked.count)
            // make sure gc pointers (including ptr_phi of union-split) are initialized to
            // NULL
    ...
    for (auto &tup : ctx.PhiNodes) {
        ...
        for (size_t i = 0; i < jl_array_len(edges); ++i) {
            ...
            // This edge value is undef, handle it the same as if the edge wasn't listed at
            // all
            if (!value)
                continue;
            ...
            if (!preds.insert(FromBB).second) {
                // Only codegen this branch once for each PHI (the expression must be the
                // same on all branches)
#ifndef NDEBUG
                for (size_t j = 0; j < i; ++j) {
            ...
            if (!terminator->getParent()->getUniqueSuccessor()) {
                ...
                BasicBlock *NewBB = BasicBlock::Create(terminator->getContext(),
                                                       FromBB->getName() + "." +
                                                           PhiBB->getName() + "_crit_edge");
                Function::iterator FBBI = FromBB->getIterator();
                ctx.f->getBasicBlockList().insert(++FBBI,
                                                  NewBB); // insert after existing block
#if JL_LLVM_VERSION >= 90000
                terminator->replaceSuccessorWith(PhiBB, NewBB);
#else
                for (unsigned Idx = 0, NumSuccessors = terminator->getNumSuccessors();
            ...
            if (val.constant)
                val =
                    mark_julia_const(val.constant); // be over-conservative at making sure
                                                    // `.typ` is set concretely, not tindex
            if (!jl_is_uniontype(phiType) || !TindexN) {
                ...
                if (val.typ == (jl_value_t *)jl_bottom_type) {
                    ...
                    if (!RTindex) {
                        assert(new_union.isboxed && new_union.Vboxed &&
                               "convert_julia_type failed");
                        RTindex = compute_tindex_unboxed(ctx, new_union, phiType);
                        if (dest) {
                            // If dest is not set, this is a ghost union, the recipient of
                            // which is often not prepared to handle a boxed representation
                            // of the ghost.
                            RTindex = ctx.builder.CreateOr(RTindex,
                                                           ConstantInt::get(T_int8, 0x80));
                        }
                    ...
                    if (dest) { // basically, if !ghost union
                        ...
                        if (new_union.Vboxed != nullptr)
                            skip = ctx.builder.CreateICmpNE( // if 0x80 is set, we won't
                                                             // select this slot anyways
                                ctx.builder.CreateAnd(RTindex,
                                                      ConstantInt::get(T_int8, 0x80)),
                                ConstantInt::get(T_int8, 0));
        ...
        for (auto *FromBB : predecessors(PhiBB)) {
            ...
            // PHI is undef on this branch. But still may need to put a valid pointer in
            // place.
            Value *RTindex = TindexN ? UndefValue::get(T_int8) : NULL;
            if (VN) {
                Value *undef = undef_value_for_type(VN->getType());
                VN->addIncoming(undef, FromBB);
                if (TindexN) // let the runtime / optimizer know this is unknown / boxed /
                             // null, so that it won't try to union_move / copy it later
                    RTindex = ConstantInt::get(T_int8, 0x80);
            }
            if (TindexN)
            ...
            if (dest) {
                ...
                if (CountTrackedPointers(dest->getAllocatedType()).count)
                    ctx.builder.CreateStore(
                        Constant::getNullValue(dest->getAllocatedType()), dest);
            }
            ctx.builder.ClearInsertionPoint();
        }
    }
        ...
        for (auto &BB : *ctx.f) {
            for (auto &I : BB) {
                ...
                if (call && !I.getDebugLoc()) {
                    // LLVM Verifier: inlinable function call in a function with debug info
                    // must have a !dbg location make sure that anything we attempt to call
                    // has some inlining info, just in case optimization messed up (except
                    // if we know that it is an intrinsic used in our prologue, which should
                    // never have its own debug subprogram)
                    Function *F = call->getCalledFunction();
                    if (!in_prologue || !F ||
                        !(F->isIntrinsic() || F->getName().startswith("julia.") ||
                          &I == restTuple)) {
...
void jl_add_code_in_flight(StringRef name, jl_code_instance_t *codeinst,
                           const DataLayout &DL);

JL_GCC_IGNORE_START("-Wclobbered")
jl_compile_result_t jl_emit_code(jl_method_instance_t *li, jl_code_info_t *src,
                                 jl_value_t *jlrettype, jl_codegen_params_t &params)
{
...
jl_compile_result_t jl_emit_codeinst(jl_code_instance_t *codeinst, jl_code_info_t *src,
                                     jl_codegen_params_t &params)
{
    JL_GC_PUSH1(&src);
    if (!src) {
    ...
    jl_compile_result_t result =
        jl_emit_code(codeinst->def, src, codeinst->rettype, params);
    ...
    if (params.cache && !f.empty()) {
        ...
        if ( // don't alter `inferred` when the code is not directly being used
            ...
            if ( // keep code when keeping everything
                ...
                jl_is_method(def) &&
                // and there is something to delete (test this before calling
...
void jl_compile_workqueue(std::map<jl_code_instance_t *, jl_compile_result_t> &emitted,
                          jl_codegen_params_t &params, CompilationPolicy policy)
{
    ...
    while (!params.workqueue.empty()) {
        ...
        std::tie(codeinst, proto_cc, proto_return_roots, protodecl, proto_specsig) =
            params.workqueue.back();
        ...
        if (params.cache && codeinst->invoke != NULL) {
            if (codeinst->invoke == jl_fptr_args) {
                preal_decl = jl_ExecutionEngine->getFunctionAtAddress(
                    (uintptr_t)codeinst->specptr.fptr, codeinst);
            }
            else if (codeinst->isspecsig) {
                preal_decl = jl_ExecutionEngine->getFunctionAtAddress(
            ...
            if (std::get<0>(result)) {
                ...
                if (policy == CompilationPolicy::Extern && codeinst->inferred &&
                    codeinst->inferred == jl_nothing) {
        ...
        if (proto_specsig) {
            ...
            if (!preal_specsig) {
                ...
                size_t nrealargs = jl_nparams(
                    codeinst->def->specTypes); // number of actual arguments being passed
                // TODO: maybe this can be cached in codeinst->specfptr?
                emit_cfunc_invalidate(protodecl, proto_cc, proto_return_roots,
                                      codeinst->def->specTypes, codeinst->rettype,
                                      nrealargs, params, preal);
...
{
    GlobalVariable *gv = new GlobalVariable(*m, T_pjlvalue, true,
...
static GlobalVariable *global_jlvalue_to_llvm(const std::string &cname, jl_value_t **addr,
                                              Module *m)
{
...
static Function *jlcall_func_to_llvm(const std::string &cname, jl_fptr_args_t addr,
                                     Module *m)
{
...
static void init_julia_llvm_env(Module *m)
{
    ...
    jl_value_dillvmt = dbuilder.createStructType(
        nullptr, "jl_value_t", julia_h,
    ...
    jl_pvalue_dillvmt = dbuilder.createPointerType(
        jl_value_dillvmt, sizeof(jl_value_t *) * 8, __alignof__(jl_value_t *) * 8);

    SmallVector<llvm::Metadata *, 1> Elts;
    std::vector<Metadata *> diargs(0);
    Elts.push_back(jl_pvalue_dillvmt);
    dbuilder.replaceArrays(jl_value_dillvmt, dbuilder.getOrCreateArray(Elts));

    jl_ppvalue_dillvmt = dbuilder.createPointerType(
        jl_pvalue_dillvmt, sizeof(jl_value_t **) * 8, __alignof__(jl_value_t **) * 8);
    ...
    diargs.push_back(
        _julia_type_to_di(NULL, (jl_value_t *)jl_int32_type, &dbuilder, false));

    jl_di_func_sig = dbuilder.createSubroutineType(dbuilder.getOrCreateTypeArray(diargs));
    jl_di_func_null_sig =
...
#endif
                      ,
                      T_int16, T_int16, T_int32};
    static_assert(sizeof(jl_array_flags_t) == sizeof(int16_t),
                  "Size of jl_array_flags_t is not the same as int16_t");
    jl_array_llvmt = StructType::create(jl_LLVMContext, makeArrayRef(vaelts), "jl_array_t");
    jl_parray_llvmt = PointerType::get(jl_array_llvmt, 0);

    global_to_llvm("__stack_chk_guard", (void *)&__stack_chk_guard, m);
    Function *jl__stack_chk_fail = Function::Create(
        FunctionType::get(T_void, false), Function::ExternalLinkage, "__stack_chk_fail", m);
    ...
    jlRTLD_DEFAULT_var = new GlobalVariable(
        *m, T_pint8, true, GlobalVariable::ExternalLinkage, NULL, "jl_RTLD_DEFAULT_handle");
    add_named_global(jlRTLD_DEFAULT_var, &jl_RTLD_DEFAULT_handle);
#ifdef _OS_WINDOWS_
    jlexe_var = new GlobalVariable(*m, T_pint8, true, GlobalVariable::ExternalLinkage, NULL,
                                   "jl_exe_handle");
    add_named_global(jlexe_var, &jl_exe_handle);
    jldll_var = new GlobalVariable(*m, T_pint8, true, GlobalVariable::ExternalLinkage, NULL,
                                   "jl_dl_handle");
    add_named_global(jldll_var, &jl_dl_handle);
#endif

    jltls_states_func =
        Function::Create(FunctionType::get(PointerType::get(T_ppjlvalue, 0), false),
    ...
    jlerror_func = Function::Create(FunctionType::get(T_void, args1, false),
                                    Function::ExternalLinkage, "jl_error", m);
    ...
    jlthrow_func = Function::Create(FunctionType::get(T_void, args1_, false),
                                    Function::ExternalLinkage, "jl_throw", m);
    ...
    jlundefvarerror_func =
        Function::Create(FunctionType::get(T_void, args1_, false),
                         Function::ExternalLinkage, "jl_undefined_var_error", m);
    ...
    jlboundserrorv_func =
        Function::Create(FunctionType::get(T_void, args2_boundserrorv, false),
                         Function::ExternalLinkage, "jl_bounds_error_ints", m);
    ...
    jlboundserror_func =
        Function::Create(FunctionType::get(T_void, args2_boundserror, false),
                         Function::ExternalLinkage, "jl_bounds_error_int", m);
    ...
    jlvboundserror_func =
        Function::Create(FunctionType::get(T_void, args3_vboundserror, false),
                         Function::ExternalLinkage, "jl_bounds_error_tuple_int", m);
    ...
    jluboundserror_func =
        Function::Create(FunctionType::get(T_void, args3_uboundserror, false),
                         Function::ExternalLinkage, "jl_bounds_error_unboxed_int", m);
    jluboundserror_func->setDoesNotReturn();
    add_named_global(jluboundserror_func, &jl_bounds_error_unboxed_int);

    jlnew_func =
        Function::Create(jl_func_sig, Function::ExternalLinkage, "jl_new_structv", m);
    ...
    args_2rptrs_.push_back(T_prjlvalue);
    jlsplatnew_func = Function::Create(FunctionType::get(T_prjlvalue, args_2rptrs_, false),
                                       Function::ExternalLinkage, "jl_new_structt", m);
...
#endif
    setjmp_func = Function::Create(FunctionType::get(T_int32, args2, false),
    ...
    memcmp_func = Function::Create(FunctionType::get(T_int32, args_memcmp, false),
    ...
    jltypeerror_func = Function::Create(FunctionType::get(T_void, te_args, false),
                                        Function::ExternalLinkage, "jl_type_error", m);
    ...
    jlcheckassign_func =
        Function::Create(FunctionType::get(T_void, args_2ptrs, false),
                         Function::ExternalLinkage, "jl_checked_assignment", m);
    ...
    jldeclareconst_func =
        Function::Create(FunctionType::get(T_void, args_1binding, false),
                         Function::ExternalLinkage, "jl_declare_constant", m);
    ...
    jlgetbindingorerror_func =
        Function::Create(FunctionType::get(T_pjlvalue, args_2ptrs_, false),
                         Function::ExternalLinkage, "jl_get_binding_or_error", m);
    add_named_global(jlgetbindingorerror_func, &jl_get_binding_or_error);

    jlboundp_func = Function::Create(FunctionType::get(T_int32, args_2ptrs_, false),
                                     Function::ExternalLinkage, "jl_boundp", m);
    ...
    builtin_func_map[jl_f_issubtype] =
        jlcall_func_to_llvm("jl_f_issubtype", &jl_f_issubtype, m);
    builtin_func_map[jl_f_isa] = jlcall_func_to_llvm("jl_f_isa", &jl_f_isa, m);
    builtin_func_map[jl_f_typeassert] =
        jlcall_func_to_llvm("jl_f_typeassert", &jl_f_typeassert, m);
    ...
    builtin_func_map[jl_f__apply_iterate] =
        jlcall_func_to_llvm("jl_f__apply_iterate", &jl_f__apply_iterate, m);
    builtin_func_map[jl_f__apply_pure] =
        jlcall_func_to_llvm("jl_f__apply_pure", &jl_f__apply_pure, m);
    builtin_func_map[jl_f__apply_latest] =
        jlcall_func_to_llvm("jl_f__apply_latest", &jl_f__apply_latest, m);
    ...
    builtin_func_map[jl_f_applicable] =
        jlcall_func_to_llvm("jl_f_applicable", &jl_f_applicable, m);
    builtin_func_map[jl_f_invoke] = jlcall_func_to_llvm("jl_f_invoke", &jl_f_invoke, m);
    builtin_func_map[jl_f_invoke_kwsorter] =
        jlcall_func_to_llvm("jl_f_invoke_kwsorter", &jl_f_invoke_kwsorter, m);
    builtin_func_map[jl_f_isdefined] =
        jlcall_func_to_llvm("jl_f_isdefined", &jl_f_isdefined, m);
    builtin_func_map[jl_f_getfield] =
        jlcall_func_to_llvm("jl_f_getfield", &jl_f_getfield, m);
    builtin_func_map[jl_f_setfield] =
        jlcall_func_to_llvm("jl_f_setfield", &jl_f_setfield, m);
    builtin_func_map[jl_f_fieldtype] =
        jlcall_func_to_llvm("jl_f_fieldtype", &jl_f_fieldtype, m);
    ...
    builtin_func_map[jl_f__typevar] =
        jlcall_func_to_llvm("jl_f__typevar", &jl_f__typevar, m);
    builtin_func_map[jl_f_arrayref] =
        jlcall_func_to_llvm("jl_f_arrayref", &jl_f_arrayref, m);
    builtin_func_map[jl_f_const_arrayref] =
        jlcall_func_to_llvm("jl_f_const_arrayref", &jl_f_arrayref, m);
    builtin_func_map[jl_f_arrayset] =
        jlcall_func_to_llvm("jl_f_arrayset", &jl_f_arrayset, m);
    builtin_func_map[jl_f_arraysize] =
        jlcall_func_to_llvm("jl_f_arraysize", &jl_f_arraysize, m);
    builtin_func_map[jl_f_apply_type] =
        jlcall_func_to_llvm("jl_f_apply_type", &jl_f_apply_type, m);
    jltuple_func = builtin_func_map[jl_f_tuple];
    jlgetfield_func = builtin_func_map[jl_f_getfield];

    jlapplygeneric_func =
        Function::Create(jl_func_sig, Function::ExternalLinkage, "jl_apply_generic", m);
    ...
    jlinvoke_func = Function::Create(FunctionType::get(T_prjlvalue, invokeargs, false),
                                     Function::ExternalLinkage, "jl_invoke", m);
    ...
    args_topeval.push_back(T_pjlvalue);
    jltopeval_func = Function::Create(FunctionType::get(T_pjlvalue, args_topeval, false),
                                      Function::ExternalLinkage, "jl_toplevel_eval", m);
    ...
    jlcopyast_func = Function::Create(FunctionType::get(T_prjlvalue, args_copyast, false),
                                      Function::ExternalLinkage, "jl_copy_ast", m);
    ...
    jlnsvec_func = Function::Create(FunctionType::get(T_pjlvalue, args5, true),
                                    Function::ExternalLinkage, "jl_svec", m);
    ...
    jlmethod_func = Function::Create(FunctionType::get(T_void, mdargs, false),
                                     Function::ExternalLinkage, "jl_method_def", m);
    ...
    jlegal_func =
        Function::Create(FunctionType::get(T_int32, args_2vals_callee_rooted, false),
                         Function::ExternalLinkage, "jl_egal", m);
    add_named_global(jlegal_func, &jl_egal);

    std::vector<Type *> args_2vals_tracked(0);
    args_2vals_tracked.push_back(T_prjlvalue);
    args_2vals_tracked.push_back(T_prjlvalue);
    jlisa_func = Function::Create(FunctionType::get(T_int32, args_2vals_tracked, false),
                                  Function::ExternalLinkage, "jl_isa", m);
    add_named_global(jlisa_func, &jl_isa);

    jlsubtype_func = Function::Create(FunctionType::get(T_int32, args_2vals_tracked, false),
                                      Function::ExternalLinkage, "jl_subtype", m);
    add_named_global(jlsubtype_func, &jl_subtype);

    jltypeassert_func =
        Function::Create(FunctionType::get(T_void, args_2vals_tracked, false),
                         Function::ExternalLinkage, "jl_typeassert", m);
    ...
    jlapplytype_func =
        Function::Create(FunctionType::get(T_prjlvalue, applytype_args, false),
                         Function::ExternalLinkage, "jl_instantiate_type_in_env", m);
    ...
    jl_object_id__func = Function::Create(FunctionType::get(T_size, objectid__args, false),
                                          Function::ExternalLinkage, "jl_object_id_", m);
    ...
    jl_alloc_obj_func =
        Function::Create(FunctionType::get(T_prjlvalue, gc_alloc_args, false),
    ...
    jl_alloc_obj_func->addFnAttr(
        Attribute::getWithAllocSizeArgs(jl_LLVMContext, 1, None)); // returns %1 bytes
    ...
    jl_newbits_func = Function::Create(FunctionType::get(T_prjlvalue, newbits_args, false),
                                       Function::ExternalLinkage, "jl_new_bits");
    ...
    jl_loopinfo_marker_func =
        Function::Create(FunctionType::get(T_void, {}, false), Function::ExternalLinkage,
    ...
    jl_typeof_func = Function::Create(FunctionType::get(T_prjlvalue, {T_prjlvalue}, false),
                                      Function::ExternalLinkage, "julia.typeof");
    ...
    jl_write_barrier_func =
        Function::Create(FunctionType::get(T_void,
                                           {
                                               T_prjlvalue,
                                           },
                                           true),
                         Function::ExternalLinkage, "julia.write_barrier");
    ...
    jldlsym_func = Function::Create(FunctionType::get(T_pvoidfunc, dlsym_args, false),
                                    Function::ExternalLinkage, "jl_load_and_lookup", m);
    ...
    getcfunctiontrampoline_args.push_back(
        FunctionType::get(T_pint8, {T_pint8, T_ppjlvalue}, false)
            ->getPointerTo()); // trampoline
    ...
    jlgetcfunctiontrampoline_func =
        Function::Create(FunctionType::get(T_prjlvalue, getcfunctiontrampoline_args, false),
                         Function::ExternalLinkage, "jl_get_cfunction_trampoline", m);
    ...
    jlgetnthfieldchecked_func =
        Function::Create(FunctionType::get(T_prjlvalue, getnthfld_args, false),
                         Function::ExternalLinkage, "jl_get_nth_field_checked", m);
    ...
    diff_gc_total_bytes_func =
        Function::Create(FunctionType::get(T_int64, false), Function::ExternalLinkage,
                         "jl_gc_diff_total_bytes", m);
    add_named_global(diff_gc_total_bytes_func, &jl_gc_diff_total_bytes);

    sync_gc_total_bytes_func =
        Function::Create(FunctionType::get(T_int64, {T_int64}, false),
                         Function::ExternalLinkage, "jl_gc_sync_total_bytes", m);
    ...
    jlarray_data_owner_func =
        Function::Create(FunctionType::get(T_prjlvalue, array_owner_args, false),
                         Function::ExternalLinkage, "jl_array_data_owner", m);
    ...
    gcroot_flush_func = Function::Create(FunctionType::get(T_void, false),
                                         Function::ExternalLinkage, "julia.gcroot_flush");
    add_named_global(gcroot_flush_func, (void *)NULL, /*dllimport*/ false);

    gc_preserve_begin_func = Function::Create(
        FunctionType::get(Type::getTokenTy(jl_LLVMContext), ArrayRef<Type *>(), true),
        Function::ExternalLinkage, "llvm.julia.gc_preserve_begin");
    add_named_global(gc_preserve_begin_func, (void *)NULL, /*dllimport*/ false);

    gc_preserve_end_func = Function::Create(
        FunctionType::get(T_void, ArrayRef<Type *>(Type::getTokenTy(jl_LLVMContext)),
                          false),
        Function::ExternalLinkage, "llvm.julia.gc_preserve_end");
    add_named_global(gc_preserve_end_func, (void *)NULL, /*dllimport*/ false);

    pointer_from_objref_func = Function::Create(
        FunctionType::get(
            T_pjlvalue,
    ...
    except_enter_func = Function::Create(FunctionType::get(T_int32, false),
                                         Function::ExternalLinkage, "julia.except_enter");
    except_enter_func->addFnAttr(Attribute::ReturnsTwice);
    add_named_global(except_enter_func, (void *)NULL, /*dllimport*/ false);

    jlgetworld_global = new GlobalVariable(
        *m, T_size, false, GlobalVariable::ExternalLinkage, NULL, "jl_world_counter");
...
extern "C" void jl_init_llvm(void)
{
    ...
    const char *const argv_tailmerge[] = {
        "", "-enable-tail-merge=0"}; // NOO TOUCHIE; NO TOUCH! See #922
    cl::ParseCommandLineOptions(sizeof(argv_tailmerge) / sizeof(argv_tailmerge[0]),
                                argv_tailmerge, "disable-tail-merge\n");
#if defined(_OS_WINDOWS_) && defined(_CPU_X86_64_)
    const char *const argv_copyprop[] = {"", "-disable-copyprop"}; // llvm bug 21743
    cl::ParseCommandLineOptions(sizeof(argv_copyprop) / sizeof(argv_copyprop[0]),
                                argv_copyprop, "disable-copyprop\n");
#endif
#if defined(_CPU_X86_) || defined(_CPU_X86_64_)
    const char *const argv_avoidsfb[] = {
        "",
        "-x86-disable-avoid-SFB"}; // llvm bug 41629, see
                                   // https://gist.github.com/vtjnash/192cab72a6cfc00256ff118238163b55
    cl::ParseCommandLineOptions(sizeof(argv_avoidsfb) / sizeof(argv_avoidsfb[0]),
                                argv_avoidsfb, "disable-avoidsfb\n");
#endif
    ...
    jl_TargetMachine =
        TheTarget->createTargetMachine(TheTriple.getTriple(), TheCPU, FeaturesStr, options,
...
extern "C" void jl_init_codegen(void)
{
    ...
    SBOX_F_PERM(int8, int8);
    UBOX_F_PERM(uint8, uint8);
    SBOX_F(int16, int16);
    UBOX_F(uint16, uint16);
    SBOX_F(int32, int32);
    UBOX_F(uint32, uint32);
    SBOX_F(int64, int64);
    UBOX_F(uint64, uint64);
    BOX_F(float32, float32, T_prjlvalue);
    BOX_F(float64, float64, T_prjlvalue);
    UBOX_F(char, char);
