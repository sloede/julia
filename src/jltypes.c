...
static int jl_has_bound_typevars(jl_value_t *v, jl_typeenv_t *env) JL_NOTSAFEPOINT
{
    ...
    if (jl_is_unionall(v)) {
        ...
        if (te)
            te->var = NULL; // temporarily remove this var from env
...
int jl_find_union_component(jl_value_t *haystack, jl_value_t *needle,
                            unsigned *nth) JL_NOTSAFEPOINT
{
...
static void flatten_type_union(jl_value_t **types, size_t n, jl_value_t **out,
                               size_t *idx) JL_NOTSAFEPOINT
{
    size_t i;
    for (i = 0; i < n; i++) {
        jl_value_t *e = types[i];
...
static unsigned typekey_hash(jl_typename_t *tn, jl_value_t **key, size_t n,
                             int nofail) JL_NOTSAFEPOINT;

/* returns val if key is in hash, otherwise NULL */
static jl_datatype_t *lookup_type_set(jl_svec_t *cache, jl_value_t **key, size_t n,
                                      uint_t hv)
{
...
static int is_cacheable(jl_datatype_t *type)
{
    ...
    if (jl_svec_len(t) == 0 &&
        jl_emptytuple_type !=
            NULL) // Tuple{} is the only type eligible for this that doesn't have parameters
        return 0;
...
static jl_value_t *inst_datatype_inner(jl_datatype_t *dt, jl_svec_t *p,
                                       jl_value_t **iparams, size_t ntp, int cacheable,
...
static jl_value_t *inst_datatype_env(jl_value_t *dt, jl_svec_t *p, jl_value_t **iparams,
                                     size_t ntp, int cacheable, jl_typestack_t *stack,
                                     jl_typeenv_t *env, int c)
{
    if (jl_is_datatype(dt))
        return inst_datatype_inner((jl_datatype_t *)dt, p, iparams, ntp, cacheable, stack,
                                   env);
...
jl_value_t *jl_apply_type(jl_value_t *tc, jl_value_t **params, size_t n)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!jl_has_free_typevars(ua->var->lb) && !jl_has_free_typevars(ua->var->ub) &&
            ...
            if (!iswrapper)
                jl_type_error_rt("Type", jl_symbol_name(ua->var->name),
                                 (jl_value_t *)ua->var, pi);
        }
...
static jl_value_t *inst_type_w_(jl_value_t *t, jl_typeenv_t *env, jl_typestack_t *stack,
                                int check);
                                     ...
                                     jl_value_t **iparams)
{
    ...
    while (stack != NULL) {
        ...
        if (stack->tt->name == tn && ntp == jl_svec_len(stack->tt->parameters) &&
...
static unsigned type_hash(jl_value_t *kj, int *failed) JL_NOTSAFEPOINT
{
    ...
    if (jl_is_datatype(uw)) {
        ...
        if (!hash) {
            ...
            hash = typekey_hash(dt->name, jl_svec_data(dt->parameters),
                                jl_svec_len(dt->parameters), *failed);
        }
        return hash;
    }
...
void jl_precompute_memoized_dt(jl_datatype_t *dt, int cacheable)
{
    ...
    for (i = 0; i < l; i++) {
        ...
        if (istuple && dt->isconcretetype)
            dt->isconcretetype =
                (jl_is_datatype(p) && ((jl_datatype_t *)p)->isconcretetype) ||
                p == jl_bottom_type;
        if (dt->isdispatchtuple) {
            dt->isdispatchtuple =
                jl_is_datatype(p) &&
                ((!jl_is_kind(p) && ((jl_datatype_t *)p)->isconcretetype) ||
                 (((jl_datatype_t *)p)->name == jl_type_typename &&
                  !((jl_datatype_t *)p)->hasfreetypevars));
        }
    }
    if (dt->name == jl_type_typename)
        cacheable = 0; // the cache for Type ignores parameter normalization, so it can't be
                       // used as a regular hash
...
static jl_value_t *_jl_instantiate_type_in_env(jl_value_t *ty, jl_unionall_t *env,
                                               jl_value_t **vals, jl_typeenv_t *prev,
                                       ...
                                       jl_typestack_t *stack, jl_typeenv_t *env)
{
    ...
    if (cacheable) {
        ...
        for (i = 0; i < ntp; i++) {
            ...
            if (tw && tw != pi &&
                (tn != jl_type_typename || jl_typeof(pi) == jl_typeof(tw)) &&
    ...
    if (istuple && ntp > 0 && jl_is_vararg_type(last)) {
        ...
        if (ntp == 1 && (last == (jl_value_t *)jl_vararg_type || // Tuple{Vararg} == Tuple
                         (va0 == (jl_value_t *)jl_any_type && jl_is_unionall(last) &&
                          va1 == (jl_value_t *)((jl_unionall_t *)last)->var))) {
            JL_GC_POP();
    ...
    if (istuple) {
        ...
        if (!jl_has_free_typevars(names_tup) && !jl_has_free_typevars(values_tt)) {
            if (!jl_is_tuple(names_tup))
                jl_type_error_rt("NamedTuple", "names", (jl_value_t *)jl_anytuple_type,
                                 names_tup);
            ...
            for (size_t i = 0; i < nf; i++) {
                ...
                if (!jl_is_symbol(ni))
                    jl_type_error_rt("NamedTuple", "name", (jl_value_t *)jl_symbol_type,
                                     ni);
                for (size_t j = 0; j < i; j++) {
                    if (ni == jl_svecref(names, j))
                        jl_errorf(
                            "duplicate field name in NamedTuple: \"%s\" is not unique",
                            jl_symbol_name((jl_sym_t *)ni));
                }
...
static jl_tupletype_t *jl_apply_tuple_type_v_(jl_value_t **p, size_t np, jl_svec_t *params)
{
    ...
    return (jl_datatype_t *)inst_datatype_inner(jl_anytuple_type, params, p, np, cacheable,
                                                NULL, NULL);
}
...
{
    return (jl_datatype_t *)inst_datatype_inner(jl_anytuple_type, p, jl_svec_data(p),
                                                jl_svec_len(p), 1, NULL, NULL);
}

jl_datatype_t *jl_inst_concrete_tupletype_v(jl_value_t **p, size_t np)
{
    return (jl_datatype_t *)inst_datatype_inner(jl_anytuple_type, NULL, p, np, 1, NULL,
                                                NULL);
}
...
static jl_value_t *inst_tuple_w_(jl_value_t *t, jl_typeenv_t *env, jl_typestack_t *stack,
                                 int check)
{
                                ...
                                int check)
{
    ...
    if (jl_is_unionall(t)) {
        ...
        if (newbody == (jl_value_t *)jl_emptytuple_type) {
            ...
            // if t's parameters are not bound in the environment, return it uncopied
            // (#9378)
...
static jl_value_t *instantiate_with(jl_value_t *t, jl_value_t **env, size_t n,
                                    jl_typeenv_t *te)
{
    if (n > 0) {
...
JL_DLLEXPORT jl_value_t *jl_instantiate_type_in_env(jl_value_t *ty, jl_unionall_t *env,
                                                    jl_value_t **vals)
{
    jl_value_t *typ = ty;
...
void jl_init_types(void) JL_GC_DISABLED
{
    ...
    jl_any_type = (jl_datatype_t *)jl_new_abstracttype((jl_value_t *)jl_symbol("Any"), core,
                                                       NULL, jl_emptysvec);
    ...
    jl_type_type = (jl_unionall_t *)jl_new_abstracttype((jl_value_t *)jl_symbol("Type"),
                                                        core, jl_any_type, jl_emptysvec);
    ...
    jl_datatype_type->name->names =
        jl_perm_symsvec(19, "name", "super", "parameters", "types", "names", "instance",
                        "layout", "size", "ninitialized", "hash", "abstract", "mutable",
                        "hasfreetypevars", "isconcretetype", "isdispatchtuple",
                        "isbitstype", "zeroinit", "isinlinealloc", "has_concrete_subtype");
    jl_datatype_type->types =
        jl_svec(19, jl_typename_type, jl_datatype_type, jl_simplevector_type,
    ...
    // NOTE: types are not actually mutable, but we want to ensure they are heap-allocated
    ...
    jl_typename_type->name->names =
        jl_perm_symsvec(9, "name", "module", "names", "wrapper", "cache", "linearcache",
                        "hash", "mt", "partial");
    jl_typename_type->types =
        jl_svec(9, jl_symbol_type, jl_any_type, jl_simplevector_type, jl_type_type,
                jl_simplevector_type, jl_simplevector_type, jl_any_type, jl_any_type,
                jl_any_type);
    ...
    jl_methtable_type->name->names =
        jl_perm_symsvec(11, "name", "defs", "cache", "max_args", "kwsorter", "module",
                        "backedges", "", "", "offs", "");
    jl_methtable_type->types =
        jl_svec(11, jl_symbol_type, jl_any_type, jl_any_type, jl_any_type /*jl_long*/,
                jl_any_type, jl_any_type /*module*/, jl_any_type /*any vector*/,
                jl_any_type /*long*/, jl_any_type /*int32*/, jl_any_type /*uint8*/,
                jl_any_type /*uint8*/);
    ...
    jl_typeofbottom_type =
        jl_new_datatype(jl_symbol("TypeofBottom"), core, type_type, jl_emptysvec,
                        jl_emptysvec, jl_emptysvec, 0, 0, 0);
    ...
    jl_tvar_type =
        jl_new_datatype(jl_symbol("TypeVar"), core, jl_any_type, jl_emptysvec,
                        jl_perm_symsvec(3, "name", "lb", "ub"),
                        jl_svec(3, jl_symbol_type, jl_any_type, jl_any_type), 0, 1, 3);

    jl_unionall_type = jl_new_datatype(jl_symbol("UnionAll"), core, type_type, jl_emptysvec,
                                       jl_perm_symsvec(2, "var", "body"),
                                       jl_svec(2, jl_tvar_type, jl_any_type), 0, 0, 2);

    jl_svec_t *tv;
    tv = jl_svec2(tvar("T"), tvar("N"));
    jl_vararg_type = (jl_unionall_t *)jl_new_abstracttype((jl_value_t *)jl_symbol("Vararg"),
                                                          core, jl_any_type, tv)
                         ->name->wrapper;
    jl_vararg_typename =
        ((jl_datatype_t *)jl_unwrap_unionall((jl_value_t *)jl_vararg_type))->name;

    jl_svec_t *anytuple_params =
        jl_svec(1, jl_wrap_vararg((jl_value_t *)jl_any_type, (jl_value_t *)NULL));
    jl_anytuple_type =
        jl_new_datatype(jl_symbol("Tuple"), core, jl_any_type, anytuple_params,
                        jl_emptysvec, anytuple_params, 0, 0, 0);
    jl_tuple_typename = jl_anytuple_type->name;
    // fix some miscomputed values, since we didn't know this was going to be a Tuple in
    ...
    jl_type_typename->wrapper =
        jl_new_struct(jl_unionall_type, tttvar, (jl_value_t *)jl_type_type);
    ...
    jl_ssavalue_type =
        jl_new_datatype(jl_symbol("SSAValue"), core, jl_any_type, jl_emptysvec,
                        jl_perm_symsvec(1, "id"), jl_svec1(jl_long_type), 0, 0, 1);

    jl_abstractslot_type = jl_new_abstracttype((jl_value_t *)jl_symbol("Slot"), core,
                                               jl_any_type, jl_emptysvec);

    jl_slotnumber_type =
        jl_new_datatype(jl_symbol("SlotNumber"), core, jl_abstractslot_type, jl_emptysvec,
                        jl_perm_symsvec(1, "id"), jl_svec1(jl_long_type), 0, 0, 1);

    jl_typedslot_type = jl_new_datatype(jl_symbol("TypedSlot"), core, jl_abstractslot_type,
                                        jl_emptysvec, jl_perm_symsvec(2, "id", "typ"),
                                        jl_svec(2, jl_long_type, jl_any_type), 0, 0, 2);

    jl_init_int32_int64_cache();

    jl_bool_type = NULL;
    jl_bool_type = jl_new_primitivetype((jl_value_t *)jl_symbol("Bool"), core, jl_any_type,
                                        jl_emptysvec, 8);
    jl_false = jl_permbox8(jl_bool_type, 0);
    jl_true = jl_permbox8(jl_bool_type, 1);

    jl_abstractstring_type = jl_new_abstracttype((jl_value_t *)jl_symbol("AbstractString"),
                                                 core, jl_any_type, jl_emptysvec);
    jl_string_type = jl_new_datatype(jl_symbol("String"), core, jl_abstractstring_type,
    ...
    *(size_t *)jl_an_empty_string = 0;
    ...
    jl_typemap_level_type =
        jl_new_datatype(jl_symbol("TypeMapLevel"), core, jl_any_type, jl_emptysvec,
                        jl_perm_symsvec(4, "arg1", "targ", "list", "any"),
                        jl_svec(4, jl_any_type, jl_any_type, jl_any_type, jl_any_type), 0,
                        1, 4);

    jl_typemap_entry_type = jl_new_datatype(
        jl_symbol("TypeMapEntry"), core, jl_any_type, jl_emptysvec,
        jl_perm_symsvec(10, "next", "sig", "simplesig", "guardsigs", "min_world",
                        "max_world", "func", "isleafsig", "issimplesig", "va"),
        jl_svec(10,
    ...
    jl_function_type = jl_new_abstracttype((jl_value_t *)jl_symbol("Function"), core,
                                           jl_any_type, jl_emptysvec);
    jl_builtin_type = jl_new_abstracttype((jl_value_t *)jl_symbol("Builtin"), core,
                                          jl_function_type, jl_emptysvec);
    jl_function_type->name->mt =
        NULL; // subtypes of Function have independent method tables
    jl_builtin_type->name->mt = NULL; // so they don't share the Any type table

    tv = jl_svec2(tvar("T"), tvar("N"));
    jl_abstractarray_type =
        (jl_unionall_t *)jl_new_abstracttype((jl_value_t *)jl_symbol("AbstractArray"), core,
                                             jl_any_type, tv)
            ->name->wrapper;

    tv = jl_svec2(tvar("T"), tvar("N"));
    jl_densearray_type = (jl_unionall_t *)jl_new_abstracttype(
                             (jl_value_t *)jl_symbol("DenseArray"), core,
                             (jl_datatype_t *)jl_apply_type(
                                 (jl_value_t *)jl_abstractarray_type, jl_svec_data(tv), 2),
                             tv)
                             ->name->wrapper;

    tv = jl_svec2(tvar("T"), tvar("N"));
    jl_array_type = (jl_unionall_t *)jl_new_datatype(
                        jl_symbol("Array"), core,
                        (jl_datatype_t *)jl_apply_type((jl_value_t *)jl_densearray_type,
                                                       jl_svec_data(tv), 2),
                        tv, jl_emptysvec, jl_emptysvec, 0, 1, 0)
                        ->name->wrapper;
    jl_array_typename =
        ((jl_datatype_t *)jl_unwrap_unionall((jl_value_t *)jl_array_type))->name;
    jl_compute_field_offsets(
        (jl_datatype_t *)jl_unwrap_unionall((jl_value_t *)jl_array_type));

    jl_array_any_type = jl_apply_type2((jl_value_t *)jl_array_type,
                                       (jl_value_t *)jl_any_type, jl_box_long(1));
    jl_array_symbol_type = jl_apply_type2((jl_value_t *)jl_array_type,
                                          (jl_value_t *)jl_symbol_type, jl_box_long(1));
    jl_array_uint8_type = jl_apply_type2((jl_value_t *)jl_array_type,
                                         (jl_value_t *)jl_uint8_type, jl_box_long(1));
    jl_array_int32_type = jl_apply_type2((jl_value_t *)jl_array_type,
                                         (jl_value_t *)jl_int32_type, jl_box_long(1));

    jl_expr_type = jl_new_datatype(jl_symbol("Expr"), core, jl_any_type, jl_emptysvec,
                                   jl_perm_symsvec(2, "head", "args"),
                                   jl_svec(2, jl_symbol_type, jl_array_any_type), 0, 1, 2);

    jl_module_type = jl_new_datatype(jl_symbol("Module"), core, jl_any_type, jl_emptysvec,
                                     jl_perm_symsvec(2, "name", "parent"),
                                     jl_svec(2, jl_symbol_type, jl_any_type), 0, 1, 2);

    jl_value_t *symornothing[2] = {(jl_value_t *)jl_symbol_type,
                                   (jl_value_t *)jl_void_type};
    ...
    jl_lineinfonode_type =
        jl_new_datatype(jl_symbol("LineInfoNode"), core, jl_any_type, jl_emptysvec,
                        ...
                        jl_svec(4, jl_any_type, jl_symbol_type, jl_long_type, jl_long_type),
                        0, 0, 4);

    jl_gotonode_type =
        jl_new_datatype(jl_symbol("GotoNode"), core, jl_any_type, jl_emptysvec,
                        jl_perm_symsvec(1, "label"), jl_svec(1, jl_long_type), 0, 0, 1);

    jl_pinode_type = jl_new_datatype(jl_symbol("PiNode"), core, jl_any_type, jl_emptysvec,
    ...
    jl_phicnode_type = jl_new_datatype(jl_symbol("PhiCNode"), core, jl_any_type,
                                       jl_emptysvec, jl_perm_symsvec(1, "values"),
                                       jl_svec(1, jl_array_any_type), 0, 0, 1);

    jl_upsilonnode_type =
        jl_new_datatype(jl_symbol("UpsilonNode"), core, jl_any_type, jl_emptysvec,
                        jl_perm_symsvec(1, "val"), jl_svec(1, jl_any_type), 0, 0, 0);

    jl_quotenode_type =
        jl_new_datatype(jl_symbol("QuoteNode"), core, jl_any_type, jl_emptysvec,
                        jl_perm_symsvec(1, "value"), jl_svec(1, jl_any_type), 0, 0, 1);

    jl_newvarnode_type = jl_new_datatype(jl_symbol("NewvarNode"), core, jl_any_type,
                                         jl_emptysvec, jl_perm_symsvec(1, "slot"),
    ...
    jl_code_info_type = jl_new_datatype(
        jl_symbol("CodeInfo"), core, jl_any_type, jl_emptysvec,
        jl_perm_symsvec(18, "code", "codelocs", "ssavaluetypes", "ssaflags",
                        "method_for_inference_limit_heuristics", "linetable", "slotnames",
                        "slotflags", "slottypes", "rettype", "parent", "edges", "min_world",
                        "max_world", "inferred", "inlineable", "propagate_inbounds",
                        "pure"),
        jl_svec(18, jl_array_any_type, jl_any_type, jl_any_type, jl_array_uint8_type,
                jl_any_type, jl_any_type, jl_array_symbol_type, jl_array_uint8_type,
                jl_any_type, jl_any_type, jl_any_type, jl_any_type, jl_ulong_type,
                jl_ulong_type, jl_bool_type, jl_bool_type, jl_bool_type, jl_bool_type),
        0, 1, 18);

    jl_method_type = jl_new_datatype(
        jl_symbol("Method"), core, jl_any_type, jl_emptysvec,
        jl_perm_symsvec(24, "name", "module", "file", "line", "primary_world",
                        "deleted_world", "sig", "ambig", "resorted", "specializations",
                        "speckeyset", "slot_syms", "source", "unspecialized", "generator",
                        "roots", "ccallable", "invokes", "nargs", "called", "nospecialize",
                        "nkw", "isva", "pure"),
        jl_svec(24, jl_symbol_type, jl_module_type, jl_symbol_type, jl_int32_type,
                jl_ulong_type, jl_ulong_type, jl_type_type,
                jl_any_type, // Union{Vector, Nothing}
                jl_any_type, // Union{Vector, Nothing}
                jl_simplevector_type, jl_array_type, jl_string_type, jl_any_type,
                jl_any_type, // jl_method_instance_type
                jl_any_type, jl_array_any_type, jl_simplevector_type, jl_any_type,
                jl_int32_type, jl_int32_type, jl_int32_type, jl_int32_type, jl_bool_type,
                jl_bool_type),
        0, 1, 12);

    jl_method_instance_type = jl_new_datatype(
        jl_symbol("MethodInstance"), core, jl_any_type, jl_emptysvec,
        jl_perm_symsvec(7, "def", "specTypes", "sparam_vals", "uninferred", "backedges",
                        "cache", "inInference"),
        jl_svec(7, jl_new_struct(jl_uniontype_type, jl_method_type, jl_module_type),
                jl_any_type, jl_simplevector_type, jl_any_type, jl_any_type, jl_any_type,
                jl_bool_type),
        0, 1, 3);

    jl_code_instance_type = jl_new_datatype(
        jl_symbol("CodeInstance"), core, jl_any_type, jl_emptysvec,
        jl_perm_symsvec(10, "def", "next", "min_world", "max_world", "rettype",
                        "rettype_const", "inferred",
                        //"edges",
                        //"absolute_max",
                        "isspecsig", "invoke", "specptr"), // function object decls
        jl_svec(10, jl_method_instance_type, jl_any_type, jl_ulong_type, jl_ulong_type,
                jl_any_type, jl_any_type, jl_any_type,
                // jl_any_type,
                // jl_bool_type,
                jl_bool_type, jl_any_type, jl_any_type), // fptrs
        0, 1, 1);
    ...
    jl_intrinsic_type = jl_new_primitivetype((jl_value_t *)jl_symbol("IntrinsicFunction"),
                                             core, jl_builtin_type, jl_emptysvec, 32);

    tv = jl_svec1(tvar("T"));
    jl_ref_type = (jl_unionall_t *)jl_new_abstracttype((jl_value_t *)jl_symbol("Ref"), core,
                                                       jl_any_type, tv)
                      ->name->wrapper;

    tv = jl_svec1(tvar("T"));
    jl_pointer_type =
        (jl_unionall_t *)jl_new_primitivetype(
            (jl_value_t *)jl_symbol("Ptr"), core,
            (jl_datatype_t *)jl_apply_type((jl_value_t *)jl_ref_type, jl_svec_data(tv), 1),
            tv, sizeof(void *) * 8)
            ->name->wrapper;
    jl_pointer_typename =
        ((jl_datatype_t *)jl_unwrap_unionall((jl_value_t *)jl_pointer_type))->name;

    // AddrSpacePtr{T, AS} where {T, AS}
    tv = jl_svec2(tvar("T"), tvar("AS"));
    jl_svec_t *tv_base = jl_svec1(tvar("T"));
    jl_addrspace_pointer_type =
        (jl_unionall_t *)jl_new_primitivetype(
            (jl_value_t *)jl_symbol("AddrSpacePtr"), core,
            (jl_datatype_t *)jl_apply_type((jl_value_t *)jl_ref_type, jl_svec_data(tv_base),
                                           1),
            tv, sizeof(void *) * 8)
            ->name->wrapper;
    jl_addrspace_pointer_typename =
        ((jl_datatype_t *)jl_unwrap_unionall((jl_value_t *)jl_addrspace_pointer_type))
            ->name;

    // Type{T} where T<:Tuple
    tttvar = jl_new_typevar(jl_symbol("T"), (jl_value_t *)jl_bottom_type,
                            (jl_value_t *)jl_anytuple_type);
    jl_anytuple_type_type = (jl_unionall_t *)jl_new_struct(
        jl_unionall_type, tttvar, (jl_value_t *)jl_wrap_Type((jl_value_t *)tttvar));

    // Type{T}
    jl_tvar_t *typetype_tvar = tvar("T");
    jl_typetype_type = (jl_unionall_t *)jl_new_struct(
        jl_unionall_type, typetype_tvar,
    ...
    jl_task_type = (jl_datatype_t *)jl_new_datatype(
        jl_symbol("Task"), NULL, jl_any_type, jl_emptysvec,
        jl_perm_symsvec(11, "next", "queue", "storage", "state", "donenotify", "result",
                        "exception", "backtrace", "logstate", "code", "sticky"),
        jl_svec(11, jl_any_type, jl_any_type, jl_any_type, jl_symbol_type, jl_any_type,
                jl_any_type, jl_any_type, jl_any_type, jl_any_type, jl_any_type,
                jl_bool_type),
        0, 1, 9);
    ...
    jl_value_t *pointer_void =
        jl_apply_type1((jl_value_t *)jl_pointer_type, (jl_value_t *)jl_nothing_type);
    ...
    // It seems like we probably usually end up needing the box for kinds (used in an Any
    // context)--but is that true?
