...
// Copied from MINGW_FLOAT_H which may not be found due to a collision with the builtin gcc
// float.h eventually we can probably integrate this into OpenLibm.
...
void __cdecl crt_sig_handler(int sig, int num)
{
    ...
    case SIGFPE:
        ...
        case _FPE_OVERFLOW:
        case _FPE_UNDERFLOW:
        default: jl_errorf("Unexpected FPE Error 0x%X", num); break;
        case _FPE_ZERODIVIDE: jl_throw(jl_diverror_exception); break;
        }
        break;
    case SIGINT:
...
static void JL_NORETURN start_backtrace_fiber(void)
{
    ...
    ptls->bt_size =
        rec_backtrace_ctx(ptls->bt_data, JL_MAX_BT_SIZE, error_ctx, ptls->pgcstack, 0);
...
LONG WINAPI jl_exception_handler(struct _EXCEPTION_POINTERS *ExceptionInfo)
{
    ...
    if (ExceptionInfo->ExceptionRecord->ExceptionFlags == 0) {
        ...
        case EXCEPTION_ARRAY_BOUNDS_EXCEEDED:
            ...
            break;
        case EXCEPTION_BREAKPOINT: jl_safe_printf("EXCEPTION_BREAKPOINT"); break;
        ...
        case EXCEPTION_FLT_INVALID_OPERATION:
            ...
            break;
        case EXCEPTION_FLT_OVERFLOW: jl_safe_printf("EXCEPTION_FLT_OVERFLOW"); break;
        case EXCEPTION_FLT_STACK_CHECK: jl_safe_printf("EXCEPTION_FLT_STACK_CHECK"); break;
        case EXCEPTION_FLT_UNDERFLOW: jl_safe_printf("EXCEPTION_FLT_UNDERFLOW"); break;
        case EXCEPTION_ILLEGAL_INSTRUCTION:
            jl_safe_printf("EXCEPTION_ILLEGAL_INSTRUCTION");
            break;
        case EXCEPTION_IN_PAGE_ERROR: jl_safe_printf("EXCEPTION_IN_PAGE_ERROR"); break;
        case EXCEPTION_INT_DIVIDE_BY_ZERO:
            jl_safe_printf("EXCEPTION_INT_DIVIDE_BY_ZERO");
            break;
        case EXCEPTION_INT_OVERFLOW: jl_safe_printf("EXCEPTION_INT_OVERFLOW"); break;
        ...
        case EXCEPTION_PRIV_INSTRUCTION:
            ...
            break;
        case EXCEPTION_SINGLE_STEP: jl_safe_printf("EXCEPTION_SINGLE_STEP"); break;
        case EXCEPTION_STACK_OVERFLOW: jl_safe_printf("EXCEPTION_STACK_OVERFLOW"); break;
        default: jl_safe_printf("UNKNOWN"); break;
        }
        jl_safe_printf(" at 0x%Ix -- ",
                       (size_t)ExceptionInfo->ExceptionRecord->ExceptionAddress);
        jl_print_native_codeloc(
            (uintptr_t)ExceptionInfo->ExceptionRecord->ExceptionAddress);

        jl_critical_error(0, ExceptionInfo->ContextRecord, ptls->bt_data, &ptls->bt_size);
    ...
    while (1) {
        if (bt_size_cur < bt_size_max) {
            ...
            if (running) {
                ...
                ctxThread.ContextFlags = CONTEXT_CONTROL | CONTEXT_INTEGER;
                if (!GetThreadContext(hMainThread, &ctxThread)) {
                    fputs("failed to get context from main thread. aborting profiling.",
                          stderr);
                    break;
                }
                // Get backtrace data
...
JL_DLLEXPORT int jl_profile_start_timer(void)
{
    running = 1;
    if (hBtThread == 0) {
        hBtThread = CreateThread(NULL, // default security attributes
