...
/*
  ...
  https://github.com/JeffBezanson/phdthesis/ This code adds the following features to the
  core algorithm:
...
typedef struct jl_varbinding_t {
    ...
    int8_t
        concrete; // 1 if another variable has a constraint forcing this one to be concrete
...
static jl_varbinding_t *lookup(jl_stenv_t *e,
                               jl_tvar_t *v) JL_GLOBALLY_ROOTED JL_NOTSAFEPOINT;
#else
static jl_varbinding_t *lookup(jl_stenv_t *e,
                               jl_tvar_t *v) JL_GLOBALLY_ROOTED JL_NOTSAFEPOINT
{
...
static void restore_env(jl_stenv_t *e, jl_value_t *root, jl_savedenv_t *se) JL_NOTSAFEPOINT
{
    ...
    while (v != NULL) {
        if (root)
            v->lb = jl_svecref(root, i);
        i++;
        if (root)
            v->ub = jl_svecref(root, i);
        i++;
        if (root)
            v->innervars = (jl_array_t *)jl_svecref(root, i);
        i++;
        assert(se->buf);
...
static int obviously_egal(jl_value_t *a, jl_value_t *b)
{
    if (a == (jl_value_t *)jl_typeofbottom_type->super)
        a = (jl_value_t *)jl_typeofbottom_type; // supertype(typeof(Union{})) is equal to,
                                                // although distinct from, itself
    if (b == (jl_value_t *)jl_typeofbottom_type->super)
        b = (jl_value_t *)jl_typeofbottom_type; // supertype(typeof(Union{})) is equal to,
                                                // although distinct from, itself
    if (a == b)
        return 1;
    ...
    if (jl_is_datatype(a)) {
        ...
        if (ad->name != bd->name)
            return 0;
        if (ad->isconcretetype || bd->isconcretetype)
            return 0;
...
static int obviously_unequal(jl_value_t *a, jl_value_t *b)
{
    ...
    if (jl_is_datatype(a)) {
        ...
        if (jl_is_datatype(b)) {
            ...
            if (jl_is_concrete_type(a) || jl_is_concrete_type(b)) {
                if (!istuple &&
                    ad->name !=
                        jl_type_typename) // HACK: can't properly normalize Tuple{Float64}
                                          // == Tuple{<:Float64} like types or Type{T} types
                    return 1;
            }
            size_t i, np;
            if (istuple) {
    ...
    if (jl_is_typevar(a) && jl_is_typevar(b) &&
        obviously_unequal(((jl_tvar_t *)a)->ub, ((jl_tvar_t *)b)->ub))
        return 1;
    if (jl_is_long(a)) {
    ...
    if (jl_is_datatype(a) && jl_is_datatype(b)) {
        ...
        if (istuple) {
            ...
            if (jl_is_va_tuple(ad)) {
                ...
                // note: some disjoint types (e.g. tuples of different lengths) can be more
                // specific
                return 1;
            }
            np = na < nb ? na : nb;
...
static jl_value_t *simple_join(jl_value_t *a, jl_value_t *b)
{
    ...
    if (!jl_has_free_typevars(a) && !jl_has_free_typevars(b) &&
        ...
        !(jl_is_type_type(a) && jl_is_type_type(b) &&
...
static jl_value_t *pick_union_element(jl_value_t *u JL_PROPAGATES_ROOT, jl_stenv_t *e,
                                      int8_t R) JL_NOTSAFEPOINT
{
    ...
    do {
        ...
        if (ui == 0) {
            state->more =
                state->depth; // memorize that this was the deepest available choice
...
static jl_value_t *intersect_aside(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, int R,
                                   int d);
...
static int var_lt(jl_tvar_t *b, jl_value_t *a, jl_stenv_t *e, int param)
{
    ...
    if (bb->ub == a)
        return 1;
    if (!((bb->lb == jl_bottom_type && !jl_is_type(a) && !jl_is_typevar(a)) ||
          subtype_ccheck(bb->lb, a, e)))
        return 0;
    ...
    if (jl_is_typevar(a)) {
        ...
        if (aa && !aa->right && in_union(bb->lb, a) && bb->depth0 != aa->depth0 &&
            var_outside(e, b, (jl_tvar_t *)a)) {
...
static int var_gt(jl_tvar_t *b, jl_value_t *a, jl_stenv_t *e, int param)
{
    ...
    if (!((bb->ub == (jl_value_t *)jl_any_type && !jl_is_type(a) && !jl_is_typevar(a)) ||
          subtype_ccheck(a, bb->ub, e)))
        return 0;
    bb->lb = simple_join(bb->lb, a);
    assert(bb->lb != (jl_value_t *)b);
    if (jl_is_typevar(a)) {
        jl_varbinding_t *aa = lookup(e, (jl_tvar_t *)a);
        if (aa && !aa->right && bb->depth0 != aa->depth0 && param == 2 &&
            var_outside(e, b, (jl_tvar_t *)a))
            return subtype_left_var(aa->ub, aa->lb, e, param);
    }
    return 1;
...
static int var_occurs_inside(jl_value_t *v, jl_tvar_t *var, int inside,
                             int want_inv) JL_NOTSAFEPOINT;
...
static int with_tvar(tvar_callback callback, void *context, jl_unionall_t *u, int8_t R,
                     jl_stenv_t *e, int param)
{
    jl_varbinding_t vb = {
        u->var, u->var->lb, u->var->ub, R,      0, 0, 0, 0, R ? e->Rinvdepth : e->invdepth,
        0,      NULL,       0,          e->vars};
    ...
    if (R) {
        ...
        if (e->envidx < e->envsz) {
            ...
            if (!vb.occurs_inv && vb.lb != jl_bottom_type)
                val = is_leaf_bound(vb.lb) ?
                          vb.lb :
                          (jl_value_t *)jl_new_typevar(u->var->name, jl_bottom_type, vb.lb);
            else if (vb.lb == vb.ub)
                val = vb.lb;
            ...
            // TODO: substitute the value (if any) of this variable into previous envout
            // entries
        }
    }
    else {
    ...
    if (vb.lb != vb.ub) {
        while (btemp != NULL) {
            ...
            if (btemp->depth0 != vb.depth0 &&
                ((vu != (jl_value_t *)vb.var && btemp->var->ub != vu &&
                  var_occurs_inside(vu, vb.var, 0, 1)) ||
                 (vl != (jl_value_t *)vb.var && btemp->var->lb != vl &&
                  var_occurs_inside(vl, vb.var, 0, 1)))) {
                ans = 0;
                break;
            }
            btemp = btemp->prev;
...
static int subtype_unionall_callback(struct subtype_unionall_env *env, int8_t R,
                                     jl_stenv_t *s, int param)
{
                            ...
                            int param)
{
    ...
    JL_GC_PUSH1(&u);
    int res =
        with_tvar((tvar_callback)subtype_unionall_callback, (void *)&env, u, R, e, param);
    JL_GC_POP();
    return res;
}
...
static int subtype_tuple_varargs(struct subtype_tuple_env *env, jl_stenv_t *e, int param)
{
    ...
    jl_value_t *xp0 = jl_tparam0(xva);
    jl_value_t *xp1 = jl_tparam1(xva);
    jl_value_t *yp0 = jl_tparam0(yva);
    jl_value_t *yp1 = jl_tparam1(yva);
    ...
    if (!subtype(xp0, yp0, e, param))
        return 0;
    if (!subtype(xp0, yp0, e, 1))
        return 0;
...
static int subtype_tuple_tail(struct subtype_tuple_env *env, int8_t R, jl_stenv_t *e,
                              int param)
{
    int x_reps = 1;
loop: // while (i <= lx) {
        ...
        if (env->j == env->ly - 1 && env->vvy) {
            if (!env->vty) {
                ...
                if (jl_is_unionall(yi)) {
                    ...
                    JL_GC_PUSH1(&u);
                    int res =
                        with_tvar((tvar_callback)subtype_tuple_tail, env, u, 1, e, param);
                    JL_GC_POP();
                    return res;
                }
                env->vty = yi;
    ...
    if (x_reps > 2) {
        ...
        // element type on the right more than twice.
             ...
             ((yi == env->lasty && !jl_has_free_typevars(xi) &&
               !jl_has_free_typevars(yi)) ||
              (yi == env->lasty && !env->vx && env->vy && jl_is_concrete_type(xi)))) {
        // fast path for repeated elements
    }
    else if (e->Runions.depth == 0 && e->Lunions.depth == 0 && !jl_has_free_typevars(xi) &&
             !jl_has_free_typevars(yi)) {
    ...
    return (env->lx + env->vx == env->ly + env->vy) ||
           (env->vy && (env->lx >= (env->vx ? env->ly : (env->ly - 1))));
}
        ...
        if (env.vvx == JL_VARARG_UNBOUND || (xbb && !xbb->right)) {
            ...
            if (env.vvy == JL_VARARG_NONE || env.vvy == JL_VARARG_INT)
                ...
                return 0;
                 ...
                 !check_vararg_length(jl_tparam(env.xd, env.lx - 1), env.ly + 1 - env.lx,
                                      e)) {
            return 0;
        }
    }
...
static int subtype_naked_vararg(jl_datatype_t *xd, jl_datatype_t *yd, jl_stenv_t *e,
                                int param)
{
    // Vararg: covariant in first parameter, invariant in second
    jl_value_t *xp1 = jl_tparam0(xd), *xp2 = jl_tparam1(xd), *yp1 = jl_tparam0(yd),
               *yp2 = jl_tparam1(yd);
    // in Vararg{T1} <: Vararg{T2}, need to check subtype twice to
    // simulate the possibility of multiple arguments, which is needed
    // to implement the diagonal rule correctly.
    if (!subtype(xp1, yp1, e, param))
        return 0;
    if (!subtype(xp1, yp1, e, 1))
        return 0;
    e->invdepth++;
    e->Rinvdepth++;
    // Vararg{T,N} <: Vararg{T2,N2}; equate N and N2
...
static int subtype(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, int param)
{
    ...
    if (jl_is_uniontype(y)) {
        ...
        int ui = 1;
        if (jl_is_typevar(x)) {
            // The `convert(Type{T},T)` pattern, where T is a Union, required changing
            // priority of unions and vars: if matching `typevar <: union`, first try to
            // match the whole union against the variable before trying to take it apart to
            // see if there are any variables lurking inside.
    ...
    if ((x != ux || y != uy) && y != (jl_value_t *)jl_any_type && jl_is_datatype(ux) &&
        jl_is_datatype(uy) && !jl_is_type_type(ux)) {
        assert(ux);
    ...
    if (jl_is_datatype(x) && jl_is_datatype(y)) {
        ...
        if (jl_is_type_type(x) && !jl_is_type_type(y)) {
            ...
            if (!jl_is_typevar(tp0)) {
                // TODO this is not strictly correct, but we don't yet have any other way
                // for e.g. the argument `Int` to match a `::DataType` slot. Most correct
                // would be: Int isa DataType, Int isa Type{Int}, Type{Int} more specific
                // than DataType,
                // !(Type{Int} <: DataType), !isleaftype(Type{Int}), because non-DataTypes
                // can be type-equal to `Int`.
        ...
        if (jl_is_type_type(y) && !jl_is_type_type(x) &&
            ...
            if (!jl_is_typevar(tp0) || !jl_is_kind(x))
                return 0;
            // DataType.super is special, so `DataType <: Type{T}` (T free) needs special
            // handling. The answer is true iff `T` has full bounds (as in `Type`), but this
            // needs to be checked at the same depth where `Type{T}` occurs --- the depth of
            // the LHS doesn't matter because it (e.g. `DataType`) doesn't actually contain
            // the variable.
        ...
        while (xd != jl_any_type && xd->name != yd->name) {
            if (xd->super == NULL)
                jl_errorf("circular type parameter constraint in definition of %s",
                          jl_symbol_name(xd->name->name));
            xd = xd->super;
        }
        if (xd == jl_any_type)
            return 0;
...
static int exists_subtype(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, jl_value_t *saved,
                          jl_savedenv_t *se, int param)
{
    memset(e->Runions.stack, 0, sizeof(e->Runions.stack));
    int lastset = 0;
    while (1) {
...
static int concrete_min(jl_value_t *t)
{
    ...
    if (jl_is_datatype(t)) {
        if (jl_is_type_type(t))
            return 0; // Type{T} may have the concrete supertype `typeof(T)`, so don't try
                      // to handle them here
...
static int obvious_subtype(jl_value_t *x, jl_value_t *y, jl_value_t *y0, int *subtype)
{
    ...
    if (x == (jl_value_t *)jl_typeofbottom_type->super)
        x = (jl_value_t *)jl_typeofbottom_type; // supertype(typeof(Union{})) is equal to,
                                                // although distinct from, itself
    if (y == (jl_value_t *)jl_typeofbottom_type->super)
        y = (jl_value_t *)jl_typeofbottom_type; // supertype(typeof(Union{})) is equal to,
                                                // although distinct from, itself
    if (x == y || y == (jl_value_t *)jl_any_type) {
        *subtype = 1;
        return 1;
    ...
    if (jl_is_datatype(y)) {
        ...
        if (jl_is_datatype(x)) {
            ...
            // if (((jl_datatype_t*)x)->isconcretetype &&
            // ((jl_datatype_t*)y)->isconcretetype && (!istuple || !istuple_x)) {
            //    *subtype = 0;
            //    return 1;
            //}
            int uncertain = 0;
            ...
            if (!iscov && !((jl_datatype_t *)x)->hasfreetypevars) {
                // by transitivity, if `wrapper <: y`, then `x <: y` if x is a leaf type of
                // its name
                if (obvious_subtype(((jl_datatype_t *)x)->name->wrapper, y, y0, subtype) &&
                    *subtype)
                    return 1;
            }
            ...
            if (istuple) {
                ...
                // if the nparams aren't equal, or at least one of them is a typevar
                // (uncertain), they may be obviously disjoint
                if (nparams_expanded_x != nparams_expanded_y ||
                    (vx != JL_VARARG_NONE && vx != JL_VARARG_INT) ||
                    (vy != JL_VARARG_NONE && vy != JL_VARARG_INT)) {
                    // we have a stronger bound on x if:
                    if (vy == JL_VARARG_NONE ||
                        vy == JL_VARARG_INT) { // the bound on y is certain
                        if (vx == JL_VARARG_NONE || vx == JL_VARARG_INT ||
                            vx == JL_VARARG_UNBOUND || // and the bound on x is also certain
                            nparams_expanded_x > nparams_expanded_y ||
                            npx > nparams_expanded_y) { // or x is unknown, but definitely
                                                        // longer than y
                            *subtype = 0;
                            return 1; // number of fixed parameters in x are more than
                                      // declared in y
                        }
                    }
                    if (nparams_expanded_x < nparams_expanded_y) {
                        *subtype = 0;
                        return 1; // number of fixed parameters in x could be fewer than in
                                  // y
                    }
                    uncertain = 1;
                }
            ...
            for (i = 0; i < npy - (vy == JL_VARARG_NONE ? 0 : 1); i++) {
                jl_value_t *a =
                    i >= (npx - (vx == JL_VARARG_NONE ? 0 : 1)) ? vxt : jl_tparam(x, i);
                jl_value_t *b = jl_tparam(y, i);
                if (iscov || jl_is_typevar(b)) {
                    if (obvious_subtype(a, b, y0, subtype)) {
                        if (!*subtype)
                            return 1;
                        if (jl_has_free_typevars(
                                b)) // b is actually more constrained that this
                            uncertain = 1;
                    }
                    else {
                        uncertain = 1;
                    ...
                    if (!obviously_egal(a, b)) {
                        ...
                        if (!jl_has_free_typevars(b) &&
                            obvious_subtype(b, a, y0, subtype)) {
                            if (!*subtype)
                                return 1;
                            if (jl_has_free_typevars(
                                    a)) // a is actually more constrained that this
                                uncertain = 1;
                        }
                        else {
                            uncertain = 1;
            ...
            if (i < nparams_expanded_x) {
                // there are elements left in x (possibly just a Vararg), check them against
                // the Vararg tail of y too
                assert(vy != JL_VARARG_NONE && istuple && iscov);
                jl_value_t *a1 =
                    (vx != JL_VARARG_NONE && i >= npx - 1) ? vxt : jl_tparam(x, i);
                ...
                if (nparams_expanded_x > npy && jl_is_typevar(b) && concrete_min(a1) > 1) {
                    // diagonal rule for 2 or more elements: they must all be concrete on
                    // the LHS
                    *subtype = 0;
                    return 1;
                }
                ...
                for (; i < nparams_expanded_x; i++) {
                    ...
                    if (i > npy && jl_is_typevar(b)) { // i == npy implies a == a1
                        // diagonal rule: all the later parameters are also constrained to
                        // be type-equal to the first
                        jl_value_t *a2 = a;
                        if (jl_is_type_type(a) && jl_is_type(jl_tparam0(a))) {
                            // if a is exactly Type{T}, then use the concrete typeof(T)
                            // instead here
                        ...
                        if (!obviously_egal(a1, a2)) {
                            if (obvious_subtype(a2, a1, y0, subtype)) {
                                if (!*subtype)
                                    return 1;
                                if (jl_has_free_typevars(
                                        a1)) // a1 is actually more constrained that this
                                    uncertain = 1;
                            }
                            else {
                                uncertain = 1;
                            ...
                            if (obvious_subtype(a1, a2, y0, subtype)) {
                                if (!*subtype)
                                    return 1;
                                if (jl_has_free_typevars(
                                        a2)) // a2 is actually more constrained that this
                                    uncertain = 1;
                            }
                            else {
                                uncertain = 1;
...
JL_DLLEXPORT int jl_subtype_env(jl_value_t *x, jl_value_t *y, jl_value_t **env, int envsz)
{
    ...
    if (y == (jl_value_t *)jl_any_type || x == jl_bottom_type)
        return 1;
    if (x == y || (jl_typeof(x) == jl_typeof(y) &&
                   (jl_is_unionall(y) || jl_is_uniontype(y)) && jl_egal(x, y))) {
    ...
    assert(obvious_subtype == 3 || obvious_subtype == subtype || jl_has_free_typevars(x) ||
           jl_has_free_typevars(y));
#ifndef NDEBUG
    if (obvious_subtype == 0 || (obvious_subtype == 1 && envsz == 0))
        subtype = obvious_subtype; // this ensures that running in a debugger doesn't change
                                   // the result
#endif
    return subtype;
}

static int subtype_in_env_(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, int invdepth,
                           int Rinvdepth)
{
    jl_stenv_t e2;
...
JL_DLLEXPORT int jl_types_equal(jl_value_t *a, jl_value_t *b)
{
    ...
    else if (jl_typeof(a) == jl_typeof(b) && (jl_is_unionall(b) || jl_is_uniontype(b)) &&
             jl_egal(a, b)) {
        subtype_ab = 1;
    }
    ...
    else if (jl_typeof(b) == jl_typeof(a) && (jl_is_unionall(a) || jl_is_uniontype(a)) &&
             jl_egal(b, a)) {
        subtype_ba = 1;
    }
...
#endif
    {
        ...
        assert(subtype_ab == 3 || subtype_ab == subtype || jl_has_free_typevars(a) ||
               jl_has_free_typevars(b));
#ifndef NDEBUG
        if (subtype_ab != 0 &&
            subtype_ab != 1) // ensures that running in a debugger doesn't change the result
#endif
            subtype_ab = subtype;
#ifdef NDEBUG
        if (subtype_ab == 0)
...
#endif
    {
        ...
        assert(subtype_ba == 3 || subtype_ba == subtype || jl_has_free_typevars(a) ||
               jl_has_free_typevars(b));
#ifndef NDEBUG
        if (subtype_ba != 0 &&
            subtype_ba != 1) // ensures that running in a debugger doesn't change the result
#endif
            subtype_ba = subtype;
    }
...
JL_DLLEXPORT int jl_is_not_broken_subtype(jl_value_t *a, jl_value_t *b)
{
    ...
    return !jl_is_kind(b) ||
           !jl_is_type_type(
               a); // || jl_is_datatype_singleton((jl_datatype_t*)jl_tparam0(a));
}
...
// this is sufficient to determine if `isa(x, T)` can instead simply check for `typeof(x) <:
// T`
...
static jl_value_t *intersect_union(jl_value_t *x, jl_uniontype_t *u, jl_stenv_t *e,
                                   int8_t R, int param)
{
    if (param == 2 ||
        (!jl_has_free_typevars(x) && !jl_has_free_typevars((jl_value_t *)u))) {
    ...
    // try all possible choices in covariant position; union them all together at the top
    // level
    return R ? intersect(x, choice, e, param) : intersect(choice, x, e, param);
}

// set a variable to a non-type constant
static jl_value_t *set_var_to_const(jl_varbinding_t *bb, jl_value_t *v JL_MAYBE_UNROOTED,
                                    jl_varbinding_t *othervar)
{
...
static void set_bound(jl_value_t **bound, jl_value_t *val, jl_tvar_t *v,
                      jl_stenv_t *e) JL_NOTSAFEPOINT
{
...
static int subtype_in_env_existential(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, int R,
                                      int d)
{
    jl_varbinding_t *v = e->vars;
    int len = 0;
...
static jl_value_t *intersect_var(jl_tvar_t *b, jl_value_t *a, jl_stenv_t *e, int8_t R,
                                 int param)
{
    jl_varbinding_t *bb = lookup(e, b);
    if (bb == NULL)
    ...
    if (param == 2) {
        ...
        if (!jl_has_free_typevars(a)) {
            ...
            int issub = subtype_in_env_existential(bb->lb, a, e, 0, d) &&
                        subtype_in_env_existential(a, bb->ub, e, 1, d);
            restore_env(e, root, &se);
            free(se.buf);
            if (!issub) {
                JL_GC_POP();
            ...
            ub = R ? intersect_aside(a, bb->ub, e, 1, d) :
                     intersect_aside(bb->ub, a, e, 0, d);
            // TODO: we should probably check `bb->lb <: ub` here; find a test case for that
        }
        if (ub != (jl_value_t *)b) {
            if (jl_has_free_typevars(ub)) {
                // constraint X == Ref{X} is unsatisfiable. also check variables set equal
                // to X.
        ...
        return R ? intersect_aside(a, bb->ub, e, 1, d) :
                   intersect_aside(bb->ub, a, e, 0, d);
    }
    else if (bb->concrete || bb->constraintkind == 1) {
        jl_value_t *ub =
            R ? intersect_aside(a, bb->ub, e, 1, d) : intersect_aside(bb->ub, a, e, 0, d);
    ...
    save_env(e, &root, &se);
    jl_value_t *ii =
        R ? intersect_aside(a, bb->lb, e, 1, d) : intersect_aside(bb->lb, a, e, 0, d);
                             ...
                             int want_inv) JL_NOTSAFEPOINT
{
    if (v == (jl_value_t *)var) {
        ...
        if (ua->var == var)
            return 0;
        if (var_occurs_inside(ua->var->lb, var, inside, want_inv) ||
            var_occurs_inside(ua->var->ub, var, inside, want_inv))
            return 1;
...
static jl_value_t *finish_unionall(jl_value_t *res JL_MAYBE_UNROOTED, jl_varbinding_t *vb,
                                   jl_stenv_t *e)
{
        ...
        if (jl_has_typevar(btemp->lb, vb->var)) {
            ...
            if (varval) {
                ...
                btemp->lb = vb->lb;
                     ...
                     !jl_has_typevar(vb->ub, btemp->var) &&
                     jl_has_typevar(btemp->ub, vb->var)) {
                // if our variable is T, and some outer variable has constraint S = Ref{T},
                // move the `where T` outside `where S` instead of putting it here. issue
                // #21243.
    ...
    if (jl_has_typevar(res, vb->var)) {
        if (varval) {
            JL_TRY {
                ...
                while (jl_is_unionall(res) &&
                       obviously_egal(((jl_unionall_t *)res)->var->lb,
                                      ((jl_unionall_t *)res)->var->ub))
                    res = jl_instantiate_unionall((jl_unionall_t *)res,
                                                  ((jl_unionall_t *)res)->var->lb);
            }
            JL_CATCH {
                res = jl_bottom_type;
            }
...
static jl_value_t *intersect_unionall_(jl_value_t *t, jl_unionall_t *u, jl_stenv_t *e,
                                       int8_t R, int param, jl_varbinding_t *vb)
{
    jl_varbinding_t *btemp = e->vars;
    // if the var for this unionall (based on identity) already appears somewhere
    // in the environment, rename to get a fresh var.
...
static jl_value_t *intersect_unionall(jl_value_t *t, jl_unionall_t *u, jl_stenv_t *e,
                                      int8_t R, int param)
{
    ...
    if (res != jl_bottom_type) {
        if (vb.concrete || vb.occurs_inv > 1 || u->var->lb != jl_bottom_type ||
            (vb.occurs_inv && vb.occurs_cov)) {
                                   ...
                                   int param)
{
    ...
    while (1) {
        ...
        if (xi && jl_is_vararg_type(xi))
            vx = 1;
        if (yi && jl_is_vararg_type(yi))
            vy = 1;
        if (xi == NULL || yi == NULL) {
            res = jl_bottom_type;
        ...
        if (vx && vy) {
            // {A^n...,Vararg{T,N}} ∩ {Vararg{S,M}} = {(A∩S)^n...,Vararg{T∩S,N}} plus N =
            // M-n
        ...
        if (ii == jl_bottom_type) {
            if (vx && vy) {
                ...
                if ((xb && jl_is_long(xb->lb) && lx - 1 + jl_unbox_long(xb->lb) != len) ||
                    ...
                    if (xb)
                        set_var_to_const(xb, jl_box_long(len - lx + 1), yb);
                    if (yb)
                        set_var_to_const(yb, jl_box_long(len - ly + 1), xb);
...
static jl_value_t *intersect_sub_datatype(jl_datatype_t *xd, jl_datatype_t *yd,
                                          jl_stenv_t *e, int R, int param)
{
    jl_value_t *isuper =
        R ? intersect((jl_value_t *)yd, (jl_value_t *)xd->super, e, param) :
    ...
    super_pattern = jl_rewrap_unionall(
        (jl_value_t *)((jl_datatype_t *)jl_unwrap_unionall(wrapper))->super, wrapper);
...
static jl_value_t *intersect_type_type(jl_value_t *x, jl_value_t *y, jl_stenv_t *e,
                                       int8_t R)
{
    ...
    return x;
    /*
    jl_value_t *ii = R ? intersect_invariant(y, jl_tparam0(x), e) :
    intersect_invariant(jl_tparam0(x), y, e);
    // NOTE: we cannot express e.g. DataType ∩ (UnionAll T<:Integer Type{T}), so returning
    `x`
...
static jl_value_t *intersect(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, int param)
{
        ...
        if (jl_is_typevar(y)) {
            ...
            if (param == 2) {
                ...
                if (!jl_is_type(ylb) && !jl_is_typevar(ylb)) {
                    ...
                    if ((xlb == jl_bottom_type && xub == (jl_value_t *)jl_any_type) ||
                        jl_egal(xlb, ylb))
                        return ylb;
                    return jl_bottom_type;
                }
                ...
                if (var_occurs_inside(xub, (jl_tvar_t *)y, 0, 0) &&
                    ...
                    // we need to avoid computing intersect(xub, yub) since it won't
                    // terminate.
                    return y;
                }
    ...
    if (!jl_has_free_typevars(x) && !jl_has_free_typevars(y) && !jl_is_vararg_type(x) &&
        !jl_is_vararg_type(y)) {
        if (jl_subtype(x, y))
            return x;
        if (jl_subtype(y, x))
    ...
    if (jl_is_datatype(x) && jl_is_datatype(y)) {
        ...
        if (xd->name == yd->name) {
            ...
            if (jl_is_vararg_type(x)) {
                // Vararg: covariant in first parameter, invariant in second
                jl_value_t *xp1 = jl_tparam0(xd), *xp2 = jl_tparam1(xd),
                           *yp1 = jl_tparam0(yd), *yp2 = jl_tparam1(yd);
                // in Vararg{T1} <: Vararg{T2}, need to check subtype twice to
                // simulate the possibility of multiple arguments, which is needed
                // to implement the diagonal rule correctly.
                ...
                if (i2 == NULL || i2 == jl_bottom_type ||
                    (jl_is_long(i2) && jl_unbox_long(i2) < 0) ||
                    !((jl_is_typevar(i2) && ((jl_tvar_t *)i2)->lb == jl_bottom_type &&
                       ((jl_tvar_t *)i2)->ub == (jl_value_t *)jl_any_type) ||
                      jl_is_long(i2)))
                    ii = jl_bottom_type;
                else
...
// `a` might have a non-empty intersection with some concrete type b even if !(a<:b) and
// !(b<:a) For example a=`Tuple{Type{<:Vector}}` and b=`Tuple{DataType}`
...
jl_value_t *jl_type_intersection_env_s(jl_value_t *a, jl_value_t *b, jl_svec_t **penv,
                                       int *issubty)
{
    if (issubty)
        *issubty = 0;
    if (obviously_disjoint(a, b, 0)) {
        if (issubty && a == jl_bottom_type)
            *issubty = 1;
        return jl_bottom_type;
    }
    ...
    *ans = jl_bottom_type;
    ...
    if (jl_subtype_env(a, b, env, szb)) {
        ...
        *ans = intersect_all(a, b, &e);
        ...
        if (jl_is_tuple_type(jl_unwrap_unionall(a)) &&
            jl_is_tuple_type(jl_unwrap_unionall(b)) &&
            ...
            if (jl_is_uniontype(ans_unwrapped)) {
                ans_unwrapped = switch_union_tuple(((jl_uniontype_t *)ans_unwrapped)->a,
                                                   ((jl_uniontype_t *)ans_unwrapped)->b);
        ...
        if (env_from_subtype) {
            ...
            // for now, we attempt to compute env by using subtype on the intersection
            // result
...
static jl_value_t *nth_tuple_elt(jl_datatype_t *t JL_PROPAGATES_ROOT,
...
static int tuple_morespecific(jl_datatype_t *cdt, jl_datatype_t *pdt, int invariant,
                              jl_typeenv_t *env)
{
    ...
    while (1) {
        ...
        if (cva && pva && i >= clen - 1 && i >= plen - 1 &&
            (some_morespecific || (cdiag && !pdiag)))
            return 1;
        ...
        if (!cms && !sub_msp(ce, pe, env)) {
            /*
              A bound vararg tuple can be more specific despite disjoint elements in order
              to preserve transitivity. For example in A = Tuple{Array{T,N}, Vararg{Int,N}}
              where {T,N} B = Tuple{Array, Int} C = Tuple{AbstractArray, Int, Array} we need
              A < B < C and A < C.
            */
            return some_morespecific && cva && ckind == JL_VARARG_BOUND &&
                   num_occurs((jl_tvar_t *)jl_tparam1(jl_unwrap_unionall(clast)), env) > 1;
        }
...
static int args_morespecific_fix1(jl_value_t *a, jl_value_t *b, int swap, jl_typeenv_t *env)
{
    ...
    jl_value_t *e[2] = {jl_tparam1(jl_unwrap_unionall(jl_tparam(a, n - 1))),
                        jl_box_long(taillen)};
                              ...
                              jl_typeenv_t *env)
{
    ...
    if (!invariant) {
        if ((jl_datatype_t *)a == jl_any_type)
            return 0;
        if ((jl_datatype_t *)b == jl_any_type && !jl_is_typevar(a))
            return 1;
    }
    ...
    if (jl_is_uniontype(a)) {
        ...
        if (type_morespecific_(u->a, b, invariant, env) ||
            type_morespecific_(u->b, b, invariant, env)) {
            if (jl_is_uniontype(b)) {
                jl_uniontype_t *v = (jl_uniontype_t *)b;
                if (type_morespecific_(v->a, a, invariant, env) ||
                    type_morespecific_(v->b, a, invariant, env))
                    return 0;
            }
            return 1;
        }
    ...
    if (jl_is_type_type(a) && !invariant) {
        ...
        else if (b == (jl_value_t *)jl_datatype_type ||
                 b == (jl_value_t *)jl_unionall_type ||
            ...
            if (tta->name == ttb->name) {
                ...
                for (size_t i = 0; i < jl_nparams(tta); i++) {
                    ...
                    if (type_morespecific_(apara, bpara, 1, env) &&
                        (jl_is_typevar(apara) || !afree || bfree))
                        ascore += 1;
                    else if (type_morespecific_(bpara, apara, 1, env) &&
                             (jl_is_typevar(bpara) || !bfree || afree))
                        bscore += 1;
                    ...
                    if (jl_is_typevar(bpara) && !jl_is_typevar(apara) && !jl_is_type(apara))
                        ascore1 = 1;
                    else if (jl_is_typevar(apara) && !jl_is_typevar(bpara) &&
                             !jl_is_type(bpara))
                        bscore1 = 1;
    ...
    if (jl_is_typevar(a)) {
        ...
        if (invariant) {
            ...
            if (b == (jl_value_t *)jl_any_type &&
                ((jl_tvar_t *)a)->ub == (jl_value_t *)jl_any_type &&
