...
JL_DLLEXPORT jl_module_t *jl_new_module(jl_sym_t *name)
{
    ...
    jl_module_t *m = (jl_module_t *)jl_gc_alloc(ptls, sizeof(jl_module_t), jl_module_type);
    JL_GC_PUSH1(&m);
    ...
    m->uuid = uuid_zero;
    static unsigned int
        mcounter; // simple counter backup, in case hrtime is not incrementing
...
jl_binding_t *_jl_get_module_binding(jl_module_t *m JL_PROPAGATES_ROOT,
                                     jl_sym_t *var) JL_NOTSAFEPOINT;
jl_binding_t **_jl_get_module_binding_bp(jl_module_t *m JL_PROPAGATES_ROOT,
                                         jl_sym_t *var) JL_NOTSAFEPOINT;
#else
static inline jl_binding_t *_jl_get_module_binding(jl_module_t *m JL_PROPAGATES_ROOT,
                                                   jl_sym_t *var) JL_NOTSAFEPOINT
{
    return (jl_binding_t *)ptrhash_get(&m->bindings, var);
}
static inline jl_binding_t **_jl_get_module_binding_bp(jl_module_t *m JL_PROPAGATES_ROOT,
                                                       jl_sym_t *var) JL_NOTSAFEPOINT
{
        ...
        if (b->owner != m) {
            if (b->owner == NULL) {
                ...
                if (b2 == NULL || b2->value == NULL)
                    jl_errorf(
                        "invalid method definition: imported function %s.%s does not exist",
                        jl_symbol_name(b->owner->name), jl_symbol_name(var));
                // TODO: we might want to require explicitly importing types to add
                // constructors
                if (!b->imported && !jl_is_type(b2->value)) {
                    jl_errorf(
                        "error in method definition: function %s.%s must be explicitly imported to be extended",
                        jl_symbol_name(b->owner->name), jl_symbol_name(var));
                }
                return b2;
            }
...
static jl_binding_t *jl_get_binding_(jl_module_t *m JL_PROPAGATES_ROOT, jl_sym_t *var,
                                     modstack_t *st);

// find a binding from a module's `usings` list
// called while holding m->lock
static jl_binding_t *using_resolve_binding(jl_module_t *m JL_PROPAGATES_ROOT, jl_sym_t *var,
                                           modstack_t *st, int warn)
{
    jl_binding_t *b = NULL;
            ...
            if (owner != NULL && tempb->owner != b->owner && !tempb->deprecated &&
                ...
                if (warn) {
                    ...
                    jl_printf(
                        ...
                        jl_symbol_name(owner->name), jl_symbol_name(imp->name),
...
static int eq_bindings(jl_binding_t *a, jl_binding_t *b)
{
    ...
    if (a->name == b->name && a->owner == b->owner)
        return 1;
    if (a->constp && a->value && b->constp && b->value == a->value)
        return 1;
    return 0;
}
...
static void module_import_(jl_module_t *to, jl_module_t *from, jl_sym_t *s, int explici)
{
    ...
    if (b == NULL) {
        jl_printf(JL_STDERR, "WARNING: could not import %s.%s into %s\n",
                  jl_symbol_name(from->name), jl_symbol_name(s), jl_symbol_name(to->name));
    }
    else {
        if (b->deprecated) {
        ...
        if (table[i] != HT_NOTFOUND) {
            ...
            if (b->exportp && (b->owner == from || b->imported)) {
                ...
                if (*tobp != HT_NOTFOUND && (*tobp)->owner != NULL &&
                    ...
                    // see issue #4715
                    var != to->name && !eq_bindings(jl_get_binding(to, var), b)) {
...
JL_DLLEXPORT jl_binding_t *jl_get_module_binding(jl_module_t *m JL_PROPAGATES_ROOT,
                                                 jl_sym_t *var) JL_NOTSAFEPOINT
{
    JL_LOCK_NOGC(&m->lock);
    jl_binding_t *b = _jl_get_module_binding(m, var);
                                ...
                                jl_value_t *val JL_ROOTED_ARGUMENT)
{
    ...
    // In a release build, simply ignore conflicting assignments (for backwards
    // compatibility). However, we want to start asserting that they do not occur, since
    // that can cause `val` not to be rooted when the caller expected it to be.
    assert(!bp->constp);
    if (!bp->constp) {
        bp->value = val;
        jl_gc_wb(m, val);
...
jl_binding_t *jl_get_dep_message_binding(jl_module_t *m, jl_binding_t *deprecated_binding)
{
    ...
    memcpy(dep_binding_name + prefix_len, jl_symbol_name(deprecated_binding->name),
           name_len);
...
void jl_binding_deprecation_warning(jl_module_t *m, jl_binding_t *b)
{
    ...
    if (b->deprecated == 1 && jl_options.depwarn) {
        ...
        if (b->owner) {
            jl_printf(JL_STDERR, "%s.%s is deprecated", jl_symbol_name(b->owner->name),
        ...
        if (jl_options.depwarn == JL_OPTIONS_DEPWARN_ERROR) {
            if (b->owner)
                jl_errorf("deprecated binding: %s.%s", jl_symbol_name(b->owner->name),
                          jl_symbol_name(b->name));
            else
...
JL_DLLEXPORT jl_sym_t *jl_module_name(jl_module_t *m)
{
    return m->name;
}
JL_DLLEXPORT jl_module_t *jl_module_parent(jl_module_t *m)
{
    return m->parent;
}
JL_DLLEXPORT uint64_t jl_module_build_id(jl_module_t *m)
{
    return m->build_id;
}
JL_DLLEXPORT jl_uuid_t jl_module_uuid(jl_module_t *m)
{
    return m->uuid;
}

// TODO: make this part of the module constructor and read-only?
JL_DLLEXPORT void jl_set_module_uuid(jl_module_t *m, jl_uuid_t uuid)
{
    m->uuid = uuid;
}
