...
#define ADD_I(name, nargs)                                                       \
    do {                                                                         \
        Function *func =                                                         \
            Function::Create(FunctionType::get(T_prjlvalue, args##nargs, false), \
...
static Constant *julia_const_to_llvm(jl_codectx_t &ctx, const void *ptr, jl_datatype_t *bt)
{
    ...
    if (lt->isFloatingPointTy() || lt->isIntegerTy()) {
        ...
        if (lt->isFloatingPointTy()) {
            return ConstantFP::get(jl_LLVMContext, APFloat(lt->getFltSemantics(), val));
        }
...
static Value *emit_unbox(jl_codectx_t &ctx, Type *to, const jl_cgval_t &x, jl_value_t *jt,
                         Value *dest, MDNode *tbaa_dest, bool isVolatile)
{
    assert(to != T_void);
    ...
    if (!x.ispointer() || c) { // already unboxed, but sometimes need conversion
        ...
        tbaa_decorate(tbaa_dest,
                      ctx.builder.CreateAlignedStore(unboxed, dest, julia_alignment(jt)));
        return NULL;
    }
    ...
    if (jt == (jl_value_t *)jl_bool_type || to == T_int1) {
        Instruction *unbox_load = tbaa_decorate(
            x.tbaa, ctx.builder.CreateLoad(T_int8, maybe_bitcast(ctx, p, T_pint8)));
        if (jt == (jl_value_t *)jl_bool_type)
            unbox_load->setMetadata(
    ...
    if (dest) {
        emit_memcpy(ctx, dest, tbaa_dest, p, x.tbaa, jl_datatype_size(jt), alignment,
                    false);
        return NULL;
    }
    else {
        if (p->getType() != ptype && isa<AllocaInst>(p)) {
            ...
            if (!AI->isArrayAllocation() &&
                (AllocType->isFloatingPointTy() || AllocType->isIntegerTy() ||
                 AllocType->isPointerTy()) &&
...
// put a bits type tag on some value (despite the name, this doesn't necessarily actually
// change anything about the value however)
static jl_cgval_t generic_bitcast(jl_codectx_t &ctx, const jl_cgval_t *argv)
{
    ...
    if (v.ispointer() && vx == NULL) {
        ...
        if (isboxed)
            vxt = llvmt;
        vx = tbaa_decorate(v.tbaa, ctx.builder.CreateLoad(emit_bitcast(
                                       ctx, data_pointer(ctx, v),
...
static jl_cgval_t generic_cast(jl_codectx_t &ctx, intrinsic f, Instruction::CastOps Op,
                               const jl_cgval_t *argv, bool toint, bool fromint)
{
    ...
    if (Op == Instruction::FPExt) {
        ...
        from = ctx.builder.CreateLoad(jlfloattemp_var,
                                      /*force this to load from the stack*/ true);
#endif
    }
...
static jl_cgval_t emit_pointerref(jl_codectx_t &ctx, jl_cgval_t *argv)
{
    ...
    if (ety == (jl_value_t *)jl_any_type) {
        ...
        return mark_julia_type(
            ctx,
            ctx.builder.CreateAlignedLoad(ctx.builder.CreateGEP(T_prjlvalue, thePtr, im1),
                                          align_nb),
            true, ety);
    }
        ...
        Value *strct = emit_allocobj(ctx, size, literal_pointer_val(ctx, ety));
        im1 = ctx.builder.CreateMul(
            im1, ConstantInt::get(T_size, LLT_ALIGN(size, jl_datatype_align(ety))));
...
static jl_cgval_t emit_pointerset(jl_codectx_t &ctx, jl_cgval_t *argv)
{
    ...
    if (ety == (jl_value_t *)jl_any_type) {
        ...
        Instruction *store = ctx.builder.CreateAlignedStore(
            ctx.builder.CreatePtrToInt(emit_pointer_from_objref(ctx, boxed(ctx, x)),
                                       T_size),
        ...
        im1 = ctx.builder.CreateMul(
            im1, ConstantInt::get(T_size, LLT_ALIGN(size, jl_datatype_align(ety))));
        emit_memcpy(ctx, ctx.builder.CreateGEP(T_int8, thePtr, im1), nullptr, x, size,
                    align_nb);
    }
    else {
        bool isboxed;
...
{
    Type *t = den->getType();
    raise_exception_unless(ctx, ctx.builder.CreateICmpNE(den, ConstantInt::get(t, 0)),
...
struct math_builder {
    ...
    math_builder(jl_codectx_t &ctx, bool always_fast = false, bool contract = false)
      : ctxbuilder(ctx.builder), old_fmf(ctxbuilder.getFastMathFlags())
    {
        FastMathFlags fmf;
    ...
    ~math_builder() { ctxbuilder.setFastMathFlags(old_fmf); }
};

static Value *emit_untyped_intrinsic(jl_codectx_t &ctx, intrinsic f, Value **argvalues,
                                     size_t nargs, jl_datatype_t **newtyp,
                                     jl_value_t *xtyp);


static jl_cgval_t emit_ifelse(jl_codectx_t &ctx, jl_cgval_t c, jl_cgval_t x, jl_cgval_t y,
                              jl_value_t *rt_hint)
{
    ...
    if (!isboxed) {
        if (type_is_ghost(llt1))
            return x;
        ifelse_result = ctx.builder.CreateSelect(isfalse, emit_unbox(ctx, llt1, y, t1),
        ...
        if (x_tindex || y_tindex) {
            ...
            if (!x_ptr && !y_ptr) { // both ghost
                ...
                if (ifelse_tbaa == NULL) {
                    // LLVM won't return a TBAA result for the root, but mark_julia_struct
                    // requires it: make it now
                    auto *OffsetNode =
                        ConstantAsMetadata::get(ConstantInt::get(T_int64, 0));
            ...
            if (!x_tindex && x.constant) {
                x_tindex = ConstantInt::get(
                    T_int8,
                    0x80 | get_box_tindex((jl_datatype_t *)jl_typeof(x.constant), rt_hint));
            }
            if (!y_tindex && y.constant) {
                y_tindex = ConstantInt::get(
                    T_int8,
                    0x80 | get_box_tindex((jl_datatype_t *)jl_typeof(y.constant), rt_hint));
            }
            if (x_tindex && y_tindex) {
                ...
                BasicBlock *compute =
                    BasicBlock::Create(jl_LLVMContext, "compute_tindex", ctx.f);
            ...
            if (x_vboxed || y_vboxed) {
                if (!x_vboxed)
                    x_vboxed =
                        ConstantPointerNull::get(cast<PointerType>(y_vboxed->getType()));
                if (!y_vboxed)
                    y_vboxed =
                        ConstantPointerNull::get(cast<PointerType>(x_vboxed->getType()));
                ret.Vboxed = ctx.builder.CreateSelect(isfalse, y_vboxed, x_vboxed);
            }
            return ret;
        }
        ifelse_result = ctx.builder.CreateSelect(isfalse, boxed(ctx, y), boxed(ctx, x));
    }
    jl_value_t *jt = (t1 == t2 ? t1 : rt_hint);
    return mark_julia_type(ctx, ifelse_result, isboxed, jt);
}

static jl_cgval_t emit_intrinsic(jl_codectx_t &ctx, intrinsic f, jl_value_t **args,
                                 size_t nargs)
{
    ...
    if (expected_nargs && expected_nargs != nargs) {
        jl_errorf("intrinsic #%d %s: wrong number of arguments", f,
    ...
    case pointerref: return emit_pointerref(ctx, argv);
    case pointerset: return emit_pointerset(ctx, argv);
    case bitcast: return generic_bitcast(ctx, argv);
    case trunc_int: return generic_cast(ctx, f, Instruction::Trunc, argv, true, true);
    case sext_int: return generic_cast(ctx, f, Instruction::SExt, argv, true, true);
    case zext_int: return generic_cast(ctx, f, Instruction::ZExt, argv, true, true);
    case uitofp: return generic_cast(ctx, f, Instruction::UIToFP, argv, false, true);
    case sitofp: return generic_cast(ctx, f, Instruction::SIToFP, argv, false, true);
    case fptoui: return generic_cast(ctx, f, Instruction::FPToUI, argv, true, false);
    case fptosi: return generic_cast(ctx, f, Instruction::FPToSI, argv, true, false);
    case fptrunc: return generic_cast(ctx, f, Instruction::FPTrunc, argv, false, false);
    case fpext: return generic_cast(ctx, f, Instruction::FPExt, argv, false, false);
    ...
    default: {
        ...
        Value *r = emit_untyped_intrinsic(ctx, f, argvalues, nargs,
                                          (jl_datatype_t **)&newtyp, xinfo.typ);
...
static Value *emit_untyped_intrinsic(jl_codectx_t &ctx, intrinsic f, Value **argvalues,
                                     size_t nargs, jl_datatype_t **newtyp, jl_value_t *xtyp)
{
    ...
    switch (f) {
    case neg_int: return ctx.builder.CreateNeg(x);
    ...
    case add_ptr: {
        return ctx.builder.CreatePtrToInt(
            ctx.builder.CreateGEP(T_int8, ctx.builder.CreateIntToPtr(x, T_pint8), y), t);
    }

    case sub_ptr: {
        return ctx.builder.CreatePtrToInt(
            ctx.builder.CreateGEP(T_int8, ctx.builder.CreateIntToPtr(x, T_pint8),
                                  ctx.builder.CreateNeg(y)),
            t);
    }
    ...
    case fma_float: {
        ...
        FunctionCallee fmaintr =
            Intrinsic::getDeclaration(jl_Module, Intrinsic::fma, makeArrayRef(t));
                 ...
                 (f == checked_uadd_int ?
                      ...
                      (f == checked_ssub_int ?
                           ...
                           (f == checked_usub_int ?
                                ...
                                (f == checked_smul_int ? Intrinsic::smul_with_overflow :
    ...
    case checked_sdiv_int: {
        Value *typemin =
            ctx.builder.CreateShl(ConstantInt::get(t, 1), t->getPrimitiveSizeInBits() - 1);
    ...
    case checked_udiv_int:
        raise_exception_unless(ctx, ctx.builder.CreateICmpNE(y, ConstantInt::get(t, 0)),
                               literal_pointer_val(ctx, jl_diverror_exception));
        return ctx.builder.CreateUDiv(x, y);

    case checked_srem_int: return emit_checked_srem_int(ctx, x, y);
    ...
    case fpislt: {
        *newtyp = jl_bool_type;
        ...
        return ctx.builder.CreateOr(
            ctx.builder.CreateAnd(ctx.builder.CreateFCmpORD(x, x),
                                  ctx.builder.CreateFCmpUNO(y, y)),
            ctx.builder.CreateAnd(
    ...
    case shl_int:
        return ctx.builder.CreateSelect(
            ...
            ConstantInt::get(t, 0), ctx.builder.CreateShl(x, uint_cnvt(ctx, t, y)));
    case lshr_int:
        return ctx.builder.CreateSelect(
            ctx.builder.CreateICmpUGE(y, ConstantInt::get(y->getType(),
                                                          t->getPrimitiveSizeInBits())),
            ConstantInt::get(t, 0), ctx.builder.CreateLShr(x, uint_cnvt(ctx, t, y)));
    case ashr_int:
        return ctx.builder.CreateSelect(
            ctx.builder.CreateICmpUGE(y, ConstantInt::get(y->getType(),
                                                          t->getPrimitiveSizeInBits())),
    ...
    case bswap_int: {
        FunctionCallee bswapintr =
            Intrinsic::getDeclaration(jl_Module, Intrinsic::bswap, makeArrayRef(t));
        return ctx.builder.CreateCall(bswapintr, x);
    }
    case ctpop_int: {
        FunctionCallee ctpopintr =
            Intrinsic::getDeclaration(jl_Module, Intrinsic::ctpop, makeArrayRef(t));
        return ctx.builder.CreateCall(ctpopintr, x);
    }
    case ctlz_int: {
        FunctionCallee ctlz =
            Intrinsic::getDeclaration(jl_Module, Intrinsic::ctlz, makeArrayRef(t));
        y = ConstantInt::get(T_int1, 0);
        return ctx.builder.CreateCall(ctlz, {x, y});
    }
    case cttz_int: {
        FunctionCallee cttz =
            Intrinsic::getDeclaration(jl_Module, Intrinsic::cttz, makeArrayRef(t));
        y = ConstantInt::get(T_int1, 0);
        return ctx.builder.CreateCall(cttz, {x, y});
    }

    case abs_float: {
        FunctionCallee absintr =
            Intrinsic::getDeclaration(jl_Module, Intrinsic::fabs, makeArrayRef(t));
    ...
    case flipsign_int: {
        ...
        if (cy) {
            APInt iy = cy->getValue();
            return iy.isNonNegative() ? x :
                                        ctx.builder.CreateSub(ConstantInt::get(t, 0), x);
        }
        Value *tmp = ctx.builder.CreateAShr(
            y, ConstantInt::get(t, cast<IntegerType>(t)->getBitWidth() - 1));
        return ctx.builder.CreateXor(ctx.builder.CreateAdd(x, tmp), tmp);
    }
    case ceil_llvm: {
        FunctionCallee ceilintr =
            Intrinsic::getDeclaration(jl_Module, Intrinsic::ceil, makeArrayRef(t));
        return ctx.builder.CreateCall(ceilintr, x);
    }
    case floor_llvm: {
        FunctionCallee floorintr =
            Intrinsic::getDeclaration(jl_Module, Intrinsic::floor, makeArrayRef(t));
        return ctx.builder.CreateCall(floorintr, x);
    }
    case trunc_llvm: {
        FunctionCallee truncintr =
            Intrinsic::getDeclaration(jl_Module, Intrinsic::trunc, makeArrayRef(t));
        return ctx.builder.CreateCall(truncintr, x);
    }
    case rint_llvm: {
        FunctionCallee rintintr =
            Intrinsic::getDeclaration(jl_Module, Intrinsic::rint, makeArrayRef(t));
        return ctx.builder.CreateCall(rintintr, x);
    }
    case sqrt_llvm: {
        FunctionCallee sqrtintr =
            Intrinsic::getDeclaration(jl_Module, Intrinsic::sqrt, makeArrayRef(t));
    ...
    default: assert(0 && "invalid intrinsic"); abort();
    }
    assert(0 && "unreachable");
}

#define BOX_F(ct, jl_ct, rt) \
    box_##ct##_func = boxfunc_llvm(ft1arg(rt, T_##jl_ct), "jl_box_" #ct, &jl_box_##ct, m);

#define SBOX_F(ct, jl_ct)          \
    BOX_F(ct, jl_ct, T_prjlvalue); \
    box_##ct##_func->addAttribute(1, Attribute::SExt);
#define UBOX_F(ct, jl_ct)          \
    BOX_F(ct, jl_ct, T_prjlvalue); \
    box_##ct##_func->addAttribute(1, Attribute::ZExt);
#define SBOX_F_PERM(ct, jl_ct)    \
    BOX_F(ct, jl_ct, T_pjlvalue); \
    box_##ct##_func->addAttribute(1, Attribute::SExt);
#define UBOX_F_PERM(ct, jl_ct)    \
    BOX_F(ct, jl_ct, T_pjlvalue); \
    box_##ct##_func->addAttribute(1, Attribute::ZExt);

template<typename T>
static Function *boxfunc_llvm(FunctionType *ft, const std::string &cname, T *addr,
                              Module *m)
{
