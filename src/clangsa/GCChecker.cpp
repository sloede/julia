...
class GCChecker
  : public Checker<eval::Call, check::BeginFunction, check::EndFunction, check::PostCall,
                   check::PreCall, check::PostStmt<CStyleCastExpr>,
                   check::PostStmt<ArraySubscriptExpr>, check::PostStmt<MemberExpr>,
                   check::PostStmt<UnaryOperator>, check::Bind, check::Location> {
    ...
    void report_value_error(CheckerContext &C, SymbolRef Sym, const char *message,
...
public:
    struct ValueState {
        ...
        static ValueState getForArgument(const FunctionDecl *FD, const ParmVarDecl *PVD)
        {
...
private:
    ...
    bool processArgumentRooting(const CallEvent &Call, CheckerContext &C,
                                ProgramStateRef &State) const;
    bool rootRegionIfGlobal(const MemRegion *R, ProgramStateRef &, CheckerContext &C,
                            ValueState *ValS = nullptr) const;
    static const ValueState *getValStateForRegion(ASTContext &AstC,
                                                  const ProgramStateRef &State,
                                                  const MemRegion *R, bool Debug = false);
...
public:
    ...
    void checkDerivingExpr(const Expr *Result, const Expr *Parent, bool ParentIsLoc,
    ...
    public:
        ...
        PDP ExplainNoPropagationFromExpr(const clang::Expr *FromWhere,
                                         const ExplodedNode *N, PathDiagnosticLocation Pos,
                                         BugReporterContext &BRC, BugReport &BR);
                                                               ...
                                                               BugReport &BR)
{
    ...
    SymbolRef Parent =
        walkToRoot([&](SymbolRef Sym, const ValueState *OldVState) { return !OldVState; },
                   N->getState(), Region);
    if (!Parent && Region) {
        Parent = walkToRoot([&](SymbolRef Sym,
    ...
    if (!Parent) {
        ...
        if (VR) {
            ...
            return MakePDP(Pos, "Argument value was derived from global, but the checker "
    ...
    if (ValS->isPotentiallyFreed()) {
        ...
        return MakePDP(Pos,
                       "Root not propagated because it may have been freed. Tracking.");
    }
...
PDP GCChecker::GCValueBugVisitor::ExplainNoPropagation(const ExplodedNode *N,
                                                       PathDiagnosticLocation Pos,
                                                       BugReporterContext &BRC,
                                                       BugReport &BR)
{
                                            ...
                                            BugReporterContext &BRC, BugReport &BR)
{
    ...
    PathDiagnosticLocation Pos;
    if (Stmt)
        Pos = PathDiagnosticLocation{Stmt, BRC.getSourceManager(), N->getLocationContext()};
    else
    ...
    if (!OldValueState) {
        if (NewValueState->isRooted()) {
            ...
            if (NewValueState->FD) {
                ...
                (void)maybeUnrooted;
                Pos = PathDiagnosticLocation{NewValueState->PVD, BRC.getSourceManager()};
    ...
    else if (NewValueState->isPotentiallyFreed() && OldValueState->isJustAllocated()) {
        // std::make_shared< in later LLVM
        return MakePDP(Pos, "Value may have been GCed here.");
    }
    else if (NewValueState->isPotentiallyFreed() && !OldValueState->isPotentiallyFreed()) {
        // std::make_shared< in later LLVM
...
void GCChecker::report_value_error(CheckerContext &C, SymbolRef Sym, const char *message,
...
bool GCChecker::propagateArgumentRootedness(CheckerContext &C, ProgramStateRef &State) const
{
    ...
    for (const auto P : FD->parameters()) {
        ...
        SymbolRef ArgSym = walkToRoot(
            [](SymbolRef Sym, const ValueState *OldVState) { return !OldVState; }, State,
            Arg.getAsRegion());
        if (!ArgSym) {
            continue;
        }
        ...
        if (!ValS) {
            report_error(
                [&](BugReport *Report) {
                    Report->addNote("Tried to find root for this parameter in inlined call",
    ...
    for (const auto P : FD->parameters()) {
        if (declHasAnnotation(P, "julia_require_rooted_slot")) {
            ...
            if (!AssignedSym)
                continue;
            assert(AssignedSym);
            State = State->set<GCValueMap>(AssignedSym, ValueState::getForArgument(FD, P));
            Change = true;
        }
    }
    if (Change) {
...
{
    return isJuliaType([](StringRef Name) { return Name.endswith_lower("jl_sym_t"); }, QT);
}
...
bool GCChecker::processPotentialSafepoint(const CallEvent &Call, CheckerContext &C,
                                          ProgramStateRef &State) const
{
    if (!isSafepoint(Call))
        return false;
    ...
    if (FD) {
        for (unsigned i = 0; i < FD->getNumParams(); ++i) {
            ...
            if (declHasAnnotation(FD->getParamDecl(i), "julia_temporarily_roots")) {
                ...
                SpeciallyRootedSymbol = walkToRoot(
...
bool GCChecker::processAllocationOfResult(const CallEvent &Call, CheckerContext &C,
                                          ProgramStateRef &State) const
{
...
SymbolRef GCChecker::getSymbolForResult(const Expr *Result, const ValueState *OldValS,
                                        ProgramStateRef &State, CheckerContext &C) const
{
    ...
    if (Loaded.isUnknown() || !Loaded.getAsSymbol()) {
        ...
        if (OldValS || GCChecker::isGCTracked(Result)) {
            Loaded = C.getSValBuilder().conjureSymbolVal(
                nullptr, Result, C.getLocationContext(), Result->getType(), C.blockCount());
...
void GCChecker::checkDerivingExpr(const Expr *Result, const Expr *Parent, bool ParentIsLoc,
                                  CheckerContext &C) const
{
    ...
    if (!isGCTracked(Result)) {
        ...
        bool ParentIsModule =
            isJuliaType([](StringRef Name) { return Name.endswith_lower("jl_module_t"); },
                        Parent->getType());
        bool ResultIsArrayList =
            isJuliaType([](StringRef Name) { return Name.endswith_lower("arraylist_t"); },
                        Result->getType());
    ...
    if (ResultVal.isUnknown()) {
        ...
        ResultVal = C.getSValBuilder().conjureSymbolVal(Result, C.getLocationContext(),
...
void GCChecker::checkPreCall(const CallEvent &Call, CheckerContext &C) const
{
    ...
    for (unsigned idx = 0; idx < NumArgs; ++idx) {
        ...
        if (!MaybeUnrooted && isCalleeSafepoint) {
            report_value_error(
                C, Sym, "Passing non-rooted value as argument to function that may GC",
                range);
        }
    }
}
                         ...
                         CheckerContext &C) const
{
    ...
    if (name == "JL_GC_POP") {
    ...
    else if (name == "JL_GC_PUSH1" || name == "JL_GC_PUSH2" || name == "JL_GC_PUSH3" ||
             name == "JL_GC_PUSH4" || name == "JL_GC_PUSH5" || name == "JL_GC_PUSH6") {
        ProgramStateRef State = C.getState();
        ...
        for (unsigned i = 0; i < NumArgs; ++i) {
            ...
            if (!ValState->isRooted()) {
                State = State->set<GCValueMap>(Sym,
                                               ValueState::getRooted(Region, CurrentDepth));
            }
        }
        CurrentDepth += 1;
        ...
        if (FD) {
            ...
            if (Items.isUnknown()) {
                Items = C.getSValBuilder().conjureSymbolVal(CE, C.getLocationContext(),
                                                            FD->getType(), C.blockCount());
    ...
    if (IsLoad && (RS = State->get<GCRootMap>(SLoc.getAsRegion()))) {
        ...
        if (LoadedSym) {
            ...
            if (!ValS || !ValS->isRooted() || ValS->RootDepth > RS->RootedAtDepth) {
                DidChange = true;
                State = State->set<GCValueMap>(LoadedSym,
                                               ValueState::getRooted(SLoc.getAsRegion(),
                                                                     RS->RootedAtDepth));
            }
        }
    }
...
{
    registry.addChecker<GCChecker>("julia.GCChecker", "Validates julia gc invariants"
#if LLVM_VERSION_MAJOR >= 8
                                   ,
