...
JL_DLLEXPORT jl_taggedvalue_t *
_jl_astaggedvalue(jl_value_t *v JL_PROPAGATES_ROOT) JL_NOTSAFEPOINT;
...
#else
#define jl_astaggedvalue(v) ((jl_taggedvalue_t *)((char *)(v) - sizeof(jl_taggedvalue_t)))
#define jl_valueof(v) ((jl_value_t *)((char *)(v) + sizeof(jl_taggedvalue_t)))
#define jl_typeof(v) ((jl_value_t *)(jl_astaggedvalue(v)->header & ~(uintptr_t)15))
#endif
...
// which split the tree when possible, for example based on the key into the tuple type at
// `offs` when key is a leaftype, (but only when the tree has enough entries for this to be
...
typedef jl_value_t *(jl_call_t)(jl_value_t *, jl_value_t **, uint32_t,
                                struct _jl_code_instance_t *);
...
typedef struct _jl_code_info_t {
    ...
    // miscellaneous data:
    jl_value_t
        *method_for_inference_limit_heuristics; // optional method used during inference
    jl_value_t *linetable; // Table of locations [TODO: make this volatile like slotnames]
    jl_array_t *slotnames; // names of local variables
    jl_array_t *slotflags; // local var bit flags
    // the following are optional transient properties (not preserved by compression--as
    // they typically get stored elsewhere):
...
typedef struct _jl_method_t {
    ...
    // list of potentially-ambiguous methods (nothing = none, Vector{Any} of TypeMapEntry
    // otherwise)
    jl_value_t *ambig;
    ...
    jl_value_t *source; // original code template (jl_code_info_t, but may be compressed),
                        // null for builtins
    struct _jl_method_instance_t
        *unspecialized; // unspecialized executable method instance, or null
    ...
    jl_svec_t
        *ccallable; // svec(rettype, sig) if a ccallable entry point is requested for this
...
// This type is a placeholder to cache data for a specType signature specialization of a
// Method can can be used as a unique dictionary key representation of a call to a
// particular Method with a particular set of argument types
struct _jl_method_instance_t {
    JL_DATA_TYPE
    union {
    ...
    jl_value_t *uninferred; // cached uncompressed code, for generated functions, top-level
                            // thunks, or the interpreter
    jl_array_t *backedges; // list of method-instances which contain a call into this
                           // method-instance
...
typedef struct _jl_code_instance_t {
    ...
    uint8_t
        isspecsig; // if specptr is a specialized function signature for specTypes->rettype
...
// in little-endian, isptr is always the first bit, avoiding the need for a branch in
// computing isptr
typedef struct {
    uint8_t isptr : 1;
    uint8_t size : 7;
...
typedef struct _jl_typemap_entry_t {
    ...
    int8_t
        issimplesig; // all(isleaftype | isAny | isType | isVararg, sig) : sorted and fast
...
#define JL_GC_PUSH4(arg1, arg2, arg3, arg4)                                 \
    void *__gc_stkf[] = {                                                   \
        (void *)JL_GC_ENCODE_PUSH(4), jl_pgcstack, arg1, arg2, arg3, arg4}; \
    jl_pgcstack = (jl_gcframe_t *)__gc_stkf;

#define JL_GC_PUSH5(arg1, arg2, arg3, arg4, arg5)                                 \
    void *__gc_stkf[] = {                                                         \
        (void *)JL_GC_ENCODE_PUSH(5), jl_pgcstack, arg1, arg2, arg3, arg4, arg5}; \
    jl_pgcstack = (jl_gcframe_t *)__gc_stkf;

#define JL_GC_PUSH6(arg1, arg2, arg3, arg4, arg5, arg6)                                 \
    void *__gc_stkf[] = {                                                               \
        (void *)JL_GC_ENCODE_PUSH(6), jl_pgcstack, arg1, arg2, arg3, arg4, arg5, arg6}; \
    jl_pgcstack = (jl_gcframe_t *)__gc_stkf;
...
JL_DLLEXPORT void jl_gc_queue_multiroot(jl_value_t *root,
                                        jl_value_t *stored) JL_NOTSAFEPOINT;

STATIC_INLINE void jl_gc_wb(void *parent, void *ptr) JL_NOTSAFEPOINT
{
    // parent and ptr isa jl_value_t*
    if (__unlikely(jl_astaggedvalue(parent)->bits.gc ==
                       3 && // parent is old and not in remset
...
STATIC_INLINE jl_value_t *jl_svecset(void *t JL_ROOTING_ARGUMENT JL_PROPAGATES_ROOT,
...
#define jl_array_data_owner_offset(ndims) \
    (offsetof(jl_array_t, ncols) +        \
     sizeof(size_t) * (1 + jl_array_ndimwords(ndims))) // in bytes
#define jl_array_data_owner(a) \
    (*((jl_value_t **)((char *)a + jl_array_data_owner_offset(jl_array_ndims(a)))))
...
STATIC_INLINE jl_value_t *jl_array_ptr_ref(void *a JL_PROPAGATES_ROOT,
                                           size_t i) JL_NOTSAFEPOINT;
STATIC_INLINE jl_value_t *jl_array_ptr_set(void *a JL_ROOTING_ARGUMENT, size_t i,
...
JL_DLLEXPORT jl_svec_t *jl_compute_fieldtypes(jl_datatype_t *st JL_PROPAGATES_ROOT,
                                              void *stack);
#define jl_get_fieldtypes(st) \
    ((st)->types ? (st)->types : jl_compute_fieldtypes((st), NULL))
...
STATIC_INLINE jl_value_t *jl_field_type_concrete(jl_datatype_t *st JL_PROPAGATES_ROOT,
                                                 size_t i) JL_NOTSAFEPOINT
{
    assert(st->types);
...
#define DEFINE_FIELD_ACCESSORS(f)                                                 \
    static inline uint32_t jl_field_##f(jl_datatype_t *st, int i) JL_NOTSAFEPOINT \
    {                                                                             \
...
static inline int jl_field_isptr(jl_datatype_t *st, int i) JL_NOTSAFEPOINT
{
    const jl_datatype_layout_t *ly = st->layout;
    assert(i >= 0 && (size_t)i < ly->nfields);
    return ((const jl_fielddesc8_t *)(jl_dt_layout_fields(ly) +
                                      jl_fielddesc_size(ly->fielddesc_type) * i))
        ->isptr;
}
...
#define jl_is_namedtuple(v) \
    (((jl_datatype_t *)jl_typeof(v))->name == jl_namedtuple_typename)
...
#define jl_array_isbitsunion(a) \
    (!(((jl_array_t *)(a))->flags.ptrarray) && jl_is_uniontype(jl_tparam0(jl_typeof(a))))
...
{
    return (jl_is_datatype(v) && jl_is_immutable(v) && ((jl_datatype_t *)(v))->layout &&
            jl_datatype_nfields(v) == 0 && jl_datatype_size(v) > 0);
}
...
{
    return (jl_is_datatype(t) && ((jl_datatype_t *)(t))->name == jl_array_typename);
}
...
{
    return (jl_is_datatype(t) && ((jl_datatype_t *)(t))->name == jl_namedtuple_typename);
}

STATIC_INLINE int jl_is_vecelement_type(jl_value_t *t) JL_NOTSAFEPOINT
{
    return (jl_is_datatype(t) && ((jl_datatype_t *)(t))->name == jl_vecelement_typename);
}
...
// object identity
JL_DLLEXPORT int jl_egal(jl_value_t *a JL_MAYBE_UNROOTED,
                         jl_value_t *b JL_MAYBE_UNROOTED) JL_NOTSAFEPOINT;
...
JL_DLLEXPORT jl_datatype_t *jl_new_datatype(jl_sym_t *name, jl_module_t *module,
...
JL_DLLEXPORT jl_value_t *jl_generic_function_def(jl_sym_t *name, jl_module_t *module,
...
STATIC_INLINE int jl_is_vararg_type(jl_value_t *v) JL_NOTSAFEPOINT
{
    ...
    return (jl_is_datatype(v) && ((jl_datatype_t *)(v))->name == jl_vararg_typename);
}
...
JL_DLLEXPORT jl_value_t *jl_get_nth_field_noalloc(jl_value_t *v JL_PROPAGATES_ROOT,
                                                  size_t i) JL_NOTSAFEPOINT;
...
JL_DLLEXPORT int jl_islayout_inline(jl_value_t *eltype, size_t *fsz,
                                    size_t *al) JL_NOTSAFEPOINT;
...
JL_DLLEXPORT jl_array_t *jl_reshape_array(jl_value_t *atype, jl_array_t *data,
                                          jl_value_t *dims);
JL_DLLEXPORT jl_array_t *jl_ptr_to_array_1d(jl_value_t *atype, void *data, size_t nel,
                                            int own_buffer);
JL_DLLEXPORT jl_array_t *jl_ptr_to_array(jl_value_t *atype, void *data, jl_value_t *dims,
                                         int own_buffer);
...
JL_DLLEXPORT jl_array_t *jl_alloc_array_3d(jl_value_t *atype, size_t nr, size_t nc,
                                           size_t z);
...
JL_DLLEXPORT jl_value_t *jl_ptrarrayref(jl_array_t *a JL_PROPAGATES_ROOT,
                                        size_t i) JL_NOTSAFEPOINT; // 0-indexed
JL_DLLEXPORT void jl_arrayset(jl_array_t *a JL_ROOTING_ARGUMENT,
...
// eq hash tables
JL_DLLEXPORT jl_array_t *jl_eqtable_put(jl_array_t *h, jl_value_t *key, jl_value_t *val,
                                        int *inserted);
...
JL_DLLEXPORT jl_value_t *jl_vexceptionf(jl_datatype_t *exception_type, const char *fmt,
                                        va_list args);
JL_DLLEXPORT void JL_NORETURN jl_error(const char *str);
JL_DLLEXPORT void JL_NORETURN jl_errorf(const char *fmt, ...);
JL_DLLEXPORT void JL_NORETURN jl_exceptionf(jl_datatype_t *ty, const char *fmt, ...);
...
JL_DLLEXPORT void JL_NORETURN jl_bounds_error_int(jl_value_t *v JL_MAYBE_UNROOTED,
                                                  size_t i);
JL_DLLEXPORT void JL_NORETURN jl_bounds_error_tuple_int(jl_value_t **v, size_t nv,
                                                        size_t i);
JL_DLLEXPORT void JL_NORETURN jl_bounds_error_unboxed_int(void *v, jl_value_t *vt,
                                                          size_t i);
...
#define JL_NARGS(fname, min, max)     \
    if (nargs < min)                  \
        jl_too_few_args(#fname, min); \
    else if (nargs > max)             \
        jl_too_many_args(#fname, max);
...
// front end interface
JL_DLLEXPORT jl_value_t *jl_parse_all(const char *str, size_t len, const char *filename,
                                      size_t filename_len);
...
JL_DLLEXPORT jl_value_t *jl_load_file_string(const char *text, size_t len, char *filename,
...
JL_DLLEXPORT int jl_dlsym(jl_uv_libhandle handle, const char *symbol, void **value,
                          int throw_err);
...
JL_DLLEXPORT jl_value_t *jl_call3(jl_function_t *f, jl_value_t *a, jl_value_t *b,
                                  jl_value_t *c);
...
__declspec(noreturn)
    __attribute__((__nothrow__)) void(jl_longjmp)(jmp_buf _Buf, int _Value);
#else
...
#define JL_CATCH                                            \
    else for (i__ca = 1, jl_eh_restore_state(&__eh); i__ca; \
              i__ca = 0, jl_restore_excstack(__excstack_state))
...
#ifdef __GNUC__
#define _JL_FORMAT_ATTR(type, str, arg) __attribute__((format(type, str, arg)))
#else
...
#define JULIA_DEFINE_FAST_TLS()                                                           \
    ...
    {                                                                                     \
        static                                                                            \
            __attribute__((tls_model("local-exec"))) __thread jl_tls_states_t tls_states; \
        return &tls_states;                                                               \
    }                                                                                     \
