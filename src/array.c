...
char *jl_array_typetagdata(jl_array_t *a) JL_NOTSAFEPOINT
{
    ...
    return ((char *)jl_array_data(a)) +
           ((jl_array_ndims(a) == 1 ? (a->maxsize - a->offset) : jl_array_len(a)) *
            a->elsize) +
           a->offset;
}
                               ...
                               int isunboxed, int hasptr, int isunion, int elsz)
{
    ...
    if (tot <= ARRAY_INLINE_NBYTES) {
        ...
        if (tot > 0 && (!isunboxed || hasptr || isunion)) // TODO: check for zeroinit
            // need to zero out isbits union array selector bytes to ensure a valid type
            // index
            memset(data, 0, tot);
    }
...
static inline jl_array_t *_new_array(jl_value_t *atype, uint32_t ndims, size_t *dims)
{
    ...
    int hasptr = isunboxed && (jl_is_datatype(eltype) &&
                               ((jl_datatype_t *)eltype)->layout->npointers > 0);
    if (!isunboxed) {
        elsz = sizeof(void *);
        al = elsz;
    }
                                          ...
                                          jl_value_t *_dims)
{
    ...
    int ndimwords = jl_array_ndimwords(ndims);
    int tsz =
        JL_ARRAY_ALIGN(sizeof(jl_array_t) + ndimwords * sizeof(size_t) + sizeof(void *),
                                            ...
                                            int own_buffer)
{
    ...
    if (((uintptr_t)data) & ((align > JL_HEAP_ALIGNMENT ? JL_HEAP_ALIGNMENT : align) - 1))
        jl_exceptionf(jl_argumenterror_type,
                      "unsafe_wrap: pointer %p is not properly aligned to %u bytes", data,
                      align);
    ...
    a->elsize = LLT_ALIGN(elsz, align);
    a->flags.ptrarray = !isunboxed;
    a->flags.hasptr = isunboxed && (jl_is_datatype(eltype) &&
                                    ((jl_datatype_t *)eltype)->layout->npointers > 0);
    a->flags.ndims = 1;
    a->flags.isshared = 1;
...
JL_DLLEXPORT jl_array_t *jl_ptr_to_array(jl_value_t *atype, void *data, jl_value_t *_dims,
                                         int own_buffer)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    size_t nel = 1;
...
JL_DLLEXPORT jl_value_t *jl_array_to_string(jl_array_t *a)
{
    ...
    if (a->flags.how == 3 && a->offset == 0 && a->elsize == 1 &&
        (jl_array_ndims(a) != 1 || ((a->maxsize + sizeof(void *) + 1 <= GC_MAX_SZCLASS) ==
                                    (len + sizeof(void *) + 1 <= GC_MAX_SZCLASS)))) {
...
JL_DLLEXPORT jl_value_t *jl_pchar_to_string(const char *str, size_t len)
{
    ...
    jl_value_t *s =
        jl_gc_alloc_(jl_get_ptls_states(), sz, jl_string_type); // force inlining
    *(size_t *)s = len;
...
JL_DLLEXPORT jl_value_t *jl_arrayref(jl_array_t *a, size_t i)
{
    ...
    if (jl_is_uniontype(eltype)) {
        // isbits union selector bytes are always stored directly after the last array
        // element
    ...
    return undefref_check((jl_datatype_t *)eltype,
...
static int NOINLINE array_resize_buffer(jl_array_t *a, size_t newlen)
{
    ...
    if (a->flags.how == 2) {
        ...
        a->data = jl_gc_managed_realloc(olddata, nbytes, oldnbytes, a->flags.isaligned,
                                        (jl_value_t *)a);
    }
...
STATIC_INLINE void jl_array_grow_at_beg(jl_array_t *a, size_t idx, size_t inc, size_t n)
{
    ...
    if (a->offset >= inc) {
        ...
        if (inc > (a->maxsize - n) / 2 - (a->maxsize - n) / 20) {
            ...
            if (isbitsunion)
                newtypetagdata = newdata + (a->maxsize - a->offset) * elsz + a->offset;
            if (idx > 0 && newdata < data) {
                memmove(newdata, data, nb1);
                if (isbitsunion) {
                    memmove(newtypetagdata, typetagdata, idx);
...
STATIC_INLINE void jl_array_grow_at_end(jl_array_t *a, size_t idx, size_t inc, size_t n)
{
    ...
    if (__unlikely(reqmaxsize > a->maxsize)) {
        ...
        if (newbuf) {
            ...
            if (isbitsunion) {
                ...
                if (has_gap)
                    memcpy(newtypetagdata + idx + inc, typetagdata + idx, n - idx);
                memset(newtypetagdata + idx, 0, inc);
            }
            if (has_gap)
...
STATIC_INLINE void jl_array_del_at_beg(jl_array_t *a, size_t idx, size_t dec, size_t n)
{
    // no error checking
    ...
    if (__unlikely(newoffs != offset) || idx > 0) {
        ...
        if (newoffs != offset) {
            ...
            if (isbitsunion)
                memmove(newtypetagdata + idx, typetagdata + idx + dec, n - idx);
        }
        a->data = newdata;
    }
    else {
...
STATIC_INLINE void jl_array_del_at_end(jl_array_t *a, size_t idx, size_t dec, size_t n)
{
    // no error checking
    // assume inbounds, assume unshared
    char *data = (char *)a->data;
...
JL_DLLEXPORT jl_array_t *jl_array_copy(jl_array_t *ary)
{
    ...
    jl_array_t *new_ary =
        _new_array_(jl_typeof(ary), jl_array_ndims(ary), &ary->nrows, !ary->flags.ptrarray,
...
static NOINLINE ssize_t jl_array_ptr_copy_forward(jl_value_t *owner, void **src_p,
                                                  void **dest_p, ssize_t n)
{
...
static NOINLINE ssize_t jl_array_ptr_copy_backward(jl_value_t *owner, void **src_p,
                                                   void **dest_p, ssize_t n)
{
    for (ssize_t i = 0; i < n; i++) {
...
JL_DLLEXPORT void jl_array_ptr_copy(jl_array_t *dest, void **dest_p, jl_array_t *src,
                                    void **src_p, ssize_t n)
{
