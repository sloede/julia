...
static jl_value_t *resolve_globals(jl_value_t *expr, jl_module_t *module,
                                   ...
                                   int eager_resolve)
{
    if (jl_is_symbol(expr)) {
        ...
        if (jl_is_toplevel_only_expr(expr) || e->head == const_sym ||
            ...
            e->head == quote_sym || e->head == inert_sym || e->head == meta_sym ||
            e->head == inbounds_sym || e->head == boundscheck_sym ||
            e->head == loopinfo_sym || e->head == aliasscope_sym ||
            ...
            if (e->head == cfunction_sym) {
                ...
                if (!jl_is_type(rt)) {
                    ...
                    JL_CATCH {
                        if (jl_typeis(jl_current_exception(), jl_errorexception_type))
                            jl_error(
                                "could not evaluate cfunction return type (it might depend on a local variable)");
                        else
                            jl_rethrow();
                    }
                ...
                if (!jl_is_svec(at)) {
                    ...
                    JL_CATCH {
                        if (jl_typeis(jl_current_exception(), jl_errorexception_type))
                            jl_error(
                                "could not evaluate cfunction argument type (it might depend on a local variable)");
                        else
                            jl_rethrow();
                    }
                ...
                JL_TYPECHK(cfunction method definition, symbol,
                           *(jl_value_t **)jl_exprarg(e, 4));
                return expr;
            }
            if (e->head == foreigncall_sym) {
                ...
                if (!jl_is_type(rt)) {
                    ...
                    JL_CATCH {
                        if (jl_typeis(jl_current_exception(), jl_errorexception_type))
                            jl_error(
                                "could not evaluate ccall return type (it might depend on a local variable)");
                        else
                            jl_rethrow();
                    }
                ...
                JL_TYPECHK(ccall method definition, symbol,
                           *(jl_value_t **)jl_exprarg(e, 4));
                jl_exprargset(e, 0,
                              resolve_globals(jl_exprarg(e, 0), module, sparam_vals,
                                              binding_effects, 1));
                i++;
            }
            ...
            for (; i < nargs; i++) {
                ...
                jl_exprargset(e, i,
                              resolve_globals(jl_exprarg(e, i), module, sparam_vals,
                                              binding_effects, eager_resolve));
            }
            if (e->head == call_sym && jl_expr_nargs(e) == 3 &&
                jl_is_globalref(jl_exprarg(e, 0)) && jl_is_globalref(jl_exprarg(e, 1)) &&
                jl_is_quotenode(jl_exprarg(e, 2))) {
                // replace module_expr.sym with GlobalRef(module, sym)
                // for expressions pattern-matching to `getproperty(module_expr, :sym)` in a
                // top-module (this is expected to help inference performance)
                ...
                if (fe_mod->istopmod && !strcmp(jl_symbol_name(fe_sym), "getproperty") &&
                    jl_is_symbol(s)) {
                    if (eager_resolve || jl_binding_resolved_p(me_mod, me_sym)) {
                        ...
                        if (b && b->constp && b->value && jl_is_module(b->value)) {
                            return jl_module_globalref((jl_module_t *)b->value,
                                                       (jl_sym_t *)s);
                        }
                    }
                }
            }
            if (e->head == call_sym && nargs > 0 && jl_is_globalref(jl_exprarg(e, 0))) {
                ...
                if (jl_binding_resolved_p(fe_mod, fe_sym)) {
                    ...
                    if (b && b->constp && b->value == jl_builtin_tuple) {
                        ...
                        if (j == nargs) {
                            jl_value_t *val = NULL;
                            JL_TRY {
                                val = jl_interpret_toplevel_expr_in(module, (jl_value_t *)e,
                                                                    NULL, sparam_vals);
                            }
                            JL_CATCH {
                            }
                              ...
                              int binding_effects)
{
    ...
    for (i = 0; i < l; i++) {
        ...
        jl_array_ptr_set(stmts, i,
                         resolve_globals(stmt, m, sparam_vals, binding_effects, 0));
    }
}
...
static void jl_code_info_set_ir(jl_code_info_t *li, jl_expr_t *ir)
{
    ...
    size_t j;
    for (j = 0; j < nlocs; j++) {
        jl_arrayset((jl_array_t *)li->codelocs,
                    jl_box_int32(jl_unbox_long(jl_arrayref((jl_array_t *)codelocs, j))), j);
    }
    ...
    for (i = 0; i < nslots; i++) {
        ...
        jl_array_uint8_set(li->slotflags, i,
                           vinfo_mask & jl_unbox_long(jl_array_ptr_ref(vi, 2)));
    }
}
...
JL_DLLEXPORT jl_method_instance_t *jl_new_method_instance_uninit(void)
{
    ...
    jl_method_instance_t *li = (jl_method_instance_t *)jl_gc_alloc(
        ptls, sizeof(jl_method_instance_t), jl_method_instance_type);
    li->def.value = NULL;
    li->specTypes = NULL;
...
JL_DLLEXPORT jl_code_info_t *jl_new_code_info_uninit(void)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_code_info_t *src =
        (jl_code_info_t *)jl_gc_alloc(ptls, sizeof(jl_code_info_t), jl_code_info_type);
    src->code = NULL;
...
void jl_linenumber_to_lineinfo(jl_code_info_t *ci, jl_value_t *name)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (jl_is_linenode(ln)) {
            ...
            rt = jl_new_struct(jl_lineinfonode_type, name, jl_linenode_file(ln), rt,
                               jl_box_long(0));
            jl_array_ptr_set(li, i, rt);
        }
        else if (jl_is_expr(ln) && ((jl_expr_t *)ln)->head == line_sym &&
                 jl_expr_nargs(ln) == 3) {
...
STATIC_INLINE jl_value_t *jl_call_staged(jl_method_t *def, jl_value_t *generator,
                                         jl_svec_t *sparam_vals, jl_value_t **args,
                                         uint32_t nargs)
{
    ...
    if (def->isva) {
        gargs[totargs - 1] = jl_f_tuple(NULL, &gargs[1 + n_sparams + def->nargs - 1],
                                        nargs - (def->nargs - 1));
...
// Like `jl_expand`, if there is an error expanding the provided expression, the return
// value will be an error expression (an `Expr` with `error_sym` as its head), which should
// be eval'd in the caller's context.
...
JL_DLLEXPORT jl_code_info_t *jl_code_for_staged(jl_method_instance_t *linfo)
{
    ...
    JL_TRY {
        ...
        ex = jl_call_staged(def, generator, linfo->sparam_vals,
                            jl_svec_data(ttdt->parameters), jl_nparams(ttdt));
    ...
    for (i = 0; i < n; i++) {
        ...
        if (jl_is_expr(st) && ((jl_expr_t *)st)->head == meta_sym) {
            ...
            if (nargs >= 1 && jl_exprarg(st, 0) == (jl_value_t *)nospecialize_sym) {
                if (nargs == 1) // bare `@nospecialize` is special: it prevents
                                // specialization on all args
                    m->nospecialize = -1;
                size_t j;
                for (j = 1; j < nargs; j++) {
                ...
                if (nargs == 1) // bare `@specialize` is special: it causes specialization
                                // on all args
                    m->nospecialize = 0;
                st = jl_nothing;
            }
                ...
                if (jl_expr_nargs(gexpr) == 7) {
                    // expects (new (core GeneratedFunctionStub) funcname argnames sp line
                    // file expandearly)
...
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_method_t *m = (jl_method_t *)jl_gc_alloc(ptls, sizeof(jl_method_t), jl_method_type);
                                                 ...
                                                 jl_binding_t *bnd)
{
    ...
    if (*bp == NULL) {
        ...
        *bp = gf;
        if (bp_owner)
            jl_gc_wb(bp_owner, gf);
    }
    return gf;
}

static jl_methtable_t *first_methtable(jl_value_t *a JL_PROPAGATES_ROOT,
                                       int got_tuple1) JL_NOTSAFEPOINT
{
    if (jl_is_datatype(a)) {
        if (got_tuple1) {
...
JL_DLLEXPORT jl_methtable_t *
jl_argument_method_table(jl_value_t *argt JL_PROPAGATES_ROOT) JL_NOTSAFEPOINT
{
...
JL_DLLEXPORT void jl_method_def(jl_svec_t *argdata, jl_code_info_t *f, jl_module_t *module)
{
    ...
    if (jl_has_free_typevars(argtype)) {
        jl_exceptionf(jl_argumenterror_type,
                      ...
                      jl_symbol_name(name), jl_symbol_name(m->file), m->line);
    }
        ...
        if (!jl_is_type(elt) && !jl_is_typevar(elt)) {
            ...
            if (argname == unused_sym)
                jl_exceptionf(
                    ...
                    i, jl_symbol_name(name), jl_symbol_name(m->file), m->line);
            else
                jl_exceptionf(
                    jl_argumenterror_type,
                    "invalid type for argument %s in method definition for %s at %s:%d",
                    jl_symbol_name(argname), jl_symbol_name(name), jl_symbol_name(m->file),
                    m->line);
        }
