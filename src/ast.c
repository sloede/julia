...
jl_sym_t *quote_sym;
jl_sym_t *line_sym;
jl_sym_t *jl_incomplete_sym;
jl_sym_t *goto_sym;
jl_sym_t *goto_ifnot_sym;
jl_sym_t *return_sym;
...
jl_sym_t *inbounds_sym;
jl_sym_t *copyast_sym;
jl_sym_t *cfunction_sym;
jl_sym_t *pure_sym;
jl_sym_t *loopinfo_sym;
jl_sym_t *meta_sym;
jl_sym_t *inert_sym;
jl_sym_t *polly_sym;
jl_sym_t *unused_sym;
jl_sym_t *static_parameter_sym;
jl_sym_t *inline_sym;
jl_sym_t *noinline_sym;
jl_sym_t *generated_sym;
jl_sym_t *generated_only_sym;
jl_sym_t *isdefined_sym;
jl_sym_t *propagate_inbounds_sym;
jl_sym_t *specialize_sym;
jl_sym_t *nospecialize_sym;
jl_sym_t *macrocall_sym;
jl_sym_t *colon_sym;
jl_sym_t *hygienicscope_sym;
jl_sym_t *throw_undef_if_not_sym;
jl_sym_t *getfield_undefref_sym;
jl_sym_t *gc_preserve_begin_sym;
jl_sym_t *gc_preserve_end_sym;
jl_sym_t *coverageeffect_sym;
jl_sym_t *escape_sym;
jl_sym_t *aliasscope_sym;
jl_sym_t *popaliasscope_sym;
jl_sym_t *optlevel_sym;
jl_sym_t *thismodule_sym;
...
#endif
#define jl_ast_context_list_item(node) container_of(node, jl_ast_context_t, list)
...
#define JL_AST_PRESERVE_PUSH(ctx, old, inmodule) \
    ...
    ctx->module = (inmodule)
#define JL_AST_PRESERVE_POP(ctx, old) ctx->module = (old)
...
static jl_value_t *jl_expand_macros(jl_value_t *expr, jl_module_t *inmodule,
...
value_t fl_julia_logmsg(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    if (nargs < 6 || kwargs_len % 2 != 0) {
        lerror(fl_ctx, fl_ctx->ArgError,
               "julia-logmsg: bad argument list - expected "
    ...
    if (!isfixnum(arg_level) || !issymbol(arg_group) || !issymbol(arg_id) ||
        ...
        lerror(fl_ctx, fl_ctx->ArgError, "julia-logmsg: Unexpected type in argument list");
    }
...
JL_DLLEXPORT void jl_lisp_prompt(void)
{
    ...
    fl_applyn(fl_ctx, 1, symbol_value(symbol(fl_ctx, "__start")),
              fl_cons(fl_ctx, fl_ctx->NIL, fl_ctx->NIL));
...
static jl_sym_t *scmsym_to_julia(fl_context_t *fl_ctx, value_t s)
{
    ...
    if (fl_isgensym(fl_ctx, s)) {
        char gsname[16];
        char *n = uint2str(&gsname[1], sizeof(gsname) - 1, ((gensym_t *)ptr(s))->id, 10);
        *(--n) = '#';
        return jl_symbol(n);
    }
...
static jl_value_t *scm_to_julia_(fl_context_t *fl_ctx, value_t e, jl_module_t *mod)
{
    if (fl_isnumber(fl_ctx, e)) {
        ...
        if (isfixnum(e)) {
            ...
            switch (nt) {
            case T_DOUBLE: return (jl_value_t *)jl_box_float64(*(double *)cp_data(cp));
            case T_FLOAT: return (jl_value_t *)jl_box_float32(*(float *)cp_data(cp));
            case T_UINT8: return (jl_value_t *)jl_box_uint8(*(uint8_t *)cp_data(cp));
            case T_UINT16: return (jl_value_t *)jl_box_uint16(*(uint16_t *)cp_data(cp));
            case T_UINT32: return (jl_value_t *)jl_box_uint32(*(uint32_t *)cp_data(cp));
            case T_UINT64: return (jl_value_t *)jl_box_uint64(*(uint64_t *)cp_data(cp));
            default:;
            }
    ...
    if (iscons(e) || e == fl_ctx->NIL) {
        ...
        if (sym == goto_sym) {
            ...
            temp = (jl_value_t *)mod;
                 ...
                 (sym == inert_sym || (sym == quote_sym && (!iscons(car_(e)))))) {
            ex = scm_to_julia_(fl_ctx, car_(e), mod);
            ...
            jl_linenumber_to_lineinfo((jl_code_info_t *)ex,
                                      (jl_value_t *)jl_symbol("top-level scope"));
        ...
        for (i = 0; i < n; i++) {
            assert(iscons(e));
            jl_array_ptr_set(((jl_expr_t *)ex)->args, i,
                             scm_to_julia_(fl_ctx, car_(e), mod));
            e = cdr_(e);
        }
        if (sym == lambda_sym)
    ...
    if (iscprim(e) && cp_class((cprim_t *)ptr(e)) == fl_ctx->wchartype) {
        ...
        if (u < 0x80) {
            ...
            c = u < 0x00000800 ?
                    (c << 16) | 0xc0800000 :
                    u < 0x00010000 ? (c << 8) | 0xe0808000 : (c << 0) | 0xf0808080;
        }
...
static value_t julia_to_scm(fl_context_t *fl_ctx, jl_value_t *v)
{
    ...
    FL_TRY_EXTERN(fl_ctx) { temp = julia_to_scm_(fl_ctx, v); }
    FL_CATCH_EXTERN(fl_ctx) { temp = fl_ctx->lasterror; }
    return temp;
}
...
static int julia_to_scm_noalloc1(fl_context_t *fl_ctx, jl_value_t *v,
                                 value_t *retval) JL_NOTSAFEPOINT
{
    if (v == NULL)
...
static value_t julia_to_scm_noalloc2(fl_context_t *fl_ctx, jl_value_t *v) JL_NOTSAFEPOINT
{
    ...
    if (jl_is_ssavalue(v))
        lerror(fl_ctx, symbol(fl_ctx, "error"),
               "SSAValue objects should not occur in an AST");
    if (jl_is_slot(v))
...
static value_t julia_to_scm_noalloc(fl_context_t *fl_ctx, jl_value_t *v) JL_NOTSAFEPOINT
{
    ...
    if (julia_to_scm_noalloc1(fl_ctx, v, &retval))
        return retval;
    assert(!jl_is_expr(v) && !jl_typeis(v, jl_linenumbernode_type) &&
           !jl_typeis(v, jl_gotonode_type) && !jl_typeis(v, jl_quotenode_type) &&
           !jl_typeis(v, jl_newvarnode_type) && !jl_typeis(v, jl_globalref_type));
    return julia_to_scm_noalloc2(fl_ctx, v);
}

static value_t julia_to_list2_noalloc(fl_context_t *fl_ctx, jl_value_t *a,
                                      jl_value_t *b) JL_NOTSAFEPOINT
{
...
static value_t julia_to_scm_(fl_context_t *fl_ctx, jl_value_t *v)
{
    ...
    if (jl_is_expr(v)) {
        ...
        if (ex->head == lambda_sym && jl_expr_nargs(ex) > 0 &&
            jl_is_array(jl_exprarg(ex, 0))) {
    ...
    if (jl_typeis(v, jl_globalref_type)) {
        ...
        if (m == jl_core_module)
            return julia_to_list2(fl_ctx, (jl_value_t *)core_sym, (jl_value_t *)sym);
                                      ...
                                      size_t filename_len)
{
    ...
    value_t e =
        fl_applyn(fl_ctx, 2, symbol_value(symbol(fl_ctx, "jl-parse-all")), s, files);
...
JL_DLLEXPORT jl_value_t *jl_parse_string(const char *str, size_t len, int pos0, int greedy)
{
    ...
    fl_context_t *fl_ctx = &ctx->fl;
    value_t s = cvalue_static_cstrn(fl_ctx, str, len);
    value_t p = fl_applyn(fl_ctx, 3, symbol_value(symbol(fl_ctx, "jl-parse-one")), s,
                          fixnum(pos0), greedy ? fl_ctx->T : fl_ctx->F);
                              ...
                              jl_module_t *inmodule, jl_value_t *mapexpr)
{
    ...
    JL_TRY {
        ...
        while (iscons(ast)) {
            expression = car_(ast);
            {
                JL_TIMING(LOWERING);
                if (fl_ctx->T ==
                    fl_applyn(fl_ctx, 1, symbol_value(symbol(fl_ctx, "contains-macrocall")),
                              expression)) {
    ...
    if (err) {
        if (jl_loaderror_type == NULL)
            ...
            jl_rethrow_other(
                jl_new_struct(jl_loaderror_type, form, result, jl_current_exception()));
    }
    JL_GC_POP();
    return result;
}
...
JL_DLLEXPORT int jl_is_operator(char *sym)
{
    jl_ast_context_t *ctx = jl_ast_ctx_enter();
    fl_context_t *fl_ctx = &ctx->fl;
    int res = fl_applyn(fl_ctx, 1, symbol_value(symbol(fl_ctx, "operator?")),
                        symbol(fl_ctx, sym)) == fl_ctx->T;
    jl_ast_ctx_leave(ctx);
    return res;
}
...
JL_DLLEXPORT int jl_is_unary_operator(char *sym)
{
    jl_ast_context_t *ctx = jl_ast_ctx_enter();
    fl_context_t *fl_ctx = &ctx->fl;
    int res = fl_applyn(fl_ctx, 1, symbol_value(symbol(fl_ctx, "unary-op?")),
                        symbol(fl_ctx, sym)) == fl_ctx->T;
    jl_ast_ctx_leave(ctx);
    return res;
}
...
JL_DLLEXPORT int jl_is_unary_and_binary_operator(char *sym)
{
    jl_ast_context_t *ctx = jl_ast_ctx_enter();
    fl_context_t *fl_ctx = &ctx->fl;
    int res = fl_applyn(fl_ctx, 1, symbol_value(symbol(fl_ctx, "unary-and-binary-op?")),
                        symbol(fl_ctx, sym)) == fl_ctx->T;
    jl_ast_ctx_leave(ctx);
    return res;
}
...
JL_DLLEXPORT int jl_operator_precedence(char *sym)
{
    ...
    fl_context_t *fl_ctx = &ctx->fl;
    int res =
        numval(fl_applyn(fl_ctx, 1, symbol_value(symbol(fl_ctx, "operator-precedence")),
                         symbol(fl_ctx, sym)));
    jl_ast_ctx_leave(ctx);
    return res;
}
                                    ...
                                    struct macroctx_stack *macroctx, int onelevel)
{
    if (!expr || !jl_is_expr(expr))
        return expr;
    jl_expr_t *e = (jl_expr_t *)expr;
    if (e->head == inert_sym || e->head == module_sym ||
        // e->head == toplevel_sym || // TODO: enable this once julia-expand-macroscope is
        // fixed / removed
        e->head == meta_sym) {
        return expr;
    }
    ...
    if (e->head == macrocall_sym) {
        ...
        if (!onelevel)
            result =
                jl_expand_macros(result, inmodule, wrap ? &newctx : macroctx, onelevel);
        if (wrap) {
                                                 ...
                                                 const char *file, int line)
{
    ...
    value_t arg = julia_to_scm(fl_ctx, expr);
    value_t e =
        fl_applyn(fl_ctx, 4, symbol_value(symbol(fl_ctx, "jl-expand-to-thunk-warn")), arg,
                                                 ...
                                                 const char *file, int line)
{
    ...
    expr = jl_expand_macros(expr, inmodule, NULL, 0);
    expr =
        jl_call_scm_on_ast_and_loc("jl-expand-to-thunk-stmt", expr, inmodule, file, line);
    JL_GC_POP();
    return expr;
}
