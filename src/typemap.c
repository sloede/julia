...
static int jl_is_any(jl_value_t *t1)
{
    return (t1 == (jl_value_t *)jl_any_type ||
            (jl_is_typevar(t1) && ((jl_tvar_t *)t1)->ub == (jl_value_t *)jl_any_type));
}
...
static int sig_match_by_type_simple(jl_value_t **types, size_t n, jl_tupletype_t *sig,
                                    size_t lensig, int va)
{
    size_t i;
    if (va)
        lensig -= 1;
    for (i = 0; i < lensig; i++) {
        ...
        if (jl_is_type_type(unw)) {
            ...
            else if (!jl_is_kind(a) || !jl_is_typevar(tp0) ||
                     ((jl_tvar_t *)tp0)->ub != (jl_value_t *)jl_any_type) {
...
static inline int sig_match_leaf(jl_value_t *arg1, jl_value_t **args, jl_value_t **sig,
                                 size_t n)
{
...
static inline int sig_match_simple(jl_value_t *arg1, jl_value_t **args, size_t n,
                                   jl_value_t **sig, int va, size_t lensig)
{
...
static jl_typemap_t **mtcache_hash_lookup_bp(jl_array_t *cache JL_PROPAGATES_ROOT,
                                             jl_value_t *ty) JL_NOTSAFEPOINT
{
...
static void mtcache_hash_insert(jl_array_t **cache, jl_value_t *parent, jl_value_t *key,
                                jl_typemap_t *val)
{
    int inserted = 0;
    jl_array_t *a = *cache;
...
static jl_typemap_t *mtcache_hash_lookup(jl_array_t *cache JL_PROPAGATES_ROOT,
                                         jl_value_t *ty) JL_NOTSAFEPOINT
{
    if (cache == (jl_array_t *)jl_an_empty_vec_any)
        return NULL;
...
static int jl_typemap_array_visitor(jl_array_t *a, jl_typemap_visitor_fptr fptr,
                                    void *closure)
{
...
static int jl_typemap_node_visitor(jl_typemap_entry_t *ml, jl_typemap_visitor_fptr fptr,
                                   void *closure)
{
...
static int jl_typemap_intersection_node_visitor(jl_typemap_entry_t *ml,
                                                struct typemap_intersection_env *closure)
{
    ...
    while (ml != (void *)jl_nothing) {
        if (closure->type == (jl_value_t *)ml->sig) {
            ...
            closure->ti = jl_type_intersection_env_s(closure->type, (jl_value_t *)ml->sig,
                                                     penv, &closure->issubty);
            if (closure->ti != (jl_value_t *)jl_bottom_type) {
                // In some corner cases type intersection is conservative and returns
                // something for intersect(A, B) even though A is a dispatch tuple and !(A
                // <: B). For dispatch purposes in such a case we know there's no match.
                // This check fixes issue #30394.
                                    ...
                                    struct typemap_intersection_env *closure)
{
    ...
    //    register jl_typemap_intersection_visitor_fptr fptr = closure->fptr;
    //        struct jl_typemap_assoc search = {(jl_value_t*)closure->type, world,
    //        closure->env, 0, ~(size_t)0}; jl_typemap_entry_t *ml =
    //        jl_typemap_assoc_by_type(map, search, offs, /*subtype*/1); if (ml) {
    ...
    if (jl_typeof(map) == (jl_value_t *)jl_typemap_level_type) {
        ...
        if (ty) {
            ...
            if (cache->targ != (jl_array_t *)jl_an_empty_vec_any) {
                ...
                if (typetype) {
                    ...
                    // first, fast-path: optimized pre-intersection test to see if `ty`
                    // could intersect with any Type
                    if (typetype ||
                        !jl_has_empty_intersection((jl_value_t *)jl_type_type, ty))
                        if (!jl_typemap_intersection_array_visitor(cache->targ, ty, 1, offs,
                                                                   closure))
                            return 0;
                }
            }
            if (cache->arg1 != (jl_array_t *)jl_an_empty_vec_any) {
                if (is_cache_leaf(ty)) {
                    ...
                    // else an array scan is required to check subtypes
                    if (!jl_typemap_intersection_array_visitor(cache->arg1, ty, 0, offs,
                                                               closure))
                        return 0;
                }
            }
        ...
        return jl_typemap_intersection_node_visitor((jl_typemap_entry_t *)map, closure);
    }
}
...
*/
static jl_typemap_entry_t *jl_typemap_entry_assoc_by_type(jl_typemap_entry_t *ml,
    ...
    for (; ml != (void *)jl_nothing; ml = ml->next) {
        ...
        if (lensig == n || (ml->va && lensig <= n + 1)) {
            ...
            if (ml->simplesig != (void *)jl_nothing && !isua) {
                ...
                int isva = lensimplesig > 0 &&
                           jl_is_vararg_type(jl_tparam(ml->simplesig, lensimplesig - 1));
                if (lensig == n || (isva && lensimplesig <= n + 1))
                    ismatch = sig_match_by_type_simple(
                        jl_svec_data(((jl_datatype_t *)types)->parameters), n,
            ...
            if (ismatch == 0)
                ...
                ismatch = sig_match_by_type_leaf(
                    jl_svec_data(((jl_datatype_t *)types)->parameters), ml->sig, lensig);
            else if (ml->issimplesig && !typesisva && !isua)
                ismatch = sig_match_by_type_simple(
                    jl_svec_data(((jl_datatype_t *)types)->parameters), n, ml->sig, lensig,
                    ml->va);
            else {
                ismatch = jl_subtype_matching(types, (jl_value_t *)ml->sig,
                                              search->env ? &search->env : NULL);
            ...
            if (ismatch) {
                ...
                if (ismatch) {
                    if (search->world < ml->min_world) {
                        ...
                        // ignore method table entries that have been replaced in the
                        // current world
...
static jl_typemap_entry_t *jl_typemap_entry_lookup_by_type(jl_typemap_entry_t *ml,
                                                           struct jl_typemap_assoc *search)
{
    for (; ml != (void *)jl_nothing; ml = ml->next) {
        if (search->world < ml->min_world || search->world > ml->max_world)
            continue;
        // unroll the first few cases here, to the extent that is possible to do fast and
        // easily
        jl_value_t *types = search->types;
        JL_GC_PROMISE_ROOTED(types);
...
jl_typemap_entry_t *jl_typemap_assoc_by_type(jl_typemap_t *ml_or_cache,
                                             ...
                                             uint8_t subtype)
{
    if (jl_typeof(ml_or_cache) == (jl_value_t *)jl_typemap_level_type) {
        ...
        return subtype ? jl_typemap_entry_assoc_by_type(leaf, search) :
                         jl_typemap_entry_lookup_by_type(leaf, search);
    }
}

jl_typemap_entry_t *jl_typemap_entry_assoc_exact(jl_typemap_entry_t *ml, jl_value_t *arg1,
                                                 jl_value_t **args, size_t n, size_t world)
{
    // some manually-unrolled common special cases
    while (ml->simplesig == (void *)jl_nothing && ml->guardsigs == jl_emptysvec &&
           ml->isleafsig) {
        ...
        if (lensig == n || (ml->va && lensig <= n + 1)) {
            if (ml->simplesig != (void *)jl_nothing) {
                ...
                int isva = lensimplesig > 0 &&
                           jl_is_vararg_type(jl_tparam(ml->simplesig, lensimplesig - 1));
                if (lensig == n || (isva && lensimplesig <= n + 1)) {
                    if (!sig_match_simple(arg1, args, n,
                                          jl_svec_data(ml->simplesig->parameters), isva,
                                          lensimplesig))
                        continue;
                }
                else {
            ...
            if (ml->isleafsig) {
                ...
                if (!sig_match_simple(arg1, args, n, jl_svec_data(ml->sig->parameters),
                                      ml->va, lensig))
                    continue;
            }
            else {
            ...
            if (ml->guardsigs != jl_emptysvec) {
                for (i = 0, l = jl_svec_len(ml->guardsigs); i < l; i++) {
                    ...
                    // expensive subtype check, since guard entries added for @nospecialize
                    // might be abstract. this fixed issue #12967.
                    if (jl_tuple1_isa(arg1, args, n,
                                      (jl_tupletype_t *)jl_svecref(ml->guardsigs, i))) {
                        goto nomatch;
                    }
                }
            }
                                                 ...
                                                 size_t n, int8_t offs, size_t world)
{
    if (n > offs) {
        ...
        if (ty == (jl_value_t *)jl_datatype_type &&
            cache->targ != (jl_array_t *)jl_an_empty_vec_any && is_cache_leaf(a1)) {
            jl_typemap_t *ml_or_cache = mtcache_hash_lookup(cache->targ, a1);
            jl_typemap_entry_t *ml =
                jl_typemap_assoc_exact(ml_or_cache, arg1, args, n, offs + 1, world);
            if (ml)
                return ml;
        }
    ...
    if (cache->linear != (jl_typemap_entry_t *)jl_nothing) {
        jl_typemap_entry_t *ml =
            jl_typemap_entry_assoc_exact(cache->linear, arg1, args, n, world);
        if (ml)
            return ml;
    }
...
static void jl_typemap_level_insert_(jl_typemap_t *map, jl_typemap_level_t *cache,
                                     jl_typemap_entry_t *newrec, int8_t offs,
                                     const struct jl_typemap_info *tparams);
static void jl_typemap_list_insert_sorted(jl_typemap_t *map, jl_typemap_entry_t **pml,
                                          jl_value_t *parent, jl_typemap_entry_t *newrec,
                                          const struct jl_typemap_info *tparams);

static jl_typemap_level_t *jl_new_typemap_level(void)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_typemap_level_t *cache = (jl_typemap_level_t *)jl_gc_alloc(
        ptls, sizeof(jl_typemap_level_t), jl_typemap_level_type);
...
static jl_typemap_level_t *
jl_method_convert_list_to_cache(jl_typemap_t *map, jl_typemap_entry_t *ml, int8_t offs,
                                const struct jl_typemap_info *tparams)
{
...
static void jl_typemap_list_insert_(jl_typemap_t *map, jl_typemap_entry_t **pml,
                                    jl_value_t *parent, jl_typemap_entry_t *newrec,
                                    const struct jl_typemap_info *tparams)
{
...
static void jl_typemap_insert_generic(jl_typemap_t *map, jl_typemap_t **pml,
                                      jl_value_t *parent, jl_typemap_entry_t *newrec,
                                      int8_t offs, const struct jl_typemap_info *tparams)
{
    ...
    if (count > MAX_METHLIST_COUNT) {
        *pml = (jl_typemap_t *)jl_method_convert_list_to_cache(
            map, (jl_typemap_entry_t *)*pml, offs, tparams);
        jl_gc_wb(parent, *pml);
        jl_typemap_level_insert_(map, (jl_typemap_level_t *)*pml, newrec, offs, tparams);
        return;
    }

    jl_typemap_list_insert_(map, (jl_typemap_entry_t **)pml, parent, newrec, tparams);
}

static int jl_typemap_array_insert_(jl_typemap_t *map, jl_array_t **cache, jl_value_t *key,
                                    jl_typemap_entry_t *newrec, jl_value_t *parent,
                                    int8_t tparam, int8_t offs,
                                    const struct jl_typemap_info *tparams)
{
                                     ...
                                     const struct jl_typemap_info *tparams)
{
    ...
    if (t1 && !isva) {
        ...
        if (t1 != (jl_value_t *)jl_typetype_type && jl_is_type_type(t1)) {
            ...
            if (jl_typemap_array_insert_(map, &cache->targ, a0, newrec, (jl_value_t *)cache,
                                         1, offs, tparams))
                return;
        }
        if (jl_typemap_array_insert_(map, &cache->arg1, t1, newrec, (jl_value_t *)cache, 0,
                                     offs, tparams))
            return;
    }
                                      ...
                                      size_t min_world, size_t max_world)
{
    ...
    jl_typemap_entry_t *newrec = (jl_typemap_entry_t *)jl_gc_alloc(
        ptls, sizeof(jl_typemap_entry_t), jl_typemap_entry_type);
    newrec->sig = type;
    ...
    newrec->issimplesig =
        !jl_is_unionall(type); // a TypeVar environment needs a complex matching test
    newrec->isleafsig =
        newrec->issimplesig && !newrec->va; // entirely leaf types don't need to be sorted
    JL_GC_PUSH1(&newrec);
    ...
    for (i = 0, l = jl_nparams(ttype); i < l && newrec->issimplesig; i++) {
        ...
        if (jl_is_kind(decl))
            ...
            newrec->isleafsig =
                0; // Type{} may need special processing to compute the match
            ...
            newrec->isleafsig = 0; // Any needs to go in the general cache
                     ...
                     decl)) // anything else needs to go through the general subtyping test
            newrec->isleafsig = newrec->issimplesig = 0;
    }
    // TODO: assert that guardsigs == jl_emptysvec && simplesig == jl_nothing if isleafsig
    // and optimize with that knowledge?
                                          ...
                                          const struct jl_typemap_info *tparams)
{
    ...
    while ((jl_value_t *)l != jl_nothing) {
        if (!l->isleafsig) { // quickly ignore all of the leafsig entries (these were
                             // handled by caller)
            if (jl_type_morespecific((jl_value_t *)newrec->sig, (jl_value_t *)l->sig)) {
                if (l->simplesig == (void *)jl_nothing ||
                    ...
                    // might need to insert multiple entries for a lookup differing only by
                    // their simplesig when simplesig contains a kind
                    // TODO: make this test more correct or figure out a better way to
                    // compute this
                    break;
                }
            }
