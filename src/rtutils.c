...
jl_value_t *jl_vexceptionf(jl_datatype_t *exception_type, const char *fmt, va_list args)
{
...
JL_DLLEXPORT void JL_NORETURN jl_exceptionf(jl_datatype_t *exception_type, const char *fmt,
                                            ...)
{
    va_list args;
    va_start(args, fmt);
...
{
    jl_exceptionf(jl_argumenterror_type, "%s: too many arguments (expected %d)", fname,
                  max);
}
                                               ...
                                               jl_value_t *expected, jl_value_t *got)
{
    ...
    jl_value_t *ex =
        jl_new_struct(jl_typeerror_type, jl_symbol(fname), ctxt, expected, got);
    jl_throw(ex);
}
...
JL_DLLEXPORT void JL_NORETURN jl_bounds_error_v(jl_value_t *v, jl_value_t **idxs,
                                                size_t nidxs)
{
    jl_value_t *t = NULL;
...
JL_DLLEXPORT void JL_NORETURN jl_bounds_error_unboxed_int(void *data, jl_value_t *vt,
                                                          size_t i)
{
...
void jl_reserve_excstack(jl_excstack_t **stack JL_REQUIRE_ROOTED_SLOT, size_t reserved_size)
{
...
// optimization of setfield which bypasses boxing of the idx (and checking field type
// validity)
...
static size_t jl_show_svec(JL_STREAM *out, jl_svec_t *t, const char *head, const char *opn,
                           const char *cls) JL_NOTSAFEPOINT
{
...
static size_t jl_static_show_x(JL_STREAM *out, jl_value_t *v,
                               struct recur_list *depth) JL_NOTSAFEPOINT;
static size_t jl_static_show_next_(JL_STREAM *out, jl_value_t *v, jl_value_t *prev,
                                   struct recur_list *depth) JL_NOTSAFEPOINT;
...
static jl_datatype_t *first_arg_datatype(jl_value_t *a JL_PROPAGATES_ROOT,
                                ...
                                struct recur_list *depth) JL_NOTSAFEPOINT
{
    ...
    // exist only by pointer identity in early startup
        ...
        if (jl_core_module && (dv->name->module != jl_core_module ||
                               !jl_module_exports_p(jl_core_module, sym))) {
        ...
        if (dv->parameters && (jl_value_t *)dv != dv->name->wrapper &&
            (jl_has_free_typevars(v) || (jl_value_t *)dv != (jl_value_t *)jl_tuple_type)) {
        ...
        n += jl_printf(out, "\"");
        jl_uv_puts(out, jl_string_data(v), jl_string_len(v));
        n += jl_string_len(v);
        n += jl_printf(out, "\"");
    }
        ...
        while (jl_is_uniontype(v)) {
            // tail-recurse on b to flatten the printing of the Union structure in the
            // common case
        ...
        n += jl_printf(out, "SSAValue(%" PRIuPTR ")", (uintptr_t)((jl_ssavalue_t *)v)->id);
    }
        ...
        if (ndims == 1)
            ...
            for (i = 0; i < ndims; i++)
                n += jl_printf(out, (i > 0 ? ", %" PRIdPTR : "%" PRIdPTR),
                               jl_array_dim(v, i));
        n += jl_printf(out, ")}[");
        ...
        if (nb > 0 && tlen == 0) {
            ...
            if (vt == jl_typemap_entry_type) {
                ...
                n += jl_static_show_next_(
                    out, (jl_value_t *)((jl_typemap_entry_t *)v)->next, v, depth);
            }
        }
        n += jl_printf(out, ")");
    }
                                   ...
                                   struct recur_list *depth) JL_NOTSAFEPOINT
{
    ...
    // while being careful about avoiding any recursion due to malformed (circular)
    // references
    if (v == NULL) {
    ...
    unsigned int dist = 1;
    struct recur_list this_item = {depth, v}, *newdepth = &this_item, *p = depth;
    while (p) {
            ...
            while (m && jl_typeis(m, jl_typemap_entry_type)) {
                if (m == v) {
                    return jl_printf(out, "<typemap reference #%u @-%u ", nid, dist) +
                           jl_static_show_x(
                               out, (jl_value_t *)((jl_typemap_entry_t *)m)->sig, depth) +
                           jl_printf(out, ">");
                }
                if (m == prev) {
                    newdepth = depth;
            ...
            jl_value_t *file, jl_value_t *line, jl_value_t *kwargs, jl_value_t *msg)
{
    ...
    if (!logmsg_func && jl_base_module) {
        ...
        if (corelogging && jl_is_module(corelogging)) {
            logmsg_func =
                jl_get_global((jl_module_t *)corelogging, jl_symbol("logmsg_shim"));
        }
    }
    if (!logmsg_func) {
        ios_t str_;
        ...
        jl_safe_printf("%s [Fallback logging]: %.*s\n",
                       level < JL_LOGLEVEL_INFO ?
                           "Debug" :
                           level < JL_LOGLEVEL_WARN ?
                           "Info" :
