...
static int NOINLINE compare_fields(jl_value_t *a, jl_value_t *b,
                                   jl_datatype_t *dt) JL_NOTSAFEPOINT
{
...
static int egal_types(jl_value_t *a, jl_value_t *b, jl_typeenv_t *env) JL_NOTSAFEPOINT
{
    ...
    if (dt == jl_unionall_type) {
        ...
        if (ua->var->name != ub->var->name)
            return 0;
        if (!(egal_types(ua->var->lb, ub->var->lb, env) &&
              egal_types(ua->var->ub, ub->var->ub, env)))
            return 0;
    ...
    for (f = 0; f < nf; f++) {
        ...
        if (jl_field_isptr(dt, f)) {
            ...
            if (jl_is_uniontype(fieldtype)) {
                ...
                fieldtype =
                    (jl_datatype_t *)jl_nth_union_component((jl_value_t *)fieldtype, sel);
            }
...
JL_CALLABLE(jl_f_sizeof)
{
    ...
    if (jl_is_datatype(x)) {
        ...
        if (dx->layout == NULL) {
            if (dx->abstract)
                jl_errorf("Abstract type %s does not have a definite size.",
                          jl_symbol_name(dx->name->name));
            else
                jl_errorf(
                    "Argument is an incomplete %s type and does not have a definite size.",
                    jl_symbol_name(dx->name->name));
        }
        if (jl_is_layout_opaque(dx->layout))
            jl_errorf("Type %s does not have a definite size.",
                      jl_symbol_name(dx->name->name));
...
void STATIC_INLINE _grow_to(jl_value_t **root, jl_value_t ***oldargs, jl_svec_t **arg_heap,
                            size_t *n_alloc, size_t newalloc, size_t extra)
{
...
static jl_value_t *do_apply(jl_value_t *F, jl_value_t **args, uint32_t nargs,
                            jl_value_t *iterate)
{
...
JL_CALLABLE(jl_f_setfield)
{
    ...
    jl_datatype_t *st = (jl_datatype_t *)vt;
    if (!st->mutabl)
        jl_errorf("setfield! immutable struct of type %s cannot be changed",
    ...
    if (args[0] == (jl_value_t *)jl_anytuple_type) {
        for (i = 1; i < nargs; i++) {
            ...
            if (jl_is_vararg_type(pi)) {
                if (i != nargs - 1)
                    jl_type_error_rt("Tuple", "non-final parameter",
...
JL_CALLABLE(jl_f_applicable)
{
    ...
    return jl_method_lookup(args, nargs, 1, world) != NULL ? jl_true : jl_false;
}
...
JL_CALLABLE(jl_f_invoke_kwsorter)
{
    ...
    if (jl_is_tuple_type(argtypes)) {
        // construct a tuple type for invoking a keyword sorter by putting the kw container
        // type and the type of the function at the front.
...
jl_expr_t *jl_exprn(jl_sym_t *head, size_t n)
{
    ...
    JL_GC_PUSH1(&ar);
    jl_expr_t *ex = (jl_expr_t *)jl_gc_alloc(ptls, sizeof(jl_expr_t), jl_expr_type);
    ex->head = head;
    ex->args = ar;
    JL_GC_POP();
    return ex;
...
JL_CALLABLE(jl_f__structtype)
{
    ...
    dt = jl_new_datatype((jl_sym_t *)args[1], (jl_module_t *)args[0], NULL,
                         (jl_svec_t *)args[2], (jl_svec_t *)fieldnames, NULL, 0,
                         args[4] == jl_true ? 1 : 0, jl_unbox_long(args[5]));
...
JL_CALLABLE(jl_f__abstracttype)
{
    ...
    jl_datatype_t *dt =
        jl_new_abstracttype(args[1], (jl_module_t *)args[0], NULL, (jl_svec_t *)args[2]);
    return dt->name->wrapper;
}
...
JL_CALLABLE(jl_f__primitivetype)
{
    ...
    if (nb < 1 || nb >= (1 << 23) || (nb & 7) != 0)
        jl_errorf("invalid number of bits in primitive type %s",
                  jl_symbol_name((jl_sym_t *)name));
    jl_datatype_t *dt = jl_new_primitivetype(args[1], (jl_module_t *)args[0], NULL,
                                             (jl_svec_t *)args[2], nb);
    return dt->name->wrapper;
}
...
{
    if (!jl_is_datatype(super) || !jl_is_abstracttype(super) || tt->super != NULL ||
        tt->name == ((jl_datatype_t *)super)->name ||
        jl_subtype(super, (jl_value_t *)jl_vararg_type) || jl_is_tuple_type(super) ||
        jl_is_namedtuple_type(super) || jl_subtype(super, (jl_value_t *)jl_type_type) ||
...
static int equiv_type(jl_value_t *ta, jl_value_t *tb)
{
    ...
    if (!(jl_typeof(dta) == jl_typeof(dtb) && dta->name->name == dtb->name->name &&
    ...
    if (!jl_types_equal(a, b))
        goto no;
    JL_TRY {
        a = jl_apply_type(dtb->name->wrapper, jl_svec_data(dta->parameters),
                          jl_nparams(dta));
    }
    JL_CATCH {
        ok = 0;
...
JL_CALLABLE(jl_f_intrinsic_call)
{
    ...
    switch (fargs) {
    case 1: return fptr.call1(args[0]);
    case 2: return fptr.call2(args[0], args[1]);
    case 3: return fptr.call3(args[0], args[1], args[2]);
    case 4: return fptr.call4(args[0], args[1], args[2], args[3]);
    default: assert(0 && "unexpected number of arguments to an intrinsic function");
    }
...
static void add_intrinsic(jl_module_t *inm, const char *name,
                          enum intrinsic f) JL_GC_DISABLED
{
...
#define ADD_I(name, nargs) \
    add_intrinsic_properties(name, nargs, (void (*)(void)) & jl_##name);
#define ADD_HIDDEN ADD_I
#define ALIAS(alias, base) \
    add_intrinsic_properties(alias, intrinsic_nargs[base], runtime_fp[base]);
    INTRINSICS
#undef ADD_I
#undef ADD_HIDDEN
#undef ALIAS
...
void jl_init_primitives(void) JL_GC_DISABLED
{
    ...
    jl_value_t *ikws =
        jl_new_generic_function_with_supertype(itn->name, jl_core_module, jl_builtin_type);
    itn->mt->kwsorter = ikws;
    jl_gc_wb(itn->mt, ikws);
    jl_mk_builtin_func((jl_datatype_t *)jl_typeof(ikws), jl_symbol_name(jl_gf_name(ikws)),
