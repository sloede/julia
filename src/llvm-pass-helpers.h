...
struct IntrinsicDescription final {
    ...
    IntrinsicDescription(const llvm::StringRef &name, const DeclarationFunction &declare)
...
struct JuliaPassContext {
    ...
    // `nullptr` is returned.
    llvm::Function *getOrNull(const jl_intrinsics::IntrinsicDescription &desc) const;

    // Gets the intrinsic or well-known function that conforms to
    // the given description if it exists in the module. If not,
    // declares the intrinsic or well-known function and adds it
    // to the module.
    llvm::Function *getOrDeclare(const jl_intrinsics::IntrinsicDescription &desc);
